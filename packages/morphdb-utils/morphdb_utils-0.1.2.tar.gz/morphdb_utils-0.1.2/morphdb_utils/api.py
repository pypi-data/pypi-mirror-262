...
def execute_sql(
    ...
    response = requests.post(url_sql, headers=headers, json={"sql": sql}, verify=True)
...
def execute_sql_external_source(
    sql: str,
    canvas_cell_id: str,
    database_id: str | None = None,
    base_url: str | None = None,
    ...
    url_sql = urllib.parse.urljoin(
        _canonicalize_base_url(base_url),
        f"/{database_id}/sql/external/python",
    )

    response = requests.post(
        url_sql, headers=headers, json={"sql": sql, "canvasCellId": canvas_cell_id}
    )
    if response.status_code != 200:
        raise MorphApiError(response.text)
