...
class ToscaWidgets2ConfigurationComponent(ConfigurationComponent):
    ...
    def _setup_validation(self, conf, app):
        ...
        from .validation import _explode_tw2validationerror

        validation_exceptions = conf["validation.exceptions"]
        if _Tw2ValidationError not in validation_exceptions:
            validation_exceptions.append(_Tw2ValidationError)

        validation_explode = conf["validation.explode"]
        if _Tw2ValidationError not in validation_explode:
            validation_explode[_Tw2ValidationError] = _explode_tw2validationerror
