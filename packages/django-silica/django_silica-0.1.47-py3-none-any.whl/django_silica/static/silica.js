...
class SilicaComponent {
    ...
    processInitCalls() {
        this.querySelectorAll("[silica\\:init]").forEach((el) => {
            ...
            this.callMethodOrSetProperty(methodOrExpressionString);
        });
    ...
    sendMessage(action) {
        ...
        fetch("/silica/message", {
            ...
            .then((response) => {
                ...
                // this.updateDomWithMorphdom(response.html);
    ...
    callMethodOrSetProperty(methodOrExpressionString) {
        if (methodOrExpressionString.includes("(")) {
            ...
            this.setProperty(propertyName.trim(), value.trim().replace(/^['"]+|['"]+$/g, ''));
        } else {
            // Assumed the string is a function without parentheses
            this.callMethod(methodOrExpressionString);
        }
    }
