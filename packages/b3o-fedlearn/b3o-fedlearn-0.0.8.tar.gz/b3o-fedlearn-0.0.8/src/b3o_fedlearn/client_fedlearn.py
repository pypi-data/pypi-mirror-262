...
import numpy as np
import json
from .client_repo.MODEL import MLMODEL
import boto3
...
class FLClient(object):
    ...
    def localTraining (self, model_params_w_file, s3_fl_model_registry, cross_account_s3_role=None):
        ...
        bucket_name = 'b3o'
        s3_file_path = f'fedlearn/model/{self.FLTaskName}-model.py'

        local_file_path = 'models/model_downloaded.py'
        s3_client.download_file(bucket_name, s3_file_path, local_file_path)

        spec = importlib.util.spec_from_file_location("model_downloaded", local_file_path)
        model_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(model_module)

        if (self.roundId == 0):
            print("2: Download a global model")
            mlmodel = model_module.MLMODEL()
