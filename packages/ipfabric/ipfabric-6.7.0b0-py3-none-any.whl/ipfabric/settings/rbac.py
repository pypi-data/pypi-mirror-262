...
class Policies(BaseModel):
    ...
    def _check_policy(self, name, description, api_scope_ids=None, attribute_filters=None):
        if not (name and description):
            raise SyntaxError("No Policy name or description provided.")
        if name in self.policies_by_name:
    ...
    def _fmt_api_scopes(api_scope_ids):
        ...
        for _ in api_scope_ids:
            if isinstance(_, Endpoint):
                ...
                logger.error(f"Invalid Scope: {_}")
        return list(tmp)
    ...
    def create_policy(
        ...
        self._check_policy(name, description, api_scope_ids, attribute_filters)
        payload = {
            "name": valid_slug(name),
            "description": description,
        }
        if api_scope_ids:
            payload["apiScopeIds"] = self._fmt_api_scopes(api_scope_ids)
        else:
    ...
    def delete_policies(self, policies: List[Union[int, str, Policy]]) -> Dict[str, bool]:
        ...
        for policy in policies:
            ...
            try:
                deleted[policy_id] = self.delete_role(policy_id)
            except HTTPStatusError:
...
class Roles(BaseModel):
    ...
    def search_roles_for_policy(self, policy_name: str = None, policy_id: Union[str, int] = None) -> List[Role]:
        ...
        if policy_name:
            ...
            logger.error("No Policy Name or ID provided")
            return list()
    ...
    def create_roles(self, roles: List[Dict[str, Union[str, List[Union[int, str, Policy]]]]]) -> Dict[str, Role]:
        ...
        for role in roles:
            if set(role.keys()) != {"name", "description", "policy_ids"} and not (
