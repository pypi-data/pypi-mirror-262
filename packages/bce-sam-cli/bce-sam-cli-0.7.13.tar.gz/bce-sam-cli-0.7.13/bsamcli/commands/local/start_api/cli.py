...
Allows you to run your Serverless application locally for quick development & testing.
 When run in a directory that contains your Serverless functions and your BCE SAM template, it will create a local HTTP
 server hosting all of your functions. When accessed (via browser, cli etc), it will launch a Docker container locally
 to invoke the function. It will read the CodeUri property of BCE::Serverless::Function resource to find the path in
 your file system containing the CFC Function code. This could be the project's root directory for interpreted
...
def cli(ctx,
    ...
    do_cli(ctx, host, port, static_dir, template, env_vars, debug_port, debug_args,
