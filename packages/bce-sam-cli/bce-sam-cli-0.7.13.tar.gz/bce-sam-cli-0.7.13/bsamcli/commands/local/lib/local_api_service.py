...
from bsamcli.local.apigw.local_apigw_service import LocalApigwService, Route
...
class LocalApiService(object):
    ...
    def __init__(self,
        ...
        self.api_provider = SamHttpProvider(lambda_invoke_context.template, cwd=self.cwd)
    ...
    def start(self):
        ...
        LOG.info("You can now browse to the above endpoints to invoke your functions. "
                 "You do not need to restart/reload BSAM CLI while working on your functions "
                 "changes will be reflected instantly/automatically. You only need to restart "
                 "BSAM CLI if you update your BCE SAM template")
    ...
    def _make_routing_list(api_provider):
        ...
        Parameters
        ----------
        api_provider : bsamcli.commands.local.lib.sam_http_provider.SamHttpProvider

        Returns
        -------
        list(bsamcli.local.apigw.service.Route)
            List of Routes to pass to the service
        """

        routes = []
        for api in api_provider.get_all():
            route = Route(methods=api.method, function_name=api.function_name, path=api.resource_path,
                          is_binary=api.is_binary)
            routes.append(route)
    ...
    def _print_routes(api_provider, host, port):
        ...
        :param bsamcli.commands.local.lib.provider.ApiProvider api_provider: API Provider that can return a list of APIs
        :param string host: Host name where the service is running
        :param int port: Port number where the service is running        
        """

        for api in api_provider.get_all():        
            methods_str = '[' + ','.join([m for m in api.method]) + ']'
            is_binary_str = "[isBinary:%s]" % (api.is_binary)
