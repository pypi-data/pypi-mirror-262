...
from bsamcli.local.services.base_local_service import BaseLocalService, LambdaOutputParser, CaseInsensitiveDict
from bsamcli.local.lambdafn.exceptions import FunctionNotFound
from bsamcli.local.events.api_event import ContextIdentity, RequestContext, ApiGatewayLambdaEvent
...
class Route(object):
    ...
    def __init__(self, methods, function_name, path, is_binary=False):
        """
...
class LocalApigwService(BaseLocalService):
    ...
    def __init__(self, routing_list, lambda_runner, static_dir=None, port=None, host=None, stderr=None):
        ...
        :param bsamcli.commands.local.lib.local_lambda.LocalLambdaRunner lambda_runner: The Lambda runner class capable
    ...
    def _request_handler(self, **kwargs):
        ...
        * We then transform the response or errors we get from the Invoke and return the data back to
        ...
        try:
            event = self._construct_event(request, self.port, route.is_binary)
    ...
    def _parse_lambda_output(lambda_output, is_binary, flask_request):
        ...
        if not isinstance(json_output, dict):
            raise TypeError("CFC returned %{s} instead of dict", type(json_output))
    ...
    @staticmethod
    def _construct_event(flask_request, port, is_binary):
        """
        ...
        if is_binary:
            LOG.debug("Base64 encoding the request data before sending to CFC function.")
        ...
        event = ApiGatewayLambdaEvent(http_method=method,
                                      ...
                                      is_base_64_encoded=is_binary)

        event_str = json.dumps(event.to_dict())
        LOG.debug("Constructed String representation of Event to invoke CFC. Event: %s", event_str)
        return event_str
    ...
    def _query_string_params(flask_request):
        ...
        for query_string_key, query_string_list in dict(flask_request.args).items():
            ...
            if not query_string_value_length:
                ...
                query_string_dict[query_string_key] = query_string_list
