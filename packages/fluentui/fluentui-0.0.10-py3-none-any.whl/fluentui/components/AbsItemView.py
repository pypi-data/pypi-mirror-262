...
from PySide6.QtWidgets import QAbstractItemView, QTableWidget, QWidget, QStyleOptionViewItem, QStyledItemDelegate, \
    QTableView
from typing_extensions import Optional, Callable, Self

from .AbsItemDelegate import AbsItemDelegate
from .ScrollArea import ScrollAreaMix
from ..core import PersistentIndex, AbsItemModel
from ..gui import StdModel, ColumnModel
...
class AbsItemViewMix(ScrollAreaMix):
    ...
    def __init__(self: QAbstractItemView | Self, *,
        ...
        if not isinstance(self, QTableWidget):
            if isinstance(self, QTableView):
                self.setModel(model or ColumnModel())
            else:
    ...
    def setModelData(self, editor: QWidget, model: AbsItemModel, index: QModelIndex) -> None:
        """ 设置模型数据 """
        if d := self.itemDelegate():
            d.super().setModelData(editor, model, index)
        return None
    ...
    def displayText(self, value: object, locale: QLocale) -> str:
        """ 项目文本 """
        if isinstance(d := self.itemDelegate(), QStyledItemDelegate):
            return d.super().displayText(value, locale)
        return None

    def itemDelegate(self) -> Optional[AbsItemDelegate]:
