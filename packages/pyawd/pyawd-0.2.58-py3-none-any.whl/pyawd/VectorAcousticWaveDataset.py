...
def solve_vector_pde(u: dvt.VectorTimeFunction, f: dvt.VectorTimeFunction, nx: int, ndt: int, ddt: float,
    ...
    op = dvt.Operator(dvt.Eq(u.forward,
                             dvt.solve(dvt.Eq(u.dt2, f +
                                              ((max_velocity * velocity_model) ** 2) * u.laplace),
...
class VectorAcousticWaveDataset(torch.utils.data.Dataset):
    ...
    def generate_data(self):
        ...
        for i in tqdm(range(self.size)):
            data = solve_vector_pde(self._u, self._f, self.nx, self.ndt, self.ddt, self.epicenters[i],
