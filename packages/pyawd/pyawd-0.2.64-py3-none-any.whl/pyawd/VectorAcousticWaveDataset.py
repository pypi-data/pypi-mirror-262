...
def solve_vector_pde(u: dvt.VectorTimeFunction, f: dvt.VectorTimeFunction, a: dvt.Function, nx: int, ndt: int, ddt: float,
                     epicenter: np.ndarray, velocity_model: dvt.Function, max_velocity: np.ndarray, f_delay: np.ndarray,
                     amplitude_factor: np.ndarray, dim: int = 2):
    """
    ...
    The returned array has its both component in the shape
            ...
            ↓
    Args:
        u (devito.VectorTimeFunction): A Devito VectorTimeFunction Object
        f (devito.VectorTimeFunction): A Devito VectorTimeFunction Object
        a (devito.Function): A Devito Function Object
    ...
    if dim == 2:
        ...
        f[0].data[:] = (np.tile(s_x, (s_t.shape[0], 1, 1)) * s_t[:, None, None])
        f[1].data[:] = (np.tile(s_y, (s_t.shape[0], 1, 1)) * s_t[:, None, None])
        a.data[:] = create_inverse_distance_matrix(nx, x0=int(epicenter[0]), y0=int(epicenter[1]), tau=nx**2)**2
    else:
        ...
        s_x, s_y, s_z = create_explosive_source(nx, x0=int(epicenter[0]), y0=int(epicenter[1]), z0=int(epicenter[2]),
                                                dim=dim)
        f[0].data[:] = (np.tile(s_x, (s_t.shape[0], 1, 1, 1)) * s_t[:, None, None, None])
        f[1].data[:] = (np.tile(s_y, (s_t.shape[0], 1, 1, 1)) * s_t[:, None, None, None])
        f[2].data[:] = (np.tile(s_z, (s_t.shape[0], 1, 1, 1)) * s_t[:, None, None, None])
        a.data[:] = create_inverse_distance_matrix(nx, x0=int(epicenter[0]), y0=int(epicenter[1]),
                                                   z0=int(epicenter[2]), tau=nx ** 2) ** 2
    op = dvt.Operator(dvt.Eq(u.forward,
                             dvt.solve(dvt.Eq(u.dt2, f +
                                              ((max_velocity * velocity_model) ** 2) * u.laplace * a),
                                       u.forward)), opt=('advanced'))
    op.apply(dt=ddt)
    return np.array([u[i].data for i in range(u.shape[0])])
