...
class ColumnExpression(ABC, Serializable):
    ...
    def is_today(self) -> "ColumnExpression":
        ...
        from .sql_function import func

        return (self >= func.now().by_day) & (
            self < func.now().by_day + datetime.timedelta(days=1)
        )
    ...
    def is_yesterday(self) -> "ColumnExpression":
        ...
        return (self >= func.now().by_day - datetime.timedelta(days=1)) & (
            self < func.now().by_day
        )
    ...
    def is_this_week(self) -> "ColumnExpression":
        ...
        return self._is_between_timestamps(
            func.now().by_week,
            func.now().by_week + datetime.timedelta(days=7),
        )
    ...
    def is_this_month(self) -> "ColumnExpression":
        ...
        return self._is_between_timestamps(
            func.now().by_month,
            func.now().by_month + _DateShift(months=1),
        )
    ...
    def is_this_quarter(self) -> "ColumnExpression":
        ...
        return self._is_between_timestamps(
            func.now().by_quarter,
            func.now().by_quarter + _DateShift(months=3),
        )
    ...
    def is_this_year(self) -> "ColumnExpression":
        ...
        return self._is_between_timestamps(
            func.now().by_year,
            func.now().by_year + _DateShift(years=1),
        )
