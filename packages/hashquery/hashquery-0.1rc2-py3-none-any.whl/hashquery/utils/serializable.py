    ...
    def _make_error_message(cls, expected: int, found: int):
        ...
        cause_str = (
            ...
            if is_found_ahead
                ...
                + f"You may be using a prerelease version of Hashquery not yet supported by {HashboardAPI.base_uri}. "
                + f"This issue will be resolved when {HashboardAPI.base_uri} is next updated. In the meantime, you "
                + "may be able to resolve the issue by downgrading to an earlier version of Hashquery. "
            )
        )
