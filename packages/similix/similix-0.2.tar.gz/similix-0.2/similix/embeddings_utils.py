...
class TextEmbeddingsAnalyzer:
    def __init__(self, api_key=os.environ.get('OPENAI_API_KEY'), max_retries=5):
        self.client = OpenAI(max_retries=max_retries, api_key=api_key)

    def _clean_text(self, text):
        return text.replace("\n", " ")

    def get_embedding(self, text: str, model="text-embedding-3-large", **kwargs) -> List[float]:
    ...
    def get_embeddings(self, list_of_text: List[str], model="text-embedding-3-large", **kwargs) -> List[List[float]]:
        assert len(list_of_text) <= 2048, "The batch size should not be larger than 2048."
        list_of_text = [self._clean_text(text) for text in list_of_text]
        data = self.client.embeddings.create(input=list_of_text, model=model, **kwargs).data

        return [d.embedding for d in data]

    async def aget_embeddings(self, list_of_text: List[str], model="text-embedding-3-large", **kwargs) -> List[
        List[float]]:
        assert len(list_of_text) <= 2048, "The batch size should not be larger than 2048."
        list_of_text = [self._clean_text(text) for text in list_of_text]
        data = await self.client.embeddings.create(input=list_of_text, model=model, **kwargs).data
    ...
    def chart_from_components_3D(self,
    ...
    def tsne_components_from_embeddings(self,
    ...
    def chart_from_components(self,
                              components: np.ndarray,
                              labels: Optional[List[str]] = None,
                              strings: Optional[List[str]] = None,
                              x_title="Component 0",
    ...
    def pca_components_from_embeddings(self,
