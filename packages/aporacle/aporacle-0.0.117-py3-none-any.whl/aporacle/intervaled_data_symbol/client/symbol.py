...
class Symbol(KomoBase):
    ...
    def __init__(self, symbol: str, input_trading_data_queue: asyncio.Queue):
        ...
        self.trades_data_task: Optional[asyncio.Task] = None

    def start(self):
        safe_ensure_future(self.cursor.start())
        self.trades_data_task = safe_ensure_future(self.trades_data_loop())

    def stop(self):
        self.cursor.stop()
        if self.trades_data_task is not None:
            self.trades_data_task.cancel()
            self.trades_data_task = None
            del self.trades_data_task
        ...
        try:
            ...
            while True:
                ...
                if timestamp not in self.cursor.ticks:
                    print(
                        f"Creating Timestamp Not Available {timestamp} {trade['timestamp_at_exchange']} {self.symbol}".upper())
