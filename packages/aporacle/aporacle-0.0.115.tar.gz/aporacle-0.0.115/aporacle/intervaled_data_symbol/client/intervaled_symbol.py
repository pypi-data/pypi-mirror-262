...
class IntervaledSymbol(Symbol):
    ...
    def run(self, chain: str, epoch: int):
        try:
            self.log_with_clock(log_level=logging.INFO, msg=f"Running interval symbol for chain {chain} epoch {epoch}. ")
            ...
            self.log_with_clock(log_level=logging.INFO, msg=f"Chain {chain} epoch {epoch} took {t2 - t1} seconds. ")
