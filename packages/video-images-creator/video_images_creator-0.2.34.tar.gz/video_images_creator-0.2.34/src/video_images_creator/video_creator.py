...
def set_transparency(img, opacity):
    ...
    if not np.any(img_with_alpha[:, :, 3]):
        img_with_alpha[:, :, 3] = 255  # Set alpha channel to fully opaque

    # Apply opacity
    alpha_float = img_with_alpha[:, :, 3].astype(float)
    alpha_float = alpha_float * opacity
    img_with_alpha[:, :, 3] = alpha_float.astype(img_with_alpha.dtype)
...
def add_images_in_single_frame_V2(feature_images_url, font, directory_name, current_index, feature_title):
    ...
    if first_image == True:
        ...
        file_name = f'{directory_name}/frame_{current_index}_180.jpg' 
        cv2.imwrite(file_name, background)
