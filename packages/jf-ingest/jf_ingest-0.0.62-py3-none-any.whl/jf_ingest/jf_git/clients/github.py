...
from jf_ingest.jf_git.exceptions import GqlRateLimitedException
...
class GithubClient:
    """
    NOTE: This client is a combintation of the GQL client in D.C. and the client in Agent.
    It has A LOT of functionality in it that we currently don't use, but we will use once
    we start using JF Ingest for ingesting Git Data
    - Gavin
    """
    ...
    def get_raw_result(self, query_body: str) -> dict:
        ...
        while True:
            try:
                response = self.session.post(url=self.gql_base_url, json={'query': query_body})
            ...
            # Do a simple retry loop here
                ...
                rate_limit_info = self.get_rate_limit(base_url=self.gql_base_url)
                reset_at: datetime = self.github_gql_format_to_datetime(rate_limit_info['resetAt'])
