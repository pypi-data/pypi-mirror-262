...
class Interface(QMainWindow):
    ...
    def __init__(self, delay, language, music):
        ...
        self.credentials = Credentials(self, language, music)

        self.tabs.addTab(self.credentials, "Settings")
    ...
    def change(self, index):
        ...
        if index == 1:
            self.game.Sound = Noise(
                language=self.parameters["language"],
                playlist=self.parameters["music"]["playlist"],
                spotify=self.parameters["music"]["spotify"]
            )

        self.parameters["previous"] = index
