...
import MySQLdb,traceback,re,sys
...
class HDBppDB(ArchivingDB,SingletonMap):
        ...
        if not all((db_name,host,user,passwd)):
            if other:
                ...
                if sch:
                    ...
                    db_name = sch.get('dbname',sch.get('db_name','hdbpp'))
                    host = host or sch.get('host','localhost')
                    user = user or sch.get('user','hdbpp')
    ...
    @Cached(expire=60.)
    def get_null_attributes(self):
        nll = [d for d in self.get_subscribers(exclude='') if '/null' in d]
        if nll:
            return self.get_archiver_attributes(nll[0])
        else:
            return []
    ...
    def get_next_archiver(self,errors=False,use_freq=False, attrexp=''):
        ...
        """
        self.info('In get_next_archiver(%s,%s,%s)' % (errors,use_freq,attrexp))
        self.clear_caches()
        ...
        if not len(loads):
            ...
            self.debug('attrexp: {}'.format(attrexp))
            attrs = [a for a in self.get_attributes(True) 
                     if fn.clmatch(attrexp,a)]
            
            self.debug('get_next_archiver({}): attributes found: {}'.format(
                attrexp, attrs))
    ...
    def get_data_types(self):
        return dict(self.Query(
            "select data_type,att_conf_data_type_id from att_conf_data_type"))
    
    def get_data_tables(self):
        return sorted('att_'+t for t in self.get_data_types())
    
    def get_conf_tables(self):
        dt = self.get_data_tables()
        return [t for t in self.getTables() if t not in self.get_data_tables()]
    ...
    def get_subscribed_attributes(self,search=''):
        ...
        """
        self.debug('get_subscribed_attributes(%s)'%str(search))
        attrs = []
        subs = self.get_subscribers() #/null is excluded here
        if self.db is not None:
            self.get_att_conf_table()
        [self.get_archiver_attributes(d,from_db=True) for d in subs] 
        
        for d in subs:
            dattrs = self.dedicated[d]
            for a in dattrs:
                try:
                    ...
                    self.warning('Unable to get {} config, archived by {}'.format(a,d))
                    continue
    ...
    def get_attribute_ID(self,attr):
        ...
        if attr in self.attributes:
            ...
            #name = fqdn = get_fqdn_name(attr).lower()
            model = parse_tango_model(attr,fqdn=True)
            name = fqdn = model.fullname
            name not in self.attributes and self.get_att_conf_table()
            #if name not in self.attributes:
                #norms = {k: get_normal_name(k) for k in self.attributes()}
            if name not in self.attributes:
                nr = get_normal_name(attr)
                self.debug('get_attribute_ID(%s): trying normal name %s)'
                        % (attr, nr))
                name = fn.first((a for a in self.attributes if 
                    fn.clsearch('/%s$' % get_normal_name(attr),a)), '')
                
        if not name:
            self.debug('%s(%s != %s): attribute not found!' % (self.db_name,attr,fqdn))
            self.debug(str(self.attributes)[:80] + '...')
            
        #traceback.print_stack()
        return getattr(self.attributes.get(name,None),'id',None)
      
    def get_attributes_IDs(self,name='%',as_dict=1):
        # returns all matching IDs
        self.get_att_conf_table() #cached
        name = name.replace('%','*')
        ids = [(k,v.id) for k,v in self.attributes.items() if 
                name == '*' or fn.clmatch('.*'+name,k)]

        return ids if not ids or not as_dict else dict(ids)
    
    @Cached(depth=10000)
    def get_attribute_by_ID(self,ID):
        self.get_att_conf_table() #cached
        attr = [k for k,v in  self.attributes.items() if v.id==ID]
        if attr:
            return attr[0]
        else:
            raise Exception('%s: wrong ID %s' % (self.db_name,ID))
    
    def get_attribute_name(self,attr):
        try:
            ID = self.get_attribute_ID(attr)
            return self.get_attribute_by_ID(ID) if ID is not None else ''
        except:
            return ''
    ...
    def get_attr_id_type_table(self,attr):
        try:
            attr = self.get_attribute_name(attr)            
    ...
    def get_attribute_subscriber(self,attribute):
        ...
        m = self.get_attribute_name(attribute)
        for k in self.get_subscribers():
            for l in self.get_archiver_attributes(k):
                if m in l.split(';'):
                    return k
    ...
    def is_attribute_archived(self,attribute,active=None,cached=True):
        """
        @ DEPRECATED by is_attribute_subscribed/is_periodic_archived
        
        If success, this method should always return the attribute name
        EXACTLY as it is used within the DB tables

        # @TODO active argument not implemented
        """
        model = parse_tango_model(attribute,fqdn=True)
        d = self.get_manager()
        self.debug(str(('is_attribute_archived',model,active,cached,d)))
        if d and cached:
            self.debug('query on properties')
            self.get_archived_attributes()
            l = map(str.lower,self.attributes)
            ms =  map(str.lower,(attribute,model.fullname,model.normalname))
            self.debug(str(('model',ms)))
            if ms[0] in l or ms[1] in l:
                return model.fullname
            else:
                return False
        elif d:
            self.debug('manager.AttributeSearch')
            ...
            self.debug('trying to match simple names on cache')
    ...
    def is_attribute_subscribed(self,attribute,exclude='.*/null'):
        ...
        if it is, then return proper attribute name within archiver
        """
        #print('%s.%s(%s,%s)' % (type(self),'is_attribute_subscribed',attribute,active))
        s = self.get_attribute_subscriber(attribute)
        return (self.get_attribute_by_ID(self.get_attribute_ID(attribute))
                    if s and not fn.clmatch(exclude,str(s)) else False)
    ...
    def add_event_subscriber(self,srv,dev,libname=''):
        ...
        put_device_property(dev,'LibConfiguration',[
          'user='+self.user,
          'password='+self.passwd,
          'port='+(getattr(self,'port',None) or '3306'),
          'host='+self.host,
          'dbname='+self.db_name,
          'libname='+libname,
          'lightschema=1',
        ...
        try:
            ...
            print('ArchiverAdd failed: %s' % str(e))
    ...
    def add_attribute(self,attribute,archiver=None,period=0,
        ...
        locked = False
        try:
            attribute = get_full_name(attribute,fqdn=True).lower()
            self.debug('full name: {}'.format(attribute))
            if archiver:
                archiver = get_full_name(archiver,fqdn=True).lower()
                
            archiver = archiver or self.get_next_archiver(
                use_freq = use_freq, attrexp=fn.tango.get_dev_name(attribute)+'/*')
            self.debug('-'*80)
            self.debug('next archiver: {}'.format(archiver))
            ...
            self.debug('checking if {} is subscribed ...'.format(attribute))
            ...
            d.lock()
            locked = True
            self.debug('SetAttributeName: %s'%attribute)
            ...
            if abs_event not in (None,-1,0):
                self.debug('SetAbsoluteEvent: %s'%abs_event)
            ...
            self.info('SetArchiver(%s)' % archiver)
            time.sleep(.2)
            try:
                d.AttributeAdd()
            except Exception as e:
                ...
                if 'already archived' in str(e).lower():
                    self.info('stop/start needed')
                    self.reassign_attribute(attribute,archiver,context,ttl) 
                else:
                    raise e
        ...
        self.info('%s added to %s' % (attribute,archiver))
        return archiver
    ...
    def add_attributes(self,attributes,*args,**kwargs):
        ...
        try:
            ...
            for dev,attrs in sorted(devs.items()):
                self.info('adding %d attributes from %s' % (len(attrs),dev))
    ...
    def start_archiving(self,attribute,archiver,period=0,
        ...
        try:
            if isSequence(attribute):
                ...
                d = self.get_manager()
                fullname = get_full_name(attribute,fqdn=True).lower()
                archiver = get_full_name(archiver,fqdn=True).lower()
                
                if not self.get_attribute_subscriber(fullname):
                    ...
                    if start:
                        ...
                        fullname = self.get_attribute_name(attribute)
                if start:
        ...
        try:
            if fn.isSequence(attribute):
                ...
                if attribute:
                    ...
                    self.error('%s seems not archived!' % attribute)
            if clear:
                self.clear_caches()
            return attribute
        except:
    ...
    def reassign_attribute(self,attr,subscriber,context=None,ttl=None):
        ...
        """
        attr = self.get_attribute_name(attr)
        curr = self.get_attribute_subscriber(attr)
        dp = fn.get_device(curr,keep=True)
        if fn.check_device(curr):
            if context is None:
            ...
            self.get_manager().AttributeRemove(attr)
    ...
    def restart_attribute(self,attr, d='', wait=3.):
        """
        execute AttributeStop/Start on subscriber device
        """
        try:
            a = self.get_attribute_name(attr)
            if not a:
