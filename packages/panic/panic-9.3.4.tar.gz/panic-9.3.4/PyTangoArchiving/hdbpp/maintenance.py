...
def rename_hdbpp_attributes(old_names, new_names, do_it=False):
    """
    This method replaces [old_names] by [new_names] in DB and archiving devices
    
    (not really tested yet)
    
    BUT!! Alarms should be also modified!!! (and other clients, by the way)
    """
    olds = [fn.tango.get_simple_name(a) for a in old_names]
    for a in new_names:
        try:
    ...
    for schema in pta.get_hdbpp_databases():
        db = pta.api(schema)
        ...
        for new,old in sorted(names.items()):
            ...
            if not db.is_attribute_archived(old):
                #print('%s is not archived!, skipping' % old)
                continue
            if db.is_attribute_archived(new):
        ...
        for d in devs:
            ...
            for i,l in enumerate(attrs):
                for new, old in names.items():
                    if fn.clsearch(old, l):
                        ln = fn.clsub(old, new, l)
                        print('Updating %s: %s => %s' % (d,l,ln))
                        attrs[i] = ln
                        dones.append(new)
            if do_it:
                fn.put_device_property(d,'AttributeList',attrs)
    ...
    print('pos ya estaria, %d renamed, do_it = %s' % (len(dones),do_it))
