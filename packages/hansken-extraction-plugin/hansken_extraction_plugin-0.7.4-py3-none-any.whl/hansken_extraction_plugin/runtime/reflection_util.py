...
def get_plugin_class(plugin_file: str) -> Type:
    ...
    if not os.path.isfile(plugin_file):
        raise ReflectionError('File not found or path is directory: ' + plugin_file)

    if not plugin_file.endswith('.py'):
        raise ReflectionError('Not a python file: ' + plugin_file)
    ...
    for name, cls in inspect.getmembers(module, inspect.isclass):
        if issubclass(cls, BaseExtractionPlugin) and not is_abstract(cls):
            ...
            return cls
    raise ReflectionError('No Extraction Plugin class found in ' + plugin_file)
