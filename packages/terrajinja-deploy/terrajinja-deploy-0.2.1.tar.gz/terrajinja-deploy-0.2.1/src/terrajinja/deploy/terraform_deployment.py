    ...
    def action(self, action):
        ...
        if action == 'apply':
            ...
            for output in self.run(['cdktf', 'apply', '--auto-approve', '--skip-synth']):
                print(output, end="")
        if action == 'delete':
            allow_delete = os.getenv('TJCLI_ALLOW_DELETE')
            if not allow_delete:
                print("to delete your stack, please set the environment variable 'TJCLI_ALLOW_DELETE' first.")
                exit(1)
            print('executing cdktf delete:')
            for output in self.run(['cdktf', 'delete', '--auto-approve', '--skip-synth']):
                print(output, end="")
