...
#ifdef USE_CUDA
void GBRL::_fit_gpu(dataSet *dataset){
    ...
    if (err != cudaSuccess) {
        ...
        std::cerr << "CUDA allocate fit_gpu data error: " << cudaGetErrorString(err)
...
float GBRL::_fit_sl_gpu(dataSet *dataset, float *targets, const int n_iterations){
    const int output_dim = this->metadata->output_dim;
    const int n_bins = this->metadata->n_bins;
    const int n_num_features = this->metadata->n_num_features, n_cat_features = this->metadata->n_cat_features;
    const int n_samples = dataset->n_samples;
    ...
    for (int i = 0 ; i < n_iterations; ++i){
       ...
       cuda_dataset.norm_grads = gpu_grads_norm;
        ...
        if (this->metadata->verbose == 0)
            ...
            std::cout << this->metadata->iteration << " - MultiRMSE: " << loss << std::endl;
        }
...
#endif
void GBRL::fit(const float *obs, const char *categorical_obs, float *grads, const int n_samples, const int n_num_features, const int n_cat_features){
    if (this->metadata->iteration == 0){
        this->metadata->n_num_features = n_num_features;
        this->metadata->n_cat_features = n_cat_features;
    }
...
#ifdef USE_CUDA
    if (this->device == gpu)
        this->_fit_gpu(&dataset);
#endif
    if (this->device == cpu)
        Fitter::fit_cpu(&dataset, this->edata, this->metadata);
}

float GBRL::fit_sl(float *obs, char *categorical_obs, float *targets, int iterations, const int n_samples, const int n_num_features, const int n_cat_features, bool shuffle, std::string _loss_type){
...
#ifdef USE_CUDA
    if (this->device == gpu)
        full_loss = this->_fit_sl_gpu(&dataset, training_targets, iterations);
#endif
    if (this->device == cpu){
       full_loss = Fitter::fit_sl_cpu(&dataset, training_targets, this->edata, this->metadata, iterations, loss_type, this->opts);
    }
...
void GBRL::plot_tree(int tree_idx, const std::string &filename){
    ...
    for (int leaf_idx = edata_cpu->tree_indices[tree_idx]; leaf_idx < stop_leaf_idx; ++leaf_idx){
        ...
        for (int i = 1; i < depth; ++i){
            ...
            printf("working on node_index %d inequality direction %d!\n", nodeIndex, inequality_direction);
