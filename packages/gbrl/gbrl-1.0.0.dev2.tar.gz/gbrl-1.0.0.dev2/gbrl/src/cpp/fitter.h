...
class Fitter {
    public:
        static void fit_cpu(dataSet *dataset, ensembleData *edata, ensembleMetaData *metadata);
        static float fit_sl_cpu(dataSet *dataset, const float *targets, ensembleData *edata, ensembleMetaData *metadata, const int iterations, lossType loss_type, std::vector<Optimizer*> opts);
