...
void Fitter::fit_cpu(dataSet *dataset, ensembleData *edata, ensembleMetaData *metadata){
...
float Fitter::fit_sl_cpu(dataSet *dataset, const float* targets, ensembleData *edata, ensembleMetaData *metadata, const int iterations, lossType loss_type, std::vector<Optimizer*> opts){
    ...
    for (int i = 0; i < iterations; ++i){
        ...
        if (metadata->verbose > 0){
            std::cout << metadata->iteration << " - MultiRMSE: " << batch_loss << std::endl;
        }
        metadata->iteration++;
        ...
        if (to_split){
            ...
            #pragma omp parallel
            {
                ...
                for (int j = start_idx; j < end_idx; ++j) {
                    float score = crnt_node->getSplitScore(dataset, parent_score, metadata->split_score_func, split_candidates[j], metadata->min_data_in_leaf);
        ...
        #pragma omp parallel
        {
            ...
            for (int j = start_idx; j < end_idx; ++j) {
                ...
                for (int node_idx = 0; node_idx < (1 << depth); ++node_idx){
                    ...
                    score += crnt_node->getSplitScore(dataset, 0.0f, metadata->split_score_func, split_candidates[j], metadata->min_data_in_leaf);
                    if (score == -INFINITY)
                        break;
                }
                // std::cout << " cand: " <<  j << " score: " <<  score << " parent score: " <<  parent_score << " info: " << split_candidates[j] << std::endl;
