...
float TreeNode::getSplitScore(dataSet *dataset, const float parent_score, scoreFunc split_score_func, const splitCandidate &split_candidate, const int min_data_in_leaf){
    ...
    switch (split_score_func) {
        case L2: {
            if (is_numeric)
                return this->splitScoreL2(dataset->obs, dataset->build_grads, parent_score, split_candidate, min_data_in_leaf);
            else
                return this->splitScoreL2Categorical(dataset->categorical_obs, dataset->build_grads, parent_score, split_candidate, min_data_in_leaf);
        }
        case Cosine: {
            if (is_numeric)
                return this->splitScoreCosine(dataset->obs, dataset->build_grads, dataset->norm_grads, parent_score, split_candidate, min_data_in_leaf);
            else
...
float TreeNode::splitScoreCosine(const float *obs, const float *grads, const float *grads_norm, const float parent_score, const splitCandidate &split_candidate, const int min_data_in_leaf){
    int left_count = 0, right_count = 0;
    ...
    return left_cosine + right_cosine - parent_score;
}

float TreeNode::splitScoreCosineCategorical(const char *obs, const float *grads, const float *grads_norm, const float parent_score, const splitCandidate &split_candidate, const int min_data_in_leaf){
    int left_count = 0, right_count = 0;
...
float TreeNode::splitScoreL2(const float *obs, const float *grads, const float parent_score, const splitCandidate &split_candidate, const int min_data_in_leaf){
    int left_count = 0, right_count = 0;
    ...
    else if (left_count < min_data_in_leaf || right_count < min_data_in_leaf){
        delete[] left_mean;
        delete[] right_mean;
        return -INFINITY;
    } 
    ...
    delete[] left_mean;
    delete[] right_mean;
    return left_count_f*left_mean_norm + right_count_f*right_mean_norm - parent_score;
}

float TreeNode::splitScoreL2Categorical(const char *obs, const float *grads, const float parent_score, const splitCandidate &split_candidate, const int min_data_in_leaf){
    int left_count = 0, right_count = 0;
