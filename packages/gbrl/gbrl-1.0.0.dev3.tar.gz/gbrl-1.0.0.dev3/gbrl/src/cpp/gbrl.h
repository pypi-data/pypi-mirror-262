...
class GBRL {
    public:
        ...
        void fit(const float *obs, const char *categorical_obs, float *grads, const int n_samples, const int n_num_features, const int n_cat_features);
#ifdef USE_CUDA
        void _fit_gpu(dataSet *dataset);
        float _fit_sl_gpu(dataSet *datasett, float *targets, const int n_iterations);
#endif
        float fit_sl(float *obs, char *categorical_obs, float *targets, int iterations, const int n_samples, const int n_num_features, const int n_cat_features, bool shuffle = true, std::string _loss_type = "MultiRMSE");
