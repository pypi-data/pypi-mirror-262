...
PYBIND11_MODULE(gbrl_cpp, m) {
    ...
    gbrl.def("fit", [](GBRL &self, py::object &obs, py::object &categorical_obs, py::array_t<float> &grads) {
        ...
        py::gil_scoped_release release; 
        self.fit(obs_ptr, cat_obs_ptr, grads_ptr, n_samples, n_num_features, n_cat_features); 
    },  py::arg("obs"),
        py::arg("categorical_obs"),
        py::arg("grads"),
    "Fit a decision tree with the given observations and gradients");
    gbrl.def("fit_sl", [](GBRL &self, py::object &obs, py::object &categorical_obs, py::array_t<float> &targets, int iterations, bool shuffle, std::string loss_type) -> float {
