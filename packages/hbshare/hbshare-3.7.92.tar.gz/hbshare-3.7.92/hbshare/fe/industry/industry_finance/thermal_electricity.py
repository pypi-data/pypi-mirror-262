...
class IndustryFinanceData:
    ...
    def load(self):
        def read_stock_bs_ch_given_date(date):
            sql = "SELECT CompanyCode AS COMPANY_CODE, to_char(ReportDate, 'yyyyMMdd') AS END_DATE, to_char(PublishDate, 'yyyyMMdd') AS PUBLISH_DATE, ReportStyle AS REPORT_STYLE, AdjustYear AS ADJUST_YEAR, ReportUnit AS REPORT_UNIT, \
                   CBSheet46 AS ASSET, CBSheet77 AS LIABILITY, CBSheet5 AS DIVIDEND_PAYABLE, CBSheet21 AS CURRENT_ASSET,CBSheet29 FIXED_ASSET_INITIAL\
        ...
        def read_stock_is_ch_given_date(date):
            sql = "SELECT CompanyCode AS COMPANY_CODE, to_char(ReportDate, 'yyyyMMdd') AS END_DATE, to_char(PublishDate, 'yyyyMMdd') AS PUBLISH_DATE, ReportStyle AS REPORT_STYLE, AdjustYear AS ADJUST_YEAR, ReportUnit AS REPORT_UNIT, BeginDate as BEGIN_DATE, \
                   CINST65 AS TOTAL_OPERATION_COST, CINST1 AS OPERATING_INCOME, CINST3 AS OPERATING_COST,CINST24 AS NET_PROFIT \
        ...
        self.stock_bs = self.stock_bs[['TICKER_SYMBOL', 'END_DATE', 'ASSET', 'LIABILITY', 'DIVIDEND_PAYABLE', 'CURRENT_ASSET','FIXED_ASSET_INITIAL']]
        self.stock_is = self.stock_is[['TICKER_SYMBOL', 'END_DATE',  'TOTAL_OPERATION_COST' ,'OPERATING_INCOME', 'OPERATING_COST', 'NET_PROFIT']]
    ...
    def get_all(self):
        ...
        neg_market_value = self.stock_daily_k.pivot(index='TRADE_DATE', columns='TICKER_SYMBOL', values='NEG_MARKET_VALUE').sort_index()
        neg_market_value = neg_market_value[neg_market_value.index.isin(self.report_trade_df['TRADE_DATE'].unique().tolist())]
        neg_market_value.index = map(lambda x: x[:6] + '31' if x[4:6] == '03' or x[4:6] == '12' else x[:6] + '30', neg_market_value.index)
        neg_market_value = neg_market_value[neg_market_value.index<= self.report_date]

        neg_market_value = cal_ttm(neg_market_value)
        neg_market_value = neg_market_value.dropna(how='all', axis=0)
        neg_market_value = neg_market_value.divide(neg_market_value.sum(axis=1), axis=0)
        industry_hhi = pd.DataFrame((neg_market_value ** 2.0).sum(axis=1), columns=['HHI'])
        ...
        stock_oi = operating_income_ttm.dropna(how='all', axis=0)
        industry_oi = pd.DataFrame((stock_oi * stock_weight).dropna(how='all', axis=0).sum(axis=1), columns=['OI'])
        ...
        stock_margin = margin_ttm.dropna(how='all', axis=0)
        industry_margin = pd.DataFrame((stock_margin * stock_weight).dropna(how='all', axis=0).sum(axis=1),columns=['MARGIN'])
        ...
        stock_net_profit = net_profit_ttm.dropna(how='all', axis=0)
        industry_net_profit = pd.DataFrame((stock_net_profit * stock_weight).dropna(how='all', axis=0).sum(axis=1),columns=['NP'])
        ...
        #营业成本/总成本（季度）：营业成本TTM/总成本TTM
        # total_opc = self.stock_finance.pivot(index='END_DATE', columns='TICKER_SYMBOL',values='TOTAL_OPERATION_COST').sort_index()
        # total_opc_ttm = cal_ttm(total_opc)
        # opc = self.stock_finance.pivot(index='END_DATE', columns='TICKER_SYMBOL',values='OPERATING_COST').sort_index()
        # opc_ttm = cal_ttm(opc)
        # operation_cost_ratio = opc_ttm / total_opc_ttm
        # stock_operation_cost_ratio = operation_cost_ratio.dropna(how='all', axis=0)
        # industry_operation_cost_ratio = pd.DataFrame((stock_operation_cost_ratio * stock_weight).dropna(how='all', axis=0).sum(axis=1), columns=['OPERATION_COST_RATIO'])

        #营业成本增速
        opc = self.stock_finance.pivot(index='END_DATE', columns='TICKER_SYMBOL',values='OPERATING_COST').sort_index()
        opc_ttm = cal_ttm(opc)
        stock_costyoy = (opc_ttm - opc_ttm.shift(4)) / abs(opc_ttm.shift(4))
        stock_costyoy = stock_costyoy.dropna(how='all', axis=0)
        industry_costyoy = pd.DataFrame((stock_costyoy * stock_weight).dropna(how='all', axis=0).sum(axis=1), columns=['NP_YOY'])


        # 应付股利/ 流通股数
        dividend_payable = self.stock_finance.pivot(index='END_DATE', columns='TICKER_SYMBOL',values='DIVIDEND_PAYABLE').sort_index()
        # dividend_payable_ttm = cal_ttm(dividend_payable)

        stock_daily_k_temp = self.stock_daily_k[['TRADE_DATE','TICKER_SYMBOL','NEG_MARKET_VALUE','CLOSE_PRICE']]
        stock_daily_k_temp['neg_stock_volumn'] = stock_daily_k_temp.NEG_MARKET_VALUE / stock_daily_k_temp.CLOSE_PRICE
        neg_stock_volumn = stock_daily_k_temp.pivot(index='TRADE_DATE', columns='TICKER_SYMBOL', values='neg_stock_volumn').sort_index()
        neg_stock_volumn = neg_stock_volumn[neg_stock_volumn.index.isin(self.report_trade_df['TRADE_DATE'].unique().tolist())]
        neg_stock_volumn.index = map(lambda x: x[:6] + '31' if x[4:6] == '03' or x[4:6] == '12' else x[:6] + '30', neg_stock_volumn.index)
        neg_stock_volumn = neg_stock_volumn[neg_stock_volumn.index<= self.report_date]

        # net_profit = self.stock_finance.pivot(index='END_DATE', columns='TICKER_SYMBOL',values='NET_PROFIT').sort_index()
        # net_profit_ttm = cal_ttm(net_profit)
        bonus_ratio = dividend_payable / neg_stock_volumn
        stock_bonus_ratio = bonus_ratio.dropna(how='all', axis=0)
        industry_bonus_ratio = pd.DataFrame((stock_bonus_ratio * stock_weight).dropna(how='all', axis=0).sum(axis=1), columns=['BONUS_RATIO'])
        ...
        FIXED_ASSET_INITIAL1 = FIXED_ASSET_INITIAL1[['600011', '600021', '600023', '600027', '600795', '600863', '600886','601991']].diff()
        FIXED_ASSET_INITIAL2 = FIXED_ASSET_INITIAL[(FIXED_ASSET_INITIAL.month_day == '1231')]
        FIXED_ASSET_INITIAL2 = FIXED_ASSET_INITIAL2[['600011', '600021', '600023', '600027', '600795', '600863', '600886','601991']].diff()
        FIXED_ASSET_INITIAL3 = pd.concat([FIXED_ASSET_INITIAL1, FIXED_ASSET_INITIAL2])
        capital_exp = pd.merge(asset.reset_index()[['END_DATE']], FIXED_ASSET_INITIAL3, how='left',
                               on='END_DATE').set_index('END_DATE')
        ...
        # 写入excel
        industry_indic_name_list = ['行业集中度', '资产负债率', '营收增速','营业收入', '毛利增速','毛利润', '盈利增速','净利润', '营业成本增速', '应付股利 / 流通股数','资本开支增速','CAPEX / DA','经营现金流', '投资现金流', '融资现金流']
        industry_indic_data_list = [industry_hhi, industry_ltoa, industry_oiyoy,industry_oi, industry_myoy,industry_margin,industry_npyoy,industry_net_profit,
                                    industry_costyoy,industry_bonus_ratio,industry_rdexpyoy,industry_capextoda,
                                    industry_cfo, industry_cfi, industry_cff]
        stock_indic_name_list = ['资产负债率', '营收增速','营业收入', '毛利增速','毛利润', '盈利增速','净利润', '营业成本增速', '应付股利 / 流通股数','资本开支增速','CAPEX / DA','经营现金流', '投资现金流', '融资现金流']
        stock_indic_data_list = [stock_ltoa, stock_oiyoy,stock_oi, stock_myoy,stock_margin,stock_npyoy,stock_net_profit,
                                 stock_costyoy,stock_bonus_ratio,stock_rdexpyoy,stock_capextoda,
