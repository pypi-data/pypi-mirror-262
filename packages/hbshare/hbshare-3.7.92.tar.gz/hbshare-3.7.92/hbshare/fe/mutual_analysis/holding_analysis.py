...
def fund_valuation(date, fund_zc_holding, fund_zc_holding_diff):
    ...
    fund_aum_last_date = fund_aum_last_date[fund_aum_last_date['bblb'] == 13]
...
class holding_analysis_v2:
    ...
    def load(self):
        ...
        for idx, td in enumerate(self.report_dates):
            ...
            fund_aum_date = fund_aum_date if len(fund_aum_date) > 0 else pd.DataFrame(columns=['zcjz', 'bblb', 'jsrq', 'jjdm', 'ggrq'])
            fund_aum_date = fund_aum_date[fund_aum_date['bblb'] == 13]
            fund_aum_date = fund_aum_date.sort_values(['jjdm', 'jsrq', 'ggrq']).drop_duplicates(['jjdm', 'jsrq'], keep='last')
