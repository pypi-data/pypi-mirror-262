...
class BinanceDataProvider:
    ...
    async def update_tickers(self, pairs: [str], timeframes: [str], fallback_starting_date: datetime = None):
        ...
        if fallback_starting_date is None:
            fallback_starting_date = datetime.combine(date.today() - relativedelta(years=2), datetime.min.time()).replace(tzinfo=UTC)
        else:
        ...
        for timeframe in timeframes:
            for pair in pairs:
                ...
                task = (asyncio.create_task(
                    self.data_downloader.download_one_ticker(ticker, download_from, today_datetime, timeframe, tqdm_position=len(tasks)-1)))
                tasks.append(task)
...
class BinanceDataDownloader:
    ...
    async def download_and_process(self, session, url: str, ticker: str, date_of_cycle: datetime, is_csv=True, tqdm_position=0):
            ...
            try:
                ...
                if self.pbars[tqdm_position] is not None:
                    if self.__minimum_achieved_date is not None and (
                        ...
                        self.pbars[tqdm_position].write(f"A hole in the cycle has been found, minimum achieved date is {self.__minimum_achieved_date}, url: {url.split('/klines')[1]}, {e}")

                    self.pbars[tqdm_position].write(f"Error({retry_count}/{self.max_retry_count} retries): {url.split('/klines')[1]}, {e}")
                    self.pbars[tqdm_position].update(1)
    ...
    async def download_one_ticker(self, ticker, start_date: datetime, end_date: datetime, timeframe, spot=False, max_retry_count=5, tqdm_position=0):
        ...
        async with httpx.AsyncClient() as session:
            ...
            while current_date < end_date_plus_one:
                ...
                if current_date.date() >= date.today() - timedelta(days=1):
                    ...
                    task = asyncio.create_task(self.download_and_process(session, url, ticker, current_date, False, tqdm_position))
                    tasks.append(task)
                    ...
                    task = asyncio.create_task(self.download_and_process(session, url, ticker, current_date, True, tqdm_position))
                    tasks.append(task)
