...
def test_missing_scenario():
    rp = RP(
        "tests/test_rest_playbook/no_scenario.playbook",
        "",
        "tests/test_rest_playbook/file_test.vars"
    )
    results = rp.run_playbook()
...
def test_missing_playbook():
    rp = RP(
        "",
        "tests/test_rest_playbook/simple_test.scenario",
        "tests/test_rest_playbook/file_test.vars"
    )
    results = rp.run_playbook()
    assert len(results) == 0
...
def test_success_playbook():
    rp = RP(
        "tests/test_rest_playbook/simple.playbook",
        "tests/test_rest_playbook/simple_test.scenario",
        "tests/test_rest_playbook/file_test.vars"
    )
    results = rp.run_playbook()
