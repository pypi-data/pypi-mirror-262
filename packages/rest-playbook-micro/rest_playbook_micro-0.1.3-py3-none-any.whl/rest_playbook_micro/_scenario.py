...
from rest_client_micro import Response as Resp
from rest_client_micro import BasicAuth as BA
...
class Scenario():
    ...
    def run_scenario(self, variables: V, return_body=False) -> R:
        ...
        if self.properties.get('AUTH', False):
            auth = self._build_auth(self.properties.get('AUTH'))
            if isinstance(auth, R):
                return auth
            rest_object.basic_auth = auth
    ...
    def _build_auth(self, auth_file: str) -> BA | R:
        username, password = self.u.parse_auth_file(auth_file)
        if username is None:
            return R(response="Invalid authentication file")
        return BA(username, password)
