...
from .common import image_height, image_width, logger, logging_decorator, \
                    colors, settings, params_from_filename
...
def analyse_image(filepath, outputfilepath, variables=None, open_mode="a+"):
    ...
    if variables is None:
        ...
        variables = ['heart', 'slice_nr', 'section', 'resolution', 'abblation_state',
                    'segment_type', 'segment_nr',
                    'red_pixels','yellow_pixels','tissue_pixels','fibrosis_perc',
                    'edge_counter','edge_count_min','edge_count_max','edge_score',
                    'pixarea','tissue_mm2','VM_mm2','fibrosis_mm2',
                    'pixlength','border_mm']
    ...
    with Image.open(filepath) as im:
        data_heart['red_pixels'], data_heart['yellow_pixels'], \
            data_heart['edge_counter'] = count_pixels_and_edges(im)

    # calculate values to export.
    data_heart['tissue_pixels'] = data_heart['red_pixels'] + data_heart['yellow_pixels']
    try:
        data_heart['fibrosis_perc'] = data_heart['red_pixels'] / data_heart['tissue_pixels'] * 100
    ...
    with open(outputfilepath, open_mode, encoding='utf-8') as fh:
        ...
        if not fh.readline() == header:
            # ...if not add it.
            logger.info(f"...adding header to output file {os.path.split(outputfilepath)[1]}")
            fh.write(header)
...
@logging_decorator
def counter_sc2(directory: str|None=None, outputfilename: str='output.csv',
                file_per_folder=False, analyze_models=None):
    """
            ...
            if directory in analyze_models:
                ...
                # check if there is an outputfilename file
                    if os.path.exists(outputfile):
                        ...
                        logger.info(f"Skipped directory - {outputfile} already exists\n")
                        continue
                else:
