...
def cag_bullseye_plot(plot_type, fig, ax, data, value_type,
					  border=None, cmap=None, norm=None, label=None,
					  norm_border=None, title=None):
	"""
	...
	cmap = get_cmap_by_valuetype(value_type,
							  norm_range=[min(data), max(data)])
	...
	# based on: https://matplotlib.org/gallery/pie_and_polar_charts/
		...
		ax.bar(x = x,
				...
				color = [cmap(norm(value)) for value in plot_data[i]],  # TODO: check if corect
...
def create_bullseye_plot(df, heart, plot_type, value_type):
	...
	if settings['save_diagram_to_file']:
		savefile = os.path.join(settings['diagram_outputfolder'],
						  f"{heart}_{value_type}_{plot_type}.png")
...
if __name__ == '__main__':
	...
	if False:
		...
		folder = os.path.join(os.path.split(settings['diagram_inputfile'])[0], settings['diagram_outputfolder'])
