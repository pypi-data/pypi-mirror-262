...
def test_default_not_implement(func_base, func_name):
    """
    Importing dvcx.sql.functions.path should register a custom compiler
...
def test_parent(warehouse: "AbstractWarehouse", path):
...
@pytest.mark.parametrize("path", PATHS)
def test_name(warehouse: "AbstractWarehouse", path):
...
def test_file_stem(warehouse: "AbstractWarehouse", path):
...
@pytest.mark.parametrize("path", EXT_PATHS)
def test_file_ext(warehouse: "AbstractWarehouse", path):
