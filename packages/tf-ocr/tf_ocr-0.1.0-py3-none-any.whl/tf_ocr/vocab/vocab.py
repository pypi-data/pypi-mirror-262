...
NUM2CHAR = hk.Thunk(num2char)

@overload
def vectorize(chars: Shaped[tf.RaggedTensor, "*n"], char2num: StringLookup = None) -> Int32[tf.RaggedTensor, "*n"]: ...
@overload
def vectorize(chars: Shaped[tf.Tensor, "*n"], char2num: StringLookup = None) -> Int32[tf.Tensor, "*n"]: ...
def vectorize(chars: Shaped[tf.Tensor, "*n"], char2num: StringLookup = None) -> Int32[tf.Tensor, "*n"]:
  """Vectorize a string tensor using a `StringLookup`. Defaults to the original vocabulary"""
  f = char2num or CHAR2NUM()
  return f(chars)

@overload
def stringify(indices: Int[tf.RaggedTensor, "*n"], num2char: StringLookup = None) -> Shaped[tf.RaggedTensor, "*n"]: ...
@overload
def stringify(indices: Int[tf.Tensor, "*n"], num2char: StringLookup = None) -> Shaped[tf.Tensor, "*n"]: ...
def stringify(indices: Int[tf.Tensor, "*n"], num2char: StringLookup = None) -> Shaped[tf.Tensor, "*n"]:
  """Stringify tensor of vocabulary indices using an inverted `StringLookup`. Defaults to the original vocabulary"""
  f = num2char or NUM2CHAR()
  return f(indices)