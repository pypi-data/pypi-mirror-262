...
class FovusApiAdapter:
        ...
        for _, monolithic_list_item in enumerate(copy.deepcopy(request[ENVIRONMENT][MONOLITHIC_LIST])):
            ...
            for license_info in license_list:
                if (
                    ...
                    # check for vendor
                    license_relation_from_vendor = self._get_software_license_relationship(
                    ...
                    # check for software
                    license_relation_from_software = self._get_software_license_relationship(
            ...
            if not is_license_valid:
                raise UserException(
                    HTTPStatus.BAD_REQUEST,
                    self.__class__.__name__,
                    f"licenseAddress or licenseName is incorrect for the software '{software_name}'",
                )
    ...
    def _get_software_license_relationship(
        self, software_license_relationships, vendor_name, license_id, software_name=None
    ) -> dict:
