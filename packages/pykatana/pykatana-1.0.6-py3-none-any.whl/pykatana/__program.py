...
def _init():
    ...
    if os.path.exists("./src"):
        ...
        try:
            ...
            if os.path.exists(os.path.join(shipath, "pykatana")):
                shipath = os.path.join(shipath, "pykatana")
        except:
            shipath = os.path.abspath(
                __file__.replace("run.py", "").replace("__program.py", "")
            )
        shutil.copytree(os.path.join(shipath, "_pykatana_init/src"), "./src")
        os.makedirs("./src/meta", exist_ok=True)
        os.makedirs("./archive", exist_ok=True)
        os.makedirs("./build", exist_ok=True)
        if not os.path.exists("./.gitignore"):
            shutil.copyfile(
                os.path.join(shipath, "_pykatana_init/gitignore"), "./.gitignore"
            )
        print("""[Done]""")
