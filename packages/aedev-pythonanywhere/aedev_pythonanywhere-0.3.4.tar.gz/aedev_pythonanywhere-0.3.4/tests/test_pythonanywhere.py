...
class TestHostRunningOnlyLocallyWithPatchedSkipper:
    ...
    def test_deployed_code_files(self, con_pkg):
        ...
        assert con_pkg.error_message == ""
        assert found_paths == set()                         # media and media_ini folders filtered out

        found_paths = con_pkg.deployed_code_files(['*/**/migrations/*'])
        assert con_pkg.error_message == ""
        assert found_paths == set()                         # migrations folders get filtered out by web_file_exclude()

        found_paths = con_pkg.deployed_code_files(['**/migrations/' + mig_file_name])
        assert con_pkg.error_message == ""
        assert found_paths == set()

        found_paths = con_pkg.deployed_code_files(['**/static/*'], lean=True)
        assert con_pkg.error_message == ""
        assert found_paths == {pkg2_static_file}            # static_ini_file + pkg0_static_file get excluded
...
class TestPythonAnywhereWithoutConnection:
    def test_web_file_exclude(self):
        instance = PythonanywhereApi("any domain", "any user", "any token", TEST_PROJECT_NAME)
