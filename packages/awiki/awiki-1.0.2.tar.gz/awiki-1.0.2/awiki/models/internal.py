...
class InterfaceModel:
    ...
    def _from_json(cls: ModelT, data: dict) -> ModelT:
        logging.debug(
            f"BEGIN DESERIALIZATION [cls {cls.__name__}] FROM [keys {list(data or {})}]"
        )
        ...
        for m_name, m_type in get_annotations(cls).items():
            ...
            print(typ, issubclass(typ, InterfaceModel))
            ...
            try:
                value = data.get(m_name) if primary_is_option else data[m_name]
