...
class OIDCProvider(OAuth2Provider):
    ...
    def build_identity(self, state):
        ...
        if self.email_verified(user_info.get("email"), bearer_token):
            return {
                "id": user_id,
        ...
        logger.error("Email not in group: %s" % user_info.get("email"))
        return {}

    def email_verified(self, email, bearer_token):
        r = requests.get(AUTH_URL + "/users?groups.project="+ PROJECT,
            headers={"X-Auth-project":PROJECT,"X-Access-token": bearer_token})
        for retry in range(10):
            if 10 < retry:
                return {}
            if not str(r.status_code).startswith("20"):
                ...
                return any(i["email"] == email for i in r.json()['items'])