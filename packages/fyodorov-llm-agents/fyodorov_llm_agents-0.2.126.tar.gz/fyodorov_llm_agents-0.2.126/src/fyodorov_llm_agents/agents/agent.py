...
class Agent(BaseModel):
    ...
    def call_with_fn_calling(self, input: str = "", history = []) -> dict:
        ...
        if tools and litellm.supports_function_calling(model=model):
            print(f"calling litellm with model {self.model}, tools: {tools}, messages: {messages}, max_retries: 0, history: {history}, base_url: {self.api_url}")
            ...
            print(f"calling litellm with model {self.model}, messages: {messages}, max_retries: 0, history: {history}, base_url: {self.api_url}")
