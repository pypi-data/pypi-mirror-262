...
columns = ['종류', '종목코드', '종목명', '기준년월', '매출액', '영업이익', '영업이익_발표기준', '세전계속사업이익',  
           '당기순이익',  '당기순이익_지배',
            '당기순이익_비지배', '자산총계', '부채총계', '자본총계', '자본총_지배', '자본총계_비지배',
            '자본금', '영업활동현금흐름', '투자활동현금흐름', '재무활동현금흐름', 'CAPEX', 'FCF', '이자발생부채',
            '영업이익률', '순이익률', 'ROE', 'ROA', '부채비율', '자본유보율', 'EPS', 'PER',
            'BPS', 'PBR', '현금DPS', '현금배당수익률', '현금배당성향', '주식수_보통주']

def GetFinInfoNaver(srchItem='', rptGb='연간'):
    data = []
    shCodeList = []
    ...
    return pd.DataFrame(data, columns = columns).astype({ '매출액':'float', '영업이익':'float', '영업이익_발표기준':'float', 
            '세전계속사업이익':'float', '당기순이익':'float',  '당기순이익_지배':'float', '당기순이익_비지배':'float', '자산총계':'float',
            '부채총계':'float', '자본총계':'float', '자본총_지배':'float', '자본총계_비지배':'float', '자본금':'float', '영업활동현금흐름':'float',
            ...
            'BPS':'float', 'PBR':'float', '현금DPS':'float', '현금배당수익률':'float', '현금배당성향':'float', '주식수_보통주':'int64' })
# acntInfo['columns'])
...
def GetCashFlowInfo(srchItem='', unit="억"):   
