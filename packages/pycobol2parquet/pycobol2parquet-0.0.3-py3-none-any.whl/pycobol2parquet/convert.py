...
def convert_cobol_file(
    copybook_filename, data_filename, output_filename, codepage, debug=False
):
    """
    Conver cobol file to parquet file
    Make sure the source Cobol file is in a reasonable size
    We don't need the config file for csv format any mpre
    """
    ...
    result_table = pa.table([pa.array(item["data"]) for item in cobol_struc],names=headers)
    pq.write_table(result_table, output_filename)