...
class DfSave(object):
    ...
    def df2sql(cls, df, tablename, connector, db, table_prefix='', folder_path=None, exampletable=None, exampledb=None,
               auto_incre_col=False, rm_csv=False):
        """
        ...
        DfSave.csv2mysql(engine, para, tablename, csv_path, columns=cols, header=True, table_prefix=table_prefix, db=db,
                         exampletable=exampletable, exampledb=exampledb, auto_incre_col=auto_incre_col)
        if rm_csv:
            os.remove(csv_path)
        engine.dispose()
...
class ConnectMysql(object):
    ...
    def df2sql(self, df, table, db=None, csv_store_path='./data/', auto_incre_col=False, rm_csv=False):
        ...
        DfSave.df2sql(df,
                      ...
                      exampletable=None,
                      exampledb=db,
                      auto_incre_col=auto_incre_col, rm_csv=rm_csv)
    ...
    def _SelfConnect(self):
        ...
        conn = pymysql.connect(host=self._para.host, port=self._para.port, user=self._para.user,
                               ...
                               charset=self._para.charset, db=self._para.db)
        return conn
...
class MySQLNode(MysqlConnEnforcePandas):
    def __init__(self, src: str, mysql_name='mysql', ):
        result = parse_rfc1738_args(src)
        ...
        charset = 'UTF8'
        db = result['database']
        super(MySQLNode, self).__init__(mysql_name, host, port, user, passwd, charset, db)
        self.MysqlCommands = MysqlCommands
        self.__name__ = self.__class__.__name__
