...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    ##################
        ...
        label_plot = os.path.join(plot_folder, f"frame_{now.strftime('%Y-%m-%d_%H')}{frame_suffix}.jpg")
        file_path_data_csv = os.path.join(data_folder, f"frame_{now.strftime('%Y-%m-%d_%H')}{frame_suffix}.csv")
       ...
       #######################
        ...
        column_names = ['frame_datetimes', "frame_numbers","std_frames", "min_displacements", "max_displacements", "mean_displacement", "shift_list_x", "shift_list_y", "latency_times"]

        # Creating a DataFrame
        df = pd.DataFrame(list(zip(frame_datetimes, frame_numbers,std_frames, min_displacements, max_displacements, mean_displacement, shift_list_x, shift_list_y, latency_times)), columns=column_names)
