...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    while True:
        ...
        detected_shift, error, diffphase = phase_cross_correlation(prev_frame, frame_gray, upsample_factor = 20, reference_mask=ref_mask )
       ...
       # Create a figure and two subplots for min and max displacements
        ...
        ax1.plot(datetimes, std_frames, label='Standard-Deviation', color='green', linestyle='--', marker='x')
        ...
        ax3 = ax1.twinx()
        color = 'tab:blue'
        ax3.plot(datetimes, shift_list, color=color, label='Image-Shift', linestyle='--', marker='*')
        ax3.tick_params(axis='y', labelcolor=color)
        ax3.set_ylabel('Image-Shift', fontsize=12, fontweight='bold', color=color)
       ...
       # Pause the Python program for 2 seconds
        ...
        if mask_path.exists==True:
            p1=draw_circles_on_image(poly_aoi, combined_frame)
            p2=draw_circles_on_image(poly_aoi, combined_mag1)
        else:  
