...
def measure_displacement_from_camera(hls_url, alpha=0.1, save_output=False, output_filename=None, ssim_threshold=0.4, 
    ...
    while True:
        ...
        # Example actions, using the strtime for naming
        # label_plot = plot_folder + "/" + strtime + '.png'
        # file_path_data_csv = data_folder + "/" + strtime + ".csv"
    ...
    ##################
        ...
        #     label_plot = os.path.join(plot_folder, f"frame_{frame_suffix}.jpg")
        #     file_path_data_csv = os.path.join(data_folder, f"frame_{frame_suffix}.csv")
        
         # Perform histogram matching
        #frame= match_histograms(frame, prev_frame_init, channel_axis=-1)
        ...
        ref_mask = ref_mask if mask_path.exists() else None

        # pixel precision first
        detected_shift, error, diffphase = phase_cross_correlation(prev_frame, frame_gray, upsample_factor = 20 )
        ...
        # Step 3: Create the CLAHE object
        # clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(3, 3))

        # # Step 4: Apply the CLAHE filter to the grayscale image
        # prev_frame = clahe.apply(prev_frame)
        
        prev_frame = cv2.normalize(prev_frame, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        ...
        bottom_right = (min(max_loc[0] + radius, magnitude.shape[1] - 1), min(max_loc[1] + radius, magnitude.shape[0] - 1))
        
        # if ssim_threshold is not None:
        #     vel_2d[ssim_map > ssim_threshold]=np.mean(vel_2d)

        # Crop the mask
        window = magnitude[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
        
        
        
        vel_2d_crop=vel_2d[top_left[1]:bottom_right[1], top_left[0]:bottom_right[0]]
        ...
        vel_2d_crop=replace_outliers_with_nan_fixed(vel_2d_crop)
        ##########
        ...
        if mask_path.exists():
            ...
            bool_mask = masked_poly.astype(bool)
            window = magnitude.copy()  # Copy the array to avoid modifying the original
            if ssim_threshold is not None:
                window[ssim_map > ssim_threshold]=0
            window[~bool_mask] = 0  # Zero out elements where the mask is False
            vel_2d_crop=window
            vel_2d_crop=replace_outliers_with_nan_fixed(vel_2d_crop)
        ...
        if alpha is not None:
            ...
            frame_gray = frame_gray.astype('uint8')  # Convert frame_gray to uint8 if it's not already
            background_mask = background_mask.astype('uint8')  # Convert background_mask to uint8 if it's not already
        ...
        ax.imshow(frame_gray_crop)
        # Improved imshow with a professional color map
        cax = ax.imshow(vel_2d_crop, cmap='hsv', alpha=0.6, vmin=-1, vmax=1)  

        # Adding a color bar with a label
        cbar = fig.colorbar(cax, ax=ax)
        cbar.set_label('Displacement (pixels)',  rotation=270, labelpad=15, fontsize=14, fontweight='bold')

        # Adding titles and labels
        ax.set_title('Displacement AOI', fontsize=14, fontweight='bold')
        
        # Enhancing the aesthetics of the tick labels (optional, based on your data)
        ax.tick_params(axis='both', which='major', labelsize=10)

        # Optionally, you can add grid lines if it helps in interpreting the data
        ...
        fig.savefig('window.png', dpi=300)
        
        try:   
            plt_img=cv2.imread('window.png')
        ...
        # norm = plt.Normalize(vmin=vel_2d_crop.min(), vmax=vel_2d_crop.max())
        ...
        # ticks = [vel_2d_crop.min(), vel_2d_crop.max()/2, vel_2d_crop.max()/4, vel_2d_crop.max()/6, vel_2d_crop.max()]
        # cb.set_ticks(ticks)
        # plt.axis('off')
        ...
        try:
            ...
            colorbar = cv2.resize(plt_img, (frame_width // 2  , frame_height))
            # Update the last successful colorbar
            last_successful_colorbar  = colorbar
        except Exception as e:
            print("Resize operation failed:", str(e))
            if last_successful_colorbar  is not None:
                colorbar = last_successful_colorbar 
            else:
       ...
       # Create a figure and two subplots for min and max displacements
        ...
        # # Create inset of the bounded region of the sine wave
        # ax_inset = fig2.add_axes([0.5, 0.35, 0.2, 0.8]) # [x, y, width, height] in figure coordinate
        # w=ax_inset.imshow(vel_2d_crop, cmap='hot')
        # ax_inset.set_title('AOI Displacement')
        
        # # Creating colorbar axes manually to match the height of ax_inset
        # cbaxes = fig2.add_axes([0.4, 0.5, 0.03, 0.3]) # Adjust the location and size as needed to align with ax_inset
        # plt.colorbar(w, cax=cbaxes)
       ...
       #######################
        ...
        combined_frame=np.hstack((frame_init, colorbar))
