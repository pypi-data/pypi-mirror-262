...
def create_user(email, password, username=""):
    ...
    with session_scope() as session:
        if (
            config.get("USERNAME")
            and session.query(User).filter_by(username=username).first()
        ):
            raise exceptions.UserNameTaken
        if (
            not config.get("USERNAME")
            and session.query(User).filter_by(email=email).first()
        ):
...
def authenticate(**kwargs):
    ...
    with session_scope() as session:
        ...
        if not user or not security.verify(kwargs["password"], user.salt, user.pwhash):
...
def login(**kwargs):
    ...
    with session_scope() as session:
        ...
        presession_record = (
            session.query(Session).filter_by(csrf_token=presession_csrf_token).first()
        )
...
def logout(request, response):
    ...
    with session_scope() as session:
        sid = session.query(Session).filter_by(session_token=hashed).first()
        logger.info(
...
def validate_CSRF(request):
    ...
    try:
        ...
        with session_scope() as sql_session:
            record = sql_session.query(Session).filter_by(csrf_token=token).first()
