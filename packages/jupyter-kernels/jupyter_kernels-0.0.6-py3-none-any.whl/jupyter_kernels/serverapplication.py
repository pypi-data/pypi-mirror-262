...
class JupyterKernelsExtensionApp(ExtensionAppJinjaMixin, ExtensionApp):
    ...
    jupyter_server_http_url = Unicode(
        "https://dev1.datalayer.tech",
        config=True,
        help="Cloud jupyter server base URL."
    )

    jupyter_server_ws_url = Unicode(
        "wss://dev1.datalayer.tech",
        config=True,
        help="Cloud jupyter server base Websocket URL."
    )

    operator_server = Unicode(
        "https://dev1.datalayer.tech",
        config=True,
        help="Cloud operator server."
    )

    server_name = Unicode(
        "default",
        config=True,
        help="User server name"
    )
