...
class QuarkProxy(object):
    ...
    def __init__(self) -> None:
        from .app import connect, sql

        self.router = {}
        for chip, host in startup.get('quarkproxy', {}):
            self.router[chip] = connect('QuarkServer', host=host)
    ...
    def run(self, task: dict):
        ...
        cloud = {'metainfo': {'name': f'testtask:/{task["name"]}',
                              'priority': 15,
                              'mutex': True,
                              'other': {'shots': 1024,
                                        'signal': 'count',
        ...
        backend = self.router.get(chip, '')
        if backend:
            t: Task = submit(cloud, block=False, backend=backend)
        else:
            t: Task = submit(cloud, block=False)  # local machine
    ...
    def result(self, tid: int):
        ...
        try:
            ...
            return record  # get_data_by_tid(tid, 'count')
        except Exception as e:
