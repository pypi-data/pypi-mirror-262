...
class SerpapiSearch(KomodoTool):
    ...
    def __init__(self):
        super().__init__(shortcode=self.shortcode,
                         ...
                         action=self.action)
        self.api_key = self.get_serp_api_key()
        if not self.api_key:
            print_warning("No SerpAPI key found. Please set the environment variable SERP_API_KEY")
...
if __name__ == "__main__":
    ...
    print(tool.action({"query": "Nvidia news"}))
