...
from komodo.models.framework.assistant import AssistantRequest, AssistantResponse, build_context
...
def run_appliance_streamed(appliance, prompt):
    agent = coordinator_agent(appliance)
    for response in run_agent_streamed(appliance, agent, prompt):
        yield response


def build_request(appliance, agent, history, prompt):
    user = KomodoUser.default_user().to_dict()
    assistant = agent.to_dict()
    assistant['tools'] = agent.tools + appliance.tools
    context = build_context(appliance.generate_context()) + build_context(agent.generate_context())
    request = AssistantRequest(user=user, assistant=assistant, prompt=prompt, context=context, history=history)
    return request


def run_agent(appliance, agent, prompt, history=None) -> AssistantResponse:
    request = build_request(appliance, agent, history, prompt)
