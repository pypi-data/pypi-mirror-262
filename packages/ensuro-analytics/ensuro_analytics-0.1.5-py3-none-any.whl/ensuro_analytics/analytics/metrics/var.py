...
def current_value(
    ...
    **kwargs,
    ...
    Given time_window days and a time_col column, the function draws n_simulation samples of policies of sizes all equal to the currently active ones, with all samples being made of policies that are within time_window days from each other.
    The function then computes the loss to exposure for each sample and returns the level-th percentile of the loss to exposure distribution.
    ...
    ), "Can not compute VaR with the number of currently active policies: not enough historical data"
...
def at_t(
    ...
    **kwargs,
    ...
    Given time_window days and a time_col column, the function draws n_simulation samples of policies of sizes all equal to the ones active at the given date, with all samples being made of policies that are within time_window days from each other.
    The function then computes the loss to exposure for each sample and returns the level-th percentile of the loss to exposure distribution.
    :param data: DataFrame with the data.
    ...
    assert (
        _data.shape[0] > n_policies
    ), "Can not compute VaR with the number of active policies at the given date: not enough historical data"
