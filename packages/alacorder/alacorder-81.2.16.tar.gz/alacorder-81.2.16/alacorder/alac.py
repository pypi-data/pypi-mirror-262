...
def read(
    ...
    if isinstance(source, list):
        ...
        with progress_bar as bar:
            for path_ in bar.track(source, description="Reading PDFs…"):
                all_text += [extract_text(path_)]
        output = pl.DataFrame(
    ...
    def enforcement(self: "Cases", *, debug: bool = False) -> pl.DataFrame:
        ...
        with console.status("Parsing enforcement…"):
            df = (
                ...
                .with_columns(
                    pl.when(
                        pl.col("OverUnderPaid").list.len() > pl.col("payor_count")
                    )
                    .then(pl.col("OverUnderPaid").list.head(pl.col("payor_count")))
                    .otherwise(pl.col("OverUnderPaid"))
                    .alias("OverUnderPaid"),
                    pl.when(
                        pl.col("WarrantMailer").list.len() != pl.col("payor_count")
                    )
                    .then(
                    ...
                    .alias("WarrantMailer"),
                    pl.when(
                        pl.col("TurnOverDate").list.len() != pl.col("payor_count")
                    )
                    .then(
                        pl.col("AllPagesText").str.extract_all(
                    ...
                    .alias("TurnOverDate"),
                    pl.when(
                        pl.col("PreTrialDate").list.len() != pl.col("payor_count")
                    )
                    .then(
                        pl.col("AllPagesText").str.extract_all(
                            r"(?s)D999[^\n]+?\n\s*[^\n]+PreTrail Date:([^\n]+)"
                    ...
                    .alias("DelinquentDate"),
                    pl.when(
                        pl.col("WarrantMailerDate").list.len()
                        != pl.col("payor_count")
                    )
                    .then(
                        pl.col("AllPagesText").str.extract_all(
                    ...
                    .alias("Frequency"),
                    pl.when(
                        pl.col("TurnOverAmt").list.len() > pl.col("payor_count")
                    )
                    .then(
                    ...
                    .alias("TurnOverAmt"),
                    pl.when(
                        pl.col("PreTrialTerms").list.len() != pl.col("payor_count")
                    )
                    .then(
                        pl.col("AllPagesText").str.extract_all(
                            r"(?s)D999[^\n]+?\n\s*[^\n]+PreTrial Terms([^\n]+)"
                    ...
                    .alias("DAMailer"),
                    pl.when(
                        pl.col("LastUpdate").list.len() != pl.col("payor_count")
                    )
                    .then(
                        pl.col("AllPagesText").str.extract_all(
                            r"(?s)DA Mailer Date:[^\n]+?\n\s*[^\n]+Last"
                    ...
                    .alias("LastUpdate"),
                    pl.when(
                        pl.col("DAMailerDate").list.len() != pl.col("payor_count")
                    )
                    .then(
