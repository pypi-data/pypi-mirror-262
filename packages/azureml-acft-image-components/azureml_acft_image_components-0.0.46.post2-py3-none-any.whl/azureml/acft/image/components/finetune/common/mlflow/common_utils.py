...
from azureml.acft.image.components.finetune.common.mlflow.common_constants import DistributedConstants
...
def get_current_device() -> torch.device:
    ...
    if torch.cuda.is_available():
        if os.environ.get(DistributedConstants.LOCAL_RANK) is None:
            msg = "LOCAL_RANK parameter is missing from settings in distributed mode."
            logger.error(f"{msg}")
            raise ACFTSystemException(msg, has_pii=False)
