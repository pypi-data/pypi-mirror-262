...
def make_tech_plot(ticker, df, tech_type, TECH_LIST, idx):
    ...
    if tech_type in ['sma','ema','macd','adx','ppo','psar','ichmoku','stoch','roc','ao',
        ...
        current_slope = find_current_slope(_df)

        # Here is Buttom, 분할매수 시작, slope 이 마이너스(-)에서 0 로 전환된 날 리포트 : 2
        _df['pivot'] = np.where((current_slope <= abs(permit_range)) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
                                _df['SellAndBuy'] < 0, 2, 0)
        # 분할매수 끝, slope 이 플러스(+) 인 상태에서, 이동평균값이 기준선 0 위로 전환된 날 리포트 : 1 
        _df['pivot'] = np.where((current_slope > permit_range) & (_df['SellAndBuy'].shift(1) < _df['SellAndBuy']) & \
        ...
        _df['pivot'] = np.where((current_slope < permit_range*(-1)) & (_df['SellAndBuy'].shift(1) > _df['SellAndBuy']) & \
                                _df['SellAndBuy'] > 0, -1, _df['pivot'])
    ...
    return latest_date, result, current_slope
