...
def eval_duplicate_removal(df, **df_options):
    ...
    sort_ascending = True if keep_which == 'first' else False
    if sort_field is None:
        df = df.drop_duplicates(duplicate_expr, keep=keep_which)
    else:
        df = df.sort_values(sort_field, ascending=sort_ascending).drop_duplicates(duplicate_expr, keep=keep_which)
    return df
