...
from miniworldmaker_physics import (
    physics_board_connector as physics_board_connector_mod,
)
...
class PhysicsBoardEventManager(board_event_manager.BoardEventManager):
    ...
    def setup_event_list(cls):
        ...
        for token_cls in token_class_inspection.TokenClassInspection(
            token_mod.Token
        ...
        for token_cls in token_class_inspection.TokenClassInspection(
            ...
            separation_token_methods.append(
                "on_separation_from_" + token_cls.__name__.lower()
            )
    ...
    def register_event(self, member, instance):
        ...
        if member.startswith("on_touching_"):
            connector = physics_board_connector_mod.PhysicsBoardConnector(
                self.board, instance
            )
