...
from miniworldmaker_physics import physicsboard_event_manager
from miniworldmaker_physics import physics_board_connector
...
class PhysicsBoard(miniworldmaker.Board):
    ...
    def get_physics_collision_methods_for_token(self, token):
        ...
        return [ getattr(token, method_name) for method_name in methods  if hasattr(token, method_name) and callable(getattr(token, method_name))]
    ...
    def simulate_all_physics_tokens(self):
        ...
        * Convert pymunk position/direction to miniworldmaker position/direction
        ...
        if len(self.physics_tokens) > 0:
            ...
            [
                token.physics._simulation_preprocess_token()
            ...
            # post-process
            [
                token.physics._simulation_postprocess_token()
                for token in self.physics_tokens
            ]
    ...
    def pymunk_touching_collision_listener(self, arbiter, space, data):
        ...
        for method in self.touching_methods:
            ...
            token_inspection.TokenInspection(method.__self__).get_and_call_method(
    ...
    def connect(self, token1, token2) -> "shapes_mod.Line":
        l = miniworldmaker.Line(token1.center, token2.center)
