...
class PhysicsBoardPositionManager(pixel_position_manager.PixelBoardPositionManager):
    ...
    def set_position(
        ...
        if (
            hasattr(self.token, "physics")
            and self.token.physics.has_physics
            and not self.token.physics._is_in_update_mode()
            and value != pos
        ):
    ...
    def set_velocity(self, value):
        self.token.physics.velocity_x, self.token.physics.velocity_y = (
            value[0],
            value[1],
        )
