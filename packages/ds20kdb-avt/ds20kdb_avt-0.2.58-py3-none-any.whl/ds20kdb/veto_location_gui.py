...
class Object:
    ...
    def __init__(self):
        ...
        self.qrcode_entry.bind('<Key>', lambda e: self.handle_ctrl_key_qrcode(e))
        ...
        self.comment_entry.bind('<Key>', lambda e: self.handle_ctrl_key_comment(e))
    ...
    def handle_ctrl_key_qrcode(self, event):
        """
        Enable command-V paste into the QR code field.
        """
        if event.keycode == 17:

            # delete selected text in entry field (exception raised when
            # nothing is selected)
            with contextlib.suppress(tk.TclError):
                self.qrcode_entry.delete(tk.SEL_FIRST, tk.SEL_LAST)

            # paste text
            self.qrcode_entry.insert(
                'end', self.root.selection_get(selection='CLIPBOARD')
            )

    def handle_ctrl_key_comment(self, event):
        """
        Enable command-V paste into the comment field.
        """
        if event.keycode == 17:
            ...
            # nothing is selected)
            with contextlib.suppress(tk.TclError):
                self.comment_entry.delete(tk.SEL_FIRST, tk.SEL_LAST)

            # paste text
            self.comment_entry.insert(
                'end', self.root.selection_get(selection='CLIPBOARD')
            )
