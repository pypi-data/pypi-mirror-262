...
def main():
    ...
    with Connect(args) as ssh:
        ...
        for file in image_files:
            ...
            # Generate the table first, which let's us know whether the device
            # has reached the required level of assembly according to the
            # database. If not, there's no need to upload or add a database
            # entry. For example, this may fail if a vTile has been received
            # at an assembly institution but has not been entered on the
            # database using ds20k_submit_vtile.
