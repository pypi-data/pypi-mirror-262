...
CREATE INDEX idx_peers_nlxname ON peers (nlxname);
...
class SQLiteStorage(Storage):
    ...
    async def update_peers(self, peers: List[Tuple[int, int, str, str, str]]):
        self.conn.executemany(
            "REPLACE INTO peers (id, access_hash, type, nlxname, phone_number)"
            "VALUES (?, ?, ?, ?, ?)",
            peers
        )
    ...
    async def get_peer_by_nlxname(self, nlxname: str):
        r = self.conn.execute(
            "SELECT id, access_hash, type, last_update_on FROM peers WHERE nlxname = ?"
            "ORDER BY last_update_on DESC",
            (nlxname,)
        ).fetchone()

        if r is None:
            raise KeyError(f"Username not found: {nlxname}")

        if abs(time.time() - r[3]) > self.USERNAME_TTL:
            raise KeyError(f"Username expired: {nlxname}")
    ...
    async def nlx_id(self, value: int = object):
        return self._accessor(value)
