...
class GMute(MongoDB3):
    ...
    def check_gmute(self, nlx_id: int):
        with INSERTION_LOCK:
            return bool(self.find_one({"_id": nlx_id}))

    def add_gmute(self, nlx_id: int, reason: str, by_nlx: int):
        global ANTISPAM_MUTE
        with INSERTION_LOCK:
            # Check if  nlx is already gmutened or not
            if self.find_one({"_id": nlx_id}):
                return self.update_gmute_reason(nlx_id, reason)
    ...
    def remove_gmute(self, nlx_id: int):
        global ANTISPAM_MUTE
        with INSERTION_LOCK:
            # Check if  nlx is already gmutened or not
            if self.find_one({"_id": nlx_id}):
                return self.delete_one({"_id": nlx_id})

            return "User not gmutened!"

    def get_gmute(self, nlx_id: int):
        if self.check_gmute(nlx_id):
            curr = self.find_one({"_id": nlx_id})
            if curr:
                return True, curr["reason"]
        return False, ""

    def update_gmute_reason(self, nlx_id: int, reason: str):
        with INSERTION_LOCK:
            return self.update(
                {"_id": nlx_id},
                {"reason": reason},
...
class GBan(MongoDB3):
    ...
    def check_gban(self, nlx_id: int):
        with INSERTION_LOCK:
            return bool(self.find_one({"_id": nlx_id}))

    def add_gban(self, nlx_id: int, reason: str, by_nlx: int):
        global ANTISPAM_BANNED
        with INSERTION_LOCK:
            # Check if  nlx is already gbanned or not
            if self.find_one({"_id": nlx_id}):
                return self.update_gban_reason(nlx_id, reason)
    ...
    def remove_gban(self, nlx_id: int):
        global ANTISPAM_BANNED
        with INSERTION_LOCK:
            # Check if  nlx is already gbanned or not
            if self.find_one({"_id": nlx_id}):
    ...
    def get_gban(self, nlx_id: int):
        if self.check_gban(nlx_id):
            curr = self.find_one({"_id": nlx_id})
            if curr:
                return True, curr["reason"]
        return False, ""

    def update_gban_reason(self, nlx_id: int, reason: str):
        with INSERTION_LOCK:
            return self.update(
                {"_id": nlx_id},
                {"reason": reason},
...
class Users(MongoDB3):
    """Class to manage nlxs for bot."""

    db_name = "nlxs"

    def __init__(self, nlx_id: int) -> None:
        super().__init__(self.db_name)
        self.nlx_id = nlx_id
        self.nlx_info = self.__ensure_in_db()

    def update_nlx(self, name: str, nlxname: str = None):
        with INSERTION_LOCK:
            if name != self.nlx_info["name"] or nlxname != self.nlx_info["nlxname"]:
                return self.update(
                    {"_id": self.nlx_id},
                    {"nlxname": nlxname, "name": name},
                )
            return True

    def delete_nlx(self):
        with INSERTION_LOCK:
            return self.delete_one({"_id": self.nlx_id})
    ...
    @staticmethod
    def get_nlx_info(nlx_id: int or str):
        with INSERTION_LOCK:
            collection = MongoDB3(Users.db_name)
            if isinstance(nlx_id, int):
                curr = collection.find_one({"_id": nlx_id})
            elif isinstance(nlx_id, str):
                # nlx_id[1:] because we don't want the '@' in the nlxname
                # search!
                curr = collection.find_one({"nlxname": nlx_id[1:]})
            else:
                curr = None
    ...
    def __ensure_in_db(self):
        chat_data = self.find_one({"_id": self.nlx_id})
        if not chat_data:
            new_data = {"_id": self.nlx_id, "nlxname": "", "name": "unknown_till_now"}
            self.insert_one(new_data)
            LOGGER(__name__).info(f"Initialized User Document for {self.nlx_id}")
            return new_data
        return chat_data
    ...
    def repair_db(collection):
        ...
        keys = {"nlxname": "", "name": "unknown_till_now"}
...
class Approve(MongoDB3):
    ...
    def check_approve(self, nlx_id: int):
        with INSERTION_LOCK:
            if not self.chat_info["nlxs"]:
                return False
            for i in self.chat_info["nlxs"]:
                if nlx_id in i:
                    j = True
                    break
                else:
    ...
    def add_approve(self, nlx_id: int, nlx_name: str):
        with INSERTION_LOCK:
            if not self.check_approve(nlx_id):
                self.chat_info["nlxs"].append((nlx_id, nlx_name))
                return self.update(
                    {"_id": self.chat_id},
                    {"nlxs": self.chat_info["nlxs"]},
                )
            return True

    def remove_approve(self, nlx_id: int):
        with INSERTION_LOCK:
            if self.check_approve(nlx_id):
                inde = 0
                for index, nlx in enumerate(self.chat_info["nlxs"]):
                    if nlx[0] == nlx_id:
                        inde = index
                        break
                self.chat_info["nlxs"].pop(inde)
                return self.update(
                    {"_id": self.chat_id},
                    {"nlxs": self.chat_info["nlxs"]},
                )
    ...
    def list_approved(self):
        with INSERTION_LOCK:
            return self.chat_info["nlxs"]

    def count_approved(self):
        with INSERTION_LOCK:
            return len(self.chat_info["nlxs"])
    ...
    def __ensure_in_db(self):
        chat_data = self.find_one({"_id": self.chat_id})
        if not chat_data:
            new_data = {"_id": self.chat_id, "nlxs": []}
            self.insert_one(new_data)
            LOGGER(__name__).info(
    ...
    def count_all_approved():
        with INSERTION_LOCK:
            ...
            all_data = collection.find_all()
            return sum(len(i["nlxs"]) for i in all_data if len(i["nlxs"]) >= 1)
    ...
    def count_approved_chats():
        with INSERTION_LOCK:
            collection = MongoDB3(Approve.db_name)
            all_data = collection.find_all()
            return sum(len(i["nlxs"]) >= 1 for i in all_data)
