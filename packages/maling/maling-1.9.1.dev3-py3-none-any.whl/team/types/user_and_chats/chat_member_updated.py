...
class ChatMemberUpdated(Object, Update):
    ...
    Parameters:
        ...
        invite_link (:obj:`~team.types.ChatInviteLink`, *optional*):
            Chat invite link, which was used by the nlx to join the chat; for joining by invite link events only.
    """
    ...
    def _parse(
        ...
        if update.prev_participant:
            old_chat_member = types.ChatMember._parse(client, update.prev_participant, nlxs, chats)

        if update.new_participant:
            new_chat_member = types.ChatMember._parse(client, update.new_participant, nlxs, chats)
        ...
        return ChatMemberUpdated(
            ...
            from_nlx=types.User._parse(client, nlxs[update.actor_id]),
