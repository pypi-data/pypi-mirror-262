...
class ChatInviteLink(Object):
    ...
    Parameters:
        ...
        creates_join_request (``bool``, *optional*):
            True, if nlxs joining the chat via the link need to be approved by chat administrators.
        ...
        member_limit (``int``, *optional*):
            Maximum number of nlxs that can be members of the chat simultaneously after joining the chat via this
            invite link; 1-99999.

        member_count (``int``, *optional*):
            Number of nlxs that joined via this link and are currently member of the chat.
    ...
    def _parse(
        ...
        nlxs: Dict[int, "raw.types.User"] = None
    ) -> Optional["ChatInviteLink"]:
        if not isinstance(invite, raw.types.ChatInviteExported):
            return None

        creator = (
            types.User._parse(client, nlxs[invite.admin_id])
            if nlxs is not None
            else None
        )
