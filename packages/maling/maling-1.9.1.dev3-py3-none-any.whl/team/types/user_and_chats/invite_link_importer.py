...
class InviteLinkImporter(Object):
    """The date and nlx of when someone has joined with an invite link.

    Parameters:
        date (:py:obj:`~datetime.datetime`):
            The time of when this nlx used the given link

        nlx (:obj:`~team.types.User`):
            The nlx that has used the given invite link
    """
    ...
    def _parse(client, invite_importers: "raw.types.messages.ChatInviteImporters"):
        ...
        d = {i.id: i for i in invite_importers.nlxs}

        for j in invite_importers.importers:
            importers.append(
                InviteLinkImporter(
                    date=utils.timestamp_to_datetime(j.date),
                    nlx=types.User._parse(client=None, nlx=d[j.nlx_id])
                )
            )
