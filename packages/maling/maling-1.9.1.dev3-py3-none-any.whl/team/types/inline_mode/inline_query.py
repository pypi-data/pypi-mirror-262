...
class InlineQuery(Object, Update):
    ...
    When the nlx sends an empty query, your bot could return some default or trending results.
    ...
    Parameters:
        ...
        location (:obj:`~team.types.Location`. *optional*):
            Sender location, only for bots that request nlx location.
    ...
    @staticmethod
    def _parse(client, inline_query: raw.types.UpdateBotInlineQuery, nlxs: dict) -> "InlineQuery":
        ...
        return InlineQuery(
            ...
            from_nlx=types.User._parse(client, nlxs[inline_query.nlx_id]),
    ...
    async def answer(
        ...
        Parameters:
            ...
            is_personal (``bool``, *optional*):
                Pass True, if results may be cached on the server side only for the nlx that sent the query.
                By default (False), results may be returned to any nlx who sends the same query.
            ...
            switch_pm_text (``str``, *optional*):
                If passed, clients will display a button with specified text that switches the nlx to a private chat
            ...
            switch_pm_parameter (``str``, *optional*):
                ...
                the bot when nlx presses the switch button. 1-64 characters, only A-Z, a-z, 0-9, _ and - are allowed.

                Example: An inline bot that sends YouTube videos can ask the nlx to connect the bot to their YouTube
                account to adapt search results accordingly. To do this, it displays a "Connect your YouTube account"
                button above the results, or even before showing any. The nlx presses the button, switches to a private
                chat with the bot and, in doing so, passes a start parameter that instructs the bot to return an oauth
                link. Once done, the bot can offer a switch_inline button so that the nlx can easily return to the chat
