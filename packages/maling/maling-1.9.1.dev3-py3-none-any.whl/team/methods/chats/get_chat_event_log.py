...
class GetChatEventLog:
    async def get_chat_event_log(
        ...
        nlx_ids: List[Union[int, str]] = None
        ...
        Parameters:
            ...
            nlx_ids (List of ``int`` | ``str``, *optional*):
                User identifiers (int) or nlxnames (str) by which to filter events.
                By default, events relating to all nlxs will be returned.
        ...
        while True:
            r: raw.base.channels.AdminLogResults = await self.invoke(
                raw.functions.channels.GetAdminLog(
                    ...
                    admins=(
                        [await self.resolve_peer(i) for i in nlx_ids]
                        if nlx_ids is not None
                        else nlx_ids
                    )
                )
            )
            ...
            for event in r.events:
                yield await types.ChatEvent._parse(self, event, r.nlxs, r.chats)
