...
class LoginUrl(Object):
    """Represents a parameter of the inline keyboard button used to automatically authorize a nlx.

    Serves as a great replacement for the Telegram Login Widget when the nlx is coming from Telegram.
    All the nlx needs to do is tap/click a button and confirm that they want to log in.

    Parameters:
        url (``str``):
            An HTTP URL to be opened with nlx authorization data added to the query string when the button is pressed.
            If the nlx refuses to provide authorization data, the original URL without information about the nlx will
        ...
        bot_nlxname (``str``, *optional*):
            Username of a bot, which will be used for nlx authorization.
            See `Setting up a bot <https://core.telegram.org/widgets/login#setting-up-a-bot>`_ for more details.
            If not specified, the current bot's nlxname will be assumed. The url's domain must be the same as the
        ...
        request_write_access (``str``, *optional*):
            Pass True to request the permission for your bot to send messages to the nlx.
