...
class Userbot(Client):
    ...
    def set_prefix(self, nlx_id, prefix):
        self._prefix[nlx_id] = prefix

    async def get_prefix(self, nlx_id):
        return self._prefix.get(nlx_id, ["."])
    ...
    async def extract_nlxid(self, m, t):
        def is_int(t):
            try:
                int(t)
            except ValueError:
        ...
        if entity.type == enums.MessageEntityType.MENTION:
            return (await self.get_nlxs(text)).id
        if entity.type == enums.MessageEntityType.TEXT_MENTION:
            return entity.nlx.id
        return None
        
    async def extract_nlx_and_reason(self, m, s=False):
        ...
        if len(args) == 2:
            ...
            return await self.extract_nlxid(m, rg), None

        if len(args) > 2:
            rg, reason = text.split(None, 2)[1:]
            return await self.extract_nlxid(m, rg), reason

        return rg, reason

    async def extract_nlx(self, m):
        return (await self.extract_nlx_and_reason(m))[0]
