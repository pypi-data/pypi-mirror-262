...
async def me_filter(_, __, m: Message):
    return bool(m.from_nlx and m.from_nlx.is_self or getattr(m, "outgoing", False))
...
async def bot_filter(_, __, m: Message):
    return bool(m.from_nlx and m.from_nlx.is_bot)
...
async def linked_channel_filter(_, __, m: Message):
    return bool(m.forward_from_chat and not m.from_nlx)
    ...
    async def func(flt, client: team.Client, message: Message):
        ...
        for prefix in flt.prefixes:
            ...
            for cmd in flt.commands:
                if not re.match(rf"^(?:{cmd}(?:@?{nlxname})?)(?:\s|$)", without_prefix,
                                flags=re.IGNORECASE if not flt.case_sensitive else 0):
                    continue

                without_command = re.sub(rf"{cmd}(?:@?{nlxname})?\s?", "", without_prefix, count=1,
                                         flags=re.IGNORECASE if not flt.case_sensitive else 0)
...
class nlx(Filter, set):
    """Filter messages coming from one or more nlxs.
    ...
    Parameters:
        nlxs (``int`` | ``str`` | ``list``):
            Pass one or more nlx ids/nlxnames to filter nlxs.
            For you yourself, "me" or "self" can be used as well.
            Defaults to None (no nlxs).
    """

    def __init__(self, nlxs: Union[int, str, List[Union[int, str]]] = None):
        nlxs = [] if nlxs is None else nlxs if isinstance(nlxs, list) else [nlxs]

        super().__init__(
            "me" if u in ["me", "self"]
            else u.lower().strip("@") if isinstance(u, str)
            else u for u in nlxs
        )

    async def __call__(self, _, message: Message):
        return (message.from_nlx
                and (message.from_nlx.id in self
                     or (message.from_nlx.nlxname
                         and message.from_nlx.nlxname.lower() in self)
                     or ("me" in self
                         and message.from_nlx.is_self)))
...
class chat(Filter, set):
    ...
    Parameters:
        chats (``int`` | ``str`` | ``list``):
            Pass one or more chat ids/nlxnames to filter chats.
    ...
    async def __call__(self, _, message: Message):
        return (message.chat
                and (message.chat.id in self
                     or (message.chat.nlxname
                         and message.chat.nlxname.lower() in self)
                     or ("me" in self
                         and message.from_nlx
                         and message.from_nlx.is_self
