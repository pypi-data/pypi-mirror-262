...
class SignIn:
    async def sign_in(
        ...
        """Authorize a nlx in Telegram with a valid confirmation code.
        ...
        Returns:
            ...
            authorization completed, the nlx is returned. In case the phone number needs to be registered first AND the
        ...
        if isinstance(r, raw.types.auth.AuthorizationSignUpRequired):
            ...
            await self.storage.nlx_id(r.nlx.id)
            await self.storage.is_bot(False)

            return types.User._parse(self, r.nlx)
