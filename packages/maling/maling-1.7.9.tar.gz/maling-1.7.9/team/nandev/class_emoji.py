...
class Emojik:
    ...
    def initialize(self):
        if self.uprem == True:
            ...
            self.ping_var = (
                udB.get_var(user.me.id, "emo_ping") or "🏓"
            )
            self.emo_ping = self.ping_var
            #pong
            self.pong_var = (
                udB.get_var(user.me.id, "emo_pong") or "🎈"
            )
            self.emo_pong = self.pong_var
            #proses
            self.proses_var = (
                udB.get_var(user.me.id, "emo_proses") or "🔄"
            )
            self.emo_proses = self.proses_var
            #sukses
            self.sukses_var = (
                udB.get_var(user.me.id, "emo_sukses") or "✅"
            )
            self.emo_sukses = self.sukses_var
            #gagal
            self.gagal_var = (
                udB.get_var(user.me.id, "emo_gagal") or "❌"
            )
            self.emo_gagal = self.gagal_var
            #profil
            self.profil_var = (
                udB.get_var(user.me.id, "emo_profil") or "👤"
            )
            self.emo_profil = self.profil_var
            #alive
            self.alive_var = (
                udB.get_var(user.me.id, "emo_alive") or "🔥"
            )
            self.emo_alive = self.alive_var
            #warn
            self.warn_var = (
                udB.get_var(user.me.id, "emo_warn") or "❗"
            )
            self.emo_warn = self.warn_var
            #block
            self.block_var = (
                udB.get_var(user.me.id, "emo_block") or "🚫"
            )
