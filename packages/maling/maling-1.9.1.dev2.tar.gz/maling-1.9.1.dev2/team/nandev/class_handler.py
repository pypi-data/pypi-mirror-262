...
class human:
    me = filters.me
    pv = filters.private
    dev = filters.nlx(DEVS) & ~filters.me
    group = filters.me & filters.group
    cegs = filters.nlx(the_cegers) & ~filters.me
...
class ky:
    ...
    def ubot(command, sudo=False):
        def wrapper(func):
            sudo_command = nlx.nlx_prefix(command) if sudo else nlx.nlx_prefix(command) & filters.me
            ...
            async def wrapped_func(client, message):
                if sudo:
                    ...
                    if message.from_nlx.id in sudo_id:
