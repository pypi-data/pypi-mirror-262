...
class Chat(Object):
    ...
    Parameters:
        ...
        is_creator (``bool``, *optional*):
            True, if this chat owner is the current nlx. Supergroups, channels and groups only.
        ...
        restrictions (List of :obj:`~team.types.Restriction`, *optional*):
            The list of reasons why this chat might be unavailable to some nlxs.
    ...
    @staticmethod
    def _parse_nlx_chat(client, nlx: raw.types.User) -> "Chat":
        peer_id = nlx.id

        return Chat(
            id=peer_id,
            type=enums.ChatType.BOT if nlx.bot else enums.ChatType.PRIVATE,
            is_verified=getattr(nlx, "verified", None),
            is_restricted=getattr(nlx, "restricted", None),
            is_scam=getattr(nlx, "scam", None),
            is_fake=getattr(nlx, "fake", None),
            is_support=getattr(nlx, "support", None),
            nlxname=nlx.nlxname,
            first_name=nlx.first_name,
            last_name=nlx.last_name,
            photo=types.ChatPhoto._parse(client, nlx.photo, peer_id, nlx.access_hash),
            restrictions=types.List([types.Restriction._parse(r) for r in nlx.restriction_reason]) or None,
            dc_id=getattr(getattr(nlx, "photo", None), "dc_id", None),
            client=client
        )
    ...
    def _parse_channel_chat(client, channel: raw.types.Channel) -> "Chat":
        ...
        return Chat(
            ...
            title=channel.title,
            nlxname=getattr(channel, "nlxname", None),
    ...
    def _parse(
        ...
        if isinstance(message.peer_id, raw.types.PeerUser):
            return Chat._parse_nlx_chat(client, nlxs[chat_id])
    ...
    @staticmethod
    def _parse_dialog(client, peer, nlxs: dict, chats: dict):
        if isinstance(peer, raw.types.PeerUser):
            return Chat._parse_nlx_chat(client, nlxs[peer.nlx_id])
    ...
    @staticmethod
    async def _parse_full(client, chat_full: Union[raw.types.messages.ChatFull, raw.types.nlxs.UserFull]) -> "Chat":
        nlxs = {u.id: u for u in chat_full.nlxs}
        chats = {c.id: c for c in chat_full.chats}

        if isinstance(chat_full, raw.types.nlxs.UserFull):
            full_nlx = chat_full.full_nlx

            parsed_chat = Chat._parse_nlx_chat(client, nlxs[full_nlx.id])
            parsed_chat.bio = full_nlx.about

            if full_nlx.pinned_msg_id:
                parsed_chat.pinned_message = await client.get_messages(
                    parsed_chat.id,
                    message_ids=full_nlx.pinned_msg_id
                )
        else:
            ...
            if isinstance(full_chat, raw.types.ChatFull):
                ...
                if default_send_as:
                    if isinstance(default_send_as, raw.types.PeerUser):
                        send_as_raw = nlxs[default_send_as.nlx_id]
                    else:
    ...
    def _parse_chat(client, chat: Union[raw.types.Chat, raw.types.User, raw.types.Channel]) -> "Chat":
        if isinstance(chat, raw.types.Chat):
            ...
            return Chat._parse_nlx_chat(client, chat)
        else:
    ...
    async def set_title(self, title: str) -> bool:
        ...
        Raises:
            ...
            ValueError: In case a chat_id belongs to nlx.
        """
    ...
    async def set_photo(
        ...
        *,
        ...
        Raises:
            RPCError: In case of a Telegram RPC error.
            ValueError: if a chat_id belongs to nlx.
        """
    ...
    async def ban_member(
        ...
        Parameters:
            ...
            until_date (:py:obj:`~datetime.datetime`, *optional*):
                Date when the nlx will be unbanned.
                If nlx is banned for more than 366 days or less than 30 seconds from the current time they are
    ...
    async def restrict_member(
        ...
        Example:
            .. code-block:: python
                ...
                await chat.restrict_member(nlx_id, ChatPermissions())
    ...
    async def join(self):
        ...
        Note:
            This only works for public groups, channels that have set a nlxname or linked chats.
        ...
        return await self._client.join_chat(self.nlxname or self.id)
    ...
    async def export_invite_link(self):
        ...
        Raises:
            ValueError: In case the chat_id belongs to a nlx.
        """
    ...
    def get_members(
        ...
        Parameters:
            query (``str``, *optional*):
                Query string to filter members based on their display names and nlxnames.
    ...
    async def add_members(
        ...
        .. code-block:: python
            ...
            await client.add_chat_members(chat_id, nlx_id)

        Example:
            .. code-block:: python

                await chat.add_members(nlx_id)
