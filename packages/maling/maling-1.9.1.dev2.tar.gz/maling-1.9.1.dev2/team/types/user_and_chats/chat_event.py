...
class ChatEvent(Object):
    ...
    Parameters:
        ...
        old_nlxname, new_nlxname (``str``, *optional*):
            Previous and new chat nlxname.
    ...
    def __init__(
        ...
        old_nlxname: str = None,
        new_nlxname: str = None,
        ...
        self.old_nlxname = old_nlxname
        self.new_nlxname = new_nlxname
    ...
    async def _parse(
        ...
        nlxs: List["raw.base.User"],
        chats: List["raw.base.Chat"]
    ):
        nlxs = {i.id: i for i in nlxs}
        chats = {i.id: i for i in chats}

        nlx = types.User._parse(client, nlxs[event.nlx_id])
        action = event.action
        ...
        old_nlxname: Optional[str] = None
        new_nlxname: Optional[str] = None
        ...
        if isinstance(action, raw.types.ChannelAdminLogEventActionChangeAbout):
            ...
            old_nlxname = action.prev_value
            new_nlxname = action.new_value
            ...
            deleted_message = await types.Message._parse(client, action.message, nlxs, chats)
            action = enums.ChatEventAction.MESSAGE_DELETED

        elif isinstance(action, raw.types.ChannelAdminLogEventActionEditMessage):
            old_message = await types.Message._parse(client, action.prev_message, nlxs, chats)
            new_message = await types.Message._parse(client, action.new_message, nlxs, chats)
            action = enums.ChatEventAction.MESSAGE_EDITED

        elif isinstance(action, raw.types.ChannelAdminLogEventActionParticipantInvite):
            invited_member = types.ChatMember._parse(client, action.participant, nlxs, chats)
            ...
            old_administrator_privileges = types.ChatMember._parse(client, action.prev_participant, nlxs, chats)
            new_administrator_privileges = types.ChatMember._parse(client, action.new_participant, nlxs, chats)
            ...
            old_member_permissions = types.ChatMember._parse(client, action.prev_participant, nlxs, chats)
            new_member_permissions = types.ChatMember._parse(client, action.new_participant, nlxs, chats)
            ...
            stopped_poll = await types.Message._parse(client, action.message, nlxs, chats)
            ...
            if message.pinned:
                pinned_message = await types.Message._parse(client, message, nlxs, chats)
                action = enums.ChatEventAction.MESSAGE_PINNED
            else:
                unpinned_message = await types.Message._parse(client, message, nlxs, chats)
            ...
            old_invite_link = types.ChatInviteLink._parse(client, action.prev_invite, nlxs)
            new_invite_link = types.ChatInviteLink._parse(client, action.new_invite, nlxs)
            ...
            revoked_invite_link = types.ChatInviteLink._parse(client, action.invite, nlxs)
            ...
            deleted_invite_link = types.ChatInviteLink._parse(client, action.invite, nlxs)
        ...
        return ChatEvent(
            ...
            old_nlxname=old_nlxname,
            new_nlxname=new_nlxname,
