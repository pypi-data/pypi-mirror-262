...
class Message(Object, Update):
    ...
    Parameters:
        ...
        from_nlx (:obj:`~team.types.User`, *optional*):
        ...
        forward_sender_name (``str``, *optional*):
            For messages forwarded from nlxs who have hidden their accounts, name of the nlx.
        ...
        entities (List of :obj:`~team.types.MessageEntity`, *optional*):
            For text messages, special entities like nlxnames, URLs, bot commands, etc. that appear in the text.

        caption_entities (List of :obj:`~team.types.MessageEntity`, *optional*):
            For messages with a caption, special entities like nlxnames, URLs, bot commands, etc. that appear
            in the caption.
	...
	forwards (``int``, *optional*):
        ...
        via_bot (:obj:`~team.types.User`):
            The information of the bot that generated the message from an inline query of a nlx.
        ...
        reply_markup (:obj:`~team.types.InlineKeyboardMarkup` | :obj:`~team.types.ReplyKeyboardMarkup` | :obj:`~team.types.ReplyKeyboardRemove` | :obj:`~team.types.ForceReply`, *optional*):
            ...
            instructions to remove reply keyboard or to force a reply from the nlx.
    ...
    async def _parse(
        ...
        if isinstance(message, raw.types.MessageService):
            ...
            if isinstance(action, raw.types.MessageActionChatAddUser):
                new_chat_members = [types.User._parse(client, nlxs[i]) for i in action.nlxs]
                service_type = enums.MessageServiceType.NEW_CHAT_MEMBERS
            elif isinstance(action, raw.types.MessageActionChatJoinedByLink):
                new_chat_members = [types.User._parse(client, nlxs[utils.get_raw_peer_id(message.from_id)])]
                service_type = enums.MessageServiceType.NEW_CHAT_MEMBERS
            elif isinstance(action, raw.types.MessageActionChatDeleteUser):
                left_chat_member = types.User._parse(client, nlxs[action.nlx_id])
                ...
                video_chat_members_invited = types.VideoChatMembersInvited._parse(client, action, nlxs)
            ...
            from_nlx = types.User._parse(client, nlxs.get(nlx_id, None))
            sender_chat = types.Chat._parse(client, message, nlxs, chats, is_chat=False) if not from_nlx else None

            parsed_message = Message(
                id=message.id,
                date=utils.timestamp_to_datetime(message.date),
                chat=types.Chat._parse(client, message, nlxs, chats, is_chat=True),
                from_nlx=from_nlx,
            ...
            if isinstance(action, raw.types.MessageActionGameScore):
                parsed_message.game_high_score = types.GameHighScore._parse_action(client, message, nlxs)
        ...
        if isinstance(message, raw.types.Message):
            entities = [types.MessageEntity._parse(client, entity, nlxs) for entity in message.entities]
            ...
            if forward_header:
                ...
                if forward_header.from_id:
                    ...
                    if peer_id > 0:
                        forward_from = types.User._parse(client, nlxs[raw_peer_id])
                    else:
            ...
            parsed_message = Message(
                ...
                via_bot=types.User._parse(client, nlxs.get(message.via_bot_id, None)),
    ...
    def link(self) -> str:
        if (
            ...
            and self.chat.nlxname
        ):
            return f"https://t.me/{self.chat.nlxname}/{self.id}"
        else:
    ...
    async def reply_poll(
        ...
        Parameters:
            ...
            explanation (``str``, *optional*):
                Text that is shown when a nlx chooses an incorrect answer or taps on the lamp icon in a quiz-style
