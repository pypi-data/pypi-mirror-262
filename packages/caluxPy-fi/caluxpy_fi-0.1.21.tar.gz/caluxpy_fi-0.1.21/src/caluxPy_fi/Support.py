...
class Support ():
    ...
    def years(issuance_date, maturity_date):
        ...
        current_date = issuance_date
        years = 0

        while current_date < maturity_date:
            next_year = current_date.year + 1
            leap_day = 1 if is_leap_year(current_date.year) else 0
            year_days = 365 + leap_day
            if (current_date + timedelta(days=year_days)) <= maturity_date:
                years += 1
                current_date += timedelta(days=year_days)
            else:
                break
        
        remaining_days = (maturity_date - current_date).days
        final_fraction = remaining_days / (365 + (1 if is_leap_year(current_date.year) else 0))
        years += final_fraction
