            ...
            if self.maturity_date >= self.settlement_date:
                ...
                if self.coupon_type == 'normal':
                    ...
                    if forward_date != '': 
                        try:
                            ...
                            self._logger.info(f'Forward Date -> {self.forward_date} Type => {type(self.forward_date)}')
                        except ValueError as e:
                            self._logger.error(e)    
                    else:
                ...
                if self.payment_type == 'bullet':
                    ...
                    if amortization_start_date != '':
                        try:
                            if type(amortization_start_date) == str: self.amortization_start_date = datetime.strptime(amortization_start_date, self._date_format).date()
