...
def _db(
    ...
    **kwargs
    ...
    if candidates:
        if len(candidates) > 1:
            range_str = "SECURITY_NAME in ('" + "','".join(candidates) + "')"
        else:
            range_str = "SECURITY_NAME='" + candidates[0] + "'"
    else:
        range_str = ""
    ...
    if isinstance(kwargs["value"], str):
        for i in data:
            if i[kwargs["value"]] is None:
                continue
            if i["SECURITY_NAME"] in name_to_sources:
                name_to_sources[i["SECURITY_NAME"]][i[kwargs["key"]]] = i[kwargs["value"]]
            else:
                name_to_sources[i["SECURITY_NAME"]] = {i[kwargs["key"]]: i[kwargs["value"]]}
        ...
        for i in data:
            if i["SECURITY_NAME"] in name_to_sources:
                ...
                name_to_sources[i["SECURITY_NAME"]][i[kwargs["key"]]] = new_val
            else:
                new_val = [i[j] for j in kwargs["value"]]
                name_to_sources[i["SECURITY_NAME"]] = {i[kwargs["key"]]: new_val}            
