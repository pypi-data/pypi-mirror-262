...
class ParseConfig:
    ...
    def get_config(self) -> dict:
        ...
        if not parameters:
            print('error: no parameters were read from the provided deployment file, please provide a yaml file as input')
            exit(1)
    ...
    def init_dir(args: any):
        ...
        print(
            'initial directories created, have a look at the template sub command to add a template (tjcli template -l)')
    ...
    def use_template(args: any):
        ...
        if args.list:
            ...
            for root, dirs, filenames in os.walk(template_dir):
                if len(dirs) == 1:
                    desc_file = os.path.join(template_dir, dirs[0], 'description.txt')
            ...
            for sub in ["deployments", "templates", "parameters"]:
                ...
                if os.path.exists(template_final_dir):
                    ...
                    for filename in os.listdir(template_final_dir):
                        ...
                        shutil.copyfile(os.path.join(template_final_dir, filename),
