...
class LocationApi(object):
    ...
    """
    # 通过地址获取地理坐标url地址
    baidu_json_location_url = u'http://api.map.baidu.com/geocoding/v3/'

    # 通过地理坐标返回地址信息url地址
    baidu_json_region_url = u'http://api.map.baidu.com/reverse_geocoding/v3/'
    ...
    def get_detail_address_by_location(self, location, **kwargs):
        ...
        params = {'ak': self.scheduler.next(), 'output': 'json', 'location': '{lat},{lng}'.format(lat=location['lat'],
                                                                                                  lng=location['lng']),
                  'coordtype': kwargs.get('coordtype', 'bd09ll'), 'pois': kwargs.get('extensions_poi', 0)}
        try:
