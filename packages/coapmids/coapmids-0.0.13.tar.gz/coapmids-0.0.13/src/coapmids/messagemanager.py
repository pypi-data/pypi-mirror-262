...
class MessageManager(interfaces.TokenInterface, interfaces.MessageManager):
    ...
    def _next_message_id(self):
        """Reserve and return a new unique message ID with inverted bits."""
        if not hasattr(self, 'used_message_ids'):
        ...
        while True:
            ...
            # Invertir los bits del Message ID
            self.message_id = int(bin(self.message_id)[2:][::-1], 2)
            print(f"Siguiente Message ID: {self.message_id:04X}")

            if message_id not in self.used_message_ids:
