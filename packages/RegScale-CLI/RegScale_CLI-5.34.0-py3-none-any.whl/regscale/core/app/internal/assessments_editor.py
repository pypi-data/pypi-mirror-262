...
def map_workbook_to_dict(
    file_path: str, workbook_data: Optional[pd.DataFrame] = None
) -> dict:
    """
    Function to map workbook to dictionary

    :param str file_path: Path to workbook file
    :param pd.DataFrame workbook_data: Dataframe to map to dictionary
    ...
    wb_data["Facility"] = (
        wb_data["Facility"].astype(str).fillna("None")
    )  # Handle missing facilities
    wb_data["Organization"] = (
        wb_data["Organization"].astype(str).fillna("None")
    ...
    facilities["Facility"] = facilities["Facility"].astype(
        str
    ...
    wb_data = wb_data.merge(
        facilities, how="left", on="Facility", validate="many_to_many"
    )
    wb_data = wb_data.merge(
