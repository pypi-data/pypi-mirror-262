...
def create_asset(component_data: Dict, user_id: str, spp_id: int) -> Optional[Dict]:
    ...
    if asset := Asset(
        ...
        cpu=0,
        assetCategory=(
            "software" if component_data["type"] == "software" else "hardware"
        ),
        createdById=user_id,
...
def create_related_items(
    ...
    for item_type in ["properties", "props", "protocols", "links"]:
        if item_type in component_data and component_data[item_type]:
            ...
            logger.error(
                f"Creating {item_type} for component {component_data['uuid']} with {create_func}"
            )
...
def create_component(
    ...
    comp = Component(
        ...
        componentOwnerId=(
            ...
            if "asset_owner" in component_data
            and component_data["asset_owner"] is not None
            else user_id
        ),
        ...
        cmmcAssetType=(
            component_data["asset_type"] if "asset_type" in component_data else None
        ),
        cmmcExclusion="false",
        lastUpdatedById=user_id,
        createdById=user_id,
