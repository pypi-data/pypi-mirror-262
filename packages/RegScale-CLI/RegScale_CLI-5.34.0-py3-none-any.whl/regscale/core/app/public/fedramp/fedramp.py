...
def parse_port_or_protocol(
    key: str, protocol: dict, return_type: Union[Type[str], Type[int]]
) -> Union[str, int]:
    """
...
def post_implementations(
    ...
    if load_missing:
        new_imps = load_non_matched_profile_controls(
...
def find_control_in_mapping(
    mapping: List[ProfileMapping], friendly_control_id: str
) -> str:
    """
    ...
    if control := [
        ...
        if control.control.controlId.lower() == friendly_control_id
    ]:
...
def handle_control_implementation(
    ...
    :param dict current_implementations_dict: Dictionary of current implementations
...
def handle_parameters(
    app: Application, api: Api, fedramp_control: any, control_imp: ControlImplementation
) -> None:
    """
    Handle parameters
...
def check_control_list_length(ssp_obj: SSP, mapping: dict) -> Optional[list]:
    ...
    """
    profile_control_ids = [
        get_friendly_control_id(item.control.controlId) for item in mapping
    ]
    parsed_control_ids = [
        get_friendly_control_id(control.number) for control in ssp_obj.control_list
    ]
...
def load_non_matched_profile_controls(
    app: Application, regscale_ssp: dict, mapping: List[ProfileMapping]
) -> List:
    ...
    if ssp := [
        ...
        controls_to_add = [
            control for control in mapping if control.controlID not in existing_controls
        ]
        logger.info(
...
def parse_ssp_docx_tables(tables: Any) -> Tuple[str, str, str, str, str, list]:
    """
    Parse the SSP docx tables

    :param tables: List of tables from the SSP docx
    :return Tuple[str, str, str, str, str, list]: System Status, System Type, Title, Cloud Model, Cloud Sevice, Table Data
    ...
    return system_status, system_type, title, cloud_model, cloud_service, table_data
...
def process_fedramp_docx(
    ...
    mdeployhybrid = (
        True if "shared across all clients/agencies" in cloud_model else False
    )
...
if __name__ == "__main__":
    ...
    process_fedramp_docx_by_profile_id(
        file_path=Path(
            "/home/abelardo/Documents/RegScale/regscale-cli/artifacts/REG-6800.docx"
        ),
        profile_id=2,
        save_data=True,
        load_missing=True,
