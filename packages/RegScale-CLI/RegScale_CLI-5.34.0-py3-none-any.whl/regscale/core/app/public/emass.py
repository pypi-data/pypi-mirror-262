...
def format_controls(file_data_dict: dict, file_name: str) -> dict:
    """
    ...
    try:
        parsed_controls = {
            key: {"raw": val.lower()}
            for key, val in file_data_dict["Control Acronym"].items()
        }
        # Remove duplicate values in dictionary
        parsed_controls = dict(parsed_controls)

        # create list of formatted controls
        for key, val in parsed_controls.items():
            val["formatted"] = val["raw"].replace("(", ".").replace(")", "")
    except KeyError:
        error_and_exit(
...
def populate_controls(file_name: Path, ssp_id: int, api: Api) -> Path:
    ...
    # format the controls
    formatted_controls = format_controls(
    ...
    regscale_control_ids = [ctrl["control"]["controlId"].lower() for ctrl in controls]
    ...
    with job_progress:
        ...
        for row in formatted_controls:
            raw_control = formatted_controls[row]["raw"]
            formatted_control = formatted_controls[row]["formatted"]
            # get the row number for the current control
            row_number = row + SKIP_ROWS

            # see if the current formatted control has an assessment by
            # comparing it to our GraphQL query results
            if raw_control in regscale_control_ids:
                control = raw_control
            elif formatted_control in regscale_control_ids:
                control = formatted_control
            else:
...
def map_finish_date(
    ...
    :return: Finish date of the assessment
    :rtype: str
    """
    finish_date = (
