...
def list_jobs(job_type: str, last: int, job_status: str):
    ...
    if job_status:
        jobs = [
            job
            for job in client.exports.jobs(job_type)
            if job["status"] == str(job_status).upper()
        ]
    else:
...
def sync_vulns(regscale_ssp_id: int = 0) -> None:
    ...
    app.logger.info(
        "Building a list of Nessus Reports with a severity >= %s.", minimum_severity
    )
    NessusReport.sync_to_regscale(
        vulns=vulns, parent_id=regscale_ssp_id, client=client, config=config
    )
    logger.info("Done!")
