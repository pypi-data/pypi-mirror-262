...
def get_issues_data(reg_api: Api, url_issues: str) -> List[dict]:
    ...
    :param reg_api: RegScale API object
    :param url_issues: URL for RegScale issues
...
def create_snow_incident(snow_api: Api, incident_url: str, snow_incident: dict) -> dict:
    ...
    :param snow_api: ServiceNow API object
    :param incident_url: URL for ServiceNow incidents
    :param snow_incident: Incident data
...
def sync_snow_to_regscale(
    ...
    if issues_data := get_issues_data(reg_api, url_issues):
        ...
        logger.warning(
            "No issues found for this record in RegScale. No issues were processed."
        )


def create_snow_assignment_group(snow_assignment_group, snow_config):
    """
    Create a new assignment group in ServiceNow or if one already exists,
    a 403 is returned from SNOW.

    :param snow_assignment_group: ServiceNow assignment group
    :param snow_config: ServiceNow configuration
    """
    ...
    if response.status_code == 201:
        ...
        logger.debug(
            "ServiceNow Assignment Group %s already exists.", snow_assignment_group
        )
    else:
        error_and_exit(
...
def process_issues(
    ...
    :param issues_data: List of issues
    :param snow_config: ServiceNow configuration
    :param snow_assignment_group: ServiceNow assignment group
    :param snow_incident_type: ServiceNow incident type
...
def create_incident(
    iss, snow_config, snow_assignment_group, snow_incident_type, config
) -> Optional[dict]:
    """
    Create a new incident in ServiceNow

    :param iss: Issue data
    :param snow_config: ServiceNow configuration
    :param snow_assignment_group: ServiceNow assignment group
    :param snow_incident_type: ServiceNow incident type
    ...
    try:
        ...
        if iss["serviceNowId"] == "":
            ...
            incident_url = urljoin(snow_config["url"], "api/now/table/incident")
            response = create_snow_incident(
                snow_config["api"], incident_url, snow_incident
            )
            ...
            if regscale_response.raise_for_status():
                ...
                if work_item := dat["work_notes"]:
                    ...
                    if work_item not in issue["description"]:
                        logger.info(
                            "Updating work item for RegScale issue # %s and ServiceNow incident "
                            + "# %s.",
                            issue["id"],
                            sys_id,
                        )
