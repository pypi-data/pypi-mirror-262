...
class Component(RegScaleModel):
    ...
    def get_list(cls) -> List[ComponentList]:  # type: ignore
        ...
        response = cls._api_handler.get(
            endpoint=cls.get_endpoint("get_list").format(
    ...
    def filter_components(cls, params: Dict) -> List["Component"]:
        ...
        if response and response.ok:
            ...
            return list(
                filter(
                    None, [cls.get_object(object_id=item.get("id")) for item in items]
                )
            )
        return []
...
class ComponentMapping(RegScaleModel):
    ...
    def find_by_unique(self, **kwargs) -> Optional["ComponentMapping"]:  # type: ignore
        """
        ...
        for mapping in mappings:
            ...
            all_fields_match = all(
                getattr(mapping, key) == getattr(self, key)
    ...
    @classmethod
    def _handle_list_response(cls, response, security_plan_id: int) -> List[T]:  # type: ignore
        """
