...
class ScanHistory(RegScaleModel):
    ...
    def prepare_scan_history_for_sync(
        ...
        :rtype: Tuple[List[ScanHistory], List[ScanHistory], List[Asset]]
        """
        new_scan_history = []
        ...
        def process_asset(asset) -> None:
            ...
            if reg_assets:
                ...
                scan = ScanHistory.create_scan_from_tenable(
            ...
            for index, file in enumerate(file_list):
                ...
                with open(file, "rb") as vuln_file_wrapper:
                    ...
                    new_scan_data.extend(
                        ScanHistory.prepare_scan_history_for_sync(
                            app, nessus_list, existing_assets
                        )
                    )
                existing_scan_history_list.extend(
                    ScanHistory.sync_scan_history_to_regscale(app, new_scan_data)
                )
    ...
    @staticmethod
    def update_scan(
        app: Application, api: Api, regscale_scan: "ScanHistory"
    ) -> Tuple[Response, "ScanHistory"]:
        """
