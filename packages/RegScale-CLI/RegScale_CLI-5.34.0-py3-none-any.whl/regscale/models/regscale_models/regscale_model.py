...
class RegScaleModel(BaseModel, ABC):
    ...
    def find_by_unique(self, parent_id_field: Optional[str] = None) -> Optional[T]:
        ...
        parent_module = getattr(
            self, "parentModule", getattr(self, "parent_module", "")
        )
    ...
    @classmethod
    def _handle_graph_response(cls, response: Dict[Any, Any], child=None) -> List[T]:
        """
    ...
    def build_graphql_fields(cls) -> str:
        ...
        :param model_class: The Pydantic model class to build the query for.
        :return: A string representing the GraphQL query.
        """
        return "\n".join(
            x
            for x in cls.get_field_names()
            if x not in cls._exclude_graphql_fields.default
        )
    ...
    def create(self: T) -> T:
        ...
        if response and response.ok:
            ...
            logger.error(
                f"Failed to insert {self.__class__.__name__} with payload:\n{payload_str}"
            )
