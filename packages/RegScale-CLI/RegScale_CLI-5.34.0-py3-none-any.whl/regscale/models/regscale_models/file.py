...
class File(BaseModel):
    ...
    def _check_compression(size_limit_mb: int = 25, file_path: str = None) -> str:
        ...
        if size > size_limit_mb * 1024 * 1024:
            app.logger.warning(
                "The file is over %i MB. It will be compressed.", size_limit_mb
            )
            # compress the file
            output_path = (
                Path(file_path).parent / Path(file_path).with_suffix(".gz").name
            )
    ...
    def _compress_file(
        ...
        with open(input_file_path, "rb") as f_in:
            # Max compression
            with gzip.GzipFile(
                filename=output_file_path, mode="wb", compresslevel=9
            ) as f_out:
    ...
    def _create_regscale_file(
        ...
        if file_size > 104857600:
            ...
            raise ValueError(
                f"File size is {mb_size} MB. This is over the max file size of {limit_size} MB"
            )
