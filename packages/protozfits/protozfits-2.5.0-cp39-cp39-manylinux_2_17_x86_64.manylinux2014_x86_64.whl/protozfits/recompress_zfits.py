...
parser.add_argument("-n", "--n-events", type=int, default=None, help="If given, stop after n events")
parser.add_argument("--rows-per-tile", type=int, help="Number of events per tile, if not given, same as inputfile will be used")
parser.add_argument("--block-size", type=int, default=100, help="Compression block size in MB")
parser.add_argument("--overwrite", action="store_true", help="Overwrite output file without asking")

parser.add_argument("--config", type=Path, help="Path to yaml config file setting up compression options")
parser.add_argument("--default-compression", type=str, help="Default compression to be used, see adh-apis for supported compressions")
parser.add_argument("-c", "--column-compression", action="append", nargs=2, type=str, help="Compression to be used for a specific column")
