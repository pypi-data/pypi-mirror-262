...
def test_can_read_R1v1_format():
    with File(sample_file_path) as f:
        ...
        assert (cam_config.pixel_id_map == np.arange(start=base_value, \
                                                    stop=base_value+arrays_size)).all()
        assert (cam_config.module_id_map == np.arange(start=base_value+1, \
                                                    stop=base_value+1+arrays_size)).all()
        ...
        for i, event in enumerate(f.Events):
            ...
            assert (event.waveform == np.arange(start=base_value, \
                                                  stop=base_value+arrays_size)).all()
            assert (event.pixel_status == np.arange(start=base_value+1,\
                                                     stop=base_value+1+arrays_size)).all()
            assert (event.first_cell_id == np.arange(start=base_value+2,\
                                                     stop=base_value+2+arrays_size)).all()
            assert (event.module_hires_local_clock_counter == np.arange(start=base_value+3,\
                                                     stop=base_value+3+arrays_size)).all()
            assert (event.pedestal_intensity == np.arange(start=base_value+4,\
                                                     stop=base_value+4+arrays_size)).all()
