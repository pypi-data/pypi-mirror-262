...
class DownloadSXData:
    ...
    def __post_init__(self):
        ...
        if self.org_id:
            ...
            # logger.info('You have supplied org_id as well as survey_ids. Survey_ids are ignored')
    ...
    def download_data(self, spss: bool = False, fail_safe: bool = True) -> None:
        ...
        for survey in self.__surveys:
            self.__download_dataset(survey)
    ...
    def __generate_spss_syntaxes(self) -> None:
        ...
        syntax_folder = Path(self.output_path, 'spss_syntax')
        syntax_folder.mkdir(exist_ok=True)
        for survey in self.__downloaded_surveys:
            ...
            generate_labels_file(survey, syntax_folder, labels_file, variables_file)
            generate_dataset_file(survey, syntax_folder, variables_file)
        generate_open_all_syntax(self.__downloaded_surveys, Path(self.output_path))
        logger.info(f'Færdig med at generere SPSS syntaxer. Er gemt i {syntax_folder.as_posix()}')
