...
impl DataArrayPreservedPCAGoodness {
    pub fn goodness<'py>(
        ...
        let Some((max_dim, max_dim_rank)) = da_a
            ...
            .iter()?
            .map(|dimension| -> Result<(&str, usize), LocationError<PyErr>> {
                dimension?.extract().map_err(LocationError::new)
            })
            .reduce(|acc, elem| {
                let (acc, elem) = (acc?, elem?);
                if acc.1 > elem.1 || (acc.1 == elem.1 && acc.0 > elem.0) {
                    Ok(acc)
                } else {
                    Ok(elem)
                }
        ...
        let max_dim = PyString::new(py, max_dim);
        ...
        let abs_correlation_sum = (|| -> Result<f64, LocationError<PyErr>> {
            let model_a = Self::fit_pca_model(py, da_a, max_dim, n_modes, seed)?;
            let model_b = Self::fit_pca_model(py, da_b, max_dim, n_modes, seed)?;
    ...
    fn fit_pca_model<'py>(
        ...
        let model = eof.call(
            ...
            Some(
                [
                    (intern!(py, "n_modes"), n_modes.into_py(py).as_ref(py)),
                    (intern!(py, "compute"), PyBool::new(py, false)),
                    ...
                    (intern!(py, "random_state"), seed.into_py(py).as_ref(py)),
                    (intern!(py, "standardize"), PyBool::new(py, false)),
                ]
                .into_py_dict(py),
            ),
        )?;
        ...
        model.call_method(
            intern!(py, "fit"),
            (),
            Some([(intern!(py, "X"), da), (intern!(py, "dim"), dim)].into_py_dict(py)),
        )?;
