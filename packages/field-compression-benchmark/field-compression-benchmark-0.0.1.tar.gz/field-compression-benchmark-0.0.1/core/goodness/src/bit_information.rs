...
use numpy::{Element, PyArray1, PyUntypedArray};
...
impl DataArrayBitInformation {
    pub fn goodness<'py>(
        py: Python<'py>,
        da_raw: &'py PyAny,
        da_compressed: &'py PyAny,
    ) -> Result<BitInformationGoodness, LocationError<PyErr>> {
        let information_content_raw = Self::information_content(py, da_raw)?;
        let information_content_compressed = Self::information_content(py, da_compressed)?;
    ...
    pub fn information_content<'py>(
        ...
        fn information_content_typed<T: BitInformationElement + Element>(
            ...
            let bit_information =
                DataArrayBitInformation::bit_information_slice(data_slice, true, 0.99);
        ...
        let flattened_values: &PyUntypedArray = da
            .getattr(intern!(py, "values"))?
            .call_method0(intern!(py, "flatten"))?
            .extract()?;
    ...
    #[allow(unused)] // FIXME
    pub fn required_bits<T: BitInformationElement>(
        data: &[T],
        information_content: f64,
        set_zero_insignificant: bool,
        confidence: f64,
    ) -> usize {
        let bit_information = Self::bit_information_slice(data, set_zero_insignificant, confidence);
        ...
        bit_information
            ...
            .scan(0.0_f64, |state, i| {
                if (*state / total_information) >= information_content {
                    return None;
                }
    ...
    pub fn bit_information_slice<T: BitInformationElement>(
        ...
        if set_zero_insignificant {
            Self::set_zero_insignificant(mutual_information.as_mut(), front.len(), confidence);
        }
...
mod tests {
    ...
    fn bit_information_f32() {
        assert_close(
            DataArrayBitInformation::bit_information_slice(&[1.0_f32, 1.0], false, 0.99),
            [0.0_f64; 32],
        );

        assert_close(
            DataArrayBitInformation::bit_information_slice(&[1.0_f32, 2.0, 3.0, 4.0], false, 0.99),
            {
                let mut bit_information = [0.0_f64; 32];
    ...
    fn bit_information_f64() {
        assert_close(
            DataArrayBitInformation::bit_information_slice(&[1.0, 1.0], false, 0.99),
            [0.0_f64; 64],
        );

        assert_close(
            DataArrayBitInformation::bit_information_slice(&[1.0, 2.0, 3.0, 4.0], false, 0.99),
            {
                let mut bit_information = [0.0_f64; 64];
