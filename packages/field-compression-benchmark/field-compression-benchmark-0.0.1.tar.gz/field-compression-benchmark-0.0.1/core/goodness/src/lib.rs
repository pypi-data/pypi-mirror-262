...
pub struct DataArrayHistogram<'py> {
    ...
    _da_min: f64,
    _da_max: f64,
    edges: &'py PyArray1<f64>,
    bins: &'py PyArray1<usize>,
}
...
impl<'py> DataArrayHistogram<'py> {
    pub fn compute(
        ...
        let edges: &PyArray1<f64> = py
            .import(intern!(py, "numpy"))?
            .getattr(intern!(py, "linspace"))?
            .call1((da_min, da_max, bins.get() + 1))?
            .extract()?;
        let bins: &PyArray1<usize> = py
            .import(intern!(py, "xhistogram.core"))?
            .getattr(intern!(py, "histogram"))?
            .call((da,), Some([(intern!(py, "bins"), edges)].into_py_dict(py)))?
            .get_item(0)?
    ...
    pub fn approximate_quantiles_tuple(
        ...
        py: Python<'py>,
        percentiles: &[f64],
    ) -> Result<&'py PyTuple, LocationError<PyErr>> {
        let numpy = py.import(intern!(py, "numpy"))?;
        let zero_bins: &PyArray1<usize> = numpy
            .getattr(intern!(py, "insert"))?
            .call1((self.bins, 0, 0_usize))?
            .extract()?;
        let cumulative_bins: &PyArray1<usize> = numpy
            .getattr(intern!(py, "cumsum"))?
            .call1((zero_bins,))?
            .extract()?;
        let cdf: &PyArray1<f64> = cumulative_bins
            .call_method1(
                intern!(py, "__truediv__"),
                (cumulative_bins.get_item(cumulative_bins.len() - 1)?,),
            )?
            .extract()?;
