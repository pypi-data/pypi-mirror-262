...
impl DataArrayUniformityGoodness {
    pub fn goodness<'py>(
        ...
        let u01_entropy = (histogram.bins.len() as f64).log2();
        let da_entropy = histogram.entropy()?;

        let entropy_goodness = da_entropy / u01_entropy;

        let morans_i = Self::morans_i(py, histogram.da)?;
    ...
    pub fn morans_i<'py>(py: Python<'py>, da: &'py PyAny) -> Result<f64, LocationError<PyErr>> {
        // da_mean = da.mean()
        ...
        let Some(neighbours_sum) = da.getattr(intern!(py, "dims"))?.iter()?.try_fold(
            None,
            |acc, dim| -> Result<Option<&PyAny>, LocationError<PyErr>> {
                let dim: &PyString = dim?.extract()?;

                let left_neighbour = da.call_method1(shift, ([(dim, -1)].into_py_dict(py),))?;
                let right_neighbour = da.call_method1(shift, ([(dim, 1)].into_py_dict(py),))?;
