...
impl UnitRegistry {
    pub fn try_new(py: Python) -> Result<&Self, LocationError<PyErr>> {
        let pint = py.import(intern!(py, "pint"))?;
        let registry = pint.getattr(intern!(py, "UnitRegistry"))?.call0()?;

        registry.extract().map_err(LocationError::new)
    }

    pub fn resolve(&self, units: &str) -> Result<DataUnit, LocationError<PyErr>> {
        ...
        let py = self.registry.py();

        let verbose_unit = self.registry.call1((&fixed_units,))?;
        let verbose = ParsedDataUnit::parse(py, self, &fixed_units, verbose_unit)?;

        let base_unit = verbose_unit.call_method0(intern!(py, "to_base_units"))?;
        let base_unit_str = base_unit.str()?.to_str()?;
        let base = ParsedDataUnit::parse(py, self, base_unit_str, base_unit)?;

        Ok(DataUnit { verbose, base })
    }
}

impl<'py> FromPyObject<'py> for &'py UnitRegistry {
    fn extract(object: &'py PyAny) -> Result<Self, PyErr> {
        let py = object.py();
        let registry = py
            .import(intern!(py, "pint"))?
            .getattr(intern!(py, "UnitRegistry"))?;

        if !object.is_instance(registry)? {
            return Err(PyErr::new::<PyTypeError, _>(
                "not an instance of pint.UnitRegistry",
            ));
        }

        #[allow(unsafe_code)]
        // Safety:
        // - object is an instance of pint.UnitRegistry
        // - UnitRegistry is a transparent newtype wrapper around PyAny
        let registry: &'py UnitRegistry =
            unsafe { &*(object as *const PyAny).cast::<UnitRegistry>() };

        Ok(registry)
    }
}

impl<'py> ToPyObject for &'py UnitRegistry {
    fn to_object(&self, _py: Python) -> PyObject {
        Py::from(&self.registry)
    }
}

impl AsRef<PyAny> for UnitRegistry {
...
impl ParsedDataUnit {
    ...
    fn parse<'py>(
        ...
        let expression = if let Some(tree) = tree {
            let operator = py.import(intern!(py, "operator"))?;
            ...
            let partial = py
                .import(intern!(py, "functools"))?
                .getattr(intern!(py, "partial"))?;

            let expression: PyUnitExpression = tree
                .call_method(
                    intern!(py, "evaluate"),
                    (partial.call1((PyUnitExpression::type_object(py), registry))?,),
                    Some(kwargs),
                )?
                .extract()?;
...
impl PyUnitExpression {
    #[new]
    fn define<'py>(py: Python<'py>, registry: &'py PyAny, expr: &'py PyAny) -> Result<Self, PyErr> {
        let expr = expr.getattr(intern!(py, "string"))?;

        let expression = if let Ok(float) = py
            .import(intern!(py, "builtins"))?
            .getattr(intern!(py, "float"))?
            .call1((expr,))
        {
            let int = float.call_method0(intern!(py, "__int__"))?;
    ...
    fn __pos__(this: PyRef<Self>) -> PyRef<Self> {
        this
    }
    ...
    fn __pow__(&self, rhs: Self, _modulo: Option<&PyAny>) -> Self {
        Self {
