...
struct DatasetVariablesSeed<'py> {
    py: Python<'py>,
    ds: &'py PyAny,
...
impl<'py, 'de> serde::de::DeserializeSeed<'de> for DatasetVariablesSeed<'py> {
...
impl<'py, 'de> serde::de::Visitor<'de> for DatasetVariablesSeed<'py> {
    type Value = (VecMap<String, DataVariable>, VecSet<String>);
...
impl<'a, 'py, 'de> serde::de::Visitor<'de> for DatasetSeed<'a, 'py> {
    ...
    fn visit_map<A: serde::de::MapAccess<'de>>(self, mut map: A) -> Result<Self::Value, A::Error> {
        ...
        let variables: VecMap<String, DataVariable> =
            DataVariable::extract_from_dataset(self.py, ds, self.unit_registry).map_err(|err| {
                serde::de::Error::custom(format!("failed to extract the dataset variables: {err}"))
            })?;
