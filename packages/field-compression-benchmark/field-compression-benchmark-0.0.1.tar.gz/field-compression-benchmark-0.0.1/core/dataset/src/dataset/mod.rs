...
impl Dataset {
    ...
    pub fn open_xarray<'py>(&self, py: Python<'py>) -> Result<&'py PyAny, LocationError<PyErr>> {
    ...
    pub fn open_xarray_sliced_variable<'py>(
        ...
        for (dim_name, dimension) in variable.dimensions() {
            data_array = dimension.slice().sel(py, data_array, dim_name)?;
        }
...
fn open_xarray_dataset<'py>(
    ...
    let xarray = py.import(intern!(py, "xarray"))?;
