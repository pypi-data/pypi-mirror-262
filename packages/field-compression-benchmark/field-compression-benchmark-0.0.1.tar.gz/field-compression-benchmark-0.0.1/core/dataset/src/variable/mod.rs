...
use crate::units::{DataUnit, DataUnitSummary, UnitRegistry};
    ...
    pub fn extract_from_dataset<'py>(
        ...
        for name in dataset.iter()? {
            ...
            let units = match variable.getattr(intern!(py, "units")) {
                Ok(units) => Some(unit_registry.resolve(units.extract()?)?),
                Err(_) => None,
            };
            let dtype = match variable.getattr(intern!(py, "dtype"))?.str()?.extract()? {
...
impl<'a> DataVariableReductionIterator<'a> {
    pub fn next<'py>(&mut self, py: Python<'py>) -> Option<&'py PyTuple> {
        if self.all_done {
            return None;
        }
