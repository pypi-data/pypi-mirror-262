...
use core_goodness::{
    ...
    pca::DataArrayPreservedPCAGoodness, uniformity::DataArrayUniformityGoodness,
    DataArrayHistogram,
};
    ...
    for (derivatives, measurements) in derivatives.iter().zip(measurements.iter_mut()) {
            ...
            if prev.head.is_none() || Some(&derivatives.head) == prev.head {
                if let Some(suffix) = derivatives.tail.strip_prefix(prev.tail) {
                    ...
                    if let Some(next) = match prev.head {
                        ...
                        let (py_data_array, py_data_array_compressed) =
        ...
        Python::with_gil(|py| {
            compute_goodness_measurements(
                py,
                py_data_array_derivative.as_ref(py),
                py_data_array_compressed_derivative.as_ref(py),
                measurements,
                error_settings,
                seed,
            )
...
pub(crate) fn compute_goodness_measurements<'py>(
    ...
    let py_data_array_error_histogram =
        DataArrayHistogram::compute(py, py_data_array_error, error_settings.bins)?;
    let py_data_array_error_abs_histogram =
        DataArrayHistogram::compute(py, py_data_array_error_abs, error_settings.bins)?;
    let py_data_array_error_rel_histogram =
        DataArrayHistogram::compute(py, py_data_array_error_rel, error_settings.bins)?;
    let py_data_array_error_rel_abs_histogram =
        DataArrayHistogram::compute(py, py_data_array_error_rel_abs, error_settings.bins)?;
    ...
    measurements.preserved_pca.push({
        let py_data_array = py_data_array.to_object(py);
        let py_data_array_compressed = py_data_array_compressed.to_object(py);

        py.with_pool(|py| {
            DataArrayPreservedPCAGoodness::goodness(
                py,
                py_data_array.as_ref(py),
                py_data_array_compressed.as_ref(py),
                seed,
            )
        })?
    });

    measurements.bit_information.push({
        let py_data_array = py_data_array.to_object(py);
        let py_data_array_compressed = py_data_array_compressed.to_object(py);

        py.with_pool(|py| {
            DataArrayBitInformation::goodness(
                py,
                py_data_array.as_ref(py),
                py_data_array_compressed.as_ref(py),
            )
