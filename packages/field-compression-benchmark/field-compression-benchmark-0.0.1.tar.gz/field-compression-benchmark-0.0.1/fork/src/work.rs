...
use ipc_channel::ipc::{channel, IpcError, IpcReceiver, TryRecvError};
...
pub enum DistributedWorkError {
    ...
    #[error("failed to create a channel to the worker process")]
    ...
    fn fill_up_with_work(&mut self) {
        while self.max_processes > 0 {
            ...
            let (result_sender, result_receiver) = match channel() {
                Ok(result_channel) => result_channel,
                Err(err) => {
                    self.queue.push_back(WorkItem::Done {
                        args: work,
                        result: Err(DistributedWorkError::CreateChannel(err).into()),
                    });
