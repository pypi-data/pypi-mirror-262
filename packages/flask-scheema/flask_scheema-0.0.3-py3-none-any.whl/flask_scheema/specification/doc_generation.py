...
def make_endpoint_description(schema: Schema, http_method: str, **kwargs):
    ...
    name = (
        ...
        if kwargs.get("model")
        else schema.get_model().__name__ if schema.get_model() else schema.__name__
    ).replace("Schema", "")
...
def generate_query_params_from_rule(rule, methods, schema, many, model) -> List[dict]:
    """
            ...
            if rule:
                ...
                for http_method in methods:
                    ...
                    query_params = generate_query_params_from_rule(
                        rule, methods, output_schema, many, model
                    )
...
def register_schemas(
    ...
    if input_schema:
        ...
        case = get_config_or_model_meta(
            "API_SCHEMA_CASE", model=input_schema.get_model(), default="camel"
        )
...
def initialize_spec_template(method, many=False, rate_limit=False):
    ...
    # Adjust for non-POST methods when not returning multiple items
    if method == "DELETE" and not many:
        responses["409"] = {"description": "Delete Conflict - Related resources exist"}
...
def append_parameters(
    ...
    if output_schema:
        ...
        name = output_schema.__name__.replace("Schema", "")
