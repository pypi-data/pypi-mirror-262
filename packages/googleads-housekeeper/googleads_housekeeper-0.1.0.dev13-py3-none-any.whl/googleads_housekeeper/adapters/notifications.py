...
class GoogleCloudAppEngineEmailNotifications(BaseNotifications):
    ...
    def _prepare_message(self, payload: MessagePayload) -> mail.Message:
        ...
        if placements_excluded := payload.placements_excluded_sample:
            ...
            body = self._generate_notification_html(
                task_name, payload.task_formula, placements_excluded)
        else:
    ...
    def _generate_notification_html(self, task_name: str, task_formula: str, placements: gaarf.report.GaarfReport) -> str:
        html_body = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        html_body += f"""
                ...
                <p><strong>Formula:</strong> {task_formula} (<a href="" class="task-link"></a>)
                </p>"""
        ...
        for placement in placements:
            html_body += f"""
                <ul class="placement-list">
                    ...
                    Reason: {placement.reason.lower().replace('_', ' ')}</li>
                </ul>
            """
        html_body += """
            </div>
