...
def __datatime_widget(df, column, ss_name, container_widget: DeltaGenerator, page_id=''):
    # df[column] = pd.to_datetime(df[df[column].notna()][column], format='%Y%m%d')
    max = df[column].max()
    min = df[column].min()
    container_widget.date_input(
        f"{column.title()}",
        (min, max),
        min,
        max,
        ...
        with container_widget.expander("Filter Condition", expanded=True):
            ...
            for ctype, column in zip(df.dtypes, df.columns):
                ...
                if column in custom_column_widget_type:
                    ...
                    elif custom_column_widget_type[column] == "date_input":
                        __datatime_widget(df, column, column.lower(), container_widget, page_id=page_id)
    ...
    for widget in all_widgets:
        ...
        if data:
            if ctype == "text":
                ...
                if len(data) == 1:
                    min = data[0] 
                    res = res.loc[(res[column] >= str(min))]
                else:
                    min, max = data
                    min = np.datetime64(min)
                    max = np.datetime64(max)
                    res = res.loc[(res[column] >= str(min)) & (res[column] <= str(max))]
