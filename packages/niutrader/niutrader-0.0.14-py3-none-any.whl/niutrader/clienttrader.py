...
from niutrader import grid_strategies, pop_dialog_handler, refresh_strategies
from niutrader.config import client
from niutrader.grid_strategies import IGridStrategy
from niutrader.log import logger
from niutrader.refresh_strategies import IRefreshStrategy
from niutrader.utils.misc import file2dict
from niutrader.utils.perf import perf_clock

if not sys.platform.startswith("darwin"):  # 打印电脑系统，win32是windows的，darwin是苹果的，linux是linux的
    import pywinauto
...
class ClientTrader(IClientTrader):
    ...
    grid_strategy: Union[IGridStrategy, Type[IGridStrategy]] = grid_strategies.Copy  # 实例化一个对象
    ...
    def connect(self, exe_path=None, **kwargs):  # 连接客户端
        """
        直接连接登陆后的客户端
        ...
        self._app = pywinauto.Application().connect(path=connect_path, timeout=10)  # 连接客户端
        self._close_prompt_windows()  # 关闭提示窗口
    ...
    def _get_balance_from_statics(self):  # 获取资金信息
        result = {}
        for key, control_id in self._config.BALANCE_CONTROL_ID_GROUP.items():  # 循环一个字典
            result[key] = float(
                self._main.child_window(
                    control_id=control_id, class_name="Static"  # 根据id获取资金信息
                ).window_text()
            )
        return result
    ...
    @perf_clock
    def close_pop_dialog(self):   # 关闭弹窗
        try:
    ...
    def _get_grid_data(self, control_id):  # self._get_grid_data(self._config.COMMON_GRID_CONTROL_ID)， COMMON_GRID_CONTROL_ID = 1047
    ...
    @perf_clock
    def _switch_left_menus(self, path, sleep=0.2):   # 这里path是 _switch_left_menus(["撤单[F3]"])
        self.close_pop_dialog()   # 关闭弹窗
        self._get_left_menus_handle().get_item(path).select()
        self._app.top_window().type_keys('{F5}')  # 这里又什么作用呢？是不是刷新？
        self.wait(sleep)
        ...
        while True:
            try:
                handle = self._main.child_window(
                    control_id=129, class_name="SysTreeView32"   # 这里129是左边栏所有的，classname也是的
                )
                if count <= 0:
                    return handle
