...
class AutoSampler(BaseSampler):
  ...
  """
  def __init__(self, configuration, project, num_random_samples=10, min_delta_score=0.01):
    super().__init__(configuration)
    self.min_delta_score = min_delta_score
    self.num_random_samples = num_random_samples
    self.project = project
    self.optimize_keys = self.config_grad_keys()

    self.random_sampler = RandomSampler(configuration)
    self.best_random_experiment = None

    # TODO: Adam optimizer still worse than the simpler base optimizer...
    self.optimizer = BaseOptimizer(self.configuration.keys(), self.optimize_keys, learning_rate=0.1)
  ...
  # TODO: Need to handle non-numeric config params, currently assume numeric only.
  def __next__(self):
