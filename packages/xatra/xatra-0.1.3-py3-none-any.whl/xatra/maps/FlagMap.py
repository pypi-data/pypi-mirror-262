...
class FlagMap:
    ...
    Attributes:
        ...
        options (Dict[str, any]): Dict with keys:
            ...
            "text_outline_width" (str): Width of the text outline. Defaults to 1px. Set to None to disable.
            "text_outline_color" (str): Color of the text outline. Defaults to '#FFFFFF'. Set to None to disable.
            "font_size" (str): Font size of the flag labels. Defaults to "10px".
    ...
    def _draw_flag_label(self, flag_name, year="static"):
        ...
        if self.text_outline_width is not None and self.text_outline_color is not None:
            text_outline = (
                f"text_shadow: -{self.text_outline_width} -{self.text_outline_width} 0 {self.text_outline_color}, "
        ...
        return folium.Marker(
            ...
            icon=folium.DivIcon(
                ...
                icon_anchor=(75, 18),
                html=(
                    f'<div style="font-size: {self.font_size}; color: {text_color}; font-weight: bold; '
                    f'filter: brightness(0.5); text-align: center; {text_outline}">{flag_name}</div>'
                ),
            ),
        )
            ...
            for year in self.breakpoints:
                ...
                layer.add_child(
                    folium.GeoJson(
                        ...
                        tooltip=folium.GeoJsonTooltip(fields=["NAME_max"]),
                    )
                )
                flag_marker = self._draw_flag_label(flag, year)
                if flag_marker:
...
class AbstractMap(ABC):
    """Abstract class for Maps: subclass this with custom methods .flags(), .geojson(), and
    ...
    def _geojson_with_flags(self, year=None):
        ...
        for feature in self.geojson:
            if self.verbose:
                print(
                    f"Map {self.__class__.__name__}: Adding flags to feature {NAME_max(feature)}"
                )
            feature_copies = []
            matching_flags = [
                flag.name
            ...
            for flag_name in matching_flags:
                ...
                feature_copy["properties"]["NAME_max"] = NAME_max(
                    feature_copy
                )  # for tooltip
    ...
    def _color_map(self):
        ...
        cmap = mpl.cm.get_cmap("tab20", len(flag_names))
        color_mapping = {}
        colors_per_segment = max(
    ...
    def plot(self, path_out=None, return_map=False):
        ...
        for year, features in self.geojson_by_year.items():
            ...
            for feature in features:
                if self.verbose:
                    print(
                        f"Map {self.__class__.__name__}: Adding feature {feature['properties']}"
                    )
