...
from Classes.elution_profile import ElutionProfile  # Add missing import statement
import numpy as np
...
with open(
    "/home/agrimaud/Desktop/Thesis/Projects/ProteoformQuant2/ProteoformQuant2/src/proteoformquant/Output/multi_rank/obj_3BKO_3_2_param_5.pkl",
    "rb",
) as inp:
    exp = pickle.load(inp)
    ...
    children=[
        ...
        html.Div(
            children=[
                ...
                dcc.Graph(
                    id="plot_envelopes_parameters", style={"display": "inline-block"}
                ),
    ...
    for node in G.nodes:
        delta_mods_tot = sum(
            [
                ...
                for mod in exp.proteoforms[node].get_modification_dict()
            ]
        )
...
def plotAllEnvelopes(input):
    ...
    i = 0
    for proteoform in [
        x for x in exp.proteoforms.values() if x.get_elution_profile() != None
    ]:
        c = colors["hex"][i]
        i += 1
        if proteoform.get_elution_profile() != None:
...
def plotRelativeAbundanceProteo(input):
    proteoformsBrno = [
        proteo.get_modification_brno() for proteo in exp.proteoforms.values()
    ]
    ...
    proteoformsIntens_auc = [
        proteo.update_proteoform_total_intens(method="AUC")
        for proteo in exp.proteoforms.values()
    ]
    ...
    proteoformsRatio_auc = [
        proteoIntens / sum(proteoformsIntens_auc)
        for proteoIntens in proteoformsIntens_auc
    ]
    ...
    fig = px.bar(
        x=proteoformsBrno,
        y=[proteoformsRatio_prec, proteoformsRatio_auc],
        barmode="group",
    )
