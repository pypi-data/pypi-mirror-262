        ...
        html.Div(
            ...
            children=[
                html.Div(
                    children=[
                        html.H1(
                            "Proteoformquant QC report", style={"textAlign": "center"}
                        )
                    ],
                    id="title_div",
                ),
                ...
                html.Div(
                    children=[
                        html.Div(
                            children=[
                                html.H6(
                                    "Proteoform grouping", style={"textAlign": "center"}
                                ),
        ...
        html.Div(
            ...
            children=[
                html.Div(
                    children=[
                        html.H6(
                            "Proteoforms' elution profiles",
                            style={"textAlign": "center"},
                        ),
                        dcc.Dropdown(
                            id="dropdown_proteoforms",
                            multi=True,
...
def load_data(list_of_names, list_of_contents):
    ...
    return html.Ul(
        children=[html.Li(i) for i in file_names],
    ...
    data_table = [
        {
            ...
            dt_tbl_col[2]: len(
                [
                    s
                    for s in msruns[j].spectra.values()
                    if s.get_number_validated_psm() > 0
                ]
            ),
            dt_tbl_col[3]: len(
                [
                    s
                    for s in msruns[j].spectra.values()
                    if s.get_number_validated_psm() > 1
                ]
            ),
            ...
            dt_tbl_col[5]: len(
                [
                    p
                    for p in msruns[j].proteoforms.values()
                    if p.get_number_validated_linked_psm() > 0
                ]
            ),
        }
        for j in range(len(msruns))
...
def dpca_sample_plot(x):
    ...
    for j in range(len(msruns)):
        ...
        df = pd.DataFrame(
            {"proforma": proteo_proforma, file_names[j]: proteo_intens_prec}
        )
...
@app.callback(
    [
        Output("dropdown_sample_page3", "options"),
        Output("dropdown_sample_page6", "options"),
    ],
...
@app.callback(
    Output("plot_network_isobaric", "figure"), Input("dropdown_sample_page3", "value")
)
    ...
    for node in G.nodes:
        delta_mods_tot = sum(
            [
                ...
                for mod in run.proteoforms[node].get_modification_dict()
            ]
        )
...
@app.callback(
    Output("plot_all_enveloppes", "figure"), Input("dropdown_sample_page3", "value")
)
def plotAllEnvelopes(value):
    ...
    i = 0
    for proteoform in [
        x for x in run.proteoforms.values() if x.get_elution_profile() != None
    ]:
        c = colors["hex"][i]
        i += 1
        if proteoform.get_elution_profile() != None:
            env = (
                proteoform.get_elution_profile()
            )  # if envelope has been computed add line to the plot

            data_yEnv = list(range(int(min(rt_range)), int(max(rt_range)), 1))
            zDataEnv = [proteoform.getMzFirstPsm() for x in data_yEnv]
            proteoform_key = np.repeat(
                proteoform.get_modification_proforma(), [len(data_yEnv)], axis=0
            )

            yDataEnvFitted, parametersFitted = list(
                env.get_y_serie(data_yEnv, method="fitted")
            )
            if yDataEnvFitted[0] != None:
                ...
                yDataEnvEstim, parametersEstim = list(
                    env.get_y_serie(data_yEnv, method="estimated")
                )
                ...
                dbc.ModalBody(
                    html.Div(
                        [
                            ...
                            dcc.Graph(
                                figure=fragments_plot(spectrum), id="fragment_plot"
                            ),
                            html.Div(
                                children=[
                                    dcc.Graph(
                            ...
                            dcc.Graph(
                                figure=spectrum_plot(spectrum), id="plot_spectrum"
                            ),
                        ]
                    )
                ),
                ...
                dbc.ModalBody(
                    html.Div(
                        [
                            html.P(str_info, style={"fontFamily": "monospace"}),
                            dcc.Graph(
                                figure=plot_psms_rank_distribution(
                                    proteoform, msruns[run_i].max_rank
                                ),
...
def fragments_plot(spectrum):
    ...
    for psm in spectrum.get_psms():
        ...
        for dir in ["n-term", "c-term"]:
            ...
            full_annot_intens = [
                psm.get_intensity_at_pos(pos, dir)
                for pos in range(1, len(psm.proteoform.peptideSequence))
            ]
...
def spectrum_plot(spectrum):
    ...
    for psm in spectrum.get_validated_psm():
        ...
        fragMz = [
            j
            for i in [fragType["mzTheo"] for fragType in psm.annotation.values()]
            for j in i
        ]
        fragIntens = [
            j
            for i in [fragType["intens"] for fragType in psm.annotation.values()]
            for j in i
        ]
        fragIntens = [i + (m * 1000) for i in fragIntens]
        frag_code = [
            j
            for i in [fragType["fragCode"] for fragType in psm.annotation.values()]
            for j in i
        ]
        # print(frag_code)
...
def plot_psms_rank_distribution(proteoform, max_rank):
    ...
    fig.add_trace(
        go.Bar(
            x=ranks, y=counts, base=0, marker=dict(color=ranks, colorscale="Bluered")
        )
    )
    return fig
...
def ms2_chromatogram_plot(proteoform, top_n_frag=40):
    ...
    mean_elution_profile_model = (
        proteoform.get_elution_profile().get_x_at_max_y()
    )  # get RT of elution peak
    ...
    annotated_frag_code_intens = peak_psm.get_annotation_pair_format(
        "fragCode", "intens"
    )
    frag_codes, intensities = zip(*annotated_frag_code_intens)  # unzip values

    if top_n_frag < len(intensities):
        idx_top_frags = sorted(
            range(len(intensities)), key=lambda i: intensities[i], reverse=True
        )[:top_n_frag]
        # print(idx_top_frags)
    else:
    ...
    for psm in linked_psms:
        ...
        intens_row = [
            dict_psm_annot_pair[code] if code in dict_psm_annot_pair.keys() else 0
            for code in frag_codes_top
        ]
...
@app.callback(
    Output("dropdown_proteoforms", "value"),
    Input("plot_network_isobaric", "selectedData"),
)
...
@app.callback(
    Output("dropdown_proteoforms", "options"), Input("dropdown_sample_page3", "value")
)
...
@app.callback(
    Output("plot_elution_profiles", "figure"), [Input("dropdown_proteoforms", "value")]
)
def plot_elution_profiles(proteoforms_input):
    ...
    for proteo in proteoforms_input:
        x_val = [
            psm.spectrum.get_rt() for psm in exp.proteoforms[proteo].get_linked_psm()
        ]
        ...
        y_val = [
            psm.spectrum.get_prec_intens()
            for psm in exp.proteoforms[proteo].get_linked_psm()
        ]
    ...
    for proteo in proteoforms_input:
        if len(exp.proteoforms[proteo].get_validated_linked_psm()) > 0:
            ...
            spectrum_key = [
                psm.spectrum.id for psm in exp.proteoforms[proteo].get_linked_psm()
            ]
            fig.add_scatter(
                x=data_x_all,
                y=data_y_all,
            ...
            data_y = [
                psm.spectrum.get_rt()
                for psm in exp.proteoforms[proteo].get_validated_linked_psm()
            ]
            data_y_spectrum = [
                psm.spectrum.get_prec_intens()
                for psm in exp.proteoforms[proteo].get_validated_linked_psm()
            ...
            if (
                elution_profile != None
            ):  # if elution profile model has been computed add line to the plot
                data_x_elution_profile = list(
                    range(int(x_min_max[0]), int(x_min_max[1]), 1)
                )
                ...
                data_y_elution_profile_estimated, params_fitted = list(
                    elution_profile.get_y_serie(
                        data_x_elution_profile, method="estimated"
                    )
                )

                proteoform_key = np.repeat(
                    proteo, [len(data_x_elution_profile)], axis=0
                )
...
def plotAllEnvelopes3d(is_displayed):
    ...
    i = 0
    for proteoform in [
        x for x in proteoFilt.values() if x.get_elution_profile() != None
    ]:
    ...
    for proteoform in [x for x in proteoFilt.values()]:
        precIntens = [
            psm.spectrum.get_prec_intens()
            for psm in proteoform.get_validated_linked_psm()
        ]
        rt = [psm.spectrum.get_rt() for psm in proteoform.get_validated_linked_psm()]
        mz = [
            psm.spectrum.get_prec_mz() for psm in proteoform.get_validated_linked_psm()
        ]
        spectrum_key = [
            psm.spectrum.get_id() for psm in proteoform.get_validated_linked_psm()
        ]
    ...
    fig.update_layout(
        template=template, height=1000, scene=dict(zaxis=dict(range=[minMz, maxMz]))
    )
...
@app.callback(
    Output("barplot_peptide_quant", "figure"), Input("dropdown_sample_page6", "value")
)
def plot_relab_peptide(value):
    ...
    proteoformsBrno = [
        proteo.get_modification_brno() for proteo in run.proteoforms.values()
    ]
    ...
    proteoformsRatio_prec = [
        proteoIntens / sum(proteoformsIntens_prec)
        for proteoIntens in proteoformsIntens_prec
    ]
    ...
    proteoforms_protein = [
        ";".join(proteo.get_protein_ids()) for proteo in run.proteoforms.values()
    ]
    ...
    fig = px.bar(
        x=proteoformsBrno,
        y=proteoformsRatio_prec,
        color=proteoforms_protein,
        barmode="group",
    )
