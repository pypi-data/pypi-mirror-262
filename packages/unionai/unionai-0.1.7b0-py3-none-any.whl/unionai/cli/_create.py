...
@create.command()
@click.pass_context
@click.option("--auth", type=click.Choice(["device-flow", "pkce"]), default="pkce")
def config(ctx: click.Context, auth: str):
    f"""Create config at {_UNIONAI_DEFAULT_CONFIG_PATH}."""
    ...
    try:
        ...
        remote.client
        config_path = _write_config_to_path(config.platform.endpoint, auth_mode.value)
        click.echo(
            f"Config was created and written to {config_path}. The `unionai` CLI will now use this configuration."
        )

    except Exception as e:
        raise click.ClickException(f"Unable to create config.\n{e}") from e
