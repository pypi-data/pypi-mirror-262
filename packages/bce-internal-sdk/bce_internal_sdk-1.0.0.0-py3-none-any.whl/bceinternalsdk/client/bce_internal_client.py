...
class BceInternalClient(BceBaseClient):
    ...
    def _send_request(self, http_method, path, headers=None, params=None, body=None):
        ...
        if self.context is not None:
            headers = dict()
            headers[b"x-impersonate-target-org-id"] = self.context.get("OrgID", "").encode('utf-8')
            headers[b"x-impersonate-target-user-id"] = self.context.get("UserID", "").encode('utf-8')
            headers[b"x-impersonate-target-project-id"] = self.context.get("ProjectID", "").encode('utf-8')
        return bce_http_client.send_request(self.config, sign([b'host', b'x-bce-date']),
                                            [parse_json],
