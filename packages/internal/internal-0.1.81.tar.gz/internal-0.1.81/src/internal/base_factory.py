...
class BaseFactory(metaclass=ABCMeta):
    ...
    def create_app(self, title=None) -> FastAPI:
        ...
        mongodb = database.MongoDB(self.get_app_config().DATABASE_URL, self.get_app_config().DATABASE_NAME, self.get_app_config().DATABASE_SSL, self.get_app_config().DATABASE_SSL_CA_CERTS)
