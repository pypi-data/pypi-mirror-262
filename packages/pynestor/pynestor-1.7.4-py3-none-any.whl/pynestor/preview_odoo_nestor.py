...
class ScriptNestor:
    ...
    def wait_all_down(self):
        """Attend que la BDD et odoo soit down"""
        self.inst.exit_if_failed(self.inst.wait(up=False, postgres=True, timeout=5 * 60))
        self.inst.exit_if_failed(self.inst.wait(up=False, timeout=5 * 60))
