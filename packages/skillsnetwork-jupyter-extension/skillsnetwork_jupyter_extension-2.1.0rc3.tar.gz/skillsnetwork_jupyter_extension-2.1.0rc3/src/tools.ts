...
export const openIndependentLab = async (
  ...
  if (ENV_TYPE() !== EnvType.JUPYTERLITE) {
    // Outside of JupyterLite we want to use a relative path
    filePath = `./${filePath}`;
  ...
  Globals.TOKENS.set(filePath, awb_token);
  ...
  if (!prevFile) {
    nbPanel = await createNotebook(docManager, filePath, instructions_content);
  } else {
...
export const openLab = async (
  ...
  let labFilename = getLabFileName(instructions_file_path);
  ...
  if (ENV_TYPE() !== EnvType.JUPYTERLITE) {
    ...
    // Inside of JupyterLite we don't want persistent storage for authors, so we delete the existing file if it exists
    await docManager.deleteFile(labFilename);
  }
