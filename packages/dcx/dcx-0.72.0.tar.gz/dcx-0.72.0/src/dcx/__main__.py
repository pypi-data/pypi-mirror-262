...
def process_report():
    if args.report:
        ...
        for report_i in range(0, len(play)):
            ...
            pdf.page(the_page).text( 7,10, "STEP (1..)                 :  %d" % active_report_i)
            pdf.page(the_page).text( 8,10, "STEP ENTERED               :  -")
            pdf.page(the_page).text( 9,10, "STEP LEFT                  :  -")
            pdf.page(the_page).text(10,10, "STEP DURATION              :  -")
            pdf.page(the_page).text(11,10, "TOTAL DURATION (incl this) :  -")
            
            pdf.page(the_page).text(13,10, "URL BEFORE :  %s" % "https://")
            pdf.page(the_page).text(14,10, "URL AFTER  :  %s" % "https://")
            ...
            pdf.page(the_page).text(70,1, "\n".join("after"))
            pdf.page(the_page).text(60,4, "\n".join("|"*40))
            #pdf.page(the_page).text(60,5, "\n".join("|"*40))

            pdf.page(the_page).text(70,135, "\n".join("after"))
            #pdf.page(the_page).text(60,4+127, "\n".join("|"*40))
            pdf.page(the_page).text(60,5+127, "\n".join("|"*40))

            pdf.page(the_page).text(15,10, "_"*52 + " [ BEFORE  ] " + "_"*52)
            pdf.page(the_page).text(59,10, "_"*52 + " [ AFTER   ] " + "_"*52)
            ...
            if jpg_landscape:
                ...
                image_params = [25, 200, screenshot_w_default_mm, calclulated_height, jpg]
            else:
                #TODO: porttrait mode
                pass
            ...
            if jpg_landscape:
                ...
                jpg_ratio = float(jpg_width) / float(jpg_height) # 1.XXXX
                calclulated_height = int(float(screenshot_w_default_mm) / jpg_ratio)
                image_params = [25, 20, screenshot_w_default_mm, calclulated_height, jpg]
            else:
                #TODO: porttrait mode
                pass
            pdf.page(the_page).image_manual_mm(*image_params)
