...
co_expression_r <- function(final_exp, power, module_merging_threshold, network_type, cor_method, w=400,h=400, verbose = FALSE){

    if (power == 0) {power = NULL}
    
    net <- exp2gcn(final_exp, module_merging_threshold=module_merging_threshold, net_type=network_type, 
                   SFTpower=power, cor_method=cor_method, verbose = FALSE)
    names(net)
