...
def local_client(host='127.0.0.1', port=19001):
    ...
    class _:
        @ensure_connect
        def set_match_url(match_url=None, fakeresponse=None, timeout=5000, qtimeout=5):
            data = json.dumps({ 
                "type": "set_match_url", 
                "match_url": match_url, 
        ...
        @ensure_connect
        def remove_match_url(match_url=None, qtimeout=5):
            data = json.dumps({ "type": "remove_match_url", "match_url": match_url })
            cu_connector.send_message_to_all(data)
            return try_json_result(que.get(timeout=qtimeout))
        @ensure_connect
        def clear_catch(qtimeout=5):
            data = json.dumps({ "type": "clear_catch" })
            cu_connector.send_message_to_all(data)
            return try_json_result(que.get(timeout=qtimeout))
        @ensure_connect
        def get_url_by_scripts(match_url=None, script=None, fakeresponse=None, timeout=5000, qtimeout=15):
            data = json.dumps({ 
        ...
        @ensure_connect
        def run_scripts(script, timeout=5000, qtimeout=15):
            data = json.dumps({ 
                "type": "run_script", 
                "script": script,
                "timeout": timeout,
        ...
        @ensure_connect
        def get_url_by_elements(match_url=None, elements=None, fakeresponse=None, timeout=5000, qtimeout=15):
            data = json.dumps({ 
