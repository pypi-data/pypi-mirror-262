...
def local_client(host='127.0.0.1', port=19001):
    ...
    class _:
        @ensure_connect
        def get_url_by_scripts(match_url=None, script=None, fakeresponse=None, timeout=5000):
            data = json.dumps({ 
            ...
            return try_json_result(que.get(timeout=3))
        get_url_by_script = get_url_by_scripts
        @ensure_connect
        def set_match_url(match_url=None, fakeresponse=None, timeout=5000):
            data = json.dumps({ 
                "type": "set_match_url", 
                "match_url": match_url, 
                "fakeresponse": fakeresponse, 
            ...
            return try_json_result(que.get(timeout=3))
        @ensure_connect
        def remove_match_url(match_url=None):
        ...
        def clear_catch():
            ...
            return try_json_result(que.get(timeout=3))
        @ensure_connect
        def run_script(script, timeout=5000):
            data = json.dumps({ 
                "type": "run_script", 
                "script": script,
                "timeout": timeout,
            ...
            cu_connector.send_message_to_all(data)
            return try_run_result(que.get(timeout=3))
    return _
