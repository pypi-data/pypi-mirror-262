...
__all__ = ['T', 'K', 'P', 'isnone', 'notnone', 'istype', 'isdict', 'isparam', 'isvarg', 'isvkws', 'issig', 'ispsa', 'ispsk',
           ...
           'unmangled_annotations', 'in_lookup', 'adjust_defaults_position', 'adjust_var_position',
           'put_var_in_signature', 'wrapper_signature_update', 'signature_from_annotations', 'typed_dict_signature',
           'typed_dict_defaults', 'typed_dict_func', 'get_typed_dict_keywords', 'typedict', 'dtyp', 'typekwds', 'tdict',
           'new', 'setkeywords']
...
def adjust_defaults_position(
    __parameters: dict[str, Parameter] | list[Parameter],
) -> list[Parameter]:
    '''
    Adjust the order of parameters so that parameters without default values
    come before those with default values, except for VAR_POSITIONAL parameters,
    which retain their original positions.

    Parameters
    ----------
    __parameters : Union[Dict[str, Parameter], List[Parameter]]
        A dictionary or list of Parameter objects to be reordered.

    Returns
    -------
    List[Parameter]
        The list of reordered Parameter objects.
    '''
    prms = list(__parameters.values()) if isdict(__parameters) else __parameters
    # Separate parameters based on their kind and whether they have a default value
    has_val = [p for p in prms if p.default != Empty and not isvarg(p)]
    missing = [p for p in prms if p.default == Empty or isvarg(p)]
    
    # The separation of has_val and missing should already ensure correct ordering,
    # but we need to ensure VAR_POSITIONAL parameters stay in their original place.
    # Since VAR_POSITIONAL parameters are included in no_default, no additional steps are
    # required to move them. However, we should avoid introducing duplicates.
    
    # Remove duplicates from no_default while preserving order
    seen = set()
    uniq = [x for x in missing if not (x in seen or seen.add(x))]
    return uniq + has_val
    
    
    print(has_val, missing)
    # Sort VAR_POSITIONAL parameters to maintain their original order
    varpos = [p for p in prms if {ParamKind.POSITIONAL_ONLY, ParamKind.POSITIONAL_OR_KEYWORD, }]
    for vp in varpos:
        idx = prms.index(vp)
        # Find where to insert the VAR_POSITIONAL parameter in missing to maintain original order
        put = 0
        for p in missing:
            if prms.index(p) > idx:
                break
            put += 1
        missing.insert(put, vp)
        # Ensure it's not duplicated if it was already included
        missing = [p for i, p in enumerate(missing) if p not in missing[:i]]
        print(idx, put, missing)
    # Combine the lists, keeping non-default parameters first
    return missing + has_val
    ...
    **kwargs : P.kwargs
    ...
    for kwd, ann in zip(kwds, anns.values()):
        ...
        if (            
            ...
            or (isnone(val) and isopt) # None is ok if it is an Optional value
            and not (
    ...
    prms = adjust_defaults_position(prms)
