...
def build_docker_image(dockerfile: Path, token: str, nocache: bool = False) -> docker.models.images.Image:
    client = docker.from_env()
...
def push(
    ...
    if url in CACHE and Path(CACHE[url][1]).exists():
        ...
        image_path = build(dockerfile, token, nocache)
