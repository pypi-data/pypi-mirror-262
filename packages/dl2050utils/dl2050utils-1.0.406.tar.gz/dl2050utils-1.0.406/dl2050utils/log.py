...
class BaseLog():
    ...
    def log(self, level, t, label='', label2='', msg=''):
        dt = datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%S')
        if level<1 or level>4: return
        LEVELS = ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']
        msg = f'{dt} {LEVELS[level]} {t} {self.service} {label} {label2} {msg}'
        print(msg)
        if self.p:
            with open(self.p, 'a') as f:
                f.write(msg)
    def __call__(self, level, t, label='', msg=''):
        self.log(level, t, label='', msg='')
