...
import argparse,sys
def _cli():
    ap=argparse.ArgumentParser(description='BFFuck CLI tool\nMakes Brainfucking Easier!\nEnjoy brainfucking with the tool\nIf you encounter any error, you can post an issue at GitHub.')
    ap.add_argument('program',help='BFFuck program file name')
    ap.add_argument('--playfield','-p',help='Number of bytes for playfield, must be an integer')
    ap.add_argument('--output','-o',help='Output brainfuck file name, default is the standard output')
    a=ap.parse_args()
    program=a.program
    ...
    try:
        bf=BFFuck(playfield).compile(code)
