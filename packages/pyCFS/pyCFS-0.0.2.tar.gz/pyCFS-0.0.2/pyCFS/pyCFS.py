...
class pyCFS:
    ...
    def _init_paths(self) -> None:
        ...
        self.hdf_res_path = (
            f"{self.cfs_proj_path}{RESULTS_HDF_DIR}/{self.project_name}/"
        )
        self.hdf_file_path = (
            f"{self.cfs_proj_path}{RESULTS_HDF_DIR}/{self.project_name}.{CFS_EXT}"
        )
 ...
 *<coordinateFile *fileName=".*" *delimiter="," *commentCharacter="#"/>\n* *</sensorArray>'
        self.result_name_pattern = (
            r'<sensorArray fileName="" type="([^"]*)" csv="yes" delimiter=",">'
        )
    ...
    def _init_sa_file_names(self) -> None:
        self.output_names = [
            f"{res}{SA_NAME_SEPARATOR}{inp}"
            for inp, res in zip(self.input_match, self.result_match)
        ]
    ...
    def _make_sa_output_paths(
        self, ind: Optional[int], for_reading: bool = False
    ) -> List[str]:
        ...
        for out_name in self.output_names:
            sarrays_output_paths.append(
                f"{SA_STORAGE_PATH}{out_name}{ind_ext}.csv{step_ext}"
            )

        return sarrays_output_paths

    def _fill_sensor_array_setup(
        self, xml_content: str, ind: Optional[int] = None
    ) -> str:

        output_names = self._make_sa_output_paths(ind)

        for sa, out_name in zip(self.sensorarray_match, output_names):
            filled_filename_pattern = self.filename_out_pattern.replace(
                '""', f'"{out_name}"'
            )
