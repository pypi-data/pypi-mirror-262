...
class MBIOTaskScheduler(MBIOTask):
    ...
    def getLocalStorageDataPath(self):
        return '%s/%s-schedulerdata.csv' % (self.getMBIO().rootPath() or '.', self.name)

    def getLocalStorageDataHCode(self):
        try:
            with open(self.getLocalStorageDataPath(), 'rb', buffering=0) as fcsv:
                return hashlib.file_digest(fcsv, 'sha256').hexdigest()
        except:
            pass
    ...
    def reload(self):
                            ...
                            if self.loadcsv(f):
                                ...
                                try:
                                    with io.BytesIO(r.content) as f:
                                        hcode=hashlib.file_digest(f, 'sha256').hexdigest()
                                    if self.getLocalStorageDataHCode()!=hcode:
                                        self.logger.warning('Scheduler(%s) CSV data has changed, update local storage' % self.name)
                                        with open(self.getLocalStorageDataPath(), 'wb') as f:
                                            f.write(r.content)
                                except:
        ...
        if self.config.type=='csv':
            if not self._reloadDataValid:
                ...
                try:
                    with open(self.getLocalStorageDataPath(), 'r') as f:
                        if self.loadcsv(f):
                            self._reloadDataValid=True
                except:
                    pass
