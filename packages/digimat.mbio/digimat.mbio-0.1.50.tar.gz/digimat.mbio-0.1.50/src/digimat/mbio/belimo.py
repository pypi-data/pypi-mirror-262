...
class MBIODeviceBelimoActuator(MBIODeviceBelimo):
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        if self.config.source=='ai':
            value=self.value('sp', unit='%', resolution=0.1)
        ...
        if item:
            ...
            if self.config.sensor=='10v':
                ...
                self.value('sensor', unit=unit, resolution=resolution)
            elif self.config.sensor in ['pt1000', 'ni1000']:
                resolution=xml.getFloat('resolution', 0.1)
                self.value('sensor', unit='C', resolution=resolution)
            elif self.config.sensor in ['di']:
                self.valueDigital('sensor')
