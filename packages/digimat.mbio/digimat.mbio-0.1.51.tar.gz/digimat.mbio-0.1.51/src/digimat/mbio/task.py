...
class MBIOTask(object):
    ...
    def getLocalStorageFilePath(self, dataname):
    ...
    def getLocalStorageDataHashCode(self, dataname):
        try:
            fpath=self.getLocalStorageFilePath(dataname)
            with open(fpath, 'rb', buffering=0) as f:
                return self.computeDataHashCodeForFile(f)
        except:
            pass

    def updateLocalStorageData(self, dataname, data):
        try:
            fpath=self.getLocalStorageFilePath(dataname)
            self.logger.warning('%s(%s)->updateLocalStorageData(%s)' % (self.__class__.__name__, self.name, fpath))
