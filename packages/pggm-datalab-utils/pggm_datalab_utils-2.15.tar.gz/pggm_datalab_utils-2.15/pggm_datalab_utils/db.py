...
def get_db_connection(server: str, database: str = None) -> Connection:
    ...
    If you specify neither `DB_AUTH` nor `DB_UID`, a fallback approach is used. If the server is detected to be
     ...
     to `{SQL Server}`, and a login attempt is made using `ActiveDirectoryIntegrated` authentication. If this fails,
     token-based authentication with a browser pop-up is attempted instead.
    """
    ...
    # Special case handling for making things "just work"
        ...
        # For some reason, on-premise connections require an older driver. Replace that here.
        if server[:3].upper().startswith('SLP') or server[:10].lower().startswith('pggm-sqlmi'):
