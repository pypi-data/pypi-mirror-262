...
class SourceProcessor(object):
    ...
    def run_all_translation(self, translation_client: TranslationClient, variant: TranslationVariant, languages: list[Language] | None = None):
        languages = languages or list(Language)
        for batch in self.list_source_by_pages(page_size=10):
            match variant:
                case "NAME":
    ...
    def update_source(self, batch: list[ProductCategory], translate_attr: str):
        attribute_updates_list = [self.create_update_dict(cat_model=cat_item, translate_attr=translate_attr) for cat_item in batch]
        ...
        for category_item in batch:
            match variant:
                ...
                case _:
                    raise ValueError("Invalid translation variant.")
            if txt:
                print("Running translation started: {}, languages: {}".format(txt, languages))
    ...
    def create_source(self, batch_generator: Generator[list[ProductCategory]]):
        self.clean_table()
