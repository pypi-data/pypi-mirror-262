...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        label.setPixmap(QPixmap(os.path.join(pPath, 'images', 'logo.png')))
        ...
        try:
            ...
            if ok:
                ...
                if not TFData.training_dataframe is None:
                    saved_file, ok = QFileDialog.getSaveFileName(self, 'Save to', os.path.join(pPath, 'data'), 'CSV Files (*.csv);;TSV Files (*.tsv);;SVM Files(*.svm);;Weka Files (*.arff)')
                    if ok:
    ...
    def mergeCodingFiles(self):
        try:
            coding_files, ok = QFileDialog.getOpenFileNames(self, 'Open coding files (more file can be selected)', os.path.join(pPath, 'data'), 'CSV Files (*.csv);;TSV Files (*.tsv);;SVM Files(*.svm);;Weka Files (*.arff)')
            merged_codings = None
            labels = None
