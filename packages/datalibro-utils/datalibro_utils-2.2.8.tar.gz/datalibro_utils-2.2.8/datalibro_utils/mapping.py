...
def get_sku_extra(df_ori, fields, return_miss=False):
    ...
    if "app_supported" in fields_list:
        try:
            ...
            app_supported_models = dr.sql_query('cfg.yaml', 's2dt', sql)
            if app_supported_models == None:
                raise ValueError("No data found in bi_app_dev")
            else:
                df['app_supported'] = df['model'].apply(lambda x: 'Y' if x in app_supported_models['model'].str[2:].values else 'N')
        except:
            raise ValueError("Must include 'model' in filed list to map 'app_supported'")
