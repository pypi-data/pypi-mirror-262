...
from redxclient.schema.areas import Area, AreaFilter, ZoneEnum
from redxclient.schema.pickup import PickupStore, PickupStoreInput, PickupStoreCreateResponse
from .schema.parcel import DeliveryStatus, ParcelCreateInput, ParcelCreateResponse, Parcel


BETA_API_URL = "https://sandbox.redx.com.bd/v1.0.0-beta"
PRODUCTION_API_URL = "https://api.redx.com.bd/v1.0.0"
...
class RedXAPIClient:
    ...
    def _perform_request(self, method: Literal["GET"] | Literal["POST"], path: str, data: Optional[dict[str, Any] | bytes] = None) -> Any:
    ...
    def create_parcel(self, data: dict[str, Any] | ParcelCreateInput) -> str:
        ...
        response = self._perform_request("POST", "parcel", valid.model_dump_json(round_trip=True).encode("utf-8"))
    ...
    def get_areas(self, filters: Optional[AreaFilter | dict[str, Any]] = None) -> List[Area]:
        """Get areas
        ...
        params = {}
        if filters:
            validator: AreaFilter = AreaFilter(**filters) if isinstance(filters, dict) else filters
            params = validator.model_dump()
        response = self._perform_request("GET", "areas", params)
        return [Area(**area) for area in response["areas"]]

    def create_pickup_store(self, data: PickupStoreInput | dict) -> PickupStoreCreateResponse:
        ...
        response = self._perform_request("POST", "pickup/store", valid.model_dump_json().encode("utf-8"))
    ...
    def get_pickup_store_details(self, store: int | PickupStoreCreateResponse) -> PickupStore:
