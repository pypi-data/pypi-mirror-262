                ...
                if binary_tuple:
                    ...
                    for group_index, binary in enumerate(binary_tuple):
                        ...
                        infos.append(self._WaveInfo(wave_name, file_name, binary,
                                                    ...
                                                    group_index,
                                                    program_name,
                                                    self.concatenated_waveforms_subdivided_info[program_name][
                                                        pos_index][0],
                                                    self.concatenated_waveforms_subdivided_info[program_name][
                                                        pos_index][1],
                                                    ))
        ...
        for wave_placeholder, wave_infos in self._concatenated_waveforms_iter():
            ...
            for group_index, wave_info in enumerate(wave_infos):
                ...
                if current_filename not in filename_list_list[group_index]:  # allegedly `in set` has O(1) complexity
                    ...
                    wave_str = ','.join(
                        ['{i},placeholder({length},true,true)'.format(i=i, length=wave_info.sample_length) for i in
                         range(2 * group_index + 1, 2 * group_index + 3)])  # should be same for all
            ...
            if ct_index >= 1024:
                raise RuntimeError(
                    'too many CT entries. Clear HardwareSetup for now. (HardwareSetup.clear_programs())')  # needs to be handled otherwise then (somehow)...
...
class ProgramWaveformManager:
    ...
    def request_list_append(self, binary_waveform: Tuple[BinaryWaveform, ...],
        ...
        self._memory.concatenated_waveforms_subdivided_info[self._program_name].append(
            (len(binary_waveform[0].ch1), sample_rate_divider))
...
class HDAWGProgramEntry(ProgramEntry):
    ...
    def compile(self,
        ...
        self._var_declarations = '{indentation}var {pos_var_name} = {pos_var_init_placeholder};'.format(
        ...
        if not self._is_fsp:
            ...
            self._waveform_manager._memory.fsp_waveforms[self._waveform_manager._waveform_name] = (
            self._loop.wf_declarations_and_ct, self._loop.wf_definitions_iter)
...
class HDAWGProgramManager:
    ...
    def add_program(self, name: str, loop: Loop,
        ...
        self._ct_dict = {i: CommandTable(s, active_validation=False) for i, s in
                         enumerate(self._ct_schema_tuple_func(tuple(range(self._awg.num_channels // 2))))}
    ...
    def _visit_nodes(self, waveform_manager: ProgramWaveformManager):
        if not self.shared:
            waveform_manager.request_list_append(self.rate_reduced_waveform(),
                                                 self.rate if self.rate is not None else 0)
