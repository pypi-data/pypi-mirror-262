...
def binary_classification(X,Y,tst):
    ...
    def build_model(hp):
            activation = hp.choice('activation',['relu','selu','elu','gelu','relu6'])
            shape = [xtrain.shape[1]]
            model = Sequential()
            model.add(Dense(16,input_shape=shape,activation=activation))
            model.add(Dense(1,activation='sigmoid'))keras activation inctf
