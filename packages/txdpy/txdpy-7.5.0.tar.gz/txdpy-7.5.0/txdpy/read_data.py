...
class ReadData:
    ...
    def connect_mysql(self,database=1):
        self.db = pymysql.connect(host=self.mysql_config["1"]['host'], port=3306, user='root', password=self.mysql_config["1"]['password'], database=self.mysql_config["1"]['database'])
    ...
    def get_th(self,table_name,table_dict=None):  # 获取表头字段
        if not self.table_dict:
            self.table_dict = {k: v for k, v in
    ...
    def read_mysql(self,name,select_sql):
        def replace_sql_str(sql):  # 替换sql语句中表名字段名
            ...
            if table_name_re:
                ...
                new_table_name = self.table_dict.get(table_name)
                if new_table_name:
