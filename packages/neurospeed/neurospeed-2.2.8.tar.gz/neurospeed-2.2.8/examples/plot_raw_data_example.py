...
window_title = "EG plot. use mouse wheel for vertical zoom, +/- keys for horizontal zoom. D key tyoggles high pass filter"

class Canvas(app.Canvas):
    def __init__(self, stream_info, scale=500, filt=True):
...
def EEG_processing_handler1(payload):
    ...
    this function retrieves raw data from a packet, reshapes it in to a form:
    list of lists, where internal list is one multichannel sample, and external list is the raw timeseries.
    the code extracts the sensor information such as sampling requency, number and name of EEG channels, and uses these for plotting. 
    
    the data is then put on queue for collection by the graphing function. 
    ...
    if EEG_sensor_information == None:
        ...
        EEG_sensor_information["EEG_channel_names"] = payload["sensor_info"]["channel_map"][0:4]
        EEG_sensor_information["EEG_channel_num"] = len(payload["sensor_info"]["channel_map"])-1    
        EEG_sensor_information["sampling_frequency"] = payload["sensor_info"]["sampling_frequency"]
    print(EEG_sensor_information)
    data = json.loads(payload["data"])["sample"]   
    # print(data)
    values = []
    for sample in data:
        values.append(sample[0:4])
...
def main():
    ...
    time.sleep(5)
    print(EEG_sensor_information)
    stream_info = {'n_chans': EEG_sensor_information["EEG_channel_num"]-1,'srate':  EEG_sensor_information["sampling_frequency"]  , 'ch_names': EEG_sensor_information["EEG_channel_names"][0:4]    }
    print(stream_info)
    Canvas(stream_info)
    app.run()
