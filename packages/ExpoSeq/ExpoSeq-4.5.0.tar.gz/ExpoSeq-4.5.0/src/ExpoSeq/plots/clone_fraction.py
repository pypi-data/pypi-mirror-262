...
class VisFrac:
    ...
    def __init__(
        ...
        self.ax = ax
        top_95_percent = self.get_top_fraction(
            report=sequencing_report,
            sample=sample,
            force_reducing=force_reducing,
            fraction=top_clone_fraction,
        )
        self.seqs = self.prepare_labels(
            top_95_percent, region_string, limit_seq_filter, seqs_viz_fraction
        )
    ...
    def get_top_fraction(report, sample, force_reducing, fraction=0.95):
        ...
        top_95_percent["cloneFraction"] = (
            top_95_percent["cloneFraction"] / fraction
        )  # normalize
    ...
    def prepare_labels(report, region_string, limit_seq_filter, fraction=0.75):
        ...
        empty_values = pd.Series(
            [""] * int((seq_length - filtered_length)), dtype="str"
        )
