...
class PrepareData:
    ...
    def check_input(samples, max_ld, min_ld, batch_size, binding_data, antigens):
        assert (
            type(samples) == list
        ), "You have to give a string as input for the sample"
        assert (
            type(max_ld) == int
    ...
    def clean_data(
        ...
        sample_report = sequencing_report.loc[
            sequencing_report[experiment_column].isin(samples)
        ]
    ...
    def calc_edges(
        ...
        for i in range(len(aa)):
            ...
            for index in range(index_1):
                ...
                if (
                    distance >= min_ld and distance <= max_ld
                ):  # sequences are not added to Graph if tehy dont meet the threshold requirements
                    G.add_edge(string1, string2)
        G_deg = G.degree()
        to_remove = [
            n for (n, deg) in G_deg if deg == 0
        ]  # degree = 0 means that these nodes are not connected to any other nodes. Since they have their own cluster they can be removed
        G.remove_nodes_from(to_remove)
        sequences_clustered = list(G.nodes)
        report = report[
            report[region_string].isin(sequences_clustered)
        ]  # remove those  which are not in self.G
        return G, report
...
class LevenshteinClustering:
    def __init__(
        ...
        self.ax = ax
        self.G, report = PrepareData().calc_edges(
            sequencing_report,
            samples,
            batch_size,
            max_ld,
        ...
        for index, g in enumerate(G):
            for i in sample_report[region_string]:
                if i == g:
                    node = math.sqrt(
                        sample_report.loc[
                            sample_report[region_string] == i, cf_column_name
                        ].values[0]
                    )
                    if node == 0:
    ...
    @staticmethod
    def map_binding(
        G, binding_data, region_of_interest, report, antigens, prefered_cmap, partition
    ):
        """_summary_
        ...
        node_colors = {}
        assert (
            region_of_interest in binding_data.columns
        ), "You do not have binding data for the corresponding region of interest"
        ...
        if maximum_binding_value == 0:
            ...
            for i, g in enumerate(G):
                ...
                right_binding_value = max(
                    [sequence_row[column].iloc[0] for column in antigens]
                )  # if you have multiple antigens this will find the right value to map
                sequence = sequence_row[region_of_interest].iloc[0]
                node_colors[sequence] = right_binding_value
            node_colors_norm_heatmap = [
                cmap(norm(node_colors[node])) if node_colors[node] != 0 else "gray"
                for node in G.nodes()
            ]  # all nodes without binding values are gray
    ...
    def create_network(
        ...
        if binding_data is not None:
            node_colors, sm, maximum_binding_value = self.map_binding(
                self.G,
                binding_data,
                region_of_interest,
                report,
