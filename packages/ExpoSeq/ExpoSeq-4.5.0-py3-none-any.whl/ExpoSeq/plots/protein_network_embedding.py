...
class PrepareData:
    ...
    def cleaning(
        ...
        sequences, sequences_filtered, selected_rows = Transformer.filter_sequences(
            sequencing_report,
...
class Network_Embedding:
    def __init__(
        ...
        self.ax = ax
        sequences_list, sequences_filtered, selected_rows = PrepareData().cleaning(
            sequencing_report,
            model_choice,
            batch_size,
            list_experiments,
    ...
    def add_edges(self, sequences_list, sequences_filtered, threshold_distance):
        ...
        for seq_1 in range(num_sequences):
            for seq_2 in range(seq_1 + 1, num_sequences):
                ...
                distance = np.sqrt(
                    np.sum(abs(sequences_list[seq_1] - sequences_list[seq_2]) ** 2)
                )  # euclidian distance of embedding coordinates for the 1024 dimensions
        ...
        for index, g in enumerate(self.G):
            ...
            for i in selected_rows[region_of_interest]:
                ...
                if i == g:
                    node = math.sqrt(
                        selected_rows.loc[
                            selected_rows[region_of_interest] == i, "cloneFraction"
                        ].values[0]
                    )
    ...
    def generate_plot(self, nodesize, cmap):
        ...
        nx.draw_networkx_nodes(
            self.G, pos, cmap=cmap, node_color=list(partition.values())
        )
