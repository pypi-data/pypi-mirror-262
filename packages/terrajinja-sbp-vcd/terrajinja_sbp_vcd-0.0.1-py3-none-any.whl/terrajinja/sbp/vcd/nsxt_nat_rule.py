...
from imports.vcd.nsxt_nat_rule import NsxtNatRule
    ...
    def nosnat_rules(self) -> list[dict]:
        ...
        for rule in self.nat_rules:
            if rule['type'].upper() == 'NO_SNAT':
                ...
                # priority = SbpVcdNsxtNatRuleDefaultPriority.nosnat if rule.get('priority') is None else rule.get('priority')
    ...
    def snat_rules(self) -> list[dict]:
        ...
        for rule in self.nat_rules:
            if rule['type'].upper() == 'SNAT':
                ...
                # priority = SbpVcdNsxtNatRuleDefaultPriority.snat if rule.get('priority') is None else rule.get('priority')
                priority = rule.get('priority')
                if not priority:
    ...
    def dnat_rules(self) -> list[dict]:
        ...
        for rule in self.nat_rules:
            if rule['type'].upper() == 'DNAT':
                ...
                # priority = SbpVcdNsxtNatRuleDefaultPriority.dnat if rule.get('priority') is None else rule.get('priority')
                priority = rule.get('priority')
                if not priority:
...
class SbpVcdNsxtNatRule:
    ...
    def __init__(
        ...
        for rule in rules:
            rule_type = rule.get('type')
            if not rule_type in allowed_types:
