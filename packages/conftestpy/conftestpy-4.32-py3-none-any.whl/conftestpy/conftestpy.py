...
class Conftest:
    def __init__(self, base_path: str, view=pyecharts, driver=...,
                 replace_js=False, index_url='Please input url here.', user='None', pwd='None'):
        """
    ...
    2. 关于钩子函数：仅pytest_runtest_makereport需要生成器，其它的直接调
    ...
    def _replace_js_(self):
        ...
        res = re.sub('seconds. </div>',
                     'seconds. </div>'
                     '<iframe style="width:30%;height:300px;margin:-200px 0px 0px 32%" width="100%" height="100%" '
                     'frameborder="no" border="0" src="assets/e1.html"></iframe>'
                     '<iframe style="width:30%;height:300px;margin:-280px 0px 0px 65%" width="100%" height="100%" '
    ...
    def _draw_by_echarts_(self, data):
        ...
        c = (
            self.view.charts.Pie(init_opts=self.view.options.InitOpts(width="380px", height="280px"))
                ...
                radius=["35%", "55%"], center=["40%", "55%"],
            )
                .set_global_opts(
                title_opts=self.view.options.TitleOpts(title=f"{rate} %", pos_top="48%", pos_right="50%"),
                legend_opts=self.view.options.LegendOpts(is_show=False)
                # legend_opts=self.view.options.LegendOpts(orient="vertical", pos_top="45%", pos_right="5%"),
            )
    ...
    def _draw_by_echarts2222_(self, data):
        """
        ...
        self.view.charts.Bar(init_opts=self.view.options.InitOpts(width="380px", height="280px")) \
            .add_xaxis(x) \
            .add_yaxis(series_name='', y_axis=y2, stack="stack1", color=self.__pfs_color__[1], category_gap="45%") \
            .add_yaxis(series_name='', y_axis=y3, stack="stack1", color=self.__pfs_color__[2], category_gap="45%") \
            .add_yaxis(series_name='', y_axis=y1, stack="stack1", color=self.__pfs_color__[0], category_gap="45%") \
            .set_global_opts(xaxis_opts={"axislabel_opt": {"rotate": -15}, "splitLine": {"show": False}},
                             yaxis_opts={"show": False}) \
            .render(self.e2_html_path)
    ...
    def _draw_by_echarts2_(self, data):
        ...
        bar.set_global_opts(
            xaxis_opts={"show": False, "inverse": True, "min": 0.4, "max": 1.2},  # min/max 控制长短，这种情况配合inverse使用
