    ...
    def score_fm_with_deployment(self,config:Configuration,headers, df:pd.DataFrame):
        ...
        for row in rows:
            # Score Request sample:
                ...
                # }
            payload = dict()   
            # Adding the model parameters from the prompt template asset
            if "prompt" in config.prompt_template_asset_json and "model_parameters" in config.prompt_template_asset_json["prompt"]:
                payload["parameters"] = config.prompt_template_asset_json["prompt"]["model_parameters"]
            # Adding the prompt variables
            payload["parameters"]["prompt_variables"] = row
