...
class CalculateTextures(BiobbObject):
    ...
    def run(self):
        ...
        try: 
            print(f'Error extractions for {fsu_mask}')
            print(f'Error extractions for {fsu_image}')
            print(f'Error extractions for {texture_params_file}')
            texture_results = calculate_textures(fsu_mask, fsu_image, self.texture_params_file, self.properties.get('resamplings'), self.properties.get('bindwiths'))
