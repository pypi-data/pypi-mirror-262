...
url = 'https://archery.wh.ksmdev.top'
...
def do_query(instance_name='merlion-risk-prod', db_name='rsk_admin', sql_content='select * from tb_company limit 100;'):
    ...
    form_data = {}
    form_data['instance_name'] = instance_name
    form_data['db_name'] = db_name
    form_data['sql_content'] = sql_content
    form_data['limit_num'] = '0'
...
def do_get_token():
    do_check_token()
    token_json = json.loads(to_list_from_txt(r'txt/csrf_token.txt')[-1])
    return token_json['csrf_token'], token_json['session_id']


def do_check_token():
    token_list = to_list_from_txt(r'txt/csrf_token.txt')
    if len(token_list) == 0:
        do_get_token_from_login()
    elif int(to_datetime().timestamp()) - int(json.loads(token_list[-1])['time']) > 60 * 20:
        do_get_token_from_login()
...
def do_get_token_from_login():
    ...
    headers['X-Csrftoken'] = csrf_token
    form_data = {}
    username_password = to_list_from_txt(r'txt/username_password.txt')
    form_data['username'] = username_password[0]
    form_data['password'] = username_password[1]
    ...
    file_path = r'txt/csrf_token.txt'
    if os.path.exists(file_path):
        os.remove(file_path)
    to_txt_file(r_obj, file_name='csrf_token')
