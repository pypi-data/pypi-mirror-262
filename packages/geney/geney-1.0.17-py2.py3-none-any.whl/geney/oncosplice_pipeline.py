...
def transform_conservation_vector(conservation_vector, plot=False, tplot=False, tid=''):
    ...
    return transformed_vector, max_val
...
def oncosplice_report(deletions, insertions, cons_vector, window_size=4, tplot=False):
    ...
    cons_vec, max_factor = transform_conservation_vector(cons_vector, tplot=tplot)
    affected_cons_scores = cons_vec * modified_positions
    affected_sum = np.sum(affected_cons_scores)
    affected_cons_scores_two = affected_cons_scores + (-min(affected_cons_scores))
    affected_cons_scores_two *= max_factor
    modified_cons_vector = np.convolve(affected_cons_scores_two, np.ones(window_size), mode='same') / window_size
    ...
    return {f'gof': gof_prob, f'oncosplice_score': max_score, 'cons_factor': max_factor, 'affected_cons_sum': affected_sum, 'cons_vector': cons_vector, 'affected_positions': modified_positions}
