...
def oncosplice_transcript(reference_transcript: Transcript, mutation: Variations, aberrant_splicing:PredictSpliceAI, prevalence_threshold=0.0, annotate=False) -> pd.DataFrame:
    ...
    for i, new_boundaries in enumerate(develop_aberrant_splicing(reference_transcript, aberrant_splicing.aberrant_splicing)):
        ...
        oncosplice_output = oncosplice_report(deleted, inserted, cons_vector,10)
...
def transform_conservation_vector(conservation_vector, window_size=15, plot=False, tplot=False):
    if window_size > len(conservation_vector):
        window_size = (len(conservation_vector) / 2) //1

    # Convolution and transformation
    factor = 100 / window_size // 1
    transformed_vector = np.convolve(conservation_vector, np.ones(window_size), mode='same') / window_size
    transformed_vector = np.exp(-transformed_vector * factor)

    # Scale down the vector based on a percentile
    seq_length_modifier = int(len(conservation_vector) * 0.05)
    max_val = np.partition(transformed_vector, -seq_length_modifier)[-seq_length_modifier]
    transformed_vector = factor * transformed_vector / max_val
    transformed_vector -= 1

    # Balance negative values
    negative_values = transformed_vector[transformed_vector < 0]
    if negative_values.size > 0:
        non_negative_sum = np.sum(transformed_vector[transformed_vector >= 0])
        negative_sum = np.sum(negative_values)
        factor = -non_negative_sum / negative_sum
        transformed_vector[transformed_vector < 0] *= factor
...
def oncosplice_report(deletions, insertions, cons_vector, window_size=4, tplot=False):
    ...
    cons_vec = transform_conservation_vector(cons_vector, tplot=tplot)
    ...
    return {f'gof': gof_prob, f'oncosplice_score': max_score, 'affected_cons_sum': affected_sum, 'cons_vector': cons_vector, 'affected_positions': modified_positions}
