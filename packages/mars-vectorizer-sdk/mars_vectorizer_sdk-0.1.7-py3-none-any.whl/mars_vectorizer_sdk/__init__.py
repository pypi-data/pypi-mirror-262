...
class Vectorizer:
    ...
    """
        ...
        Note: The call function returns a numpy zero matrix with the shape (len(key_value_set), 10), as default.
    """

    def __call__(self, key_value_set: List[Tuple[str, Union[str, float]]]) -> np.ndarray:
        return np.zeros((len(key_value_set), 10), dtype=np.float64)
