...
LLM_MODELS = (
    ...
    'auto',  # unknown?
    "stanford-crfm/alias-gpt2-small-x21",  # <500MB
...
LLM_MODEL_DICT.update({s.split('/')[-1].lower().split('-')[-1].lower(): s for s in LLM_MODELS})
...
class RAG:
    ...
    def __init__(
            ...
            client=None, db=None, self_hosted=RAG_SELF_HOSTED, temperature=RAG_TEMPERATURE,
        ...
        if self_hosted:
            self.pipe = pipeline("text-generation", model=self.llm_model)

        if not db:
            self.db = VectorDB(search_limit=self.search_limit, min_relevance=self.min_relevance)
    ...
    def run_model(self, prompt, self_hosted=None, max_new_tokens=None):
        ...
        if self.self_hosted:
            if not self.pipe:
                self.pipe = self.pipe = pipeline("text-to-text", self.llm_model)
            self.answer = self.pipe(prompt, max_new_tokens=max_new_tokens)[0]['generated_text']
        ...
        return dict(
            answers=self.answers, answer=self.answer,
            answer_id=self.answer_id, answer_logprob=self.answer_logprob)
