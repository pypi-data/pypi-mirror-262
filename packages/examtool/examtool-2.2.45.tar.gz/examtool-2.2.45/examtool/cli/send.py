...
@click.option(
    ...
    default=None,
    type=click.File("r"),
    help="Provide a template to use for the email body",
)
def send(exam, target, email, subject, filename, template=None, mailtool=False):
    """
    ...
    body = (
        ...
        "You should complete your exam on that website (https://exam.cs61a.org/{exam}).\n\n"
        "Course: {course}\n"
        "Exam: {exam}\n\n"
    ...
    if template:
        body = template.read().format(course=course, exam=exam)
    ...
    for email in roster:
        ...
        data = {
            "from": {"email": "cs61a@berkeley.edu", "name": f"{course} Exam Platform"},
