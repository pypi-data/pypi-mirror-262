...
class YamlDataSource(_abstract.DataSource):
    ...
    def _iter_data_source(self):
        ...
        for root, _dirs, files in os.walk(self._yaml_root, followlinks=True):
            ...
            for name in files:
                ...
                _logger.debug("Read Yaml filename %s", filename)
    ...
    def create_class_filename(self, class_name):
        _logger.error("create_class_filename '%s' is not umplemented", class_name)
        node = Node()
        return node

    def create_server_filename(self, server_name):
        _logger.error("create_server_filename '%s' is not umplemented", server_name)
        node = Node()
        return node
