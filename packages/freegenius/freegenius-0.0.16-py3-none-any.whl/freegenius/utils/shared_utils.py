    ...
    def checkCompletion():
        if config.llmServer == "ollama":
            if shutil.which("ollama"):
                config.print3(f"Checking ollama model: {config.ollamaDefaultModel}, {config.ollamaCodeModel}")
                Downloader.downloadOllamaModel(config.ollamaDefaultModel)
                Downloader.downloadOllamaModel(config.ollamaCodeModel)
            else:
