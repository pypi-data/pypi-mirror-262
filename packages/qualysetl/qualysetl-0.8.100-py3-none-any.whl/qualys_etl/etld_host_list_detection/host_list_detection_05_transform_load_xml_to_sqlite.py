...
def copy_database_table(
    ...
    while attempt < max_attempts:
        try:
            ...
            etld_lib_functions.logger.error(f"Operational error on attempt {attempt + 1}: {oe}")
        except sqlite3.IntegrityError as ie:
            etld_lib_functions.logger.error(f"Integrity error on attempt {attempt + 1}: {ie}")
            break  # No point in retrying if it's an integrity issue
        except sqlite3.DatabaseError as de:
            etld_lib_functions.logger.error(f"Database error on attempt {attempt + 1}: {de}")
        except sqlite3.Error as se:
            etld_lib_functions.logger.error(f"sqlite3.Error on attempt {attempt + 1}: {se}")
        except IOError as ioe:
            etld_lib_functions.logger.error(f"IO error on attempt {attempt + 1}: {ioe}")
        except Exception as e:
            etld_lib_functions.logger.error(f"Unexpected error on attempt {attempt + 1}: {e}")
