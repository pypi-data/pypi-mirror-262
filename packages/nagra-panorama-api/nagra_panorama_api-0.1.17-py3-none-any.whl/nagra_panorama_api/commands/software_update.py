...
def wait_availability(client, fw_host):
    logging.info(
    ...
    for _ in wait():
        try:
            ...
            if current is None:
                logging.warning("Device is not not answering")
            else:
                return current
        except (urllib3.exceptions.MaxRetryError, requests.exceptions.ConnectionError):
            logging.warning(f"Firewall {fw_host} is still not responding")
        except Exception as e:
            logging.debug(
                f"Unexpected error of type {type(e)} occured on firewall {fw_host}"
            )
            logging.error(f"Firewall {fw_host} is still not responding: {e}")
...
def check_availability(client, fw_host):
    ## Wait for the FW to respond
    version = wait_availability(client, fw_host)
    if not version:
        logging.error(f"Device {fw_host} never responded")
        return False
    logging.info(f"Firewall {fw_host} is available")
    return True


def check_panorama_connection(panorama_client, fw_host):
    ...
    if device is None:
        logging.error(f"Device with IP {fw_host} is not detected AT ALL to panorama")
        return False
    if not device.connected:
        ...
        return False
    logging.info(f"Firewall {fw_host} is detected has connected to panorama")
    return True


def run_checks(panorama_client, client, fw_host):
    if not check_availability(client, fw_host):
        return False
    if not check_panorama_connection(panorama_client, fw_host):
        return False
    return True
...
def upgrade_cmd(file, group, apikey, download_only, test_only):
    ...
    if not apikey:
        logging.error(
            "PANORAMA_APIKEY not provided, please check the environment variables"
        )
        exit(1)
    ...
    def worker(args):
        try:
            ...
            panorama_client = nagra_panorama_api.XMLApi(panorama_host, apikey)
            ...
            # Tests
            res = run_checks(panorama_client, client, fw_host)
            if download_only:
                logging.info(
...
def generate_configuration_file_cmd(host, apikey, version, out):
    ...
    if not host:
        logging.error("Missing Panorama host")
        exit(1)
    if not apikey:
        apikey = getenv("PANORAMA_APIKEY", "PANO_APIKEY")
    if not apikey:
