...
def get_parallelism(parallelism):
    ...
    if parallelism is True:
        ...
        if parallelism == 0:
            raise ValueError(f"Invalid parallelism setting: {parallelism}")
        if parallelism < 0:
            parallelism = multiprocessing.cpu_count() + parallelism
    else:
        raise TypeError(f"Invalid parallelism setting: {parallelism}")
...
def get_blocking(blocking, work_len, parallelism=None):
    ...
    if blocking is True:
        ...
        raise TypeError(f"Invalid blocking setting: {blocking}")
    return blocking
