...
@tf_function
def onehot_logits_sample(label: Int[tf.Tensor, "maxlen"], value: float = 1e9, blank_index: int = 0, vocabsize: int = None, num: int = None) -> Float[tf.Tensor, "2*maxlen vocabsize"]:
...
def onehot_logits_tf(label: Int[tf.SparseTensor, "batch maxlen"], value: float = 1e9, blank_index: int = 0, *, batch: int, maxlen: int, vocabsize: int = 0) -> Float[tf.Tensor, "batch 2*maxlen vocabsize"]:
  ...
  logits = [onehot_logits_sample(xs, value, blank_index, vocabsize=vocabsize, num=maxlen)[None, ...] for xs in tf.unstack(dense, num=batch)]
  return tf.concat(logits, axis=0)

def onehot_logits(label: Int[tf.SparseTensor, "batch maxlen"], value: float = 1e9, blank_index: int = 0, vocabsize: int = None) -> Float[tf.Tensor, "batch 2*maxlen vocabsize"]:
  """Logits that approach a CTC loss of 0 as `value` grows.
  ...
  return onehot_logits_tf(label, value, blank_index=blank_index, batch=int(batch), maxlen=int(maxlen), vocabsize=vocabsize or tf.sparse.reduce_max(label)+1)

def mock_logits(top_preds: list[Int[tf.SparseTensor, "batch maxlen"]], blank_index: int = 0, vocabsize: int = None) -> Float[tf.Tensor, "batch 2*maxlen vocabsize"]:
