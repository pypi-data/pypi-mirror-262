...
def beam_decode_tf(
	...
	top_paths, log_probs = tf.nn.ctc_beam_search_decoder(
...
3. Add 1 to the predicted indices (so, `0 -> 1, 1 -> 2, n-2 -> n-1`. `n-1` was blank thus there can be no `n-1` in the predicted sequences)
	...
	top_paths: list[tf.SparseTensor] = t # thank you python type linter!
	paths = [s.with_values(s.values+1) for s in top_paths] if blank_zero else top_paths
