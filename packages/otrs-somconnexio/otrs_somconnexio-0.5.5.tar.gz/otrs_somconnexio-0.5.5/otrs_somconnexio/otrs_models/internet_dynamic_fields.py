...
class InternetDynamicFields(ProvisionDynamicFields):
    def _build_specific_dynamic_fields(self):
        dynamic_fields = [
            ...
            self._asociatel_fiber_coverage(),
    ...
    def _previous_service(self):
        return DynamicField(
            name="serveiPrevi", value=self.service_data.previous_service
        )
    ...
    def _service_address(self):
        return DynamicField(
            name="direccioServei", value=self.service_data.service_address
        )
    ...
    def _service_subdivision(self):
        return DynamicField(
            name="provinciaServei", value=self.service_data.service_subdivision
        )

    def _service_subdivision_code(self):
        return DynamicField(
            name="codiProvinciaServei", value=self.service_data.service_subdivision_code
        )
    ...
    def _previous_owner_vat(self):
        return DynamicField(
            name="NIFNIEtitular", value=self.service_data.previous_owner_vat
        )
    ...
    def _mm_fiber_coverage(self):
        return DynamicField(
            name="coberturaFibraMM", value=self.service_data.mm_fiber_coverage
        )

    def _asociatel_fiber_coverage(self):
        # OTRS DF used for asociatel fiber coverage is the same as the former VDF
        return DynamicField(
            name="coberturaFibraVdf", value=self.service_data.asociatel_fiber_coverage
        )

    def _orange_fiber_coverage(self):
        return DynamicField(
            name="coberturaFibraOrange", value=self.service_data.orange_fiber_coverage
        )

    def _previous_contract_phone(self):
        return DynamicField(
            name="telefonFixAntic", value=self.service_data.previous_contract_phone
        )

    def _previous_contract_address(self):
        return DynamicField(
            name="direccioServeiAntic",
            value=self.service_data.previous_contract_address,
        )

    def _change_address(self):
        change_address = "yes" if self.service_data.type == "location_change" else "no"
        return DynamicField(name="canviUbicacioMateixTitular", value=change_address)

    def _change_address_provider(self):
        return DynamicField(
            name="proveidorPreviCU", value=self.service_data.previous_internal_provider
        )

    def _product(self):
        return DynamicField(name="productBA", value=self.service_data.product)
