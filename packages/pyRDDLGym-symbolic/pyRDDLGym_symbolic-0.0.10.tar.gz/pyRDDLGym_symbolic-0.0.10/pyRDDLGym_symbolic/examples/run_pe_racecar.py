...
def run_vi(args: argparse.Namespace):
    ...
    print(f"{model.cpfs=}")
    # print(f"{mdp.context._id_to_node[1994]}")
    ...
    ax = np.arange(-10, 10, 0.1)
    ay = np.arange(-10, 10, 0.1)

    Z = np.zeros((len(ax), len(ay)))

    bool_assign = {var_dict["reach"]: False, var_dict["violation"]: False, var_dict["intersect___b1"]: False, var_dict["intersect___b2"]: False, var_dict["intersect___b3"]: False, var_dict["intersect___b4"]: False}
    cont_assign = {var_dict["ax"]: 0, var_dict["ay"]: 0, var_dict["x"]: 1, var_dict["y"]: 0, var_dict["vx"]: 0, var_dict["vy"]: 0.1}

    for i, t1 in enumerate(tqdm(ax)):
        for j, t2 in enumerate(ay):
            cont_assign[var_dict["ax"]] = t1
            cont_assign[var_dict["ay"]] = t2
            Z[i][j] = mdp.context.evaluate(res["value_dd"][-1], bool_assign=bool_assign, cont_assign=cont_assign)

    print(Z)

    def plot_and_save(Z, title, filename):
        fig, ax = plt.subplots()
        ...
        ax.set_xticklabels([str(xtick + 40) for xtick in xticks])
        ax.set_yticklabels([str(ytick + 40) for ytick in yticks])
        plt.show()
        plt.savefig(filename)
        plt.close()

    plot_and_save(Z, f"Value Function for {args.max_iter} steps", f"./Racecar_{args.max_iter}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--domain", type=str, default="RobotLinear_1D", help="The name of the RDDL environment.")
    parser.add_argument("--instance", type=str, default="0", help="The instance number of the RDDL environment.")
    parser.add_argument("--policy_fpath", type=str, help="The file path to the policy.")
    parser.add_argument("--max_iter", type=int, default=10, help="The maximum number of iterations")
    parser.add_argument("--enable_early_convergence", action="store_true", help="Whether to enable early convergence.")
    parser.add_argument("--is_linear", action="store_true", help="Whether the MDP is linear or not.")
    parser.add_argument("--reduce_lp", action="store_true", help="Whether to perform the reduce LP function.")
    parser.add_argument("--assert_concurrency", action="store_true", help="Whether to assert concurrency or not")
    parser.add_argument("--save_graph", action="store_true", help="Whether to save the XADD graph to a file.")
