...
optimizer = dict(
    type='LARS',
    lr=3.2,  # lr=1.6 for bs2048
    momentum=0.9, weight_decay=1e-6,
    paramwise_options={
        '(bn|ln|gn)(\d+)?.(weight|bias)': dict(weight_decay=0., lr_mult=0.032, lars_exclude=True),
        'bias': dict(weight_decay=0., lr_mult=0.032, lars_exclude=True),
        # bn layer in ResNet block downsample module
        'downsample.1':
        dict(weight_decay=0, lr_mult=0.032, lars_exclude=True),
    })

# lr scheduler
lr_config = dict(
    policy='CosineAnnealing',
    by_epoch=False, min_lr=0.0032,
    warmup='linear',
    warmup_iters=10, warmup_by_epoch=True,
    warmup_ratio=3.2e-4,  # cannot be 0
)
