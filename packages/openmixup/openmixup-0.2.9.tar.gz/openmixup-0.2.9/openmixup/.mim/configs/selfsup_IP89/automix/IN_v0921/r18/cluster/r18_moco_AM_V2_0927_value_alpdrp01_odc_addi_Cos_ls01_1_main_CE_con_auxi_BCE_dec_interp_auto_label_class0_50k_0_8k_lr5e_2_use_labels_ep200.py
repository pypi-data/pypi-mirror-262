...
conv1x1=dict(
    ...
    with_last_dropout=dict(type="AlphaDropout", prob=0.1),
    with_avg_pool=False, with_residual=False)  # no res
...
model = dict(
    ...
    loss_weights=dict(
        ...
        accent_weight=[],
        weight_bb_mix=1, weight_bb_ssl=1, weight_mb_main=1, weight_mb_auxi=1,  # 1, 1
    ...
    #         debug=False)
    # ),
    clustering=dict(type="DeepCluster", nonlinear_neck=True),
    head_clst=dict(  # using neck feat (default for ODC, optional for DeepCluster)
...
# additional hooks
custom_hooks = [
    dict(type='DeepClusterAutoMixHook',  # try pretrained labels
        extractor=dict(
            imgs_per_gpu=128,
            workers_per_gpu=8,
            dataset=dict(
        ...
        unif_sampling=False,  # True for DeepCluster
        reweight=True,  # using loss reweight
        reweight_pow=0.5,
        init_memory=False,  # True for ODC
        init_params=False,  # optional for DeepCluster (default False)
        initial=True,  # call initially
        interval=9999999,  # init only
        save_cluster=False,
        pretrained_labels="work_dirs/selfsup/automix/IN_v0921/r18/cluster/r18_moco_AM_V2_0927_value_drp01_odc_addi_Cos_ls01_1_main_CE_con_auxi_BCE_dec_interp_auto_label_class0_50k_0_8k_lr5e_2_ep200/cluster/cluster_epoch_199.npy",
    ),
    # dict(type='ODCHook',
    #     # centroids_update_interval=250,  # 5004 iter per ep
    #     # deal_with_small_clusters_interval=50,  # on IN
    #     # evaluate_interval=500,  # on IN
    #     centroids_update_interval=500400,  # per 1ep, 5004iter / ep
    #     deal_with_small_clusters_interval=500400,  # per 100ep on IN
    #     evaluate_interval=50,  # per 1ep on IN
    #     reweight=True,
    #     reweight_pow=0.5),
...
optimizer = dict(type='SGD', lr=0.03, weight_decay=0.0001, momentum=0.9,
                paramwise_options={
                    'head_clst': dict(lr=0.050, momentum=0.,),    # clustering
                    'mix_block': dict(lr=0.050, momentum=0.9)},)  # required parawise_option
# optimizer args
