...
# pre_conv_cfg, v08.23
conv1x1=dict(
    in_channels=1024, hid_channels=512, out_channels=512,  # reduce channel v08.27
    num_layers=2, kernel_size=1,
    with_bias=True, with_last_bn=False, norm_cfg=dict(type='BN'),  # ori
    with_last_dropout=0, with_avg_pool=False, with_residual=False)  # no residual
# pre_attn_cfg, v08.23
embedded=dict(
    in_channels=1024, mode='embedded_gaussian', reduction=2)
# pre_head_cfg, v08.24
mix_head=dict(type='ContrastiveHead', temperature=0.1)  # SimCLR infoNCE
# pre_neck_cfg, v08.24
pre_neck=dict(
    type='NonLinearNeckSimCLR',  # reduce channel v08.27
    in_channels=512, hid_channels=256, out_channels=128, num_layers=2, with_avg_pool=True)
...
model = dict(
    ...
    mask_adjust=0.,
    pre_one_loss=0.5,  # v08.24 pre loss
    pre_mix_loss=1.0,  # v08.24 pre loss
    lam_margin=0.05,
    ...
    mix_block = dict(  # V2, update v08.24
        ...
        in_channels=512,  # reduce channel 08.27
        reduction=2, use_scale=True, double_norm=False,
        attention_mode='embedded_gaussian', unsampling_mode='bilinear',
        pre_norm_cfg=pre_norm,  # v08.27
        pre_conv_cfg=conv1x1,  # v08.23
        pre_attn_cfg=embedded,  # v08.23
        pre_neck_cfg=pre_neck,  # v08.24, SimCLR neck
        pre_head_cfg=mix_head,  # v08.24, SimCLR head
        lam_concat=False,
...
data = dict(
    imgs_per_gpu=128,  # total 128 * 2gpus = 256
    workers_per_gpu=8,
    drop_last=True,
    train=dict(
        type=dataset_type,
...
optimizer = dict(type='SGD', lr=0.03, weight_decay=0.0001, momentum=0.9,
                paramwise_options={
                    'mix_block': dict(lr=0.0005, momentum=0),
...
addtional_scheduler = dict(
    policy='CosineAnnealing', min_lr=mb_minlr,  # try
    warmup='linear',
