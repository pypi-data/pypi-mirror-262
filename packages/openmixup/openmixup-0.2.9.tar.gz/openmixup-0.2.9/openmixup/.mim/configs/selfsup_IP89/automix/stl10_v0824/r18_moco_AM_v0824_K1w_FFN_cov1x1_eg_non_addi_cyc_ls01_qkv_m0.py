...
embedded=dict(
    in_channels=256, mode='embedded_gaussian', reduction=2)
# pre_head_cfg, v08.24
mix_head=dict(type='ContrastiveHead', temperature=0.1)  # SimCLR infoNCE
# pre_neck_cfg, v08.24
pre_neck=dict(
    type='NonLinearNeckSimCLR',
    in_channels=256, hid_channels=256, out_channels=128, num_layers=2, with_avg_pool=True)
...
optimizer = dict(type='SGD', lr=0.03, weight_decay=0.0001, momentum=0.9,
                paramwise_options={
                    # 'mix_block': dict(lr=0.0005, momentum=0.9),
                    'mix_block': dict(lr=0.0005),
                    'mix_block.(query|key|value)': dict(momentum=0.),
...
addtional_scheduler = dict(
    ...
    # policy='Cyclic', target_ratio=(100, 1), cyclic_times=2, step_ratio_up=0.4,  # old good
    warmup='linear',
