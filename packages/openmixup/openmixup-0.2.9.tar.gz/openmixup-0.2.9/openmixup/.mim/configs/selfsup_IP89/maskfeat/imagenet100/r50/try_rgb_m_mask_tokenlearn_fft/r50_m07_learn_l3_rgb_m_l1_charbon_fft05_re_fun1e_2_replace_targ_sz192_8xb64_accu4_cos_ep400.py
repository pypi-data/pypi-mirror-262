...
model = dict(
    ...
    head=dict(
        type='MIMHead',
        loss=dict(type='RegressionLoss', mode='charbonnier_loss',
            loss_weight=1.0, reduction='none'),
        unmask_weight=0.,
        unmask_replace='target',
        fft_weight=0.5,
        fft_focal=False,
        fft_unmask_weight=1e-2,  # unmask in fft loss
        fft_reweight=True,
    ...
    train=dict(
        ...
        mask_pipeline=[
            dict(type='BlockwiseMaskGenerator',
                input_size=192, mask_patch_size=32, model_patch_size=16, mask_ratio=0.7,
