...
conv1x1=dict(
    type="ConvNeck",
    in_channels=320, hid_channels=64, out_channels=1,  # MixBlock v
...
model = dict(
    type='AutoMixup',
    pretrained="work_dirs/my_pretrains/IN_pretrain/OK_van_small_8xb128_fp16_ep300/epoch_300.pth",
    alpha=2.0,
    momentum=0.999,
...
# data
data = dict(imgs_per_gpu=16, workers_per_gpu=2)

# additional hooks
update_interval = 4  # 16 x 8gpus x 4 accumulates = bs512
custom_hooks = [
    dict(type='SAVEHook',
        save_interval=1621 * 4 * 10,  # plot 10ep
...
addtional_scheduler = dict(
    policy='CosineAnnealing',
    by_epoch=False, min_lr=1e-4,  # 0.1
    warmup='linear',
    warmup_iters=5, warmup_by_epoch=True,  # warmup 5 epochs.
    warmup_ratio=1e-6,
