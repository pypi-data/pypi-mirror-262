...
model = dict(
    ...
    mix_block = dict(  # automix, v05.12
        type='PixelMixBlock', in_channels=2048, reduction=2, use_scale=True,
        mode='embedded_gaussian',
        lam_concat_theta=True, lam_concat_g=True,  # lambda concat: theta + g
        post_conv=False,
        loss_mask="L1", loss_margin=0.1,  # loss: L1, 0.1
        frozen=False),
    head_one_q=dict(
        type='ClsHead', with_avg_pool=True, in_channels=2048, num_classes=200),
    head_mix_q=dict(
        type='ClsMixupHead', with_avg_pool=True, in_channels=2048, num_classes=200),
    head_one_k=dict(
        type='ClsHead', with_avg_pool=True, frozen=True, in_channels=2048, num_classes=200),
    head_mix_k=dict(
        type='ClsMixupHead', with_avg_pool=True, frozen=True, in_channels=2048, num_classes=200),
    )
...
data = dict(
    ...
    imgs_per_gpu=25,  # 25 x 4gpus = 100
    workers_per_gpu=4,
    train=dict(
        type=dataset_type,
        data_source=dict(
