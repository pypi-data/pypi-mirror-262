...
model = dict(
    ...
    alpha=[1, 1, 0.2],
    mix_mode=["cutmix", "resizemix", "manifoldmix"],
    mix_repeat=2,
    mix_args=dict(
    ...
    pretrained='work_dirs/my_pretrains/official/resnext101_32x8d.pth',
    backbone=dict(
        type='ResNeXt_Mix',
        depth=101,
        num_stages=4,
...
optimizer = dict(type='SGD', lr=0.002, momentum=0.9, weight_decay=0.0005,
                paramwise_options={
                    'head': dict(lr=0.02, momentum=0.9)},)  # required parawise_option
