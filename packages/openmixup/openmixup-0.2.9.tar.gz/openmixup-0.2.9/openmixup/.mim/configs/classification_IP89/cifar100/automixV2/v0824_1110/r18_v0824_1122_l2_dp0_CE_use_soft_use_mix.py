...
model = dict(
    ...
    head_mix=dict(  # backbone
        ...
        loss=dict(type='CrossEntropyLoss', loss_weight=1.0,
            use_soft=True, use_sigmoid=False, use_mix=True, use_eta=1.0,  # try CE local mix
        ),
    ...
    head_mix_k=dict(  # mixblock
        type='ClsMixupHead',  # mixup, soft CE (onehot label)
        loss=dict(type='CrossEntropyLoss', use_soft=True, use_sigmoid=False, loss_weight=1.0),
        with_avg_pool=True, multi_label=True,
        two_hot=True, two_hot_mode='pow', two_hot_thr=1, two_hot_idx=1, neg_weight=1,  # ori neg
        in_channels=512, num_classes=100),
