...
model = dict(
    ...
    neck_mim=dict(
        ...
        decoder_cfg=None,
        kernel_size=5,
        in_channels=2048, in_chans=2, encoder_stride=32),
    head_cls=dict(
    ...
    loss_weights=dict(
        decent_weight=[], accent_weight=['weight_cls'],
        weight_mim=1, weight_cls=2),
)

# dataset
data = dict(
    imgs_per_gpu=64, workers_per_gpu=5,
    train=dict(
...
# additional hooks
custom_hooks = [
    dict(type='CustomCosineAnnealingHook',  # 0.25 to 0
        attr_name="cos_annealing", attr_base=0.25, min_attr=0., by_epoch=False,  # by iter
