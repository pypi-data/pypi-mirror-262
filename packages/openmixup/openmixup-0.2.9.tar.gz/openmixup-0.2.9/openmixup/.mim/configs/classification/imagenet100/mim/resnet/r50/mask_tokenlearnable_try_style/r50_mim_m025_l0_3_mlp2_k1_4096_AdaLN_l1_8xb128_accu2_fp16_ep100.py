...
model = dict(
    ...
    neck_mim=dict(
        ...
        decoder_cfg=dict(
            type="ConvNeck",
            in_channels=2048, hid_channels=2048, out_channels=4096,  # 7x7x4096 -> 56x56x64
            num_layers=2, kernel_size=1, stride=1,
            norm_cfg=dict(type='BN'), act_cfg=dict(type='ELU'),  # use norm
            with_last_norm=False,
            with_avg_pool=False,
            with_pixel_shuffle=8,  # upsampling to 1/4
            norm_token="AdaLN",  # fix to stem norm
        ),
