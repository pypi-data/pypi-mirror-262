...
model = dict(
    ...
    neck_mim=dict(
        ...
        decoder_cfg=dict(
            ...
            num_layers=1, kernel_size=1,
            norm_cfg=dict(type='LN2d'), act_cfg=dict(type='ELU'),  # default
            with_last_norm=True,
            with_last_dropout=0, with_avg_pool=False, with_residual=False),  # no res & dropout
        in_channels=4096, in_chans=3, encoder_stride=32),
    head_cls=dict(
...
train_pipeline = [
    ...
    dict(type='BlockwiseMaskGenerator',
        input_size=224, mask_patch_size=32, model_patch_size=4, mask_ratio=0.40),
]
data = dict(
