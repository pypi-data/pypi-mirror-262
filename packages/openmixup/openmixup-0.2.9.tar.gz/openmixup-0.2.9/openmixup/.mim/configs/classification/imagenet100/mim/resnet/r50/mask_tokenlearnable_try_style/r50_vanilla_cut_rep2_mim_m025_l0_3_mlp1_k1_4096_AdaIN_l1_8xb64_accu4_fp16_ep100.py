...
model = dict(
    ...
    pretrained=None,
    alpha=[1, 0.2],  # float or list
    mix_mode=["vanilla", "cutmix"],  # str or list, choose a mixup mode
    mix_args=dict(),
    mix_repeat=2,
    backbone=dict(
        type='MIMResNet',
        ...
        num_stages=4,
        out_indices=(-1, 3,),  # no conv-1, x-1: stage-x
    ...
    neck_mim=dict(
        ...
        decoder_cfg=dict(
            type="ConvNeck",
            in_channels=2048, hid_channels=4096, out_channels=4096,  # 7x7x4096 -> 56x56x64
            num_layers=1, kernel_size=1, stride=1,
            norm_cfg=None, act_cfg=dict(type='ELU'),  # no norm
            with_last_norm=False,
            with_avg_pool=False,
            with_pixel_shuffle=8,  # upsampling to 1/4
            norm_token="AdaIN",  # fix to stem norm
        ),
        in_channels=64, in_chans=3, encoder_stride=4),
    head_cls=dict(
...
update_interval = 4  # total: 8 x bs128 x 2 accumulates = bs2048
