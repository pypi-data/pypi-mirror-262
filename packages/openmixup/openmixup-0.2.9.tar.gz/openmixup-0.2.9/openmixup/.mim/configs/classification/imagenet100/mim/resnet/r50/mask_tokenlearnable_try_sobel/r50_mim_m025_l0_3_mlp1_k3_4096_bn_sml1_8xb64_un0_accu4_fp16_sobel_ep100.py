...
model = dict(
    ...
    neck_mim=dict(
        ...
        decoder_cfg=dict(
            type="ConvNeck",
            in_channels=2048, hid_channels=4096, out_channels=4096,
            num_layers=1, kernel_size=3,
            norm_cfg=dict(type='SyncBN'), act_cfg=dict(type='ELU'),  # default
            with_last_norm=True,
            with_last_dropout=0, with_avg_pool=False, with_residual=False),  # no res & dropout
        in_channels=4096, in_chans=2, encoder_stride=32),
    head_cls=dict(
    ...
    head_mim=dict(
        type='MIMHead',
        loss=dict(type='RegressionLoss', mode='smooth_l1_loss',
            loss_weight=1.0, reduction='none'),
        unmask_weight=0.,
        patch_size=4, encoder_in_channels=2,  # try sobel
    ),
)
...
# validation hook
evaluation = dict(imgs_per_gpu=64, workers_per_gpu=2,)
