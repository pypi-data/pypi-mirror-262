...
class ImageViewer:
    ...
    def __init__(self, **params):
        ...
        self.imv = [[pg.ImageView(view=pg.PlotItem())].copy()[0] for _ in range(3)]+[self.gl.GLViewWidget()]
    ...
    def _cursor_event(self, pic, sp_xy, org_xy):
        ...
        def _inner(event):  # to be modified
            if event.isExit():
                self.prompt.setText(' ')
                return
            _loc = event.pos()
    ...
    def _init_volume(self, x):
        ...
        item = self.gl.GLVolumeItem(x)
        item.scale(*_re_sp)
        item.translate(*offset)
        res = [item]
        if isinstance(self._msk, list) and len(self._msk) != 0:
            shaders = [c for _, c in enumerate('rgb') if _ < len(self._msk)]
            for _msk, _c in zip(self._msk, shaders):
    ...
    def _load_data(self, **params):
        ...
        for attr in ['_data', '_msk', '_sp', '_org', '_lv']:  # clear data
        ...
        if isinstance(self._msk, list) and len(self._msk) > 3:
            warn('Support simultaneous 3 masks in maximum, drop the others')
            self._msk = self._msk[:3]

        img0 = overlap_mask(self._data, self._msk) if self._msk is not None else self._data
        img1 = overlap_mask(self._data.transpose((2, 1, 0)), [_.transpose((2, 1, 0)) for _ in self._msk]) \
            if self._msk is not None else self._data.transpose((2, 1, 0))
        img2 = overlap_mask(self._data.transpose((1, 2, 0)), [_.transpose((1, 2, 0)) for _ in self._msk]) \
    ...
    @staticmethod
    def play(**params):
        _ = ImageViewer(gui_size=params.get('gui_size', (1000, 1000)))
        _.view(**params)
