...
_polar_ends = getattr(fun, '_polar_ends')
_attach_tag = getattr(dep, '_attach_tag')
_resort = getattr(fun, '_resort')
_spans = getattr(fun, '_spans')
_start_angles = getattr(fun, '_start_angles')
...
distance_matrix = getattr(fun, 'distance_matrix')
...
_heatmap_ = _image_.update(**{'colorMap': 'CET-C4s', 'tags': np.array([])})
...
class _Sketcher:
    ...
    def __init__(self, sketch: str):
        _acceptable_type = ['line', 'scatter', 'histogram', 'box', 'beeswarm', 'heatmap', 'radar', 'pie', 'image']
...
class Canvas:
    ...
    def __init__(self, **params):
        self.configs = params
        self.items, self.src, self._meta_fig_config = None, None, FigConfigs
        self.app, self.main, self.cvs = pg.mkQApp(self.configs.get('cvs_main')), QMainWindow(), pg.PlotWidget()
        self.main.setCentralWidget(self.cvs)
        ...
        self.configs_ = eval(f"self._meta_fig_config.{str.upper(self._type)[0] + self._type[1:]}") if not \
            self.configs.get('fig_configs') else self.configs.get('fig_configs')  # guess for fig_configs
    ...
    def _prepare(self, **params):
        ...
        if _config.get('fig_type') in ['heatmap', 'radar', 'pie']:
