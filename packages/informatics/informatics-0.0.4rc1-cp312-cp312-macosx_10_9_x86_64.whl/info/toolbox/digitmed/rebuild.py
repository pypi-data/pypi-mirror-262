...
class DcmSeries:
    ...
    def _intrinsic_order(self):
        a, b, c = self.settings.get('axis_order')
        order = (0, 1, 2)
    ...
    @property
    def rcs_spacing(self):
        return np.linalg.norm(self.affine.T[:3], axis=1, ord=2)[np.array(self._intrinsic_order)]

    @property
    def rcs_origin(self):
        return self.affine.T[-1][:3][np.array(self._intrinsic_order)]
