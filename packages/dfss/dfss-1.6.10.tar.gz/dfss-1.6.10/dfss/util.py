    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            with paramiko.Transport(sock) as transport:
                ...
                th = threading.Thread(target=sftpget, args=(".sophgo_speed", file_content))
                start_time = time.time()
                th.start()
                th.join(timeout)
                end_time = time.time()
...
def get_server_info(username, isflag):
    ...
    if username.startswith('sophgo'):
        ...
        with ThreadPoolExecutor(max_workers=10) as t:
            async_res = [t.submit(is_sftp_supported, index, server, 3) for index, server in enumerate(server_list)]
        lock_output(False)
        lock_output_clear()
...
def sftp_login(username):
    ...
    try:
        ...
        try:
            ...
            print("Here's the SFTP server information above. Please use it to log in and download the files.")
            return False
    return True
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            with paramiko.Transport(sock) as transport:
                transport.connect(username=username, password=password)
                sftp = paramiko.SFTPClient.from_transport(transport)
                th = threading.Thread(target=sftpget, args=(".dfss_flags", file_content))
                th.start()
                th.join()
                sftp.close()
                transport.close()
