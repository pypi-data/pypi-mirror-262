...
from test_tele.bot.utils import (
    ...
    TTLOLI_START, TTLOLI_HELP, BASE_PROMPT
)
...
# help_prompt = "Translate into a format that is easier to understand and clearer"

# chat = model.start_chat()
# response = chat.send_message(f'{help_prompt} : \n {TTLOLI_HELP}')
...
async def subscribe_channel_handler(event):
    ...
    notes = """
    This command used to mirror a channel's posts into this chat.

    Command: `/subscribe`
    Usage: CHANNEL_ID | ID+HASH

    Note: Works only in group
    """.replace("    ", "")
