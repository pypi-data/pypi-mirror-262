...
async def download_media_url_handler(event, is_premium: bool=False):
    ...
    with yt_dlp.YoutubeDL({'format': "mp4", 'outtmpl': f'outputs/{video_title}.%(ext)s', 'postprocessors': []}) as ydl:
        info = ydl.extract_info(event.message.text, download=True)
        try:
            msg = await event.client.send_message(event.message.chat_id, 'Uploading to telegram...')
            try:
                await event.client.send_file(
                    event.message.chat_id, 
                    open(info['requested_downloads'][0]['filepath'], 'rb'), 
                    caption=f'{time.time() - start}', 
