...
def get_delta_add_pset_for_nodes(nodes, task, mapping):
    ...
    for index in range(len(ordered_nodes)):
        print("Filling node "+ordered_nodes[index].run_service("GET", "NAME"))
...
def output_space_generator_launch(setop, 
    ...
    num_procs_add = len(nodes) if mapping == 'sparse' else sum([n.run_service("GET", "NUM_CORES") for n in nodes])
