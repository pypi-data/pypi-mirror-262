...
from quac.prot import DuckSpecProtocol, DuckTypeProtocol, DuckedProtocol, DuckModuleProtocol
...
# %% ../nbs/00_duck.ipynb 7
def isduck(x) -> TypeGuard[Duck | ABCDuck | DuckTypeProtocol]:
    ...
    return isinstance(x, (Duck, ABCDuck, DuckTypeProtocol))

def isducked(x) -> TypeGuard[DuckedProtocol]:
    '''Check if `x` is `ducked` i.e. has one of the following attributes:
    `duck`, `__duck__`, `__ducktypes__`.'''
    return isinstance(x, (Duck, DuckedProtocol, ))
