...
class VolumeFileTransfer:
    ...
    def __get_gs_downscoped_credentials(self):
        federation_credentials = vessl_api.volume_federate_api(volume_id=self.volume.id)
        bearer_bytes = base64.b64decode(federation_credentials.token.gs.base64_credentials)
        bearer = json.loads(bearer_bytes)
        return GoogleCredentials(token=bearer["access_token"])
