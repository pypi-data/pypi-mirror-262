...
async def handle_websockify(websocket: WebSocket, ws: WebSocketClientProtocol):
    try:
        while True:
            websockify_data = await ws.recv()
            if websockify_data:
                await websocket.send_text(websockify_data)
    except Exception as e:
        print(f"Error handling websockify data: {e}")
...
async def websocket_proxy(
    ...
    try:
        print("\nWS websocket: ", websocket.__dict__)
        print("\nWS proxying websocket with headers: ", websocket.headers)
