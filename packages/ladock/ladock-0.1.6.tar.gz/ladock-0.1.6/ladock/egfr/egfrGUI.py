...
from ladock.utility import browse_file, mol_mmff, process_csv, help_egfr
...
from ladock.nogui import create_egfr
...
def egfr_conf(frame):
    ...
    result_text = tk.Text(result_frame, wrap="none", font=("Monospace", 11))
    result_text.grid(row=0, column=0, pady=5, sticky="news")
    result_text.config(state="disabled", background="black", foreground="white")  
    ...
    # Help Button
    ttk.Button(help_gen_frame, text="?", command=help_egfr).grid(column=0, row=3, pady=30, padx=5)
