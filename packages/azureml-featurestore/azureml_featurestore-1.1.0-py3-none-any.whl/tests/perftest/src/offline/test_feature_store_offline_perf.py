...
from .spark_perf_test_base import SparkJobPERFBase


@pytest.mark.perftest
class FeatureStoreOfflinePERF(SparkJobPERFBase):
    def test_spark_jobs_hobo(self):
        self._test_spark_jobs(
            sdk_version=getenv("SDK_BUILD_VERSION"),
            fs_scala_jar=getenv("FS_SCALA_JAR"),
        )
