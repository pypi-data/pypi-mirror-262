...
def resolve_features(features: List[Feature]):
    ...
    for f in features:
        ...
        if transformation_type not in job_map:
            job_map[transformation_type] = ([], [], set())

        job_map[transformation_type][0].append(f)
        job_map[transformation_type][1].append("{}:{}".format(feature_set.name, f.name))
        job_map[transformation_type][2].add(feature_set)
...
def _build_logger(name: str, **kwargs):
    ...
    app_insights_handler: Tuple = get_appinsights_log_handler(user_agent=USER_AGENT, properties=custom_properties)
    ...
    if isinstance(app_insights_handler[0], logging.NullHandler) and enable_telemetry:
        app_insights_handler: Tuple = _get_azuresdk_handler(name, enable_telemetry)
...
def _get_azuresdk_handler(name, enable_telemetry=False):
    ...
    custom_properties.update({"PythonVersion": platform.python_version()})
    custom_properties.update(_get_workspace_context_properties())
    ...
    handler = AzureMLSDKLogHandler(  # cspell: ignore MLSDK
