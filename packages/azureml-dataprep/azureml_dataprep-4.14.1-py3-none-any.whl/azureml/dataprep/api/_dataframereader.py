...
from ._loggerfactory import _LoggerFactory, trace, _log_dataflow_execution_activity
...
class RecordIterator:
    def __init__(
        ...
        _LoggerFactory.trace(
            _LOGGER, "RecordIterator_created", {
    ...
    def _clean_up(self):
        _LoggerFactory.trace(
            _LOGGER, "RecordIterator_cleanup", {
                "iterator_id": self._iterator_id})
...
def _log_dataflow_execution_activity(
    activity, clex_forced, fallback_allowed, rslex_failed, rslex_error, conversion_failed, conversion_error, clex_failed, clex_error, execution_succeeded, preppy, extra_props=None
):
    builder = {}
    ...
    try:
        _LoggerFactory.trace(_LOGGER, "dataflow_execution", builder)
    except Exception:
        pass
...
def _write_preppy(
    ...
    if isinstance(dataflow, (str, PyRsDataflow)):
...
def _execute(
    ...
    # turn off fallback if dataflow is yaml script or engineless
    from .engineless_dataflow import EnginelessDataflow
    if isinstance(dataflow, (str, PyRsDataflow)) or isinstance(dataflow, EnginelessDataflow):
        if force_clex:
    ...
    def rslex_execute():
        ...
        try:
            if not isinstance(dataflow, (str, PyRsDataflow)):
    ...
    def preppy_execution():
        ...
        try:
            ...
            if convert_preppy_to_pyrecords:
                records = _preppy_files_to_pyrecords(preppy_dirc)
                inner_telemetry_dict["to_pyrecords_with_clex_forced_failed"] = False
            else:
                records = _preppy_files_to_pandas_dataframe(preppy_dirc, extended_types, nulls_as_nan)
            ...
            _LoggerFactory.trace_error(_LOGGER, error)
            if convert_preppy_to_pyrecords:
                inner_telemetry_dict["to_pyrecords_with_clex_forced_failed"] = True
                inner_telemetry_dict["to_pyrecords_with_clex_forced_error"] = repr(
                    e)
            else:
        ...
        if not force_clex:
            try:
                ...
                if not allow_fallback_to_clex:
                    execution_succeeded = False
                    raise e
                _LoggerFactory.trace_warn(_LOGGER, "rslex failed, falling back to clex.")
                pass
            except Exception as e:
                if conversion_error is None:
                    rslex_failed = True
...
class _DataFrameReader:
    ...
    def register_outgoing_dataframe(
        ...
        _LoggerFactory.trace(
            _LOGGER, "register_outgoing_dataframes", {
