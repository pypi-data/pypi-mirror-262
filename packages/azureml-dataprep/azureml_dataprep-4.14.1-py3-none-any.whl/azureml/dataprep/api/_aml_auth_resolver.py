...
def _resolve_auth_from_registry(registry_name, auth_type, creds):
    ...
    return  _resolve_auth_v2(auth_type, creds, None, None, None, registry_name)
...
def _resolve_auth_from_workspace(subscription, resource_group, ws_name, auth_type, creds):
    try:
        ...
        # azureml-core is not installed try aml run token auth for remote aml run
        return _resolve_auth_v2(auth_type, creds, subscription, resource_group, ws_name)      
...
def _resolve_auth_v2(auth_type, creds, subscription=None, resource_group=None, ws_name=None, registry_name=None):
