...
from ._dataframereader import (get_dataframe_reader, get_partition_count_with_rslex, get_partition_info_with_fallback, RecordIterable, _write_preppy,
                               _execute, _preppy_files_to_pyrecords)
...
class Dataflow:
    ...
    def _add_portable_path_column(self, stream_column=_DEFAULT_STREAM_INFO_COLUMN):
        (prefix_path, _) = self._find_path_prefix()
    ...
    def _to_pyrecords(self):
        ...
        traceparent = 'Dataflow._to_pyrecords'
        with tracer.start_as_current_span(traceparent, trace.get_current_span()) as span:
            self._raise_if_missing_secrets()
            span_context = to_dprep_span_context(span.get_context())

            def original_execute():
                preppy_path = _write_preppy(activity=traceparent,
                                            dataflow=self,
                                            span_context=span_context,
                                            allow_fallback_to_clex=allow_fallback_to_clex,
                                            force_clex=force_clex)
                return _preppy_files_to_pyrecords(preppy_path)

            if self._rs_dataflow_yaml is None or force_clex or allow_fallback_to_clex:
                return original_execute()
            else:
                try:
                    from ._rslex_executor import get_rslex_executor
                    return get_rslex_executor().to_pyrecords(self._rs_dataflow_yaml, span_context.span_id)
                except Exception:  # TODO (nathof) remove fallback after dprep release
                    return original_execute()
