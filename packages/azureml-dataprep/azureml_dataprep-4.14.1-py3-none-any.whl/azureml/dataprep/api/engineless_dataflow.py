...
from .engineapi.typedefinitions import (InspectorArguments, ExecuteInspectorCommonResponse, FieldType, InvalidLineHandling, IfDestinationExists)
...
from .dataflow import Dataflow, DataflowValidationError, FilePath, PromoteHeadersMode, FileEncoding, DecimalMark, MismatchAsOption, MultiColumnSelection, SummaryColumnsValue
...
class EnginelessDataflow(Dataflow):
    ...
    def __init__(self, py_rs_dataflow, dataflow_json=None):
    ...
    def add_step(self,
        ...
        _log_not_supported_api_usage_and_raise('add_step', 'Adding a step to a Dataflow isn''t supported anymore.')
    ...
    def filter(self, expression: Expression) -> 'Dataflow':
        ...
        expression_text = f'{{"r":["Function",[[],{{"r":[]}},{{"r":["Function",[["row"],{{"r":[]}},{change_quotes(str(expression.underlying_data))}]]}}]]}}'
    ...
    def replace_datasource(self, new_datasource: DataSource) -> 'Dataflow':
        ...
        if isinstance(new_datasource, MSSQLDataSource) or isinstance(new_datasource, PostgreSQLDataSource):
            ...
            new_dataflow = Dataflow.get_files(new_datasource)
    ...
    def keep_columns(self, columns: MultiColumnSelection) -> 'EnginelessDataflow':
        """
        ...
        return self._add_transformation('keep_columns', _column_selection_to_py_rs_dataflow_selector(columns))
    ...
    def write_to_parquet(self,
        ...
        :param file_path: The path in which to store the output file, not supported for now, will be ignored.
        :param directory_path: The path in which to store the output files.
        :param single_file: Whether to store the entire Dataflow in a single file, not supported for now, will be ignored.
        :param error: String to use for error values.
        :param row_groups: Number of rows to use per row group, not supported for now, will be ignored.
        ...
        if directory_path is None:
            raise ValueError("Please provide `directory_path` to specify where to write the files.")
        ...
        if partition_keys is not None:
            transformation_arguments['writer_arguments'] = {
    ...
    def add_column(self, expression: Expression, new_column_name: str, prior_column: str) -> 'EnginelessDataflow':
        ...
        add_col = {'language': 'native', 'expressions': [{'new_column': new_column_name, 'prior_column': prior_column, 'function_source': change_quotes(str(expression.underlying_data))}]}
    ...
    def promote_headers(self) -> 'Dataflow':
        ...
        :return: The modified Dataflow.
        """
        _log_not_supported_api_usage_and_raise('promote_headers', 'use one of the facotry methods to read the data with headers instead.')
    ...
    def convert_unix_timestamp_to_datetime(self,
        ...
        :return: The modified Dataflow.
        """
        _log_not_supported_api_usage_and_raise('convert_unix_timestamp_to_datetime', 'Consider converting numeric column to date in Pandas.')
    ...
    def _get_optimal_path(common_path_list, input_path_list, is_datastore, verbose):
        ...
        if is_datastore:
            ...
            if verbose:
                print("Removing datasotre name, new optimized path: {}".format(result))
    ...
    def _set_column_types(self, type_conversions: List[Tuple[ColumnSelector, TypeConversionInfo]]) -> 'Dataflow':
        ...
        if len(columns_needing_inference) > 0:
            ...
            try:
                ...
                _log_dataflow_execution_activity(activity='set_columns_types.infer_missing_date_formats',
...
def _column_selection_to_py_rs_dataflow_selector(columns) -> Union[str, List[str], Dict[str, Any]]:
