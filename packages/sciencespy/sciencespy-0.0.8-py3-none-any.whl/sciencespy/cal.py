...
def run_mcmc_chain(chain_id, mcmc, output_samples, input_names, lower_bounds, upper_bounds, surrogate_model):
    pyro.set_rng_seed(chain_id)  # Set RNG seed for reproducibility
    ...
    def get_calibrated_parameters(self):
        ...
        for building_name in self.ubem.get_building_names():
            ...
            if self.num_chains > 1:
                ...
                chain_args = [(chain_id, mcmc, output_samples, input_names, lower_bounds, upper_bounds, surrogate_model) for chain_id in range(self.num_chains)]
                results = pool.starmap(run_mcmc_chain, chain_args)
                pool.close()
                pool.join()
                for chain_result in results:
                    for param_name, param_samples in chain_result.items():
                        if param_name not in posterior_samples:
                            posterior_samples[param_name] = param_samples
                        else:
                            posterior_samples[param_name] = np.concatenate(
                                [posterior_samples[param_name], param_samples], axis=0)
            else:
                posterior_samples = run_mcmc_chain(1, mcmc, output_samples, input_names, lower_bounds, upper_bounds, surrogate_model)
