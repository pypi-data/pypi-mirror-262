...
class SubstationRequest(Flow):
    """
    SubstationResponse represents response of Seaplane Substation Proxy.
...
class SubstationResponse(Subscription):
    """
    SubstationResponse represents response of Seaplane Substation Proxy.
    ...
    def __init__(self, substation_instance_name: str, response_sub: str):
        self.substation_instance_name = substation_instance_name
        super(SubstationResponse, self).__init__(response_sub, "new")
...
EdgeTo = Union[Flow, OutEndpoint, Subject]
...
class Dag:
    ...
    def substation_response(
        self, app_name: str, dag_name: str, substation: Substation
    ) -> Subscription:
        # TODO: app_name, dag_name should be Dag primitives for proper encapsulation.
        # This needs to be part of a larger refactoring
        response_sub = f"{substation.results_stream()}.{app_name}-{dag_name}.>"
        return SubstationResponse(substation.app_division, response_sub)
...
class App:
    ...
    def represention(self) -> Dict[str, Any]:
        ...
        for k_dag in sorted(self.dag_registry.keys()):
            ...
            # Add all substation input nodes
            # TODO(zeemo25)
            # Add all flow nodes
    ...
    def substation_dag(self, dag_name: str, input_list: Optional[List[EdgeFrom]] = None) -> Any:
        ...
        external_request = substation_dag.substation_request(substation, input_list)
        external_response = substation_dag.substation_response(self.name, dag_name, substation)

        response = substation_dag.task(
            substation.get_response,
            [external_request, external_response],
            instance_name="sub-resp",
        )

        substation_dag.respond(response)
