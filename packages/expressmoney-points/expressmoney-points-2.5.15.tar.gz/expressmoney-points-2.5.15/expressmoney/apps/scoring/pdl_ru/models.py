...
from expressmoney.apps.scoring.cache.models import NBKIScore, NBKICreditHistoryV2


class FeatureStoreAbstract(models.Model):
    ENDPOINT_ID = None
    MODEL_NAME = None
    # Order
    ...
    cache_ki = models.ForeignKey('cache.NBKICreditHistoryV2',
                                 models.SET_NULL,
                                 null=True,
                                 blank=True,
                                 related_name='%(app_label)s_%(class)s')
    cache_score = models.ForeignKey('cache.NBKIScore',
                                    models.SET_NULL,
                                    null=True,
                                    blank=True,
                                    related_name='%(app_label)s_%(class)s')
    ...
    class Meta:
        verbose_name = 'Абстрактная модель хранилища семплов из модели Scoring'
        verbose_name_plural = 'Абстрактные модели хранилища семплов из модели Scoring'
        abstract = True


class FeatureStore(FeatureStoreAbstract):
    ...
    class Meta:
        verbose_name = 'Хранилище семплов из модели Scoring'
        verbose_name_plural = 'Хранилища семплов из модели Scoring'
        ordering = ('-order_id',)
