...
class CustomModelContainer:
    ...
    def _drop_id(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'id_opt' in X.columns and 'id_xray' in X.columns:
            return X.drop(columns=['id_opt', 'id_xray'])
        if 'id_opt' in X.columns:
            return X.drop(columns=['id_opt'])
        if 'id_xray' in X.columns:
            return X.drop(columns=['id_xray'])
        return X
