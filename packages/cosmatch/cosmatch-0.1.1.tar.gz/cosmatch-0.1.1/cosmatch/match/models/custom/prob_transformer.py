...
class ProbTransformer(ProbTransformerBase):
    ...
    def _add_best_flag(df: pd.DataFrame) -> None:
        ...
        df['flag_best'] = 0
        df['max_pi'] = df.groupby('id_xray')['P_i'].transform('max')
        df.loc[df['max_pi'] >= df['P_i'] * 0.5, 'flag_best'] = 2
    ...
    def transform(df: pd.DataFrame, predict: pd.DataFrame) -> pd.DataFrame:
        ...
        temp = (1 / (df.groupby("id_xray")['P_0'].sum() + 1))
        df.drop(columns=['P_0'], inplace=True)
        df = df.join(temp, on="id_xray")
