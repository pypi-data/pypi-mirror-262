...
def generate_correlated_catalog(name_frame: str = 'frame', name_target: str = 'target', size_frame: int = 500, size_target: int = 200,
                                ...
                                deviation_frame: int = 1,  deviation_target: int = 1, ## Дописать докстринг
                                other_columns: list = [],
                                max_distance: float = 15, add_distance: bool = True,
                                add_mark: bool = False, mark_method: str = 'nearest', num_usefull_features: int = 0) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        other_columns: Дополнительные колонки каталога. В данных колонках будут генерироваться случайные значения.
    ...
    Examples:
        ...
        >>> # 200
    """
    frame = generate_catalog(name_frame, size=size_frame, random_state=random_state, num_flux=num_frame_flux)
    target = generate_catalog(name_target, size=size_target, random_state=random_state+1, other_columns=['poserr'], num_flux=num_target_flux)
    ...
    if add_mark:
        ...
        for i in range(num_usefull_features):
            df[f'feature_usefull_{i+1}'] = np.random.normal(1, 1, len(df))
            df.loc[df['mark'] == 1, f'feature_usefull_{i+1}'] = np.random.normal(1.5, 1, df.loc[df['mark'] == 1].shape[0])
