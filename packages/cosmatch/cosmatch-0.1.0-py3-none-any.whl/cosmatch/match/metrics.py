...
class Metric(ABC):
    ...
    * Задача поиска бездомных источников: - Поиск рентгеновских источников, для которых отстутвует оптический компаньон в каталоге - метрика рассчитывается по уникальным рентгеновским источникам.
    ...
    def _get_necessary_columns(self, df: pd.DataFrame, columns: list[str] = ['id_xray', 'P_0', 'P_i', 'mark']) -> pd.DataFrame:
...
class MetricHostless(Metric, ABC):
    ...
    def add_hostless_mark(self, df: pd.DataFrame) -> None:
        ...
        """
        hostfull = df.groupby('id_xray').max().query('mark==1').index
        df['mark_hostless'] = 1
        df.loc[df['id_xray'].isin(hostfull), 'mark_hostless'] = 0
...
class RocAucIdentification(MetricIdentification):
    ...
    def calculate(self, df: pd.DataFrame) -> float:
        ...
        """
        df = self._get_necessary_columns(df, ['id_xray', 'mark', 'P_i'])
...
class RocAucNearest(MetricIdentification):
    ...
    def calculate(self, df: pd.DataFrame) -> float:
        ...
        """
        df = self._get_necessary_columns(df, ['id_xray', 'P_i', 'mark'])
        true_pair = df.query('mark==1')
        false_pair = df.query('mark==0')
        group = false_pair.groupby('id_xray').max().rename(
            columns={'P_i': 'tmp'})['tmp']
        false_pair = false_pair.join(
            group, on='id_xray').query('P_i==tmp').drop(
            columns=['tmp'])
...
class Completeness(MetricIdentification, MetricHostless):
    ...
    def calculate(self, df: pd.DataFrame) -> float:
        ...
        """
        df = self._get_necessary_columns(df, ['id_xray', 'P_i', 'P_0', 'mark'])
        self.add_hostless_mark(df)
        df = df.join(df.groupby('id_xray').max().rename(columns={'P_i': 'tmp'})['tmp'],
                     on='id_xray').query('P_i==tmp').drop_duplicates()
...
class RocAucHostless(MetricHostless):
    ...
    def calculate(self, df: pd.DataFrame) -> float:
        ...
        """
        df = self._get_necessary_columns(df, ['id_xray', 'P_0', 'mark'])
        self.add_hostless_mark(df)
        df = df[['id_xray', 'mark_hostless', 'P_0']].drop_duplicates()
