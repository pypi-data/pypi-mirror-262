...
@click.option(
    "--question_limit",
    required=True,
    help="The number of questions to generate. Use 0 to not set a limit.",
)
def generate(
    api_key: str,
    in_file: list[str],
    ...
    if dataset_name is None:
        dataset_name = "questions_" + datetime.datetime.now().strftime(
            "%Y_%m_%d_%H_%M_%S"
        )

    if out_file is None:
        out_file = dataset_name + ".csv"
