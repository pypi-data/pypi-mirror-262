...
from toolboxv2.utils.extras.Style import print_to_console, Style
...
Aufgaben Name oder None:"""
    ...
    def start_widget(self, command, app):
        ...
        sender, receiver = self.app.run_any("WebSocketManager", "srqw",
        ...
        self.app.run_any("MinimalHtml", "add_group", [group_name])

        widget_data = {'name': collection_name, 'group': [
            {'name': 'nav', 'file_path': './app/1/simpchat/simpchat.html',
             'kwargs': {'chatID': widget_id}}]}

        self.app.run_any("MinimalHtml", "add_collection_to_group", [group_name, widget_data])

        isaa_widget_html_element = self.app.run_any("MinimalHtml", "generate_html", [group_name, collection_name])

        print(isaa_widget_html_element)

        # Initialize the widget ui
        ui_html_content = self.app.run_any("WebSocketManager", "construct_render",
