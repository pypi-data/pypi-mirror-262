...
from toolboxv2 import App, get_app, get_logger, Style, Spinner
from toolboxv2.mods.CloudM.ModManager import download_files
from toolboxv2.mods.isaa import Tools
from toolboxv2.mods.isaa.AgentFramwork import DivideMode, CreatePrompt, TextExtractor
from toolboxv2.mods.isaa.AgentUtils import anything_from_str_to_dict
from toolboxv2.mods.isaa.subtools.file_loder import load_from_file_system
from toolboxv2.mods.isaa.subtools.web_loder import read_git_repo, get_pdf_from_url, route_url_to_function
...
def parsing_inputs(isaa, request: str or list, existing_code_base: str, local_sources: list,
    ...
    with tqdm(total=total, unit='steps', desc='parsing input') as pbar:
        result = isaa.mini_task_completion_format(f"Your Task is to evaluate the complexity of the request."
                                                  ...
                                                  f" 0 means no actions needed 1 means its seams impossible. request : {request}"
                                                  ,
                                                  f"Expected format List[docs:bool, examples:bool, complexity:float]")
        ...
        if local_sources is not None:
            for source in local_sources:
                if source[-3] == '.':
                    ...
                    data = load_from_file_system(source)
        ...
        if remote_sources is not None:
            ...
            for source in remote_sources:
                if 'github' in source.lower():
                    read_git_repo(working_directory + "git", source)
...
if __name__ == "__main__":
    ...
    for request_ in requests:
        result = runner(request_)
        print(f"Request {request_}\nResult ============={result}=================")
