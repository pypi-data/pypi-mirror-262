...
class Tools(MainTool, FileHandler):
    ...
    def add_collection_to_group(self, group_name, collection):
        if group_name not in self.groups.keys():
            if self.app.debuge: ValueError(f"add Group '{group_name}' not found in {list(self.groups.keys())}")
...
if __name__ == '__main__':
    ...
    titels = {'name': "titels",
              'group': [{'name': 'titel1', 'file_path': './web/0/welcome/welcome.html', 'kwargs': {'test_name': 'titel1'}},
