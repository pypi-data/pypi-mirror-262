...
from chck import isnone, notnone, isstr, istuple
...
def match_channels_format(fmt: str | int4) -> str:
    ...
    Parameters
    ----------
    fmt : 'thcw' | 'tchw' | (0, 1, 2, 3) | (0, 3, 1, 2) | Channels
...
def as_channels_format(t: tensor, fmt: Channels = THWC, cidx: intq = None) -> tensor:
    return ascfmt(t, fmt, cidx)

@wraps(ascfmt, assigned=WRAPS_ASSIGN_ANNDOCS)
def to_channels_format(t: tensor, fmt: Channels = THWC, cidx: intq = None) -> tensor:
    return ascfmt(t, fmt, cidx)

@wraps(ascfmt, assigned=WRAPS_ASSIGN_ANNDOCS)
def aschnfmt(t: tensor, fmt: Channels = THWC, cidx: intq = None) -> tensor:
    return ascfmt(t, fmt, cidx)

@wraps(ascfmt, assigned=WRAPS_ASSIGN_ANNDOCS)
def format_channels(t: tensor, fmt: Channels = THWC, cidx: intq = None) -> tensor:
    return ascfmt(t, fmt, cidx)
...
def channels_resize(t: tensor, fmt: Channels = THWC, size: Size | None = None) -> tensor:
    return aschns(t, fmt=fmt, size=size)

@wraps(aschns, assigned=WRAPS_ASSIGN_ANNDOCS)
def aschannels(t: tensor, fmt: Channels = THWC, size: Size | None = None) -> tensor:
    return aschns(t, fmt=fmt, size=size)
