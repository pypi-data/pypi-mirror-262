...
async def create(
    ...
    try:
        async with async_launchflow_client_ctx() as client:
            project = await get_project(client, project)
            environment = await create_environment(client, name, project, env_type)
...
async def get(
    ...
    try:
        async with async_launchflow_client_ctx() as client:
            project = await get_project(client, project)
            environment = await client.environments.get(project.name, name)
    except Exception as e:
        typer.echo(e)
        raise typer.Exit(1)
