...
class EventTrigger(Flag):
    ...
    def simplify(trigger):
        ...
        if trigger == EventTrigger.LARGER | EventTrigger.SMALLER | EventTrigger.EQUALS or \
                trigger == EventTrigger.LARGER | EventTrigger.SMALLER | EventTrigger.NOT | EventTrigger.EQUALS:
...
class Event(object):
    ...
    def serialize(self):
        return {'severity': str(self._severity), 'trigger': str(self._trigger.value), 'target': self._target,
                'message': self._message, 'timestamp': self._timestamp.isoformat() + 'Z', 'value': self._value}
