...
class BaseHandler:
    ...
    def write(self,stream,frame_info,log,level):
        '''
        Writes to stream if level is greater than threshold
        '''
        if level >= self.level:
            data=self.prepare_log(frame_info,level,log)
    ...
    def text_to_heading(self,frameinfo,text):
        ...
        millisec = int((time_now  - self.timer_start)//datetime.timedelta(milliseconds=1))
    ...
    def prepare_log(self,frame_info,level,log):
        '''
