...
class Translation:
    ...
    def getDesanonymisation(self,value:str):
        ...
        if value.lower() in self.desanonymisations.keys():
            desanonymisation = self.desanonymisations[value.lower()]
...
def loadAllDataExternes():
    global liste_pays,clients,regions_france_metropolitaine,departements_france,villes,acronymes,chaines,prenoms,remplacements
...
def split(paragraphe,config : Configuration = None):
    ...
    if config != None and config.flagAmeliorationBasiqueTexte:
        ...
        # découpe sur les caractères de phrase
        ...
        for index,line in enumerate(lines):
            ...
            for index2,word in enumerate(words):
                ...
                # si un seul point , on regarde si c'est postentiellement un nombre à partir d'un test de conversion
                if nbDots == 1:
                    try:
                        input_1 = float(word)
                        isFloat = True
        ...
        while ". . " in phraseComplete:
            phraseComplete=phraseComplete.replace(". . ",".. ")
...
# a modifier
def is_unit(mot):
    number = re.match("\d*\.?\d+", mot)
    if (number):
        unit = mot[number.end():]
        with open(unit_path, mode='r', newline='') as fichier_csv:
...
def anonymiser_mot(text: textAnonyms,nbfakechaine,nbfakeorganisation,nbfakecity,nbfakepays,config : Configuration ):
    fakecity = ["ville"]
    fakechaine = ["chaine"]
    fakepays = ["pays"]
    fakeprenom=["PAUL", "JEAN", "PHILIPPE", "PIERRE", "MARC",  "DAVID", "GUILLAUME"]
    fakeorganisation=["entreprise"]
    ...
    try:
        ...
        if (text.textFormat == "CITY" and config.ville == True):
            ...
            #fakeData = fakeorganisation[0]+str(nbfakeorganisation)
        ...
        # ecriture dans le fichier words.csv 
        anonymisedData = pd.concat([anonymisedData, pd.DataFrame([[text.originalText, fakeData]], columns=["original", "anonymous"])])
        anonymisedData.to_csv(word_path, index=False,encoding="iso-8859-1")
        return (fakeData,nbfakechaine,nbfakeorganisation,nbfakecity,nbfakepays)
    except Exception as e:
        log("fonction anonymiser_mot " + str(e),loglevel=logging.ERROR)
        return (text.originalText,nbfakechaine,nbfakeorganisation,nbfakecity,nbfakepays)
...
def anonymiser_paragraphe(paragraphe,config:Configuration):
    ...
    for word in tokens:
        ...
        index=index+1
        listmot.append(word)
        if (word not in acronymes and (word != " " and word != "")):
            flagIsInDico=is_in_dico(word)
            # if((last_was_dot == True and is_in_dico(word.lower()) == False) or (last_was_dot == False and is_in_dico(word) == False)):
            # if not flagIsInDico:
            if flagIsInDico:
               compteurFlag+=1
    ...
    translation.displayListExcludedWords()
    ...
    for entity_type, entity_value,position in entites_nommees:
        ...
        found = False
        for index, row in anonymisedData.iterrows():
            if (entity_value == row["original"]):
                listmot[position] = row["anonymous"]
                found = True
        if found == False:
            (faketext,nbfakechaine,nbfakeorganisation,nbfakecity,nbfakepays) = anonymiser_mot(text,nbfakechaine,nbfakeorganisation,nbfakecity,nbfakepays,config)
...
def desanonymiser_paragraphe(anonymous_paragraphe,config:Configuration=None):
    ...
    tokens = split(anonymous_paragraphe,config=config)
    listmot = []
    for word in tokens:
        listmot.append(word)
    ...
    """
    anonymisedData = pd.read_csv(word_path,  encoding="iso-8859-1", dtype={"original": str, "anonymous": str,"index":int})
    for i in range(0,len(listmot)):
        for index, row in anonymisedData.iterrows():
            if (listmot[i].lower() == row["anonymous"].lower()):
                listmot[i] = row["original"]
                break
    """
