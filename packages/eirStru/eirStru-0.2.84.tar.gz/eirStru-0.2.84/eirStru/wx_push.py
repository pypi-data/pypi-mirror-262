...
class NotifyPush:
    def __init__(self):
        self.wx_template_msg_url = 'http://meian.expressgo.cn/wechat/sendTemplateMsg/'
        self.wx_text_msg_url = "http://218.0.55.172:30001/SendTextMsg_NoSrc"
    ...
    async def send_wx_text_msg(self, wx_id, text_msg):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(self.wx_text_msg_url, params=payload) as resp:
                    r_text = await resp.text()
