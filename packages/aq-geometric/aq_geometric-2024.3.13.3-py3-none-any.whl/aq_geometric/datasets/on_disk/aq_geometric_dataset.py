...
from aq_geometric.data.file.local import load_hourly_data_from_fp, load_stations_info_from_fp, load_hourly_features_from_fp
...
from aq_geometric.data.graph.nodes.compute_node_features import get_node_features
...
class AqGeometricDataset(Dataset):
    def __init__(
        ...
        targets: List[str] = ["OZONE"],
        samples_in_node_feature: int = 24,
        sample_offset_between_features_and_targets: int = 0,
        samples_in_node_target: int = 24,
        start_time: str = "2023-01-01",
        end_time: str = "2023-06-01",
        freq: str = "H",
        time_closed_interval: bool = False,
        max_samples_in_graph_on_disk: int = 360,  # used for storing bulk processed graphs on disk
        engine: Union["sqlalchemy.engine.Engine", None] = None,
        stations_info_fp: Union[str, None] = None,
        features_hourly_features_fp: Union[str, None] = None,
        targets_hourly_features_fp: Union[str, None] = None,
        ...
        node_missing_value: float = np.nan,
        verbose: bool = False
    ):
        self.engine = engine
        self.stations_info_fp = stations_info_fp
        self.features_hourly_features_fp = features_hourly_features_fp
        self.targets_hourly_features_fp = targets_hourly_features_fp
        ...
        self.sample_offset_between_features_and_targets = sample_offset_between_features_and_targets
    ...
    def from_local_fps(
        self,
        stations_info_fp: str,
        feature_hourly_features_fps: List[str],
        target_hourly_features_fps: List[str],
    ):
        self.stations_info_fp = stations_info_fp
        self.feature_hourly_features_fps = feature_hourly_features_fps
        self.target_hourly_features_fps = target_hourly_features_fps
        return self
    ...
    def raw_file_names(self) -> List[str]:
        ...
        raw_fps.extend([osp.basename(fp) for fp in glob([osp.join(self.raw_dir, f"{f}_data*.csv") for f in self.features])])
        raw_fps.extend([osp.basename(fp) for fp in glob([osp.join(self.raw_dir, f"{f}_data*.csv") for f in self.targets])])
        return raw_fps
    ...
    def download(self):
        ...
        start_time, end_time = timestamps[0], timestamps[-1] + pd.Timedelta(self.sample_freq)  # add one more sample to the end time
        ...
        stations_info.to_csv(osp.join(self.raw_dir, "stations_info.csv"), index=False)
    ...
    def process(self):
        ...
        num_samples_in_graph = self.max_samples_in_graph_on_disk // total_samples
        if self.verbose:
            print(f"max samples in graph: {self.max_samples_in_graph_on_disk}")
        ...
        for i, (start_time, end_time) in enumerate(zip(start_times, end_times)):
            ...
            graph = make_graph(
                ...
                node_feature_missing_value=np.nan,  # this will later be replaced with the missing value
                verbose=self.verbose,
            )
            if self.verbose:
        ...
        print(f"Finished processing {len(end_times)} graphs on disk ({len(timestamps) - (self.samples_in_node_feature + self.samples_in_node_target - 1)} total dataset graphs).")
    ...
    def _load_next_graph(self) -> None:
        ...
        if self.current_graph_index+1 < len(self.graph_index_ranges):
            ...
            assert all(h1 == h2 for h1, h2 in zip(g1.h3_index.tolist(), g2.h3_index.tolist())), "The h3_index must be the same for both graphs."
            assert all(h1 == h2 for h1, h2 in zip(g1.aqsid.tolist(), g2.aqsid.tolist())), "The aqsid must be the same for both graphs."
        ...
        node_features[node_features_mask == 0] = self.node_missing_value
        node_targets[node_targets_mask == 0] = self.node_missing_value
    ...
    def _load_data_from_memory(
        ...
        feature_timestamps = pd.date_range(feature_start_time, feature_end_time, periods=node_features.shape[1]).to_numpy()
        target_timestamps = pd.date_range(target_start_time, target_end_time, periods=node_targets.shape[1]).to_numpy()
