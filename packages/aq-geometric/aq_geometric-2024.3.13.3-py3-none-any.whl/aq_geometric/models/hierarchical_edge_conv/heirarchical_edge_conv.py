...
class AqHierarchicalEdgeConvModel(BaseModel):
    ...
    def __init__(
        ...
        self.learned_pool = Linear(self.hidden_channels+3, self.hidden_channels)  # learned pool for the 4 pooled or convolved features
        ...
        self.node_target_conv = ChebGraphConv(self.hidden_channels, self.hidden_channels, K=cheb_k)
        self.node_target_mlp = Seq(Linear(self.hidden_channels, self.hidden_channels),
                                   ReLU(),
                                   Linear(self.hidden_channels, self.hidden_channels))
    ...
    def forward(self, x, edge_index):
        ...
        for _ in range(self.num_resolutions_between_finest_and_coarsest//2):  # @TODO update
            # apply edge conv from high to low resolution
            xs = self.edge_conv(xs, self.edges_high_to_low_resolution[fine_res].to(x.device))
            fine_res -= 1
            coarse_res -= 1
        # apply max pool
        xp2 = self.max_pool(xs)
        # apply edge conv at each level of resolution
        for _ in range(self.num_resolutions_between_finest_and_coarsest//2):
            # apply edge conv from low to high resolution
            xs = self.edge_conv(xs, self.edges_low_to_high_resolution[fine_res].to(x.device))
            coarse_res += 1
            fine_res += 1
        # apply max pool
    ...
    def load(self, path: str):
        ...
        self.edges_high_to_low_resolution = model_data["edges_high_to_low_resolution"]
        self.edges_low_to_high_resolution = model_data["edges_low_to_high_resolution"]
...
def process_aq_geometric_dataset_edges_by_h3_resolution(
    ...
    max_h3_resolution = int(np.max([h3.h3_get_resolution(h) for h in h3_index if h != "root"]))
    ...
    if graph_includes_root_node:
        node_to_h3_resolution_map = {i: h3.h3_get_resolution(h3_index[i]) for i in range(h3_index.shape[0]-1)}  # the last node is the root node
        node_to_h3_resolution_map[graph.num_nodes-1] = -1  # the root node
    else:
        node_to_h3_resolution_map = {i: h3.h3_get_resolution(h3_index[i]) for i in range(h3_index.shape[0])}
    ...
    edges_same_resolution = {**{_: [] for _ in range(min_h3_resolution, max_h3_resolution+1)}}
    edges_high_to_low_resolution = {**{_: [] for _ in range(min_h3_resolution, max_h3_resolution+1)}}
    edges_low_to_high_resolution = {**{_: [] for _ in range(min_h3_resolution, max_h3_resolution+1)}}

    # get the self edges if needed
    self_edges = None
    if force_self_loops:
        self_edges = torch.stack([torch.arange(0, graph.num_nodes, dtype=int), torch.arange(0, graph.num_nodes, dtype=int)], dim=0)
    ...
    for _, e in enumerate(graph.edge_index.T):
        ...
        if from_h3_resolution == to_h3_resolution:
            ...
            edges_high_to_low_resolution[from_h3_resolution].append([from_, to])
            if force_bidirectional_edges:
                edges_low_to_high_resolution[to_h3_resolution].append([to, from_])

    for k in range(min_h3_resolution, max_h3_resolution+1):
        if verbose: print(f"processing edges and edge attributes at h3 resolution {k}")
        edges_same_resolution[k] = torch.cat((self_edges, torch.tensor(edges_same_resolution[k], dtype=int).T), dim=1)
        edges_high_to_low_resolution[k] = torch.cat((edges_same_resolution[k], torch.tensor(edges_high_to_low_resolution[k], dtype=int).T), dim=1)
        edges_low_to_high_resolution[k] = torch.cat((edges_same_resolution[k], torch.tensor(edges_low_to_high_resolution[k], dtype=int).T), dim=1)
        if verbose: print(f"before removing duplicates, edges have shape: {edges_same_resolution[k].shape}, {edges_high_to_low_resolution[k].shape}, {edges_low_to_high_resolution[k].shape}")
        edges_same_resolution[k] = torch.unique(edges_same_resolution[k], dim=1)
        edges_high_to_low_resolution[k] = torch.unique(edges_high_to_low_resolution[k], dim=1)
        edges_low_to_high_resolution[k] = torch.unique(edges_low_to_high_resolution[k], dim=1)
        if verbose: print(f"after removing duplicates, edges have shapes:\n  -> same resolution: {edges_same_resolution[k].shape}\n  -> high to low resolution: {edges_high_to_low_resolution[k].shape}\n  -> low to high resolution: {edges_low_to_high_resolution[k].shape}")

    # change the graph's edge_index in place
    if verbose: print("changing the graph's edge_index in place")
    new_edge_index = torch.unique(torch.cat([torch.cat((edges_same_resolution[k], edges_low_to_high_resolution[k], edges_high_to_low_resolution[k]), dim=1) for k in range(min_h3_resolution, max_h3_resolution+1)], dim=1), dim=1)
    if verbose: print(f"old edge_index shape: {graph.edge_index.shape}, new edge_index shape: {new_edge_index.shape}")
