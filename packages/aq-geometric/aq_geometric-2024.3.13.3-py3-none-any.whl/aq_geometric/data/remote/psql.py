...
def load_nodes_info(
    ...
    """
    assert table in ["daily_stations", "stations_info"], "table must be daily_stations or stations_info"
    ...
    if table == "daily_stations":
        stations_info = load_daily_stations(
            engine=engine, query_date=query_date, selected_aqsids=selected_aqsids, verbose=verbose)
    elif table == "stations_info":
        stations_info = load_stations_info(engine=engine, query_date=query_date, selected_aqsids=selected_aqsids, verbose=verbose)
    
    if verbose:
        print(f"[{datetime.now()}] loaded {len(stations_info)} stations info from table {table}")
...
def load_node_feature(
    ...
    """
    assert table in ["hourly_features", "hourly_data"], "table must be hourly_features or hourly_data"
    ...
    if verbose:
        print(f"[{datetime.now()}] loading node feature {feature} from table {table}")
    
    # load the node features
    feature_df = pd.DataFrame()
    if verbose:
        print(f"[{datetime.now()}] loading features {feature} from table {table}")
    if table == "hourly_features":
        feature_df = load_hourly_feature(
            ...
            h3_indices_filter=selected_h3_indices,
            verbose=verbose
        )
    elif table == "hourly_data":
        feature_df = load_hourly_data(
            engine=engine,
            table_name=table,
            features=[feature],
            start_time=start_time,
    ...
    if verbose:
        print(f"[{datetime.now()}] loaded feature {feature} from table {table} with shape {feature_df.shape}")
...
def load_node_features(
    ...
    if verbose:
        print(f"[{datetime.now()}] loading features {features} from table {table}")
    if table == "hourly_features":
    ...
    if verbose:
        print(f"[{datetime.now()}] loaded feature {features} from table {table} with shape {features_df.shape}")
