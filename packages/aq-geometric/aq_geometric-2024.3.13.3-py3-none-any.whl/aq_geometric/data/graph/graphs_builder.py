...
from aq_geometric.data.graph.nodes.compute_node_features import get_node_feature, get_node_features, stack_node_features
...
class GraphsBuilder(Iterable):
    ...
    def export_as_graph(self) -> Data:
        ...
        graph_kwargs = {
            "nodes": self._nodes,
            "edges": self._edges[["from", "to"]].to_numpy().T,
            "edge_attr": self._edge_attr.drop(columns=["from", "to"]).to_numpy() if self.with_edge_features else None,
    ...
    def _check_data_source(self):
        if self.engine is None:
            if self.stations_info_fp is None:
                raise ValueError("either engine or stations_info_fp must be provided")
            assert os.path.exists(self.stations_info_fp), f"file {self.stations_info_fp} does not exist"
            if self.features_hourly_features_fps is None:
                raise ValueError("features_hourly_features_fps must be provided")
            assert isinstance(self.features_hourly_features_fps, dict), "features_hourly_features_fps must be a dictionary"
            ...
            if self.targets_hourly_features_fps is None:
                raise ValueError("targets_hourly_features_fps must be provided")
            assert isinstance(self.targets_hourly_features_fps, dict), "targets_hourly_features_fps must be a dictionary"
            ...
            if self.stations_info_fp is not None:
                raise ValueError("stations_info_fp must be None if engine is provided")
            if self.features_hourly_features_fps is not None:
                raise ValueError("features_hourly_features_fps must be None if engine is provided")
            if self.targets_hourly_features_fps is not None:
                raise ValueError("targets_hourly_features_fps must be None if engine is provided")

    def _prepare_stations_info(self):
        if self.stations_info_fp is not None:
            # load the stations info from the local file
            stations_info_df = load_stations_info_from_fp(self.stations_info_fp)
        else:
            # load the stations info from the database
            stations_info_df = load_nodes_info(
                engine=self.engine,
                query_date=self.start_time,
                selected_aqsids=self.selected_aqsids,
                verbose=self.verbose,
            )
        if self.stations_info_filters is not None and len(self.stations_info_filters) > 0:
        ...
        if self.selected_aqsids is not None and len(self.selected_aqsids) > 0:
            stations_info_df = stations_info_df[stations_info_df["aqsid"].isin(self.selected_aqsids)]
        ...
        if self.features_hourly_features_fps is not None:
            ...
            for feature in self.features:
                ...
                feature = get_node_feature(
                    ...
                    feature_timestamps=self.timestamps,
                    nodes=self._nodes,
                    verbose=self.verbose,
                )
            ...
            for feature in self.features:
                ...
                node_features.append(feature)
        self._node_features = stack_node_features(node_features)
    ...
    def _compute_targets(self):
        ...
        if self.targets_hourly_features_fps is not None:
            ...
            for feature in self.targets:
                ...
                node_targets.append(feature)
        self._node_targets = stack_node_features(node_targets)
    ...
    def _generate(self, n):
        ...
        for i in range(n):
            ...
            if self.verbose:
                print(f"[{datetime.now()}] generating graph {i + 1}/{n} at timestamp {timestamp}")
...
def _get_timestamps(
    ...
    timestamps = pd.date_range(
        ...
        freq=freq,
        inclusive="both" if time_closed_interval else "left",
    )
    return timestamps
...
def _determine_num_graphs_from_timestamps(
    ...
    if verbose:
        print(f"[{datetime.now()}] determining the number of graphs from the timestamps")
    num_graphs = len(timestamps)
    if verbose:
        print(f"[{datetime.now()}] determined {num_graphs} graphs from the timestamps")
