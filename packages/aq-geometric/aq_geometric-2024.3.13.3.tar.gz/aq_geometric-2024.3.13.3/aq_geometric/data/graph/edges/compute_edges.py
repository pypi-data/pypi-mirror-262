...
def get_edges_from_df(
    ...
    df = stations_info_df.copy(deep=True).groupby(["latitude", "longitude"]).aggregate({
        "aqsid": "first"
    }).reset_index()
    ...
    if as_df:
        if with_edge_features:
            return  pd.DataFrame(edges, columns=["from", "to"]), pd.DataFrame(edge_attr, columns=["from", "to", "distance"])
