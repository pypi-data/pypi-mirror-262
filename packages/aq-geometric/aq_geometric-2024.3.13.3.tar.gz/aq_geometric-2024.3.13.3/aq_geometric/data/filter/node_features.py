...
def filter_features_by_measurement(
    ...
    if verbose: print(f"[{datetime.now()}] filtering by measurement: {measurement}")
    features_df = features_df[features_df["measurement"] == measurement]
    if verbose: print(f"[{datetime.now()}] after filtering dataframe has shape: {features_df.shape}")
    return features_df
...
def filter_features_by_h3_indices(
    ...
    """Filter the dataframe by measurement."""
    if verbose: print(f"[{datetime.now()}] filtering by h3_indices: {h3_indices}")
