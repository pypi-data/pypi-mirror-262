...
class AqGeometricInMemoryDataset(InMemoryDataset):
    ...
    def __init__(
        ...
        end_time: str = "2023-01-15",
        freq: str = "1H",
        samples_in_node_features: int = 24,
        ...
        leaf_h3_resolution: Union[int, None] = 6,
        ...
        self.engine = engine if engine is not None else get_engine(**engine_kwargs)
        ...
        self.graph_feature_start_timestamps = self.timestamps[0:-self.num_samples_in_node_targets]
        self.graph_target_start_timestamps = self.timestamps[self.num_samples_in_node_features:]
        ...
        self.num_graphs = 1 + len(self.timestamps) - self.num_samples_in_node_features - self.num_samples_in_node_targets
    ...
    def get(self, idx):
        ...
        if self.verbose:
            ...
            print(f"[{datetime.now()}] At this index, features are in the range from [{self.graph_feature_start_timestamps[idx]}, {self.graph_feature_start_timestamps[idx + self.num_samples_in_node_features]}] and targets are in the range from [{self.graph_target_start_timestamps[idx]}, {self.graph_target_start_timestamps[idx + self.num_samples_in_node_targets]}]")
    ...
    def _load_data_from_memory(
        ...
        feature_start_time = pd.to_datetime(self._graph_data.feature_start_time) + (pd.Timedelta(self.freq) * (time_index_start))
        feature_timestamps = pd.date_range(start=feature_start_time, periods=self.num_samples_in_node_features, freq=self.freq)
        feature_end_time = feature_timestamps[-1]

        target_start_time = feature_end_time + pd.Timedelta(self.freq)
        target_timestamps = pd.date_range(target_start_time, periods=self.num_samples_in_node_targets, freq=self.freq)
        ...
        node_features = self._graph_data.x[:, start_idx:start_idx+self.num_samples_in_node_features]
        node_targets = self._graph_data.y[:, start_idx+self.num_samples_in_node_features:end_idx]

        # get the node mapping attributes from the current graph
        h3_index = self._graph_data.h3_index
        aqsid = self._graph_data.aqsid
        
        node_features_mask = self._graph_data.node_features_mask[:, start_idx:start_idx+self.num_samples_in_node_features]
        node_targets_mask = self._graph_data.node_targets_mask[:, start_idx+self.num_samples_in_node_features:end_idx]
        ...
        x_mask = torch.all(torch.all(node_features_mask, dim=1), dim=1)
        y_mask = torch.all(torch.all(node_targets_mask, dim=1), dim=1)
        node_all_valid_measurements_mask = torch.all(torch.stack((x_mask, y_mask), dim=1), dim=1)
        ...
        data = Data(
            ...
            timestamps=np.hstack([feature_timestamps, target_timestamps]).shape,
