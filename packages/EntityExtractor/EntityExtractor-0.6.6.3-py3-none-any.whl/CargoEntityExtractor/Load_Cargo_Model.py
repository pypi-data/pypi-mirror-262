...
class LoadCargoModel:

    __EMAIL_PARSER_MODEL_ZIP_URL = 'https://d2olrrfwjazc7n.cloudfront.net/website/assets/entity_extraction_model/output/CARGO.zip'
    __BASE_DIR = os.getcwd()
    __local_zip_file_path = os.path.join(__BASE_DIR, 'CARGO.zip')
    ...
    def __load_model(cls):
        try:
            logging.info("Checking CARGO Model File")
            if os.path.exists(cls.__local_zip_file_path) and os.path.exists(os.path.join(cls.__extract_to, 'CARGO//model-best')):
                logging.info("Loading Pretrained CARGO Model")
                nlp = spacy.load(os.path.join(cls.__extract_to, 'CARGO//model-best'))
                logging.info("Model CARGO has been loaded")
                return nlp
                ...
                try:
                    logging.info("Downloading CARGO Model File")
                    ...
                    if response.status_code == 200:
                        ...
                        logging.error("Failed to download the CARGO Model file")
                        return None
