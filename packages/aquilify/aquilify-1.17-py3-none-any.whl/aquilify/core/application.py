...
class Aquilify:
    ...
    def _load_error_handler(self) -> Optional[
            Mapping[
                Any,
                Callable[
                    [Request, Exception],
        ...
        try:
            error_handlers: Optional[dict] = getattr(settings, 'ERROR_HANDLERS', None)
            if error_handlers is not None:
                if not isinstance(error_handlers, dict):
                    raise ImproperlyConfigured("Invalid error_handlers type, excepted dict")
            if error_handlers:
                for key, value in error_handlers.items():
                    _handler = import_string(value)
                    if inspect.isclass(_handler):
                        self.error_handlers[key] = _handler()
                    else:
                        if not (inspect.iscoroutinefunction(_handler) or inspect.isasyncgenfunction(_handler)):
                            raise TypeError("ASGI can only register asynchronous functions or class of Error handler.")
                        self.error_handlers[key] = _handler
            else:
                self.error_handlers = {} 
        except ImproperlyConfigured as config_error:
            raise ImproperlyConfigured(f"Error loading {error_handlers} Error handler: {config_error}")
        except ImportError as import_error:
            raise ImproperlyConfigured(f"Error importing {error_handlers} Error handler: {import_error}")
        except Exception as e:
            raise ImproperlyConfigured(f"An unexpected error occurred: {e}")
