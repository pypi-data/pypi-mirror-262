...
def imports_snapshot(source:str|None, model:str|None, name:str|None, type:str|None, options):
    ...
    for import_source in sources:
        try:
            ...
            print()
            rich.print(f"[yellow]The [bold]{provider.platform}[/bold] integration doesn't support loading [bold]'{err.type}'[/bold] files.")
...
def exports_delete(export, model):
    ...
    if not export:
        ...
        for source in sources:
            with Spinner(f"Removing {source.name}", f"{source.name} removed"):
                provider.delete_export(model, "", source.name)
    else:
