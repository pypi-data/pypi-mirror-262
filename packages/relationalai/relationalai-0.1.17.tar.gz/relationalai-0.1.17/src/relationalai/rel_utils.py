...
_ignore_known_problems: list[Any] = []
def ignore_known_problems(problems: Sequence[Any]):
    ...
    global _ignore_known_problems
    _ignore_known_problems.clear()
    _ignore_known_problems.extend(problems)
...
def assert_no_problems(res: api.TransactionAsyncResponse):
    ...
    for problem in res.problems:
        if problem not in _ignore_known_problems:
            break
    else:
        return

    raise ExceptionGroup("Rel Errors", [
        Exception(f'{problem["error_code"]}\n{problem["report"]}\n{problem["message"]}')
        for problem in res.problems
    ])
