...
class Resources(ResourceProvider):
    ...
    def list_graphs(self) -> List[AvailableModel]:
        results = self._exec(f"SELECT * FROM {APP_NAME}.api.databases WHERE state <> 'DELETED'")
        if not results:
            return []
        return [{"name": x[1]} for x in results.fetchall()]

    def get_graph(self, name:str):
        results = self._exec(f"SELECT * FROM {APP_NAME}.api.databases WHERE name='{name}' AND state <> 'DELETED'")
        if not results:
            return None
    ...
    def list_imports(self, model:str) -> list[Import]:
        ...
        if not results:
            return []
        return [cast(Import, {"name": row[6], "type": "snowflake_table"})
