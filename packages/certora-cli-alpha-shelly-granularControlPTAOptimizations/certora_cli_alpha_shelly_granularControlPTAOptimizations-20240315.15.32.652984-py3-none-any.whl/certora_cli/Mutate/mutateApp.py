    ...
    def config_server(args: SimpleNamespace) -> str:
        ...
        if args.server:
            ...
            with open(args.conf, 'r') as conf_file:
                conf_obj = json5.load(conf_file, allow_duplicate_keys=False)
...
class MutateApp():
    ...
    def submit(self) -> None:
        ...
        # aa = [(mutant, sources_dir, trg_dir, prover_conf, mutation_test_id) for mutant,
        # trg_dir in all_mutants_with_target_dir]
        # self.run_mutant(*aa[0])
    ...
    def generate_gambit_conf_file(self) -> None:
        ...
        if getattr(conf_obj, 'packages', None):
            filtered_packages = \
                [package for package in getattr(conf_obj, 'packages') if Path(package.split('=')[1]).exists()]
            shared_attributes['solc_remappings'] = filtered_packages
        if getattr(conf_obj, 'solc_optimize', None) or getattr(conf_obj, 'solc_optimize_map', None):
            shared_attributes['solc_optimize'] = True  # type: ignore
        if getattr(conf_obj, 'solc_allow_path', None):
