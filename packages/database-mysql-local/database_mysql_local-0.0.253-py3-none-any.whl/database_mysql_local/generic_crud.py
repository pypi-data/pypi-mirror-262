...
class GenericCRUD(metaclass=MetaLogger, object=LOGGER_CRUD_CODE_OBJECT):
    ...
    def __add_create_updated_user_profile_ids(self, data_json: dict, add_created_user_id: bool = False,
        ...
        self.logger.start(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, object={"add_created_user_id": add_created_user_id,
        ...
        if (get_environment_name() not in (EnvironmentName.DVLP1.value, EnvironmentName.PROD1.value)):
            ...
            if add_created_user_id:
                ...
                if "number" in columns_dict:
                    ...
                    self.__log_warning(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, "number", schema_name,
                                       table_name)
                if "identifier" in columns_dict:
                    ...
                    self.__log_warning(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, "identifier", schema_name,
                                       table_name)
            if "updated_user_id" in columns_dict:
                data_json["updated_user_id"] = self.user_context.get_effective_user_id()
            else:
                self.__log_warning(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, "updated_user_id", schema_name,
                                   table_name)
            ...
            if "is_test_data" in columns_dict:
                ...
                self.__log_warning(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, "is_test_data", schema_name,
                                   table_name)
        else:
            schema_name = schema_name or self.schema_name
            table_name = table_name or self.default_table_name
            ...
            data_json["updated_effective_profile_id"] = self.user_context.get_effective_profile_id()
            data_json["is_test_data"] = self.is_test_data
        self.logger.end(ADD_CREATE_UPDATED_USER_PROFILE_IDS_METHOD_NAME, object={"data_json": data_json})
        return data_json
