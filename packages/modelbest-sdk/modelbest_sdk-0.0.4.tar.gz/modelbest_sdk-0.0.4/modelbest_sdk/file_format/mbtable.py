...
class MbTable:
    def __init__(self, path: str, max_retry=5):
        self.path = path
        self.handle = lib.MbTableOpen(path.encode('utf-8'))
        retry = 0
        while not self.handle and retry < max_retry:
            retry += 1
            ...
            self.handle = lib.MbTableOpen(path.encode('utf-8'))
        if not self.handle:
            raise Exception('Failed to open mbtable after {} retries'.format(max_retry))
    ...
    def get_entry_count(self):
        return lib.MbTableGetEntryCount(self.handle)
...
class MbTableIterator:
    ...
    def __enter__(self):
        ...
        self.iterator = lib.MbTableCreateIterator(self.mbtable, self.start_index)
        return self
