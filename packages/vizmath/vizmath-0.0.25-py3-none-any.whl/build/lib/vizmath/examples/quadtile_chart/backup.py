...
#region quadtile testing
# qt = quadtile.random_quadtile(300, rotate=0)
# qt.quadtile_plot()

# ##%% bounding box for infilling as a treemap
# qt.o_squares.to_dataframe()
# print(qt.o_squares.df['x'].max()-qt.o_squares.df['x'].min())
# print(qt.o_squares.df['y'].max()-qt.o_squares.df['y'].min())

# ##%%
# poly = [ (-250, -50), (300, 200), (100, 350),(-100,-250)]
# df = qt.df.sort_values(by=['value'], ascending=False).head(67)
# # constraints: only good for rectangles!
# # qt_test = quadtile(df, 'id', 'value', packing='inc', rotate=0, overflow=6, constraints=poly)
# qt_test = quadtile(qt.df.head(5), 'id', 'value', packing='inc', rotate=0, overflow=6, constraints=poly)
# qt_test.quadtile_plot(polygon=poly)
#endregion
            ...
            if collapse:
                ...
                if left_square is not None:
                    ...
                    if merge_segment is not None: # potential merge
                        ...
                        x_seg = left_square.x + left_square.w
                    ...
                    if extend_segment is not None: # extend a connected segment
                        ...
                        if left_square is not None:
                            if left_square.x + left_square.w < new_left_segment.x: # gap exists, abs(left_square.x + left_square.w - segment.x) > 10e-12
                                # covered_segment = max((s for s in segments if s.x+s.length == left_square.x + left_square.w and s.y < new_left_segment.y), key=lambda s: s.y, default=None)
                                # covered_segment = max((s for s in segments if abs(s.x+s.length - new_left_segment.x) > 1e-13 and s.y < new_left_segment.y), key=lambda s: s.y, default=None)
...
#%%
rpqt = polyquadtile.random_polyquadtile(100, buffer=0., rotate=0, collapse=False)
print(len(rpqt.o_polysquares.viz))
rpqt.polyquadtile_plot(show_constraints=True)
df = rpqt.o_polysquares.df
constraints = rpqt.constraints
rpqt2 = polyquadtile(df, 'id', 'a', constraints=constraints, buffer=0., rotate=0, collapse=True)
print(len(rpqt2.o_polysquares.viz))
rpqt2.polyquadtile_plot(show_constraints=True)

#%%
import os

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug10.csv')

df['value'] = df['a']
val_max = df['value'].max()
df['value'] = df['value'].apply(lambda x: x / val_max)
df['value'] = df['value']*0.20029460954517128

df = df.loc[df['side']=='right']

constraints = [(-3.3391932336715566, -0.46094093956070803),
    (4.572304826604297, -1.8559096376744015),
    (0.6294445346051636, 1.8146103199764578),
    (-1.8625561275379037, 0.5022402572586514)]
pqt = polyquadtile(df, 'id', 'value', constraints=constraints, buffer=0,
    rotate=-90, auto=False, collapse=True, sides=['right'])
print(len(df))
print(len(pqt.o_polysquares.viz))
print(pqt.multiplier) #0.20029460954517128
pqt.polyquadtile_plot(show_constraints=True)
...
#%%
#%%
df.to_csv(os.path.dirname(__file__) + '/debug10.csv', encoding='utf-8', index=False)
constraints

#%%
rpqt2.o_polysquares.df.to_csv(os.path.dirname(__file__) + '/debug8.csv', encoding='utf-8', index=False)
rpqt2.constraints
...
# val_max = df['value'].max()
# df['value'] = df['value'].apply(lambda x: x / val_max)
# df['value'] = df['value']*1.875872634029388

# df = df.loc[df['side']=='top']
...
constraints = [(-3.3391932336715566, -0.46094093956070803),
    ...
    (-1.8625561275379037, 0.5022402572586514)]
# pqt = polyquadtile(df, 'id', 'value', constraints=constraints, buffer=0,
#     rotate=0., auto=True, collapse=False)#, sides=['top'])
pqt2 = polyquadtile(df, 'id', 'value', constraints=constraints, buffer=0,
    rotate=0., auto=True, collapse=True)#, sides=['top'])
# print(len(df))
# print(len(pqt.o_polysquares.viz))
# print(pqt.multiplier) #0.4172922804832459
# pqt.polyquadtile_plot(show_constraints=True)
pqt2.polyquadtile_plot(show_constraints=True)

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug7.csv')
df['value'] = df['a']
val_max = df['value'].max()
df['value'] = df['value'].apply(lambda x: x / val_max)
df['value'] = df['value']*1.875872634029388
...
constraints = [(-3.367619250045914, -0.630160378839055),
    (-3.0889091186105766, -2.690306971312919),
    (-2.422672312504454, -4.923687558016003),
    (6.4021521659454175, 0.6076968639572257),
    (2.4770485152155275, 7.636458044210753)]
pqt = polyquadtile(df, 'id', 'value', constraints=constraints, buffer=0,
    rotate=0., auto=False, collapse=True)#, sides=['top'])
print(len(df))
print(len(pqt.o_polysquares.viz))
print(pqt.multiplier) #
pqt.polyquadtile_plot(show_constraints=True)

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug6.csv')
val_max = df['value'].max()
df['value'] = df['value'].apply(lambda x: x / val_max)
df['value'] = df['value']*1.875872634029388
constraints = [(-3.367619250045914, -0.630160378839055),
...
print(len(df))
print(len(pqt.o_polysquares.viz))
print(pqt.multiplier) #0.9926888022899628
pqt.polyquadtile_plot(show_constraints=True)
pqt.o_polysquares.df.to_csv(os.path.dirname(__file__) + '/debug7.csv', encoding='utf-8', index=False)

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug5.csv')
# val_max = df['value'].max()
# df['value'] = df['value'].apply(lambda x: x / val_max)
# df['value'] = df['value']*0.5
constraints = [(-1.357514999771493, -2.3660829862334327),
    (-0.5987157035267783, -2.613550475900524),
    (3.191671712622017, 0.8899431648041118),
    (1.5942239389655974, 3.5126441623822187),
    (-2.829664948289343, 0.5770461349476266)]
pqt = polyquadtile(df, 'id', 'value', constraints=constraints, buffer=0,
    rotate=0., auto=True, collapse=True, sides=['top'])
print(len(df))
print(len(pqt.o_polysquares.viz))
print(pqt.multiplier) #0.9926888022899628
pqt.polyquadtile_plot(show_constraints=True)
pqt.df.to_csv(os.path.dirname(__file__) + '/debug7.csv', encoding='utf-8', index=False)