...
#%%
rpqt = polyquadtile.random_polyquadtile(100, buffer=0.05, rotate=0.)
print(len(rpqt.o_polysquares.viz))
rpqt.polyquadtile_plot(show_constraints=True)
print(rpqt.multiplier)

#%%
rpqt.df.to_csv(os.path.dirname(__file__) + '/debug3.csv', encoding='utf-8', index=False)

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug3.csv')
df['test'] = df['apqt_norm_i'] #*1.25 #*0.557863171863556
constraints = [(-2.9733607861376763, -7.372873456046384),
 (1.6113524486534052, -2.1473887905966196),
 (4.1858262911463076, 2.2978291225068297),
 (1.2945682572090065, 4.07991067977986),
 (-4.1183862108710425, 3.142522444356313)]
pqt = polyquadtile(df, 'id', 'test', constraints=constraints,
    rotate=0., auto=True, collapse=False)
print(len(df))
...
#%%
rpqt.df.to_csv(os.path.dirname(__file__) + '/debug2.csv', encoding='utf-8', index=False)

#%%
df = pd.read_csv(os.path.dirname(__file__) + '/debug2.csv')
df
#%%
df['test'] = df['apqt_norm_i']*0.557863171863556
constraints = [(-4.663391176191671, -2.211177135765596),
 (5.1681928869853975, -4.550271735499158),
 (1.7124235902277314, 4.122471697173289),
 (-2.2172253010214575, 2.6389771740914645)]
pqt = polyquadtile(df, 'id', 'test', constraints=constraints,
    rotate=0., sides=['top'], auto=False, collapse=False)
print(len(pqt.o_polysquares.viz))
pqt.polyquadtile_plot(show_constraints=True)
# print(pqt.multiplier) #0.557863171863556

#%%
rpqt.df['test'] = rpqt.df['apqt_norm_i']
pqt = polyquadtile(rpqt.df, 'id', 'test', constraints=rpqt.constraints,
    rotate=0., sides=['top'], auto=False, collapse=True)
print(len(pqt.o_polysquares.viz))
pqt.polyquadtile_plot(show_constraints=True)

#%%
rpqt = polyquadtile.random_polyquadtile(100, buffer=0.05)
print(len(rpqt.o_polysquares.viz))
rpqt.polyquadtile_plot(show_constraints=True)
...
#%%
df = pd.read_csv(os.path.dirname(__file__) + '/poly_test_debug_1.csv')
print(len(df))
# df['value'] = df['value']/1.63
# polygon = [(-3,-7),(-7,-3),(3,7),(7,3),(-7,-7),(7,7)]
polygon = [(-5,-5),(-5,7),(7,3),(6,-5)]
df = df.head(1).copy(deep=True)
# test
# df['value'] = df['value'].apply(lambda x: (x - df['value'].min()) / (df['value'].max() - df['value'].min()))
# df['value'] = df['value'].apply(lambda x: 0.01 + 0.99 * (x - df['value'].min()) / (df['value'].max() - df['value'].min()))
# print(len(df))
# df['value'] = df['value']*10
pqt = polyquadtile(df, 'id', 'value', 0, 0, buffer=0.1, rotate=45., constraints=polygon,
    auto=True) #, auto_max_iter=50) #, auto_max_val=2000, auto_min_val=1)
print(len(pqt.o_polysquares.viz))
pqt.polyquadtile_plot(show_constraints=True)
...
# region testing
# qt = quadtile.random_quadtile(300, rotate=0)
# df = qt.df.sort_values(by=['value'], ascending=True).head(200)
# df['value'] = df['value']/1000

# import os
# df = pd.read_csv(os.path.dirname(__file__) + '/poly_test_debug_1.csv')
# # df['value'] = df['value']/1.19
# # polygon = [(-5,-5),(-5,7),(7,3),(6,-5)]
# df['value'] = df['value']/1.63
# polygon = [(-3,-7),(-7,-3),(3,7),(7,3),(-7,-7),(7,7)]
# # df['value'] = df['value']/3.84
# # polygon = [(2,-2),(-2,2),(-7,-7),(7,7)]
# pqt = polyquadtile(df, 'id', 'value', 0, 0, buffer=0.1, rotate=45., constraints=polygon)
# print(len(pqt.o_polysquares.viz))
# pqt.polyquadtile_plot(show_constraints=True)
# # pqt.polyquadtile_plot(polygon=polygon)

# # Tableau centoid test
# pqt.o_polysquares.dataframe_rescale(-10,10,-10,10)
# pqt.o_polysquares.df.to_csv(os.path.dirname(__file__) + '/poly_centroids_test_1.csv', encoding='utf-8', index=False)
#endregion