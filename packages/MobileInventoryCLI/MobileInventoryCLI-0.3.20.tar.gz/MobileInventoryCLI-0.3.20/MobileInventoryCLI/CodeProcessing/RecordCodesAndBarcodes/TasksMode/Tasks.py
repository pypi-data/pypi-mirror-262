            ...
            if cmd.lower() in ['l','list']:
                with Session(self.engine) as session:
                    ...
                    for num,t in enumerate(tags):
                        print(f"{Fore.green}{num}{Style.reset}/{Fore.light_red}{tagCt-1}{Style.reset} -> {Fore.grey_70}{t}{Style.reset}")
                ...
                tag=input("Tag: ")
                if tag.lower() in ['help',]:
                    print(self.helpText_tags)
                elif tag.lower() in ['q','quit']:
                    exit("user quit!")
                elif tag.lower() in ['b','back']:
                    return
                with Session(self.engine) as session:
