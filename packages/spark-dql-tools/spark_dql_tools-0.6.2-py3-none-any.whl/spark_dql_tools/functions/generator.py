...
def dq_generated_mvp(table_master_name=None,
    ...
    url_master_l1t_filename = str(url_master.split("/")[-1])
    ...
    generate_kirby_l1t_conf(table_master_name=table_name_master)
    kirby_l1t_generated_zip(table_name=table_name_master)
...
def generate_kirby_l1t_conf(table_master_name=None):
    ...
    pj_dq_dir_name = os.path.join(pj_dir_workspace, pj_dq_dir_name)
    pj_dq_dir_table_master_l1t_json = os.path.join(pj_dq_dir_name, "data_kirby_l1t", "kirby", f"{uuaa_master}", f"{uuaa_tag_master_l1t}", "master", f"{dir_hocons_kirby_master_filename_json}")
    pj_dq_dir_table_master_l1t_conf = os.path.join(pj_dq_dir_name, "data_kirb_l1t", "kirby", f"{uuaa_master}", f"{uuaa_tag_master_l1t}", "master", f"{dir_hocons_kirby_master_filename_conf}")
    pj_dq_dir_table_output_l1t_schema = os.path.join(pj_dq_dir_name, "data_kirby_l1t", "kirby", f"{uuaa_master}", f"{uuaa_tag_master_l1t}", "master", f"{dir_hocons_kirby_master_output_schema}")

    pj_dq_dir_table_master_l1t_schema = os.path.join(pj_dq_dir_name, "data_schema", f"{uuaa_tag_master_l1t}", "master", f"{uuaa_tag_master_l1t}.output.schema")
    os.makedirs(os.path.dirname(pj_dq_dir_table_master_l1t_json), exist_ok=True)
    json_file = json.dumps(table_dict, indent=4)
    with open(pj_dq_dir_table_master_l1t_json, "w") as f:
        f.write(json_file)
    ...
    with open(f"{pj_dq_dir_table_master_l1t_schema}") as f:
        txt_conf = f.read()
    txt_json = json.loads(txt_conf)
    with open(pj_dq_dir_table_output_l1t_schema, "w") as f:
        f.write(txt_conf)
    ...
    conf2 = ConfigFactory.parse_string(json_file2)
    hocons_file2 = HOCONConverter.convert(conf2, "hocon")
    with open(pj_dq_dir_table_master_l1t_conf, "w") as f:
        f.write(hocons_file2)
    with open(pj_dq_dir_table_master_l1t_conf) as f:
        txt_conf = f.read()
