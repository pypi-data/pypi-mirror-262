...
from label_studio_converter.utils import (
    ...
    get_json_root_type,
    prettify_result,
    yolo_rectangle_append_format,
    yolo_populate_interpolation_videorectangles,
    yolo_write_annotations_to_file,
    yolo_polygonn_append_format,
    yolo_populate_interpolation_videopolygons,
    populate_interpolation_videorectangles,
    populate_interpolation_videopolygons,
)
...
class Converter(object):
    ...
    def _get_supported_formats(self):
        ...
        if not (
            ('Image' in input_tag_types or 'Video' in input_tag_types)
            and (
                'RectangleLabels' in output_tag_types
            ...
            or
            (
                'VideoRectangleLabels' in output_tag_types
                or 'VideoPolygonLabels' in output_tag_types
            )
        ):
    ...
    def convert_to_coco(
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            for label in labels:
                category_name = None
                for key in ['rectanglelabels', 'polygonlabels', 'videorectanglelabels', 'videopolygonlabels', 'labels']:
                ...
                if 'videorectanglelabels' in label:
                    prev_sequence = {}
                    for each_sequence in label['sequence']:
                        if (prev_sequence and not prev_sequence['enabled']) or ( not prev_sequence):
                            xywh = self.rotated_rectangle(each_sequence)
                            if xywh is None:
                                continue
                            x, y, w, h = xywh
                            ...
                            annotations.append(
                                {
                                    ...
                                    'iscrowd': 0,
                                    'area': w * h,
                                    'frame': each_sequence['frame'],
                                    'interpolate': False
                                }
                            )

                        else:
                            for each_frame in range(prev_sequence['frame']+1, each_sequence['frame']):
                                if frame_sequence := populate_interpolation_videorectangles(self, label, each_sequence, prev_sequence, each_frame):
                                    update_details ={ 
                                        'id': len(annotations),
                                        'image_id': image_id,
                                        'category_id': category_id,
                                    }
                                    frame_sequence = frame_sequence | update_details
                                    annotations.append(frame_sequence)
                                
                        prev_sequence = each_sequence
                    
                    if prev_sequence["enabled"]:
                        total_frames = label['framesCount']
                        xywh = self.rotated_rectangle(prev_sequence)
                        if xywh is None:
                            continue
                        x, y, w, h = xywh
                        x = x * label["original_width"] / 100
                        ...
                        w = w * label["original_width"] / 100
                        h = h * label["original_height"] / 100
                        for each_frame in range(prev_sequence['frame']+1, total_frames+1):
                            annotations.append(
                                {
                                    'id': len(annotations),
                                    'image_id': image_id,
                ...
                elif 'videopolygonlabels' in label:
                    prev_sequence = {}
                    for each_sequence in label['sequence']:
                        if (prev_sequence and not prev_sequence['enabled']) or ( not prev_sequence):
                            points_abs = [
                                (x / 100 * width, y / 100 * height) for x, y in each_sequence["points"]
                            ]
                            x, y = zip(*points_abs)
                            annotations.append(
                                {
                            ...
                            for each_frame in range(prev_sequence['frame']+1, each_sequence['frame']):
                                frame_sequence = populate_interpolation_videopolygons(label, each_sequence, prev_sequence, each_frame)
                                update_details ={ 
                                    'id': len(annotations),
                                    'image_id': image_id,
                                    'category_id': category_id,
                    ...
                    if prev_sequence["enabled"]:
                        total_frames = label['framesCount']
                        points_abs = [
                                (x / 100 * width, y / 100 * height) for x, y in prev_sequence["points"]
                            ]
                        x, y = zip(*points_abs)
                        bbox = get_polygon_bounding_box(x, y)
                        area = get_polygon_area(x, y)
    ...
    def convert_to_yolo(
        ...
        for item_idx, item in enumerate(item_iterator):
            ...
            for label in labels:
                ...
                for category_name in category_names:
                    ...
                    if 'videorectanglelabels' in label:
                        create_file_with_frame = True
                        prev_sequence = {}
                        for each_sequence in label['sequence']:
                            if (prev_sequence and not prev_sequence['enabled']) or ( not prev_sequence):
                                if annotation_dict := yolo_rectangle_append_format(self, each_sequence, category_id):
                                    path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_sequence["frame"]}' +'.txt')
                                    yolo_write_annotations_to_file(path, annotation_dict)
                            else:
                                for each_frame in range(prev_sequence['frame']+1, each_sequence['frame']+1):
                                    frame_sequence = yolo_populate_interpolation_videorectangles(each_sequence, prev_sequence, each_frame)
                                    if annotation_dict := yolo_rectangle_append_format(self, frame_sequence, category_id):
                                        path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_frame}' +'.txt')
                                        yolo_write_annotations_to_file(path, annotation_dict)
                        ...
                        if prev_sequence["enabled"]:
                            total_frames = label['framesCount']
                            if annotation_dict := yolo_rectangle_append_format(self, prev_sequence, category_id):
                                for each_frame in range(prev_sequence['frame']+1, total_frames+1):
                                    path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_frame}' +'.txt')
                                    yolo_write_annotations_to_file(path, annotation_dict)
                        ...
                        for each_sequence in label['sequence']:
                            if (prev_sequence and not prev_sequence['enabled']) or ( not prev_sequence):
                                annotation_dict = yolo_polygonn_append_format(each_sequence['points'], category_id)
                                path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_sequence["frame"]}' +'.txt')
                                yolo_write_annotations_to_file(path, annotation_dict)
                            else:
                                for each_frame in range(prev_sequence['frame']+1, each_sequence['frame']+1):
                                    frame_sequence = yolo_populate_interpolation_videopolygons(each_sequence, prev_sequence, each_frame)
                                    annotation_dict = yolo_polygonn_append_format(frame_sequence, category_id)
                                    path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_frame}' +'.txt')
                                    yolo_write_annotations_to_file(path, annotation_dict)
                        ...
                        if prev_sequence["enabled"]:
                            total_frames = label['framesCount']
                            annotation_dict = yolo_polygonn_append_format(prev_sequence['points'], category_id)
                            for each_frame in range(prev_sequence['frame']+1, total_frames+1):
                                path = os.path.join( output_label_dir, labeler_subfolder, filename + f'_frame_{each_frame}' +'.txt')
                                yolo_write_annotations_to_file(path, annotation_dict)
                    elif (
        ...
        with io.open(metadata_file, mode='w', encoding='utf8') as fout:
            json.dump(
                {
                    'files': files,
                },
