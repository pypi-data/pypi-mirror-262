...
def interpolate_prop(start, end, frame, prop):
    r = (frame - start['frame']) / (end['frame'] - start['frame'])
    if prop == 'rotation':
        d_angle = normalize_angle(end[prop] - start[prop])
        return normalize_angle(start[prop] + d_angle * r)

    return start[prop] + (end[prop] - start[prop]) * r

def populate_interpolation_videorectangles(self, label, cur_sequence, prev_sequence, frame):
    frame_obj = {}
    props =  ['x', 'y', 'width', 'height', 'rotation']
    for each_prop in props:
        frame_obj[each_prop] = interpolate_prop(cur_sequence, prev_sequence, frame, each_prop)
        xywh = self.rotated_rectangle(frame_obj)
        if xywh is None:
            return None
    x, y, w, h = xywh
...
def populate_interpolation_videopolygons(label, cur_sequence, prev_sequence, frame):
    polygon_points = []
    width = label["original_width"]
    height = label["original_height"]
    for each_point_indx in range(len(prev_sequence['points'])):
        x = interpolate_prop({'x': cur_sequence['points'][each_point_indx][0]} | cur_sequence, {'x': prev_sequence['points'][each_point_indx][0]} | prev_sequence, frame, 'x')
        y = interpolate_prop({'y': cur_sequence['points'][each_point_indx][1]} | cur_sequence, {'y': prev_sequence['points'][each_point_indx][1]} | prev_sequence, frame, 'y')
        polygon_points.append([x,y])
    points_abs = [
        (x / 100 * width, y / 100 * height) for x, y in polygon_points
    ]
    x, y = zip(*points_abs)
    res_dict = {
            'segmentation': [
...
def yolo_populate_interpolation_videorectangles(cur_sequence, prev_sequence, frame):
    frame_obj = {}
    props =  ['x', 'y', 'width', 'height', 'rotation']
    for each_prop in props:
        frame_obj[each_prop] = interpolate_prop(cur_sequence, prev_sequence, frame, each_prop)
...
def yolo_write_annotations_to_file(label_path, annotation):
...
def yolo_rectangle_append_format(self, label, category_id):
...
def yolo_polygonn_append_format(points, category_id):
    points_abs = [(x / 100, y / 100) for x, y in points]
    return (
        [category_id]
        + [coord for point in points_abs for coord in point]
    )

def yolo_populate_interpolation_videopolygons(cur_sequence, prev_sequence, frame):
    polygon_points = []
    for each_point_indx in range(len(prev_sequence['points'])):
        x = interpolate_prop({'x': cur_sequence['points'][each_point_indx][0]} | cur_sequence, {'x': prev_sequence['points'][each_point_indx][0]} | prev_sequence, frame, 'x')
        y = interpolate_prop({'y': cur_sequence['points'][each_point_indx][1]} | cur_sequence, {'y': prev_sequence['points'][each_point_indx][1]} | prev_sequence, frame, 'y')
