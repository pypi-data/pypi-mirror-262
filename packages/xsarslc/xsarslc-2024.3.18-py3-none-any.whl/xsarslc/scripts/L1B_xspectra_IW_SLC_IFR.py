...
def generate_IW_L1Bxspec_product(slc_iw_path, output_filename, xspeconfigname, polarization=None, dev=False,
                                 landmask=None,drop_xspectra=False):
    """
    ...
    :param drop_xspectra: bool True->drop the x-spectra variables before saving in netCDF [optional, default=False]
    :return:
    """
    ...
    if one_subswath_xspectrum_dt:
        ...
        if drop_xspectra:
            logging.info('drop x-spectra from inter and intra burst groups')

            for grp in one_subswath_xspectrum_dt.groups:
                variables_to_drop = [v for v in one_subswath_xspectrum_dt[grp].variables if 'xspectra' in v]
                logging.info('grp : %s variables to drop : %s',grp,variables_to_drop)
                if grp != '/' and variables_to_drop != []:
                    one_subswath_xspectrum_dt[grp] = one_subswath_xspectrum_dt[grp].drop_vars(variables_to_drop)
...
def main():
    ...
    parser.add_argument('--dropxspec', action='store_true', default=False,
                        help='True-> drop x-spectra variables before saving to netCDF [default=False]')
    ...
    output_filename = get_output_l1b_filepath(slc_iw_path, outputdir=args.outputdir, version=args.version)
    ...
    if os.path.exists(output_filename) and args.overwrite is False:
        ...
        generate_IW_L1Bxspec_product(slc_iw_path=slc_iw_path, output_filename=output_filename,
                                     ...
                                     polarization=polarization_from_file, landmask=landmask,drop_xspectra=args.dropxspec)
