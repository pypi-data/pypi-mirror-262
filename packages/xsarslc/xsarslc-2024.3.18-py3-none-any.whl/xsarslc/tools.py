...
import os
import datetime
def get_output_l1b_filepath(slc_iw_path,outputdir,version):
    """

    :param slc_iw_path: str full path of measurement level-1 slc
    :param outputdir: str directory where to store the new l1b measurement
    :param version: str for instance A01
    :return:
    """
    # SAFE part
    safe_basename = os.path.basename(os.path.dirname(os.path.dirname(slc_iw_path)))
    safestartdate = datetime.datetime.strptime(safe_basename.split('_')[5],'%Y%m%dT%H%M%S')
    safe_basename = safe_basename.replace('SLC', 'XSP')
    safe_basename = safe_basename.replace('.SAFE','_'+version.upper()+'.SAFE')
    # measurement part
    measu_basename = os.path.basename(slc_iw_path)
    measu_basename = measu_basename.replace('.tiff', '')+ '_' + version.lower() + '.nc'
    measu_basename = measu_basename.replace('-slc-','-xsp-')
    measu_basename = 'l1b-'+measu_basename
    output_filename = os.path.join(outputdir,safestartdate.strftime('%Y'),safestartdate.strftime('%j') , safe_basename, measu_basename )
