    ...
    # for i in xndindex(tiles_sizes):
        ...
        if water_only: # Bright target mask is applied only on water 
            ...
            mod1 = compute_modulation(np.abs(DN1), lowpass_width=lowpass_width,
                                      spacing={'sample': ground_spacing, 'line': azimuth_spacing})
            # ------------- nv ------------
            nv_f = compute_normalized_variance(mod0)
            # ------------- mean sigma0 and nesz ------------
            sigma0_f, _ = compute_mean_sigma0_interp(DN0, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            sigma0_f_overlap, _ = compute_mean_sigma0_interp(DN1, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            
            if not np.isfinite(sigma0_f): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
                sigma0_f, _ = compute_mean_sigma0_closest(DN0, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            if not np.isfinite(sigma0_f_overlap): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
                sigma0_f_overlap, nesz_f_overlap = compute_mean_sigma0_closest(DN1, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])

            sigma0_f_overlap = sigma0_f_overlap.rename(sigma0_f_overlap.name+'_overlap').drop('burst')
            sigma0_f_overlap.attrs.update({'long_name':sigma0_f_overlap.attrs['long_name']+' (latest burst overlap)'})

            sigma0_f.attrs.update({'long_name':sigma0_f.long_name+' with BT correction'})
            sigma0_f_overlap.attrs.update({'long_name':sigma0_f_overlap.long_name+' with BT correction'})
            nv_f.attrs.update({'long_name':nv_f.long_name+' with BT correction'})

            # ---------------- part of the variables to be added to the final dataset ----------------------
            variables_list+=[sigma0_f.to_dataset(name=sigma0_f.name+'_filt'), nv_f.to_dataset(name=nv_f.name+'_filt'), sigma0_f_overlap.to_dataset(name=sigma0_f_overlap.name+'_filt')]
            ...
            xspecs_v = xspecs.var(dim=['periodo_line', 'periodo_sample'], keep_attrs=True)  # variance of periodograms in each tile
            xspecs_v = xspecs_v.rename('var_'+xspecs_v.name)
            ...
            xspecs_m.attrs.update(xs_attrs)
            xspecs_v.attrs.update(xs_attrs)
