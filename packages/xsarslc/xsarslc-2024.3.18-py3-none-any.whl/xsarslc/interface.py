...
def get_low_res_tiles_from_L1BSLC(file_path, xspectra = 'intra', posting = {'sample':400,'line':400}, tile_width = {'sample':17600.,'line':17600.}, window='GAUSSIAN', **kwargs):
    ...
    res = xr.combine_by_coords([t.expand_dims(['tile_sample', 'tile_line']) for t in low_res_tiles])
    if 'land_flag' in res:
...
def get_tiles_from_L1B_SLC(L1B, polarization=None):
    ...
    for DN in DN_tiles:
        ...
        if np.any(np.isnan(range_noise_lut_mytile)):
            # mid_burst_line = int(DN.line.min().values+(DN.line.max().values-DN.line.min().values)/2)
        ...
        tile_coords = { 'tile_line':calibrated_DN['tile_line'], 'tile_sample':calibrated_DN['tile_sample']}
