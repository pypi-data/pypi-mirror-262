...
def tile_burst_to_xspectra(burst, geolocation_annotation, orbit, calibration, noise_range, noise_azimuth, tile_width, tile_overlap,
    ...
    for ii  in range(nbtiles):
        ...
        sigma0.attrs.update({'long_name':'RAW '+sigma0.long_name})
        nesz.attrs.update({'long_name':'RAW '+nesz.long_name})
        variables_list+=[sigma0.to_dataset(name=sigma0.name), nesz.to_dataset(name=nesz.name)]
        ...
        if water_only: # Bright target mask is applied only on water 
            ...
            # ------------- nv ------------
            nv_f = compute_normalized_variance(mod)
            # ------------- mean sigma0 and nesz ------------
            sigma0_f, _ = compute_mean_sigma0_interp(DN, calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])
            if not np.isfinite(sigma0_f): # should only append in IW mode. Case when line are badly indexed in noise-range LUT
                sigma0_f, _ = compute_mean_sigma0_closest(DN, burst['linesPerBurst'], calibration['sigma0_lut'], noise_range['noise_lut'], noise_azimuth['noise_lut'])


            sigma0_f.attrs.update({'long_name':sigma0_f.long_name+' with BT correction'})
            nv_f.attrs.update({'long_name':nv_f.long_name+' with BT correction'})

            # ---------------- part of the variables to be added to the final dataset ----------------------
            variables_list+=[sigma0_f.to_dataset(name=sigma0_f.name+'_filt'), nv_f.to_dataset(name=nv_f.name+'_filt')]
            ...
            if xspecs: # no xspecs have been calculated (could be undefined centroid,)
                ...
                [xspecs_v[k].attrs.update(xs_attrs) for k in xspecs_v.keys()] # this line copy the same attribute to all variables of xspecs_m dataset
...
def compute_intraburst_xspectrum(slc, mean_incidence, slant_spacing, azimuth_spacing, synthetic_duration,
    ...
    periodo = periodo.drop_vars([range_dim, azimuth_dim], errors='ignore').swap_dims({'__' + d: d for d in periodo_slices.keys()})
    ...
    ground_range_spacing = float(slant_spacing / np.sin(np.radians(out.mean_incidence)))

    # Normalisation of X-spectra
    out = out*(ground_range_spacing*azimuth_spacing)
    ...
    k_rg = xr.DataArray(np.fft.rfftfreq(nperseg[range_dim], ground_range_spacing / (2 * np.pi)),
                        ...
                        attrs={'long_name': 'wavenumber in range direction', 'units': 'rad/m', 'spacing':2 * np.pi/(nperseg[range_dim]*ground_range_spacing)})
    k_az = xr.DataArray(np.fft.fftshift(
        np.fft.fftfreq(out.sizes['freq_' + azimuth_dim], azimuth_spacing / (out.attrs.get('look_width') * 2 * np.pi))),
                        dims='freq_' + azimuth_dim, name='k_az',
                        attrs={'long_name': 'wavenumber in azimuth direction', 'units': 'rad/m', 'spacing':out.attrs.pop('look_width')*2*np.pi/(out.sizes['freq_' + azimuth_dim]*azimuth_spacing)})
...
def compute_looks(slc, azimuth_dim, synthetic_duration, nlooks=3, look_width=0.2, look_overlap=0., look_window=None,
    ...
    for l in xndindex(looks_sizes):
        ...
        look = xrft.ifft(look.assign_coords({freq_azi_dim: np.arange(-(nperlook // 2),
            ...
            mydop[freq_azi_dim].spacing)}), dim=freq_azi_dim, detrend=None, window=False, shift=True, true_phase=False,
                         true_amplitude=True)
        look = np.abs(look) ** 2
        look = look / look.mean(dim=slc.dims) # Normalisation before this point is not needed
        ...
        look = xrft.fft(look, dim=slc.dims, detrend='constant', true_phase=True, true_amplitude=False,
                        shift=False)  # shift=False is to keep zero at begnining for easier selection of half the spectrum
    ...
    for l1 in range(nlooks):
        for l2 in range(l1, nlooks):
            ...
            NM = np.prod(looks_spec[{'look': l2}].shape)
            xspecs[str(l2 - l1) + 'tau'].append(looks_spec[{'look': l2}] * np.conj(looks_spec[{'look': l1}])/NM)
    ...
    for i in range(nlooks):
        ...
        concat_spec.attrs.update(
            {'long_name':'sub-looks cross-spectra {} tau'.format(i), 'nlooks': nlooks, 'look_width': look_width, 'look_overlap': look_overlap, 'look_window': str(look_window),
             'tau': tau})
