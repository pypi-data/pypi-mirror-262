#ifndef _IN_SP2_CORE_TIME_H
#define _IN_SP2_CORE_TIME_H
...
inline TimeDelta TimeDelta::fromString( const std::string & str )
{
    ...
    if( n == 0 )
        SP2_THROW( ValueError, "Failed to convert " << str << " to TimeDelta" );
...
inline Date Date::fromYYYYMMDD( const std::string & date )
{
    ...
    if( sscanf( date.c_str(), "%04d%02d%02d", &year, &month, &day ) != 3 )
        SP2_THROW( InvalidArgument, "Date string not in YYYYMMDD format: " << date );
...
inline void Time::checkRange( int64_t raw )
{
    if( raw >= SECONDS_PER_DAY * NANOS_PER_SECOND || raw < -1 )
        SP2_THROW( ValueError, "Time value out of range: " << raw );
}

inline Time::Time( int hour, int minute, int second, int32_t nanosecond )
{
    if( hour > 23 || hour < 0 )
        SP2_THROW( ValueError, "Hour out of range: " << hour );
    if( minute > 59 || minute < 0 )
        SP2_THROW( ValueError, "Minute out of range: " << minute );
    if( second > 59 || second < 0 )
        SP2_THROW( ValueError, "Second out of range: " << second );
    if( nanosecond >= NANOS_PER_SECOND || nanosecond < 0 )
        SP2_THROW( ValueError, "Nanosecond out of range: " << nanosecond );
...
inline Time Time::fromString( const std::string & str )
{
    ...
    if( n == 0 )
        SP2_THROW( ValueError, "Failed to convert " << str << " to Time" );
...
inline const char * DateTime::asCString( char * buf, size_t buflen ) const
{
    ...
    if( ( len = strftime( buf, buflen, "%Y%m%d %H:%M:%S", &TM ) ) == 0 )
        SP2_THROW( RuntimeException, "strftime failed" );
...
namespace std
{
    template<> struct hash<sp2::DateTime>
    {
        size_t operator()( const sp2::DateTime & dt ) const
        {
            return std::hash< int64_t >()( static_cast<int64_t>( dt.asNanoseconds() ) );
        }
    };

    template<> struct hash<sp2::Date>
    {
        size_t operator()( const sp2::Date & dt ) const
        {
            return dt.hash();
        }
    };

    template<> struct hash<sp2::Time>
    {
        size_t operator()( const sp2::Time & t ) const
        {
            return std::hash< int64_t >()( static_cast<int64_t>( t.asNanoseconds() ) );
        }
    };

    template<> struct hash<sp2::TimeDelta>
    {
        size_t operator()( const sp2::TimeDelta & td ) const
        {
