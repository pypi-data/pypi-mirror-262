...
static void sp2_terminate( void );
static bool set_sigabrt_handler();

// invoke set_terminate as part of global constant initialization
// This for uncaught exceptions
static const bool SET_TERMINATE = std::set_terminate( sp2_terminate );
...
void sp2_terminate()
{
    ...
    catch( const sp2::Exception & ex )
    {
        std::cerr << __FUNCTION__ << " caught unhandled sp2::Exception. what(): "
                  << ex.what() << std::endl;
        if( ex.btsize() > 0 )
...
sp2::Exception::Exception( const sp2::Exception &orig ) :
    m_full( orig.m_full ),
...
sp2::Exception::Exception( sp2::Exception && donor ) :
...
void sp2::Exception::writeBacktrace( std::ostream & dest ) const
{
    if( m_backtracesize != 0 )
        printBacktrace( m_backtracemessages, m_backtracesize, dest );
}

std::string sp2::Exception::backtraceString() const
{
