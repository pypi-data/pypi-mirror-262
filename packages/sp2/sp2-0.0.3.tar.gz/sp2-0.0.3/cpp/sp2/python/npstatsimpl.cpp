...
#include <numpy/npy_math.h> // need to be included before sp2

//Include first so that sp2 dialect types are defined
#include <sp2/python/PySp2Type.h>
#include <sp2/python/PyObjectPtr.h>

#include <sp2/core/Time.h>
#include <sp2/cppnodes/statsimpl.h>
...
// autogen types rely on _sp2impl.so files to be created first
    ...
    public:
        ...
        NumPyIterator( PyObject * arr )
        {
            if( unlikely( !PyArray_Check( arr ) ) )
                SP2_THROW( sp2::TypeError, "Expected NumPy array type, got " << Py_TYPE( arr ) -> tp_name );
...
struct PyShape
{
    ...
    void validateShape( PyArrayObject* arr )
    {
        ...
        if( nd != m_dims.size() )
            SP2_THROW( ValueError, "Inconsistent shape of NumPy arrays for computation: different number of dimensions");

        npy_intp* dims = PyArray_DIMS( arr );
        for( size_t i = 0; i < nd; i++ )
        {
            if( dims[i] != m_dims[i] )
                SP2_THROW( ValueError, "Inconsistent shape of NumPy arrays for computation: difference in dimension " << i );
        }
    }
...
{
    if( unlikely( s_first ) )
        SP2_THROW( ValueError, NPY_SHAPE_ERROR );
...
public:
    ...
    {
        if( !sp2.valid( this -> x ) )
            SP2_THROW( ValueError, "Error: sampler called on a NumPy array before any data ticks - shape is unknown.");
    }
};
...
DECLARE_CPPNODE( _list_to_np )
{
    ...
    INVOKE()
    {
        ...
        {
            if( sp2.valid( x[i] ) && ( fillna || sp2.ticked( x[i] ) ) )
                data[i] = x[i];
            else
...
DECLARE_CPPNODE( _np_to_list )
{
    ...
    {
        if( n == 0)
            SP2_THROW( ValueError, "Must provide at least one output channel for NumPy conversion");
    }
    ...
    INVOKE()
    {
        ...
        int64_t nd = PyArray_NDIM( arr );
        if( nd != 1 )
            SP2_THROW( ValueError, "Cannot convert NumPy array of more than one dimension into listbasket");

        int64_t n_arr = PyArray_SIZE( arr );
        if( n_arr != n )
            SP2_THROW( ValueError, "Cannot convert NumPy array of size " << n_arr << " into listbasket of " << n << " elements");
...
public:
    _np_compute( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}
...
public:
    _np_bivariate( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}
...
DECLARE_CPPNODE( _np_exp_timewise )
{
    ...
    INVOKE()
    {
        ...
        if( sp2.ticked( sampler ) && sp2.ticked( x ) )
        {
            PyObject* arr = x.lastValue().get();
            if( unlikely( s_first ) )
            {
        ...
        if( sp2.ticked( additions ) )
        {
            ...
            if( unlikely( s_first ) )
            {
                ...
                if( PyArray_NDIM( (PyArrayObject * ) arr ) != 1 )
                    SP2_THROW( ValueError, "Covariance or correlation matrix called on an array of more than 1 dimension: undefined" );
                // Special setup
    ...
    INVOKE()
    {
        ...
        if( sp2.ticked( x_add ) )
        {
            ...
            if( unlikely( s_first ) )
            {
                PyObject* arr = add_x[0].get();
                if( PyArray_NDIM( (PyArrayObject * ) arr ) != 1 )
                    SP2_THROW( ValueError, "Covariance matrix called on an array of more than 1 dimension: undefined" );
                // Special setup
                int64_t n = PyArray_Size( arr );
                s_shp = PyShape( {n,n}, n );
                s_elem.reserve( n*n );
...
DECLARE_CPPNODE( _np_arg_min_max )
{
    ...
    INVOKE()
    {
        ...
        if( sp2.ticked( x ) && sp2.ticked( sampler ) )
        {
            PyObject* arr = x.lastValue().get();
            if( unlikely( s_first ) )
            {
