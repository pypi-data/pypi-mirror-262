...
DECLARE_CPPNODE( exprtk_impl )
{
    ...
    public:
        ...
        void validateArray( PyArrayObject* arr )
        {
            ...
            if( dim != 1 )
                SP2_THROW( ValueError, "sp2.exprtk recieved an array of dim " << dim << " but can only take 1D arrays" );

            if( !PyArray_CHKFLAGS( arr, NPY_ARRAY_OWNDATA ) )
                SP2_THROW( ValueError, "sp2.exprtk requires arrays be naturally strided" );

            if( !PyArray_ISFLOAT( arr ) )
                SP2_THROW( ValueError, "sp2.exprtk requires arrays to contain floats" );
        }

        void setValue( const TimeSeriesProvider *tsProvider ) override
        {
            PyArrayObject* arr = (PyArrayObject*) sp2::python::toPythonBorrowed(tsProvider -> lastValueTyped<DialectGenericType>());
            ...
            else
            {
                if( PyArray_SIZE( arr ) != m_arr_size )
                    SP2_THROW( ValueError, "sp2.exprtk NumPy array input must have same size each tick, but first saw " << m_arr_size
    ...
    struct sp2_now_fn : public exprtk::ifunction<double>
    {
    public:
        sp2_now_fn() : exprtk::ifunction<double>(0) {}
        double operator()() { return ( m_engine -> rootEngine() -> now() ).asNanoseconds() / 1e9; }
        void setEngine( sp2::Engine * engine ) { m_engine = engine; }
    private:
        sp2::Engine * m_engine;
    };
    ...
    STATE_VAR( sp2_now_fn, s_sp2_now );
    ...
    void compile_expression()
    {
        ...
        if( !s_parser.compile( expression_str, s_expr ) )
            SP2_THROW( ValueError, "cannot compile expression: " << std::string( expression_str ) << " ERROR: " << s_parser.error() );
    ...
    START()
    {
        ...
        for( size_t elem = 0; elem < inputs.size(); ++elem )
        {
            ...
            if( typ -> type() == Sp2Type::Type::DIALECT_GENERIC )
            {
                s_valuesContainer.push_back( std::make_unique<NumpyArrayValueContainer>() );
            }
            else
            {
                PartialSwitchSp2Type<Sp2Type::Type::STRING, Sp2Type::Type::DOUBLE>::invoke(
                        typ,
        ...
        for( auto it = state_vars.value() -> begin(); it != state_vars.value() -> end(); ++it )
        {
            ...
            else if( it.hasValue<double>() || it.hasValue<int64_t>() )
            {
                ...
                SP2_THROW( ValueError, "state_vars dictionary contains " << it.key() << " with unsupported type (need be string or float)" );
        }
        ...
        for( auto it = constants.value() -> begin(); it != constants.value() -> end(); ++it )
        {
            if( it.hasValue<double>() || it.hasValue<int64_t>() )
                ...
                SP2_THROW( ValueError, "constants dictionary contains " << it.key() << " with unsupported type (need be float)" );
        }
        ...
        if( functions.value() -> size() > 0 )
        {
            ...
            {
                sp2::python::PyObjectPtr fnInfo = sp2::python::PyObjectPtr::own( sp2::python::toPython( it.value<DialectGenericType>() ) );
                const char * body;
                PyObject * vars;
                ...
                {
                    SP2_THROW( sp2::python::PythonPassthrough, "could not parse function info in sp2.exprtk" );
                }
                ...
                {
                    case 0:
                        SP2_THROW( ValueError, "sp2.exprtk functions must take at least one variable" );
                        break;
                    case 1:
                        if( !PyArg_ParseTuple( vars, "s", &arg1 ) )
                        {
                            SP2_THROW( sp2::python::PythonPassthrough, "sp2.exprtk could not parse variables list" );
                        }
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        SP2_THROW( ValueError, "sp2.exprtk given too many variables (" << numVars << "), max supported is 4" );
                }
            }
        }

        s_sp2_now.setEngine( engine() );
        symbolTable.add_function( "sp2.now", s_sp2_now );
    ...
    INVOKE()
    {
        ...
        if( likely( sp2.valid( inputs ) ) )
        {
            if( unlikely( !s_isCompiled ) )
                compile_expression();

            const Sp2Type* outputType = unnamed_output().type();
            if( outputType->type() == Sp2Type::Type::DOUBLE )
            {
                RETURN( s_expr.value() );
            }
            else
            {
                ...
                RETURN( sp2::python::PyObjectPtr::own( out ) );
            }
        }
    }
...
// Base nodes
REGISTER_CPPNODE( sp2::cppnodes, sample );
REGISTER_CPPNODE( sp2::cppnodes, firstN );
REGISTER_CPPNODE( sp2::cppnodes, count );
REGISTER_CPPNODE( sp2::cppnodes, _delay_by_timedelta );
REGISTER_CPPNODE( sp2::cppnodes, _delay_by_ticks );
REGISTER_CPPNODE( sp2::cppnodes, merge );
REGISTER_CPPNODE( sp2::cppnodes, split );
REGISTER_CPPNODE( sp2::cppnodes, cast_int_to_float );
REGISTER_CPPNODE( sp2::cppnodes, filter );
REGISTER_CPPNODE( sp2::cppnodes, _drop_dups_float );
REGISTER_CPPNODE( sp2::cppnodes, drop_nans );
REGISTER_CPPNODE( sp2::cppnodes, unroll );
REGISTER_CPPNODE( sp2::cppnodes, collect );
REGISTER_CPPNODE( sp2::cppnodes, demultiplex );
REGISTER_CPPNODE( sp2::cppnodes, multiplex );
REGISTER_CPPNODE( sp2::cppnodes, times );
REGISTER_CPPNODE( sp2::cppnodes, times_ns );
REGISTER_CPPNODE( sp2::cppnodes, struct_field );
REGISTER_CPPNODE( sp2::cppnodes, struct_fromts );
REGISTER_CPPNODE( sp2::cppnodes, struct_collectts );

REGISTER_CPPNODE( sp2::cppnodes, exprtk_impl );

static PyModuleDef _sp2baselibimpl_module = {
    PyModuleDef_HEAD_INIT,
    "_sp2baselibimpl",
    "_sp2baselibimpl c++ module",
    -1,
    NULL, NULL, NULL, NULL, NULL
};

PyMODINIT_FUNC PyInit__sp2baselibimpl(void)
{
    PyObject* m;

    m = PyModule_Create( &_sp2baselibimpl_module);
    if( m == NULL )
        return NULL;
