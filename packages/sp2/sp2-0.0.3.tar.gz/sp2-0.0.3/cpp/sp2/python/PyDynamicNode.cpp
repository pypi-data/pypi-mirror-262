...
static PyObject * PyDynamicNode_create( PyObject * module, PyObject * args )
{
    ...
    for( int i = 0; i < len; ++i )
    {
        ...
        if( strcmp( arg_type, "snap" ) == 0 )
        {
            ...
            SP2_THROW( ValueError, "Internal Error: unrecognized dynamic arg type: " << arg_type );
    }
    ...
    DynamicNode::EngineBuilder builder = [pyBuilderObj,
                                          ...
                                          pyOutputsObj ]( DynamicNode * pNode, DynamicEngine * engine, const DialectGenericType & newkey )
        {
            ...
            for( auto & entry : snapMap )
            {
                ...
                if( !ts -> valid() )
                    SP2_THROW( RuntimeException, "sp2.snap input ( sub_graph arg " << arg_idx << " ) is not valid at time of dynamic creation on sp2.dynamic node '" << pNode -> name() << "'" );
            ...
            for( auto & entry : attachMap )
            {
                ...
                //python side wiring already setup the Edge for sp2.attach, just need to update the elemId properly
