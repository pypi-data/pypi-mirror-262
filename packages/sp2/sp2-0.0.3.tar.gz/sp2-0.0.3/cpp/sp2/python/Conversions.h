#ifndef _IN_SP2_PYTHON_CONVERSIONS_H
#define _IN_SP2_PYTHON_CONVERSIONS_H
...
inline sp2::Sp2TypePtr & pyTypeAsSp2Type( PyObject * pyType )
{
    return Sp2TypeFactory::instance().typeFromPyType( pyType );
}
...
{
    return switchSp2Type( pyTypeAsSp2Type( pyType ), f);
}

inline bool validatePyType( const Sp2Type * type, PyObject *pyType, PyObject * value )
{
    ...
    return type -> type() != Sp2Type::Type::DIALECT_GENERIC ||
...
struct FromPython
{
    static T impl( PyObject * o, const Sp2Type & type )
    {
...
template<typename T>
inline T fromPython( PyObject * o, const Sp2Type & type )
{
...
template<typename T>
inline PyObject * toPython( const T & value, const Sp2Type & type )
{
...
{
    if( !PyBool_Check( o ) )
        SP2_THROW( TypeError, "Invalid bool type, expected bool got " << Py_TYPE( o ) -> tp_name );
...
inline double fromPython( PyObject * o )
{
    if( !PyFloat_Check( o ) )
    {
        ...
        SP2_THROW( TypeError, "Invalid float type, expected float got " << Py_TYPE( o ) -> tp_name );
    }
...
{
    if( !PyLong_Check( o ) )
        SP2_THROW( TypeError, "Invalid int type, expected long (int) got " << Py_TYPE( o ) -> tp_name );
...
inline int32_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<int32_t>::max() || rv < std::numeric_limits<int32_t>::min() )
        SP2_THROW( OverflowError, rv << " is too big/small to fit in int32" );
    return ( int32_t ) rv;
}
...
inline uint32_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<uint32_t>::max() )
        SP2_THROW( OverflowError, rv << " is too big to fit in uint32" );
    return ( uint32_t ) rv;
}
...
inline int16_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<int16_t>::max() || rv < std::numeric_limits<int16_t>::min() )
        SP2_THROW( OverflowError, rv << " is too big/small to fit in int16" );
    return ( int16_t ) rv;
}
...
inline uint16_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<uint16_t>::max() )
        SP2_THROW( OverflowError, rv << " is too big to fit in uint16" );
    return ( uint16_t ) rv;
}
...
inline int8_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<int8_t>::max() || rv < std::numeric_limits<int8_t>::min() )
        SP2_THROW( OverflowError, rv << " is too big/small to fit in int8" );
    return ( int8_t ) rv;
}
...
inline uint8_t fromPython( PyObject * o )
{
    ...
    if( rv > std::numeric_limits<uint8_t>::max() )
        SP2_THROW( OverflowError, rv << " is too big to fit in uint8" );
    return ( uint8_t ) rv;
}

//String
template<>
inline PyObject * toPython( const sp2::Sp2Type::StringCType & value )
{
    return toPythonCheck( PyUnicode_FromStringAndSize( value.c_str(), value.size() ) );
}

template<>
inline PyObject * toPython( const sp2::Sp2Type::StringCType & value, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::STRING );
    const Sp2StringType& strType = static_cast<const Sp2StringType&>(type);
    if(strType.isBytes())
    {
...
template<>
inline sp2::Sp2Type::StringCType fromPython( PyObject * o )
{
    if( PyUnicode_Check( o ) )
    {
        ...
        return sp2::Sp2Type::StringCType( s, len );
    }
    else if(PyBytes_Check( o ))
    {
        ...
        return sp2::Sp2Type::StringCType( s, PyBytes_Size(o) );
    }
    else
    {
        SP2_THROW( TypeError, "Invalid string type, expected str got " << Py_TYPE( o ) -> tp_name );
    }
}
...
template<>
inline StructPtr fromPython( PyObject * o, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::STRUCT );

    if( !PyType_IsSubtype( Py_TYPE( o ), &PyStruct::PyType ) ||
        ( static_cast<const Sp2StructType &>( type ).meta() && //could be sp2.Struct as a type annotation which is allowed
          !StructMeta::isDerivedType( static_cast<PyStruct *>( o ) -> structMeta(),
                                      static_cast<const Sp2StructType &>( type ).meta().get() ) ) )
    {
        std::string name;
        auto meta = static_cast<const Sp2StructType &>( type ).meta();
        if( meta )
            name = " " + meta -> name();
        SP2_THROW( TypeError, "Invalid struct type, expected struct" << name << " got " << Py_TYPE( o ) -> tp_name );
    }
...
//Sp2Enum
template<>
inline PyObject * toPython( const Sp2Enum & e, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::ENUM );

    auto & enumType = static_cast<const Sp2EnumType&>( type );
    const auto * emeta = static_cast<const DialectSp2EnumMeta*>( enumType.meta().get() );

    PyObject * obj = emeta -> pyMeta() -> toPyEnum( e );
    if( unlikely( !obj ) )
        SP2_THROW( ValueError, e.value() << " is not a valid value on sp2.enum type " << emeta -> name() );
    return obj;
}

template<>
inline Sp2Enum fromPython( PyObject * o, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::ENUM );

    if( !PyType_IsSubtype( Py_TYPE( o ), &PySp2Enum::PyType ) ||
        static_cast<PySp2Enum *>( o ) -> meta() != static_cast<const Sp2EnumType &>( type ).meta().get() )
        SP2_THROW( TypeError, "Invalid enum type, expected enum type " << static_cast<const Sp2EnumType &>( type ).meta() -> name() << " got " << Py_TYPE( o ) -> tp_name );

    return static_cast<PySp2Enum *>( o ) -> enum_;
}
...
inline TimeDelta fromPython( PyObject * o )
{
    ...
    if( !PyDelta_Check( o ) )
        SP2_THROW( TypeError, "Invalid timedelta type, expected timedelta got " << Py_TYPE( o ) -> tp_name );
    ...
    {
        SP2_THROW( OverflowError, "timedelta " << PyObjectPtr::incref( o ) << " out of range for sp2 timedelta" );
    }
...
inline Date fromPython( PyObject * o )
{
    ...
    if( !PyDate_Check( o ) )
        SP2_THROW( TypeError, "Invalid date type, expected date got " << Py_TYPE( o ) -> tp_name );
...
inline Time fromPython( PyObject * o )
{
    ...
    if( !PyTime_Check( o ) )
        SP2_THROW( TypeError, "Invalid time type, expected time got " << Py_TYPE( o ) -> tp_name );

    PyDateTime_Time * t = (PyDateTime_Time * ) o;
    if( t -> hastzinfo )
        SP2_THROW( TypeError, "sp2 time type does not support timezones.  Please use ts[object] for timezone time values" );
...
inline DateTime fromPython( PyObject * o )
{
    ...
    if( !PyDateTime_Check( o ) )
        SP2_THROW( TypeError, "Invalid datetime type, expected datetime got " << Py_TYPE( o ) -> tp_name );
    ...
    {
        SP2_THROW( OverflowError, "datetime " << PyObjectPtr::incref( o ) << " is out of range for sp2 datetime" );
    }
...
inline DateTimeOrTimeDelta fromPython( PyObject * o )
{
    ...
    SP2_THROW( TypeError, "Invalid type, expected datetime or timedelta got " << Py_TYPE( o ) -> tp_name );
}
...
{
    if( !PyDict_Check( o ) )
        SP2_THROW( TypeError, "Dictionary conversion expected type dict got " << Py_TYPE( o ) -> tp_name );
    ...
    while( PyDict_Next( o, &pos, &key, &value) )
    {
        ...
        auto v = fromPython<sp2::Dictionary::Value>( value );
...
{
    if( !PyList_Check( o ) )
        SP2_THROW( TypeError, "Dictionary conversion expected type list got " << Py_TYPE( o ) -> tp_name );
...
template<typename T>
inline PyObject * toPython( const std::vector<T> & v, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::ARRAY );
...
template<>
inline PyObject * toPython( const std::vector<bool> & v, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::ARRAY );
...
#ifdef __clang__
template<>
inline PyObject * toPython( const boost::container::vector<bool> & v, const Sp2Type & type )
{
    assert( type.type() == Sp2Type::Type::ARRAY );
...
inline PyObject * toPython( const Dictionary::Value & value )
{
    ...
    {
        case Dictionary::DictDataType::MONOSTATE:          SP2_THROW( ValueError, "Monostate value is not convertible to a Python type");
        ...
        case Dictionary::DictDataType::STRUCTMETAPTR:      SP2_THROW( NotImplemented, "StructMetaPtr value is not convertible to a Python type");
    ...
    SP2_THROW( ValueError, "Given dictionary value is not a valid value type." );
}

template<>
inline PyObject * toPython( const DictionaryPtr & value)
{
    // Convert sp2::Dictionary object to Python dictionary
...
{
    static std::vector<T> impl( PyObject * o, const Sp2Type & type )
    {
        assert( type.type() == Sp2Type::Type::ARRAY );
        const Sp2Type & elemType = *static_cast<const Sp2ArrayType &>( type ).elemType();
        ...
        else if( o -> ob_type -> tp_iter )
        {
            ...
            SP2_THROW( TypeError,  "Invalid list / iterator type, expected list or iterator got " << Py_TYPE( o ) -> tp_name );
...
{
    static boost::container::vector<bool> impl( PyObject * o, const Sp2Type & type )
    {
        assert( type.type() == Sp2Type::Type::ARRAY );
        const Sp2Type & elemType = *static_cast<const Sp2ArrayType &>( type ).elemType();
