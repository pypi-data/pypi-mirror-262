...
PyListBasketInputProxy * PyListBasketInputProxy::create( PyNode * node, INOUT_ID_TYPE id, size_t shape )
{
    if( shape > InputId::maxBasketElements() )
        SP2_THROW( ValueError, "List basket size of " << shape << " exceeds basket size limit of " << InputId::maxBasketElements() << " in node " << node -> name() );
...
static PyObject * PyListBasketInputProxy_getvalue( PyListBasketInputProxy * proxy, Py_ssize_t idx )
{
     ...
     if( !in_proxy -> valid() )
         SP2_THROW( RuntimeException, "list basket element " << idx << " is not valid" );
...
PyTypeObject PyListBasketInputProxy::PyType = {
    ...
    "_sp2impl.PyListBasketInputProxy",   /* tp_name */
...
PyInputProxy * PyDictBasketInputProxy::proxyByKey( PyObject * key )
{
    ...
    if( !in_proxy )
        SP2_THROW( KeyError, "key " << PyObjectPtr::incref( key ) << " %s is not a member of the dict basket" );
    return in_proxy;
}
...
static PyObject * PyDictBasketInputProxy_getvalue( PyDictBasketInputProxy * proxy, PyObject * key )
{
     ...
     if( !in_proxy -> valid() )
         SP2_THROW( RuntimeException, "dict basket element " << PyObjectPtr::incref( key ) << " is not valid" );
...
PyTypeObject PyDictBasketInputProxy::PyType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "_sp2impl.PyDictBasketInputProxy",   /* tp_name */
...
void PyDynamicBasketInputProxy::handleShapeChange( const DialectGenericType & key, bool added, int64_t elemId, int64_t replaceId )
{
    ...
    if( added )
    {
        ...
        SP2_ASSERT( elemId == PyList_GET_SIZE( m_shape.get() ) );
    ...
    else
    {
        ...
        if( replaceId != -1 )
        {
            SP2_ASSERT( replaceId == ( PyList_GET_SIZE( m_shape.get() ) -1 ) );
        ...
        {
            SP2_ASSERT( elemId == ( PyList_GET_SIZE( m_shape.get() ) -1 ) );
            Py_DECREF( PyList_GET_ITEM( m_shape.get(), elemId ) );
        }
...
static char s_shapedoc[] = "timeseries of the dynamic basket shape ts[sp2.DynamicBasketEvents]";
...
PyTypeObject PyDynamicBasketInputProxy::PyType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "_sp2impl.PyDynamicBasketInputProxy",   /* tp_name */
    ...
    0,                         /* tp_as_buffer */
    Py_TPFLAGS_DEFAULT,        /* tp_flags */
    "sp2 dynamic basket input proxy",    /* tp_doc */
    0,                         /* tp_traverse */
    0,                         /* tp_clear */
    0,                         /* tp_richcompare */
    0,                         /* tp_weaklistoffset */
