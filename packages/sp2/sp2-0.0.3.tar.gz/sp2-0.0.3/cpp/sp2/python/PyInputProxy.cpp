...
#include <sp2/python/PyInputProxy.h>
...
PyObject *PyInputProxy::valueAt( ValueType valueType, PyObject *indexArg, PyObject *duplicatePolicyArg, PyObject *defaultValueArg ) const
{
    ...
    if( PyLong_Check( indexArg ) )
    {
        ...
        SP2_TRUE_OR_THROW_RUNTIME( valueIndex <= 0,
        ...
        if( unlikely( valueIndex >= ts() -> tickCountPolicy() ) )
            SP2_THROW( RangeError, "buffer index out of range.  requesting data at index " << valueIndex << " with buffer policy set to " <<
    ...
    else
    {
        ...
        SP2_TRUE_OR_THROW_RUNTIME( duplicatePolicyIntVal == TimeSeriesProvider::DuplicatePolicyEnum::LAST_VALUE ||
        ...
        if( dtd.isTimeDelta() )
        {
            ...
            SP2_TRUE_OR_THROW_RUNTIME( !timedelta.isNone(),
                                       "None time delta is unsupported" );
            SP2_TRUE_OR_THROW_RUNTIME( timedelta.sign() <= 0,
                                       "Positive time delta is unsupported" );

            if( unlikely( -timedelta > ts() -> tickTimeWindowPolicy() ) )
                SP2_THROW( RangeError, "buffer timedelta out of range.  requesting data at timedelta " << PyObjectPtr::incref( indexArg ) << " with buffer policy set to " <<
        ...
        {
            SP2_TRUE_OR_THROW_RUNTIME( dtd.isDateTime(), "value_at index must be integer, DateTime, or TimeDelta" );
            auto datetime = dtd.datetime();

            SP2_TRUE_OR_THROW_RUNTIME( datetime <= m_node -> now(),
                                       "requesting data from future time" );

            if( unlikely( m_node -> now() - datetime >= ts() -> tickTimeWindowPolicy() ) )
                SP2_THROW( RangeError, "requested buffer time out of range.  requesting datetime " << PyObjectPtr::incref( indexArg ) << " at time " <<
    ...
    if( valueIndex < 0 || static_cast<uint32_t>(valueIndex) >= num_ticks() )
    {
        if( defaultValueArg == constants::UNSET() )
            SP2_THROW( OverflowError, "No matching value found" );
    ...
    switch( valueType )
    {
        ...
        default:
            SP2_THROW( NotImplemented, "Unsupported value type " << valueType );
    }
}
...
int32_t PyInputProxy::computeStartIndex( DateTime startDt, autogen::TimeIndexPolicy startPolicy ) const
{
    ...
    switch( startPolicy.enum_value() )
    {
        ...
        case autogen::TimeIndexPolicy::enum_::EXTRAPOLATE:
            ...
            break;
        default:
            SP2_THROW( InvalidArgument, "Unsupported time index policy " << startPolicy.name() );
    }
...
int32_t PyInputProxy::computeEndIndex( DateTime endDt, autogen::TimeIndexPolicy endPolicy ) const
{
    ...
    switch( endPolicy.enum_value() )
    {
        ...
        case autogen::TimeIndexPolicy::enum_::EXCLUSIVE:
            ...
            break;
        default:
            SP2_THROW( InvalidArgument, "Unsupported time index policy " << endPolicy.name() );
    }
                                  ...
                                  PyObject *endIndexPolicyArg ) const
{
    ...
    auto startPolicy = static_cast<autogen::TimeIndexPolicy>( static_cast<PySp2Enum *>( startIndexPolicyArg ) -> enum_ );
    auto endPolicy = static_cast<autogen::TimeIndexPolicy>( static_cast<PySp2Enum *>( endIndexPolicyArg ) -> enum_ );
    ...
    {
        SP2_TRUE_OR_THROW_RUNTIME( PyLong_Check( endIndexArg ) || endIndexArg == Py_None,
        ...
        if( startPolicy.enum_value() != autogen::TimeIndexPolicy::enum_::INCLUSIVE )
            SP2_THROW( InvalidArgument, "Unsupported time index policy for integer indexing: " << startPolicy.name() );

        if( endPolicy.enum_value() != autogen::TimeIndexPolicy::enum_::INCLUSIVE )
            SP2_THROW( InvalidArgument, "Unsupported time index policy for integer indexing: " << endPolicy.name() );

        if( unlikely( startIndex >= ts() -> tickCountPolicy() ) && startIndexArg != Py_None )
            SP2_THROW( RangeError, "buffer index out of range.  requesting data at index " << startIndex << " with buffer policy set to " << ts() -> tickCountPolicy() << " ticks in node '" << m_node -> name() << "'" );
        if( unlikely( endIndex >= ts() -> tickCountPolicy() ) )
            SP2_THROW( RangeError, "buffer index out of range.  requesting data at index " << endIndex << " with buffer policy set to " << ts() -> tickCountPolicy() << " ticks in node '" << m_node -> name() << "'" );
    ...
    {
        SP2_TRUE_OR_THROW_RUNTIME( startIndexArg == Py_None || endIndexArg == Py_None ||
        ...
        if( startIndexArg != Py_None )
        {
            ...
            if( startDtd.isTimeDelta() )
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( startTd.sign() <= 0, "Positive timedelta is unsupported" );

                if( unlikely( -startTd > ts() -> tickTimeWindowPolicy() ) )
                    SP2_THROW( RangeError, "buffer timedelta out of range.  requesting data at timedelta " <<
            ...
            else
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( startDt <= m_node -> now(), "requesting data from future time" );
                if( unlikely( m_node -> now() - startDt > ts() -> tickTimeWindowPolicy() ) )
                    SP2_THROW( RangeError, "requested buffer time out of range.  requesting datetime " << PyObjectPtr::incref( startIndexArg ) << " at time " <<
                                           PyObjectPtr::own( toPython( m_node -> now() ) ) << " with buffer time window policy set to " <<
                                           PyObjectPtr::own( toPython( ts() -> tickTimeWindowPolicy() ) )<< " in node '" << m_node -> name() << "'" );
        ...
        if( endIndexArg != Py_None )
        {
            ...
            if( endDtd.isTimeDelta() )
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( endTd.sign() <= 0, "Positive timedelta is unsupported" );

                if( startIndexArg != Py_None )
                    SP2_TRUE_OR_THROW_RUNTIME( endTd >= startTd,
            ...
            else
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( endDt <= m_node -> now(), "requesting data from future time" );

                if( startIndexArg != Py_None )
                    SP2_TRUE_OR_THROW_RUNTIME( endDt >= startDt,
                                               "Start datetime (got " << startDt << ") must come before end datetime (got " << endDt << ")" );

                if( unlikely( m_node -> now() - endDt >= ts() -> tickTimeWindowPolicy() ) )
                    SP2_THROW( RangeError, "requested buffer time out of range.  requesting datetime " << PyObjectPtr::incref( endIndexArg ) << " at time " <<
                                           PyObjectPtr::own( toPython( m_node -> now() ) ) << " with buffer time window policy set to " <<
                                           PyObjectPtr::own( toPython( ts() -> tickTimeWindowPolicy() ) )<< " in node '" << m_node -> name() << "'" );
...
static PyObject * PyInputProxy_set_buffering_policy( PyInputProxy * proxy, PyObject * args, PyObject * kwargs )
{
    ...
    if( !tickCount && !tickHistory )
        SP2_THROW( TypeError, "sp2.set_buffering_policy expected at least one of tick_count or tick_history" );
...
static inline PyObject * PyInputProxy_value_at_impl( ValueType valueType, PyInputProxy * proxy, PyObject * args )
{
    ...
    if( !PyArg_ParseTuple( args, "OOO", &indexArg, &duplicatePolicyArg, &defaultValueArg ) )
        SP2_THROW(RuntimeException, "Invalid arguments parsed to value_at");
...
static inline PyObject * PyInputProxy_values_at_impl( ValueType valueType, PyInputProxy * proxy, PyObject * args )
{
    ...
    if( !PyArg_ParseTuple( args, "OOO!O!", &startIndexArg, &endIndexArg,
                           &PySp2Enum::PyType, &startExclusiveArg,
                           &PySp2Enum::PyType, &endExclusiveArg ) )
        SP2_THROW( RuntimeException, "Invalid arguments passed to values_at" );
