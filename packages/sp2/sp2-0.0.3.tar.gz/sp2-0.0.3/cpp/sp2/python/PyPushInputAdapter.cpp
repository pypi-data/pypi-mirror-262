...
static int PushBatch_init( PyPushBatch * self, PyObject * args, PyObject * kwargs )
{
    ...
    if( PyTuple_GET_SIZE( args ) != 1 )
        SP2_THROW( TypeError, "PushBatch expected engine as single positional argument" );

    PyObject * pyengine = PyTuple_GET_ITEM( args, 0 );
    if( pyengine -> ob_type != &PyEngine::PyType )
        SP2_THROW( TypeError, "PushBatch expected engine as single positional argument" );

    sp2::RootEngine * engine = ( ( PyEngine * ) pyengine ) -> rootEngine();
...
public:
    PyPushInputAdapter( Engine * engine, AdapterManager * manager,
                        ...
                        PushMode pushMode, PushGroup * pushGroup ) : PushInputAdapter( engine, pyTypeAsSp2Type( pyType ), pushMode, pushGroup ),
...
struct PyPushInputAdapter_PyObject
{
    ...
    static PyObject * pushTick( PyPushInputAdapter_PyObject * self, PyObject * args, PyObject **kwargs )
    {
        ...
        if( len < 1 || len > 2 )
            SP2_THROW( TypeError, "push_tick takes value and optional batch as positional arguments" );
...
PyTypeObject PyPushInputAdapter_PyObject::PyType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "_sp2impl.PyPushInputAdapter", /* tp_name */
...
static InputAdapter * pypushinputadapter_creator( sp2::AdapterManager * manager, PyEngine * pyengine,
                                                  PyObject * pyType, PushMode pushMode, PyObject * args )
{
    ...
    if( !PyType_IsSubtype( pyAdapterType, &PyPushInputAdapter_PyObject::PyType ) )
        SP2_THROW( TypeError, "Expected PushInputAdapter derived type, got " << pyAdapterType -> tp_name );
    ...
    if( pyPushGroup != Py_None )
    {
        ...
        {
            PyErr_Clear();
            SP2_THROW( TypeError, "Expected PushGroup instance for push group, got: " << PyObjectPtr::incref( pyPushGroup ) );
        }
    }
