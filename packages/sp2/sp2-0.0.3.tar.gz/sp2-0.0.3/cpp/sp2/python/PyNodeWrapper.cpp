...
PyNodeWrapper *PyNodeWrapper::create(sp2::Node *node)
{
...
static PyObject *PyNodeWrapper_linkFrom(PyNodeWrapper *self, PyObject *args)
{
    ...
    if( PyType_IsSubtype(Py_TYPE(source), &PyNodeWrapper::PyType) )
    {
        ...
        {
            SP2_ASSERT( sourceNode -> outputBasket( outputIdx ) -> isDynamicBasket() );
    ...
    else if( PyType_IsSubtype(Py_TYPE(source), &PyInputAdapterWrapper::PyType) )
    {
        ...
        SP2_THROW(TypeError, "link_from expected PyNode or PyInputAdapter as source, got " << Py_TYPE( source ) -> tp_name);
...
static PyObject *PyNodeWrapper_createAlarm(PyNodeWrapper *self, PyObject *args)
{
    ...
    auto & sp2Type = pyTypeAsSp2Type( type );
    self -> node() -> createAlarm( sp2Type, inputIdx );
    SP2_RETURN_NONE;
}
...
static PyObject *PyNodeWrapper_createOutput(PyNodeWrapper *self, PyObject *args)
{
    ...
    if( PyTuple_Check( type ) )
    {
        ...
        if( shape == Py_None )
        {
            self -> node() -> createDynamicBasketOutput( sp2Type, index );
        }
        else
        {
            ...
            else
            {
                if( !PyList_Check( shape ) )
                    SP2_THROW( TypeError, "Expected basket shape as int or list, got " << Py_TYPE( shape ) -> tp_name );
                basketSize = PyList_GET_SIZE( shape );
            }
            self -> node() -> createBasketOutput( sp2Type, index, basketSize );
        }
    }
    else
    {
        auto & sp2Type = pyTypeAsSp2Type( type );
        self -> node() -> createOutput( sp2Type, index );
    }
...
PyTypeObject PyNodeWrapper::PyType = {
        ...
        "_sp2impl.PyNodeWrapper",  /* tp_name */
