...
PyNode::PyNode( sp2::Engine * engine, PyObjectPtr gen, PyObjectPtr inputs, PyObjectPtr outputs,
                NodeDef def ) : sp2::Node( def, engine ),
                                m_gen( gen ),
                                m_localVars( nullptr ),
...
//PY311 changes
    ...
    for( int stackloc = code -> co_argcount; stackloc < code -> co_nlocalsplus; ++stackloc, ++localPlusIndex )
    {
        ...
        if( !PyTuple_Check( *var ) )
            SP2_THROW( TypeError, "expected tuple types in stack, got " << (*var) -> ob_type -> tp_name << " in node " << name() );
        ...
        {
            SP2_ASSERT( !isInputBasket( index ) );
        ...
        if( vartype == NODEREF_VAR )
            newvalue = toPython( reinterpret_cast<uint64_t>( static_cast<sp2::Node*>(this) ) );
            ...
            if( PyTuple_Check( inputType ) )
            {
                ...
                else
                {
                    if( !PyList_Check( shape ) )
                        SP2_THROW( TypeError, "Expected input basket shape as int or list, got " << Py_TYPE( shape ) -> tp_name << " on node \"" << name() << "\"" );
        ...
        else if( vartype == OUTPUT_PROXY_VAR )
        {
                ...
                else if( PyLong_Check( shape ) )
                {
                  std::uint64_t basketSize = fromPython<std::uint64_t>( shape );
                    if( basketSize > OutputId::maxBasketElements() )
                        SP2_THROW( ValueError, "output basket size exceeds limit of " << OutputId::maxBasketElements() << " on node \"" << name() << "\"");
                    newvalue = PyListBasketOutputProxy::create( outputType, this, index, basketSize );
                }
                else
                {
                    if( !PyList_Check( shape ) )
                        SP2_THROW( TypeError, "Expected output basket shape as int or list, got " << Py_TYPE( shape ) -> tp_name << " on node \"" << name() << "\"" );
            ...
            SP2_THROW( ValueError, "Unexpected var type " << vartype );
...
inline bool PyNode::makePassive( InputId id )
{
    ...
    {
        // Allocate the passive conversion count array upon first sp2.make_passive call
...
void PyNode::createAlarm( Sp2TypePtr & type, size_t id )
{
    ...
    this -> Node::createAlarm<PyObjectPtr>( Sp2Type::DIALECT_GENERIC(), id );
}
