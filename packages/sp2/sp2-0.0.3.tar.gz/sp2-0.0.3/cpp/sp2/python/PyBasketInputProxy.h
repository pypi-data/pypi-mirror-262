#ifndef _IN_SP2_PYTHON_PYBASKETINPUTPROXY_H
#define _IN_SP2_PYTHON_PYBASKETINPUTPROXY_H

#include <sp2/engine/InputId.h>
#include <sp2/python/Conversions.h>
#include <Python.h>

namespace sp2 { class InputBasketInfo; }
...
public:
    ...
    PyInputProxy * proxy( int64_t elemId )
    {
        if( elemId < 0 || size_t(elemId) >= m_proxies.size() )
            SP2_THROW( RangeError, "basket index out of range" );
