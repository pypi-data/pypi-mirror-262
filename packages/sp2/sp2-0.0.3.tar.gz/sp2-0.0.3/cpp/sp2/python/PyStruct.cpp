...
namespace sp2::python { class PyObjectStructField; }
...
public:
    ...
    void setValue( Struct * s, const DialectGenericType & obj ) const override
    {
        ...
        if( !PyObject_IsInstance( pyobj.ptr(), ( PyObject * ) m_pytype.ptr() ) )
            SP2_THROW( TypeError, "Invalid " << m_pytype -> tp_name << " type, expected " << m_pytype -> tp_name << " got " <<
...
This is done in python code where sp2.Struct is defined as class Struct( _sp2impl.PyStruct, metaclass = StructMeta ).
StructMeta is defined in python and is a derived type of _sp2impl.PyStructMeta so that we can defined some helper code in python.

This is a lot to take in... so again
In python we have:
class StructMeta( _sp2impl.PyStructMeta ):
    def __new__( ... ):
        #extract meta info
...
class Struct( _sp2impl.PyStruct, metaclass = StructMeta ):
    # python side methods...
...
when MyStruct() instance is created, it'll defer required calls to _sp2impl.PyStruct, its baseclass
...
static PyObject * PyStructMeta_new( PyTypeObject *subtype, PyObject *args, PyObject *kwds )
{
    ...
    if( !metadata )
        SP2_THROW( KeyError, "StructMeta missing __metadata__" );
    ...
    StructMeta::Fields fields;
    {
        ...
        while( PyDict_Next( metadata, &pos, &key, &type ) )
        {
            ...
            if( !PyType_Check( type ) && !PyList_Check( type ) )
                SP2_THROW( TypeError, "Struct metadata for key " << keystr << " expected a type, got " << PyObjectPtr::incref( type ) );

            std::shared_ptr<StructField> field;
            Sp2TypePtr sp2type = pyTypeAsSp2Type( type );

            switch( sp2type -> type() )
            {
                case sp2::Sp2Type::Type::BOOL:      field = std::make_shared<BoolStructField>( keystr ); break;
                case sp2::Sp2Type::Type::INT64:     field = std::make_shared<Int64StructField>( keystr ); break;
                case sp2::Sp2Type::Type::DOUBLE:    field = std::make_shared<DoubleStructField>( keystr ); break;
                case sp2::Sp2Type::Type::DATETIME:  field = std::make_shared<DateTimeStructField>( keystr ); break;
                case sp2::Sp2Type::Type::TIMEDELTA: field = std::make_shared<TimeDeltaStructField>( keystr ); break;
                case sp2::Sp2Type::Type::DATE:      field = std::make_shared<DateStructField>( keystr ); break;
                case sp2::Sp2Type::Type::TIME:      field = std::make_shared<TimeStructField>( keystr ); break;
                case sp2::Sp2Type::Type::STRING:    field = std::make_shared<StringStructField>( sp2type, keystr ); break;
                case sp2::Sp2Type::Type::ENUM:      field = std::make_shared<Sp2EnumStructField>( sp2type, keystr ); break;
                case sp2::Sp2Type::Type::STRUCT:    field = std::make_shared<StructStructField>( sp2type, keystr ); break;
                case sp2::Sp2Type::Type::ARRAY:
                {
                    const Sp2ArrayType & arrayType = static_cast<const Sp2ArrayType&>( *sp2type );
                    field = ArraySubTypeSwitch::invoke( arrayType.elemType(), [sp2type,keystr]( auto tag ) -> std::shared_ptr<StructField>
                    {
                        using CElemType = typename decltype(tag)::type;
                        return std::make_shared<ArrayStructField<CElemType>>( sp2type, keystr );
                    } );
                    break;
                }

                case sp2::Sp2Type::Type::DIALECT_GENERIC: field = std::make_shared<PyObjectStructField>( keystr, PyTypeObjectPtr::incref( ( PyTypeObject * ) type ) ); break;
                default:
                    SP2_THROW( ValueError, "Unexpected sp2 type " << sp2type -> type() << " on struct " << name );
            }
    ...
    for( Py_ssize_t idx = 0; idx < numbases; ++idx )
    {
        ...
        //Check if base is a PyStruct type.  Note that this might be the python side sp2.Struct class, but we dont create structMeta on that
        //( see above ) so it will be null here, no need to check explicitly
        if( PyType_IsSubtype( base, &PyStruct::PyType ) )
        {
            if( metabase )
                SP2_THROW( TypeError, "Struct " << name << " defined with multiple struct bases.  Only single-struct hierarchy is supported" );
            metabase = ( ( PyStructMeta * ) base ) -> structMeta;
        }
    }

    /*back reference to the struct type that will be accessible on the sp2 struct -> meta()
    ...
    */
    ...
    if( !defaults )
        SP2_THROW( KeyError, "StructMeta missing __defaults__" );   \
...
static PyObjectPtr PyStructMeta_typeinfo( const Sp2Type * type )
{
    ...
    if( type -> type() == Sp2Type::Type::ENUM )
    {
        auto const * enumType = static_cast<const Sp2EnumType*>( type );
        auto const * enumMeta = static_cast<const DialectSp2EnumMeta*>( enumType -> meta().get() );
        if( PyDict_SetItemString( out.get(), "pytype", ( PyObject * ) enumMeta -> pyType().get() ) < 0 )
            SP2_THROW( PythonPassthrough, "" );
    }
    else if( type -> type() == Sp2Type::Type::STRUCT )
    {
        auto const * structType = static_cast<const Sp2StructType*>( type );
    ...
    else if( type -> type() == Sp2Type::Type::ARRAY )
    {
        auto const * arrayType = static_cast<const Sp2ArrayType*>( type );
...
{
    PyObject *v = switchSp2Type( field -> type(), [ field, struct_ ]( auto tag )
    {
...
void PyStruct::setattr( Struct * s, PyObject * attr, PyObject * value )
{
    ...
    if( !field )
        SP2_THROW( AttributeError, "'" << s -> meta() -> name() << "' object has no attribute '" << PyUnicode_AsUTF8( attr ) << "'" );

    try
    {
        switchSp2Type( field -> type(), [field,&struct_=s,value]( auto tag )
        {
            using CType = typename decltype(tag)::type;
            auto *typedField = static_cast<const typename StructField::upcast<CType>::type *>( field );
    ...
    {
        SP2_THROW( TypeError, "on field '" << PyUnicode_AsUTF8( attr ) << "' : " << err.description() );
    }
}
...
void repr_array( const std::vector<ElemT> & val, const Sp2ArrayType & arrayType, std::string & tl_repr, bool show_unset );

#ifdef __clang__
void repr_array( const boost::container::vector<bool> & val, const Sp2ArrayType & arrayType, std::string & tl_repr, bool show_unset );
#endif
...
void repr_field( const Struct * struct_, const StructFieldPtr & field, std::string & tl_repr, bool show_unset )
{
    ...
    switch( type )
    {
        ...
        {
            auto const * arrayType = static_cast<const Sp2ArrayType*>( field -> type().get() );
            const Sp2Type * elemType = arrayType -> elemType().get();

            switchSp2Type( elemType, [ field, struct_, &arrayType, &tl_repr, show_unset ]( auto tag )
            {
                using CElemType = typename decltype( tag )::type;
                #ifdef __clang__
        ...
        case Sp2Type::Type::UNKNOWN:
        case Sp2Type::Type::NUM_TYPES:
            break;
    }
}
...
void repr_array( const std::vector<ElemT> & val, const Sp2ArrayType & arrayType, std::string & tl_repr, bool show_unset )
{
    ...
    for( auto it = val.begin(); it != val.end(); it++ )
    {
        ...
        else if constexpr( std::is_same<ElemT, StructPtr>::value )
        {
            ...
            repr_array( *it, static_cast<const Sp2ArrayType&>( arrayType.elemType() ), tl_repr, show_unset ); // recursive, allows for nested arrays!
        else
        {
...
PyObject * PyStruct_new( PyTypeObject * type, PyObject *args, PyObject *kwds )
{
    ...
    if( ! ( (PyStructMeta * ) type ) -> structMeta )
        SP2_THROW( TypeError, "sp2.Struct cannot be instantiated" );
...
int PyStruct_tp_clear( PyStruct * self )
{
    ...
    {
        if( field -> type() -> type() == Sp2Type::Type::DIALECT_GENERIC && field -> isSet( self -> struct_.get() ) )
        {
...
int PyStruct_traverse( PyStruct * self, visitproc visit, void * arg )
{
    ...
    for( auto & field : self -> structMeta() -> fields() )
    {
        if( field -> type() -> type() == Sp2Type::Type::DIALECT_GENERIC )
        {
            auto * pyfield = static_cast<PyObjectStructField*>( field.get() );
...
void PyStruct_setattrs( PyStruct * self, PyObject * args, PyObject * kwargs, const char * methodName )
{
    if( PyTuple_GET_SIZE( args ) > 0 )
        SP2_THROW( TypeError, "'" << self -> ob_type -> tp_name << '.' << methodName << "' takes 0 positional arguments but " << PyTuple_GET_SIZE( args ) << " were given" );
...
PyObject * PyStruct_copy_from( PyStruct * self, PyObject * o )
{
    ...
    if( !PyType_IsSubtype( Py_TYPE( o ), &PyStruct::PyType ) )
        SP2_THROW( TypeError, "Attempting to copy from non-struct type " << Py_TYPE( o ) -> tp_name );
...
PyObject * PyStruct_deepcopy_from( PyStruct * self, PyObject * o )
{
    ...
    if( !PyType_IsSubtype( Py_TYPE( o ), &PyStruct::PyType ) )
        SP2_THROW( TypeError, "Attempting to deepcopy from non-struct type " << Py_TYPE( o ) -> tp_name );
...
PyObject * PyStruct_update_from( PyStruct * self, PyObject * o )
{
    ...
    if( !PyType_IsSubtype( Py_TYPE( o ), &PyStruct::PyType ) )
        SP2_THROW( TypeError, "Attempting to update_from from non-struct type " << Py_TYPE( o ) -> tp_name );
