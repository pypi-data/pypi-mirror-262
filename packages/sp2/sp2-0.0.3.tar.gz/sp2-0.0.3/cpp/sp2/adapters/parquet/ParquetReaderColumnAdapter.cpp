        ...
        const std::shared_ptr<typename ArrowArrayType::TypeClass> &dateType )
{
    ...
    SP2_THROW( sp2::TypeError, "Unexpected day unit: " << ( int ) dateType -> unit() << " for column " << columnName );
}
...
//                    "Unexpected parquet column timezone: " << timestampType -> timezone() );
        ...
        case arrow::Type::DICTIONARY:
        {
            ...
            {
                SP2_THROW( ParquetColumnTypeError,
        ...
        case arrow::Type::LIST:
        {
            ...
            switch( valueType -> id() )
            {
                ...
                default:
                    SP2_THROW( TypeError, "Trying to create arrow list array reader for unsupported element type " << valueType -> name() << " for column " << field.name() );
            }
        }
                                                                                         ...
                                                                                         std::optional<utils::Symbol> symbol )
{
    ...
    {
        SP2_THROW( TypeError, "Unexpected column type for column " << getColumnName() << " , expected "
...
template< typename ValueType, typename ArrowArrayType, typename ValueDispatcherT >
void BaseTypedColumnAdapter<ValueType, ArrowArrayType, ValueDispatcherT>::ensureType( Sp2Type::Ptr sp2Type )
{
    try
    {

        CompatibleTypeSwitch::invoke( sp2Type.get(), [ sp2Type, this ]( auto tag )
        {
            // No need to check native types here, they won't be assignable so it will raise exception.
            if( sp2Type -> type() > Sp2Type::Type::MAX_NATIVE_TYPE )
            {
                SP2_TRUE_OR_THROW( ( std::is_same<ValueType, typename decltype(tag)::type>::value ), TypeError,
                                   "Unexpected column type for column " << getColumnName() << " , expected " << sp2Type -> type().asCString() <<
                                                                       " got " << ArrowArrayType::TypeClass::type_name() );
            }
        } );
    }
    ...
    {
        SP2_THROW( TypeError, "Unexpected column type for column " << getColumnName() << " , expected " <<  sp2Type -> type().asCString() <<
                                                                   " got " << ArrowArrayType::TypeClass::type_name() );
    }
...
{
    SP2_TRUE_OR_THROW_RUNTIME( data -> num_chunks() == 1,
...
ParquetStructAdapter::ParquetStructAdapter( ParquetReader &parquetReader, StructAdapterInfo adapterInfo )
        : m_parquetReader( parquetReader ),
          m_structMeta( std::static_pointer_cast<const Sp2StructType>( adapterInfo.type() ) -> meta() )
{
    ...
    for( auto it = adapterInfo.fieldMap() -> begin(); it != adapterInfo.fieldMap() -> end(); ++it )
    {
        ...
        if(fieldPtr -> type() -> type() == Sp2Type::TypeTraits::STRUCT)
        {
            auto &fieldMeta = std::static_pointer_cast<const Sp2StructType>( fieldPtr -> type() ) -> meta();
          ...
          m_structMeta( structMeta )
{
    m_resetFunc = []()
    {
        SP2_THROW( RuntimeException, "Internal error, trying to reset single column struct adapter" );
    };

    auto &arrowColumns = arrowType -> fields();

    SP2_TRUE_OR_THROW_RUNTIME( arrowColumns.size() == columnAdapters.size(), "Found mismatch between arrow and sp2 schema" );
                                              ...
                                              ParquetColumnAdapter &columnAdapter )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( fieldPtr != nullptr,
                               "No field " << fieldName << " in struct " << m_structMeta -> name() );

    FieldSetter fieldSetter;
    if( fieldPtr -> type() -> type() == Sp2Type::TypeTraits::ENUM )
    {
        columnAdapter.ensureType( Sp2Type::STRING() );
        auto enumMetaPtr = std::static_pointer_cast<const Sp2EnumType>( fieldPtr -> type() ) -> meta();
        using ColumnType = Sp2Type::Type::toCType<Sp2Type::Type::STRING>::type;
        fieldSetter = [ &columnAdapter, fieldPtr, enumMetaPtr ]( StructPtr &s )
        {
            ...
            {
                fieldPtr -> setValue<Sp2Enum>( s.get(), enumMetaPtr -> fromString( curValue.value().c_str() ) );
            }
        };
    }
    else
    {
        using TypeSwitch = PrimitiveSp2TypeSwitch::Extend<Sp2Type::Type::STRUCT>;
        columnAdapter.ensureType( fieldPtr -> type() );
        if( fieldPtr -> type() -> type() == Sp2Type::Type::STRUCT )
        {
            auto &fieldMeta = std::static_pointer_cast<const Sp2StructType>( fieldPtr -> type() ) -> meta();
        ...
        {
            fieldSetter = NativeSp2TypeSwitch::invoke(
                    ( columnAdapter.getNativeSp2Type().get() ), [ &columnAdapter, &fieldPtr ]( auto columnTag )
                    {
                                    ...
                                    return FieldSetter( [ &columnAdapter, fieldPtr ]( StructPtr &s )
                                                        {
                                                            ...
                                                            if( curValue.has_value() )
                                                            {
                                                                fieldPtr -> setValue<FieldType>( s.get(), sp2::cast<FieldType>(
                                                                        curValue.value() ) );
                                                            }
                                                        } );
                                } );
...
{
    SP2_TRUE_OR_THROW( inputAdapter -> type() -> type() == Sp2Type::Type::STRUCT, TypeError,
                       "Subscribing unexpected type " << inputAdapter -> type() -> type() << " as struct for column " );
    auto meta = static_cast<const Sp2StructType *>( inputAdapter -> type()) -> meta();
    SP2_TRUE_OR_THROW( meta == m_structMeta, TypeError,
...
void NativeTypeColumnAdapter<ValueType, ArrowArrayType>::readCurValue()
{
    ...
    {
        this -> m_curValue = sp2::cast<ValueType>( this -> m_curChunkArray -> Value( curRow ) );
    }
    else
    {
...
void DatetimeColumnAdapter<UNIT>::readCurValue()
{
    ...
    if( this -> m_curChunkArray -> IsValid( curRow ) )
    {
        this -> m_curValue = sp2::DateTime::fromNanoseconds(
...
void DurationColumnAdapter<UNIT>::readCurValue()
{
    ...
    if( this -> m_curChunkArray -> IsValid( curRow ) )
    {
        this -> m_curValue = sp2::TimeDelta::fromNanoseconds(
                m_curChunkArray -> Value( m_parquetReader.getCurRow() ) * UNIT );
    }
    else
    {
...
void TimeColumnAdapter<UNIT, ArrowTimeArray>::readCurValue()
{
    ...
    if( this -> m_curChunkArray -> IsValid( curRow ) )
    {
        this -> m_curValue = sp2::Time::fromNanoseconds(
...
{
    if( inputAdapter -> type() -> type() == Sp2Type::TypeTraits::ENUM )
    {
        auto enumMetaPtr = static_cast<const Sp2EnumType *>( inputAdapter -> type()) -> meta();
        auto callback    = std::function<void(
                ...
                [ inputAdapter, enumMetaPtr ]( const std::string *val )
                {
                    if( val )
                    {
                        inputAdapter -> pushTick<Sp2Enum>( enumMetaPtr -> fromString( val -> c_str() ) );
                    }
                    else
                    {
                        inputAdapter -> pushNullTick<Sp2Enum>();
                    }
                } );
        m_dispatcher.addSubscriber( callback, symbol );
    }
...
namespace
{
    ...
    struct ArrayValidValueProvider
    {
        ...
        static T getValue(const V& array, int index)
        {
            ...
            {
                SP2_THROW(ValueError, "Can't read empty value to array from arrow array of type " << arrow::StringArray::TypeClass::type_name());
            }
...
{
    SP2_THROW( NotImplemented, "Trying to subscribe to list column indirectly, via struct field?" );
}
...
{
    SP2_TRUE_OR_THROW_RUNTIME( m_listReader == nullptr,
                               "Trying to subscribe list column in parquet reader more than once, this is not supported" );
    SP2_TRUE_OR_THROW_RUNTIME( listReader != nullptr,
...
{
    SP2_TRUE_OR_THROW_RUNTIME( inputAdapter -> type() -> type() == Sp2Type::TypeTraits::STRUCT,
                               "Trying to subscribe with non struct type " << inputAdapter -> type() -> type().asString() );
    auto structMeta = static_cast<const Sp2StructType *>( inputAdapter -> type()) -> meta();
...
void StructColumnAdapter::handleNewBatch( const std::shared_ptr<::arrow::ChunkedArray> &data )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( childArrays.size() == m_childColumnAdapters.size(),
...
void StructColumnAdapter::initFromStructMeta( const std::shared_ptr<StructMeta> &structMeta )
{
    if( m_structAdapter )
    {
        SP2_TRUE_OR_THROW_RUNTIME( m_structAdapter -> getStructMeta() == structMeta,
