#include <sp2/adapters/parquet/FileWriterWrapper.h>
...
void FileWriterWrapper::open( const std::string &fileName, const std::string &compression, bool allowOverwrite )
{
    if( !allowOverwrite )
    {
        SP2_TRUE_OR_THROW_RUNTIME( !utils::fileExists( fileName ),
...
::arrow::Compression::type FileWriterWrapper::resolveCompression( const std::string &compression )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( it != m_compressionNameMapping.end(), "Unable to resolve compression: " << compression );
    return it->second;
}
