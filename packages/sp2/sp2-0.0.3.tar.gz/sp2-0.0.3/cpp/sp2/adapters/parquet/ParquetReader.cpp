...
#include <sp2/adapters/parquet/ArrowIPCFileReaderWrapper.h>
#include <sp2/adapters/parquet/ParquetReaderColumnAdapter.h>
#include <sp2/adapters/parquet/ParquetFileReaderWrapper.h>
#include <sp2/adapters/parquet/ParquetStatusUtils.h>
#include <sp2/core/FileUtils.h>
    ...
    if( getSymbolColumnAdapter().valid() )
    {
        switch( m_symbolType )
        {
            case Sp2Type::Type::STRING:
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( curSymbol.has_value(),
            ...
            default:
                SP2_THROW( RuntimeException, "Unexpected symbol type: " << m_symbolType );
        }
...
void ParquetReader::setSymbolColumnAdapter( ColumnAdapterReference adapter )
{
    ...
    try
    {
        m_symbolColumn -> ensureType( Sp2Type::STRING() );
        m_symbolType = Sp2Type::Type::STRING;
    }
    catch( const TypeError & )
    {
        try
        {
            m_symbolColumn -> ensureType( Sp2Type::INT64() );
            m_symbolType = Sp2Type::Type::INT64;
        }
        catch( const TypeError & )
        {
            SP2_THROW( TypeError, "Invalid symbol column type.  Only string and int64 symbols are currently supported" );
        }
    }
}
...
void ParquetReader::validateSymbolType( const utils::Symbol & symbol )
{
    ...
    {
        case Sp2Type::Type::STRING:
            SP2_TRUE_OR_THROW( std::holds_alternative<std::string>( symbol ), TypeError, "Provided symbol type does not match symbol column type (string)" );
            break;

        case Sp2Type::Type::INT64:
            SP2_TRUE_OR_THROW( std::holds_alternative<int64_t>( symbol ), TypeError, "Provided symbol type does not match symbol column type (int64)" );
            break;
        default:
            SP2_THROW( RuntimeException, "Unexpected symbol type: " << m_symbolType );
    }
...
void SingleTableParquetReader::setColumnAdaptersFromCurrentTable()
{
    ...
    for( const auto &columnName:m_columns )
    {
        ...
        {
            SP2_TRUE_OR_THROW_RUNTIME( existingRecordIt -> second == m_columnAdapters.size(),
        ...
        {
            SP2_TRUE_OR_THROW_RUNTIME( isAllowMissingColumns(), "Missing column " << columnName << " in file " << getCurFileOrTableName() );
...
bool SingleFileParquetReader::openNextFile()
{
    ...
    while(true)
    {
        ...
        {
            if(m_allowMissingFiles && !sp2::utils::fileExists(fileName))
            {
                continue;
            }
            if( isArrowIPC() )
...
bool InMemoryTableParquetReader::openNextFile()
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( table -> num_columns() > 0, "Provided in memory arrow table with 0 columns" );
    schema = table -> schema();
    ...
    {
        SP2_TRUE_OR_THROW_RUNTIME( table -> column( i ) -> num_chunks() == refNumChunks,
...
bool InMemoryTableParquetReader::readNextRowGroup()
{
    ...
    while( !endOfData )
    {
        ...
        for(auto colIndex : m_neededColumnIndices)
        {
            ...
            SP2_TRUE_OR_THROW_RUNTIME( curArrayChunk -> length() == refChunkLength,
...
bool MultipleFileParquetReader::start()
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( successCount == 0 || successCount == m_columnReaders.size(),
...
ColumnAdapterReference MultipleFileParquetReader::operator[]( const std::string &name )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( it != m_columnReaderByName.end(),
...
bool MultipleFileParquetReader::skipRow()
{
    ...
    {
        SP2_THROW(RuntimeException, "Input files are not alligned - some columns have more data than the others");
    }
    return true;
}
