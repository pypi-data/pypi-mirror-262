...
ParquetInputAdapterManager::ParquetInputAdapterManager( sp2::Engine *engine, const Dictionary &properties,
        ...
        m_reader()
{
    SP2_TRUE_OR_THROW_RUNTIME(!generatorPtr || !m_tableGenerator, "Trying to set both generatorPtr and tableGeneratorPtr");
    SP2_TRUE_OR_THROW_RUNTIME(generatorPtr || m_tableGenerator, "Either generatorPtr or tableGeneratorPtr must be set");
    ...
    SP2_TRUE_OR_THROW_RUNTIME( m_timeColumn != "", "Time column can't be empty" );
    SP2_TRUE_OR_THROW_RUNTIME( m_defaultTimezone == "UTC",
...
void ParquetInputAdapterManager::start( DateTime starttime, DateTime endtime )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( m_reader == nullptr, "Starting parquet adapter manager more than once" );
    ...
    {
        neededColumns.insert( it.first + "__sp2_value_count" );
    }
    ...
    for( auto &&it : m_dictBasketInputAdapters )
    {
        auto valueCountColumnName = it.first + "__sp2_value_count";
        std::string basketSymbolColumn = it.first + +"__sp2_symbol";
        DictBasketReaderRecord record{ ( *m_reader )[ valueCountColumnName ], nullptr };
        record.m_valueCountColumn -> ensureType( Sp2Type::UINT16() );
    ...
    SP2_TRUE_OR_THROW_RUNTIME( m_timestampColumnAdapter.valid(), "m_timestampColumnAdapter is NULL" );
    m_timestampColumnAdapter -> ensureType( Sp2Type::DATETIME() );
                                                                                    ...
                                                                                    bool nullOnEmpty) const
{
    ...
    {
        SP2_TRUE_OR_THROW_RUNTIME(m_fileNameGeneratorReplicator, "Trying to read split columns from file while reading in memory tables");
    ...
    if(!reader->isEmpty())
    {
        for( auto &symMapPair : adaptersBySymbol )
        {
            ...
            for( auto &columnAdapterPair:symMapPair.second.m_adaptersByColumnName )
            {
                ...
                SP2_TRUE_OR_THROW_RUNTIME( !columnAdapter->isListType() || listReaderInterface != nullptr,
            ...
            {
                SP2_TRUE_OR_THROW_RUNTIME( structAdapterPair.second.m_listReaderInterface == nullptr,
...
DateTime ParquetInputAdapterManager::processNextSimTimeSlice( DateTime time )
{
    ...
    SP2_TRUE_OR_THROW_RUNTIME( data_reference_time == nextDataTime, "Expected time " << nextDataTime.value() << " got " << data_reference_time );
    do
    {
        for( auto &&dictBasketRecord:m_dictBasketReaders )
        {
...
ManagedSimInputAdapter *ParquetInputAdapterManager::getInputAdapter( Sp2TypePtr &type, const Dictionary &properties, PushMode pushMode,
                                                                     const DialectGenericListReaderInterface::Ptr &listReaderInterface )
{
    SP2_TRUE_OR_THROW( !m_pushMode.has_value() || m_pushMode.value() == pushMode, NotImplemented,
    ...
    if( properties.exists( "symbol" ) )
    {
        ...
        if( std::holds_alternative<std::string>( rawSymbol ) )
            ...
            SP2_THROW( TypeError, "Parquet subscribe symbol must be a string or int type" );
    }
    ...
    {
        SP2_TRUE_OR_THROW(listReaderInterface == nullptr, NotImplemented, "Reading of baskets of arrays is unsupported");
        return getDictBasketAdapter( type, properties, pushMode, symbol, basketName );
    }
}

ManagedSimInputAdapter *
ParquetInputAdapterManager::getRegularAdapter( const Sp2TypePtr &type, const Dictionary &properties, const PushMode &pushMode,
                                               ...
                                               const DialectGenericListReaderInterface::Ptr &listReaderInterface )
{
    if( pushMode == PushMode::NON_COLLAPSING )
    {
        ...
        SP2_TRUE_OR_THROW( !m_subscribedBySymbol || !m_subscribedForAll, NotImplemented,
    ...
    {
        SP2_TRUE_OR_THROW(listReaderInterface == nullptr, NotImplemented, "Reading of arrays of structs is unsupported");
    ...
    SP2_THROW( RuntimeException, "Reached unreachable code" );
}

ManagedSimInputAdapter *ParquetInputAdapterManager::getDictBasketAdapter( const Sp2TypePtr &type,
                                                                          ...
                                                                          const std::string &basketName )
{
    ...
    {
        auto field = properties.get<std::string>( "field_map" );
        SP2_TRUE_OR_THROW_RUNTIME( field.empty(), "Non empty field map for dict basket" );
...
ParquetInputAdapterManager::getOrCreateSingleColumnAdapter( ParquetInputAdapterManager::AdaptersBySymbol &inputAdaptersContainer,
                                                            const Sp2TypePtr &type, const utils::Symbol &symbol, const std::string &field,
                                                            ...
                                                            const DialectGenericListReaderInterface::Ptr &listReaderInterface )
{
    ...
    const Sp2TypePtr& adapterType = (listReaderInterface==nullptr) ? type : Sp2Type::DIALECT_GENERIC();
...
ManagedSimInputAdapter *
ParquetInputAdapterManager::getSingleColumnAdapter( const Sp2TypePtr &type, const utils::Symbol &symbol,
...
ManagedSimInputAdapter *ParquetInputAdapterManager::getOrCreateStructColumnAdapter( AdaptersBySymbol &inputAdaptersContainer,
                                                                                    ...
                                                                                    const sp2::DictionaryPtr &fieldMap,
                                                                                    const PushMode &pushMode )
{
    auto itBySymbol = inputAdaptersContainer.find( symbol );
    if( itBySymbol == inputAdaptersContainer.end() )
...
ManagedSimInputAdapter *
ParquetInputAdapterManager::getStructAdapter( const Sp2TypePtr &type, const utils::Symbol &symbol,
