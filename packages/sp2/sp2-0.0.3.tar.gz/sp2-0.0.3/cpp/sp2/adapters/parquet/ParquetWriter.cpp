...
#include <sp2/adapters/parquet/ArrowIPCFileWriterWrapper.h>
...
ParquetWriter::ParquetWriter( ParquetOutputAdapterManager *mgr, const Dictionary & properties ) : ParquetWriter( mgr, std::optional<bool>{} )
{
    if( properties.exists( "file_metadata" ) )
    {
        ...
        for( auto it = file_meta -> begin(); it != file_meta -> end(); ++it )
        {
            ...
            if( !value )
                SP2_THROW( TypeError, "parquet metadata can only have string values" );
    ...
    if( properties.exists( "column_metadata" ) )
    {
        ...
        for( auto colIt = column_metadata -> begin(); colIt != column_metadata -> end(); ++colIt )
        {
            ...
            if( !cmeta )
                SP2_THROW( TypeError, "parquet column metadata expects dictionary entry per column, got unrecognized type for column '" << colIt.key() << "'" );
            ...
            for( auto it = (*cmeta) -> begin(); it != (*cmeta) -> end(); ++it )
            {
                const std::string * value = std::get_if<std::string>( &it.getUntypedValue() );
                if( !value )
                    SP2_THROW( TypeError, "parquet column metadata can only have string values, got non-string value for metadata on column '" << colIt.key() << "'" );
...
SingleColumnParquetOutputAdapter *ParquetWriter::getScalarOutputAdapter( Sp2TypePtr &type, const std::string &columnName )
{
    auto res = static_cast<SingleColumnParquetOutputAdapter *>(getScalarOutputHandler( type, columnName ));
    return res;
}

StructParquetOutputAdapter *ParquetWriter::getStructOutputAdapter( Sp2TypePtr &type, sp2::DictionaryPtr fieldMap )
{
    auto res = static_cast<StructParquetOutputAdapter *>(getStructOutputHandler( type, fieldMap ));
    return res;
}

SingleColumnParquetOutputHandler *ParquetWriter::getScalarOutputHandler( Sp2TypePtr &type, const std::string &columnName )
{
    SP2_TRUE_OR_THROW_RUNTIME( m_publishedColumnNames.emplace( columnName ).second,
...
StructParquetOutputHandler *ParquetWriter::getStructOutputHandler( Sp2TypePtr &type, sp2::DictionaryPtr fieldMap )
{
...
ListColumnParquetOutputHandler *ParquetWriter::getListOutputHandler( Sp2TypePtr &elemType, const std::string &columnName,
...
ListColumnParquetOutputAdapter *ParquetWriter::getListOutputAdapter(
        Sp2TypePtr &elemType, const std::string &columnName,
        const DialectGenericListWriterInterface::Ptr &listWriterInterface )
{
...
void ParquetWriter::start()
{
    ...
    if( !m_columnMetaData.empty() )
        SP2_THROW( ValueError, "parquet column metadata has unmapped column: '" << m_columnMetaData.begin() -> first << "'" );
...
{
    SP2_TRUE_OR_THROW_RUNTIME( m_fileWriterWrapperContainer, "Trying to set file name when file writer already closed" );
    writeCurChunkToFile();
    m_fileWriterWrapperContainer -> close();
    if( !fileName.empty() )
    {
...
SingleColumnParquetOutputHandler *ParquetWriter::createScalarOutputHandler( Sp2TypePtr type, const std::string &name )
{
    return m_engine -> createOwnedObject<SingleColumnParquetOutputAdapter>( *this, type, name );
}

ListColumnParquetOutputHandler *ParquetWriter::createListOutputHandler( Sp2TypePtr &elemType, const std::string &columnName,
...
StructParquetOutputHandler *ParquetWriter::createStructOutputHandler( Sp2TypePtr type,
...
void ParquetWriter::writeCurChunkToFile()
{
    if( m_curChunkSize > 0 )
    {
        if( unlikely( !isFileOpen() ) )
        {
            if(m_curChunkSize != 0 )
            {
                SP2_THROW( sp2::RuntimeException, "Trying to write to parquet/arrow file, when no file name was provided" );
