        ...
        : ParquetOutputHandler( parquetWriter, type )
{
    switch( m_type -> type() )
    {
       ...
       case Sp2Type::TypeTraits::TIME:
        ...
        case Sp2Type::TypeTraits::ENUM:
        {
            auto enumMetaPtr = std::static_pointer_cast<const Sp2EnumType>( type ) -> meta();
            createEnumColumnBuilder( columnName, enumMetaPtr );
            break;
        }
        default:
        {
            SP2_THROW( TypeError, "Writing of " << m_type -> type().asString() << " to parquet is not supported" );
        }
    }
}
...
void SingleColumnParquetOutputHandler::createEnumColumnBuilder( const std::string &columnName, Sp2EnumMeta::Ptr enumMetaPtr )
{
    ...
    m_valueHandler       = std::make_unique<ValueHandler>(
            [ this ]( const TimeSeriesProvider *input )
            {
                static_cast<StringArrayBuilder *>(this -> m_columnArrayBuilder.get())
                        -> setValue( input -> lastValueTyped<Sp2Enum>().name() );
            } );
}
...
ListColumnParquetOutputHandler::ListColumnParquetOutputHandler( Engine *engine, ParquetWriter &parquetWriter, Sp2TypePtr &elemType,
                                                                const std::string &columnName,
                                                                DialectGenericListWriterInterface::Ptr &listWriterInterface )
        : ParquetOutputHandler( parquetWriter, Sp2Type::DIALECT_GENERIC() ),
...
inline std::shared_ptr<::arrow::ArrayBuilder> makeArrayAndAttachToWriter( DialectGenericListWriterInterface::Ptr &listWriterInterface )
{
    ...
    SP2_TRUE_OR_THROW( typedWriter != nullptr, TypeError,
...
std::shared_ptr<::arrow::ArrayBuilder> ListColumnParquetOutputHandler::createValueBuilder( const Sp2TypePtr &elemType,
                                                                                           DialectGenericListWriterInterface::Ptr &listWriterInterface )
{
...
StructParquetOutputHandler::StructParquetOutputHandler( Engine *engine, ParquetWriter &parquetWriter, Sp2TypePtr &type,
                                                        DictionaryPtr fieldMap )
        : ParquetOutputHandler( parquetWriter, type )
{
    auto structMetaPtr = std::static_pointer_cast<const Sp2StructType>( type ) -> meta().get();
        ...
        const std::string *path )
{
    ...
    switch( fieldPtr -> type() -> type() )
    {
        ...
        case Sp2Type::TypeTraits::STRUCT:
            return createStructColumnBuilder( fieldPtr, columnName, path );
        default:
            SP2_THROW( TypeError, "Writing of column " << columnName << " of type " << fieldPtr -> type() -> type().asString()
        ...
        const std::string *path )
{
    ...
    auto columnBuilderRawPtr{ columnBuilderPtr.get() };
    auto enumMetaPtr = std::static_pointer_cast<const Sp2EnumType>( field -> type() ) -> meta();

    ValueHandler res = [ field, columnBuilderRawPtr ]( const Struct *s )
    {
        if( field -> isSet( s ) )
        {
            columnBuilderRawPtr -> setValue( field -> value<Sp2Enum>( s ).name() );
        }
    };
    if( path == nullptr )
    {
        ...
        const std::string *path )
{
    ...
    auto structFieldMetaPtr = std::static_pointer_cast<const Sp2StructType>( structField -> type() ) -> meta().get();
