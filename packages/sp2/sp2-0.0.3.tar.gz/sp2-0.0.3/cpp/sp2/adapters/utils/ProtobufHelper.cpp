#include <sp2/adapters/utils/ProtobufHelper.h>
...
public:
    ...
    {
        SP2_THROW( RuntimeException, "Failed to load proto schema " << filename << ":" << line << ":" << column << ": " << message );
    }
...
proto::FieldDescriptor::CppType ProtobufHelper::sp2ToProtoCppType( const Sp2Type & type )
{
    switch( type.type() )
    {
        //NOTE if new types are added make sure parsing code switch statement below is updated
        case Sp2Type::Type::BOOL:   return proto::FieldDescriptor::CPPTYPE_BOOL;
        case Sp2Type::Type::INT32:  return proto::FieldDescriptor::CPPTYPE_INT32;
        case Sp2Type::Type::UINT32: return proto::FieldDescriptor::CPPTYPE_UINT32;
        case Sp2Type::Type::INT64:  return proto::FieldDescriptor::CPPTYPE_INT64;
        case Sp2Type::Type::UINT64: return proto::FieldDescriptor::CPPTYPE_UINT64;
        case Sp2Type::Type::DOUBLE: return proto::FieldDescriptor::CPPTYPE_DOUBLE;
        case Sp2Type::Type::STRING: return proto::FieldDescriptor::CPPTYPE_STRING;
        case Sp2Type::Type::STRUCT: return proto::FieldDescriptor::CPPTYPE_MESSAGE;
        case Sp2Type::Type::ARRAY:
        {
            auto elemType = static_cast<const Sp2ArrayType &>( type ).elemType();
            return sp2ToProtoCppType( *elemType );
        }

        default:
            SP2_THROW( TypeError, "Struct field type " << type.type() << " not currently mappable to proto field" );
    }
}

bool ProtobufHelper::isCoercible( proto::FieldDescriptor::CppType src, Sp2Type::Type dest )
{
    switch( src )
    {
        case proto::FieldDescriptor::CPPTYPE_INT32:  return ( dest == Sp2Type::Type::UINT32 || dest == Sp2Type::Type::INT64 || dest == Sp2Type::Type::UINT64 );
        case proto::FieldDescriptor::CPPTYPE_UINT32: return ( dest == Sp2Type::Type::INT32  || dest == Sp2Type::Type::INT64 || dest == Sp2Type::Type::UINT64 );
        case proto::FieldDescriptor::CPPTYPE_INT64:  return dest == Sp2Type::Type::UINT64;
        case proto::FieldDescriptor::CPPTYPE_UINT64: return dest == Sp2Type::Type::INT64;
        case proto::FieldDescriptor::CPPTYPE_FLOAT:  return dest == Sp2Type::Type::DOUBLE;
        case proto::FieldDescriptor::CPPTYPE_ENUM:   return dest == Sp2Type::Type::STRING;
        default:
            return false;
    }
}
    ...
    switch( descr -> cpp_type() )
    {
        ...
        case proto::FieldDescriptor::CPPTYPE_UINT32:
        {
            ...
            if( v > uint32_t(std::numeric_limits<int32_t>::max()) )
                SP2_THROW( RangeError, "coercion out of range for UINT32 value into INT32 value for proto msg type " << message.GetTypeName() << " field " << descr -> name() );
            return ( int32_t ) v;
        }
        default:
            SP2_THROW( TypeError, "Attempting to coerce proto field type " << descr -> cpp_type_name() << " to int32_t" );
    }
}
...
uint32_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message, const proto::FieldDescriptor * descr, int index )
{
    switch( descr -> cpp_type() )
    {
        ...
        case proto::FieldDescriptor::CPPTYPE_INT32:
        {
            ...
            if( v < 0 )
                SP2_THROW( RangeError, "coercion out of range for INT32 value into uint32_t value for proto msg type " << message.GetTypeName() << " field " << descr -> name() );
            return ( uint32_t ) v;
        }
        default:
            SP2_THROW( TypeError, "Attempting to coerce proto field type " << descr -> cpp_type_name() << " to uint32_t" );
    }
}
    ...
    switch( descr -> cpp_type() )
    {
        ...
        case proto::FieldDescriptor::CPPTYPE_UINT64:
        {
            ...
            if( v > uint64_t(std::numeric_limits<int64_t>::max()) )
                SP2_THROW( RangeError, "coercion out of range for UINT64 value into int64_t value for proto msg type " << message.GetTypeName() << " field " << descr -> name() );
            return ( int64_t ) v;
        }
        case proto::FieldDescriptor::CPPTYPE_UINT32: return index == -1 ? access -> GetUInt32( message, descr ) : access -> GetRepeatedUInt32( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_INT32:  return index == -1 ? access -> GetInt32( message, descr ) : access -> GetRepeatedInt32( message, descr, index );
        ...
        default:
            SP2_THROW( TypeError, "Attempting to coerce proto field type " << descr -> cpp_type_name() << " to int64_t" );
    }
}
...
uint64_t ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message, const proto::FieldDescriptor * descr, int index )
{
    switch( descr -> cpp_type() )
    {
        ...
        case proto::FieldDescriptor::CPPTYPE_INT64:
        {
            ...
            if( v < 0 )
                SP2_THROW( RangeError, "coercion out of range for INT64 value into uint64_t value for proto msg type " << message.GetTypeName() << " field " << descr -> name() );
            return ( uint64_t ) v;
        }
        case proto::FieldDescriptor::CPPTYPE_UINT32:  return index == -1 ? access -> GetUInt32( message, descr ) : access -> GetRepeatedUInt32( message, descr, index );
        case proto::FieldDescriptor::CPPTYPE_INT32:
        {
            int32_t v = index == -1 ? access -> GetInt32( message, descr ) : access -> GetRepeatedInt32( message, descr, index );
            if( v < 0 )
                SP2_THROW( RangeError, "coercion out of range for INT32 value into uint64_t value for proto msg type " << message.GetTypeName() << " field " << descr -> name() );
            return ( uint64_t ) v;
        }
...
double ProtobufHelper::coercedValue( const proto::Reflection * access, const proto::Message & message, const proto::FieldDescriptor * descr, int index )
{
    switch( descr -> cpp_type() )
    {
        ...
        default:
            SP2_THROW( TypeError, "Attempting to coerce proto field type " << descr -> cpp_type_name() << " to double" );
    }
}

void ProtobufStructMapper::init( const Sp2TypePtr & type, const Dictionary & fieldMap, const google::protobuf::Descriptor * protoDesc )
{
    m_fields = buildFields( type, fieldMap, protoDesc );
}

ProtobufStructMapper::Fields ProtobufStructMapper::buildFields( const Sp2TypePtr & type, const Dictionary & fieldMap, const google::protobuf::Descriptor * protoDesc )
{
    auto structType = std::static_pointer_cast<const Sp2StructType>( type );
    ...
    for( auto it = fieldMap.begin(); it != fieldMap.end(); ++it )
    {
        ...
        else
        {
            ...
            if( subdict -> size() != 1 )
                SP2_THROW( ValueError, "Expected sub-fieldmap for incoming protofield " << protoField << " to have a single key : map entry" );
        ...
        if( !protoFieldDesc )
            SP2_THROW( ValueError, "field '" << protoField << "' is not a valid field on proto descriptor for " << protoDesc -> name() );

        auto protoType = ProtobufHelper::sp2ToProtoCppType( *sField -> type() );
        if( ( protoType != protoFieldDesc -> cpp_type() && !ProtobufHelper::isCoercible( protoFieldDesc -> cpp_type(), sField -> type() -> type() ) ) ||
            ( sField -> type() -> type() == Sp2Type::Type::ARRAY && !protoFieldDesc -> is_repeated() ) )
        {
            SP2_THROW( TypeError, "Type mismatch on struct field " << structField << " with type " << sField -> type() -> type() << " and protofield " <<
        ...
        {
            if( !subFieldMap )
                SP2_THROW( ValueError, "invalid field_map entry for sub-struct field " << sField -> fieldname() << " on struct type " << structMeta -> name() );
...
using SupportedArraySp2TypeSwitch = PartialSwitchSp2Type<sp2::Sp2Type::Type::INT32,
...
void ProtobufStructMapper::mapProtoToStruct( StructPtr & struct_, const proto::Message & protoMsg, const Fields & fields )
{
    ...
    for( auto & entry : fields )
    {
        ...
        {
            case Sp2Type::Type::BOOL:   sField -> setValue<bool>( struct_.get(),        protoAccess -> GetBool( protoMsg, pField ) );  break;
            case Sp2Type::Type::INT32:  sField -> setValue<int32_t>( struct_.get(),     ProtobufHelper::coercedValue<int32_t>( protoAccess,  protoMsg, pField ) ); break;
            case Sp2Type::Type::UINT32: sField -> setValue<uint32_t>( struct_.get(),    ProtobufHelper::coercedValue<uint32_t>( protoAccess, protoMsg, pField ) ); break;
            case Sp2Type::Type::INT64:  sField -> setValue<int64_t>( struct_.get(),     ProtobufHelper::coercedValue<int64_t>( protoAccess,  protoMsg, pField ) ); break;
            case Sp2Type::Type::UINT64: sField -> setValue<uint64_t>( struct_.get(),    ProtobufHelper::coercedValue<uint64_t>( protoAccess, protoMsg, pField ) ); break;
            case Sp2Type::Type::DOUBLE: sField -> setValue<double>( struct_.get(),      ProtobufHelper::coercedValue<double>( protoAccess,   protoMsg, pField ) ); break;
            ...
            case Sp2Type::Type::ARRAY:
            {
                ...
                auto elemType = static_cast<const Sp2ArrayType &>( *sField -> type() ).elemType();
#ifdef __clang__
                if (elemType.get()->type() == Sp2Type::Type::BOOL) {
...
#endif
                SupportedArraySp2TypeSwitch::invoke( elemType.get(), [&count,&protoMsg,&pField,&sField,&struct_]( auto tag )
                                                    {
            ...
            default:
                SP2_THROW( TypeError, "Struct field type " << sField -> type() -> type() << " not currently mappable to proto field" );
        }
    }
}
