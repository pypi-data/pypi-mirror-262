...
INIT_SP2_ENUM( sp2::adapters::kafka::KafkaStatusMessageType,
               "OK",
               "MSG_DELIVERY_FAILED",
               "MSG_SEND_ERROR",
               "MSG_RECV_ERROR"
...
KafkaAdapterManager::KafkaAdapterManager( sp2::Engine * engine, const Dictionary & properties ) : AdapterManager( engine ),
                                                                                                  m_consumerIdx( 0 ),
                                                                                                  m_producerPollThreadActive( false )
{
    ...
    if( m_consumerConf -> set( "event_cb", m_eventCb.get(), errstr ) != RdKafka::Conf::CONF_OK )
        SP2_THROW( RuntimeException, "Failed to set consumer error cb: " << errstr );
    ...
    if( m_producerConf -> set( "dr_cb", m_producerCb.get(), errstr ) != RdKafka::Conf::CONF_OK )
        SP2_THROW( RuntimeException, "Failed to set producer callback: " << errstr );
    if( m_producerConf -> set( "event_cb", m_eventCb.get(), errstr ) != RdKafka::Conf::CONF_OK )
        SP2_THROW( RuntimeException, "Failed to set producer error cb: " << errstr );
}
...
void KafkaAdapterManager::setConfProperties( RdKafka::Conf * conf, const Dictionary & properties )
{
    ...
    for( auto it = properties.begin(); it != properties.end(); ++it )
    {
        ...
        if( conf -> set( key, value, errstr ) != RdKafka::Conf::CONF_OK )
            SP2_THROW( RuntimeException, "Failed to set property " << key << ": " << errstr );
    }
}
...
void KafkaAdapterManager::forceShutdown( const std::string & err )
{
    ...
    try
    {
        SP2_THROW( RuntimeException, "Kafka fatal error. " +  err );
    }
...
void KafkaAdapterManager::start( DateTime starttime, DateTime endtime )
{
    ...
    if( !m_staticPublishers.empty() || !m_dynamicPublishers.empty() )
    {
        ...
        if ( !m_producer )
        {
            SP2_THROW( RuntimeException, "Failed to create producer: " << errstr );
        }
    }
    ...
    try
    {
        while( true )
        {
            ...
            if( rc && rc != RdKafka::ERR__TIMED_OUT )
                SP2_THROW( RuntimeException, "KafkaProducer failed to flush pending msgs on shutdown: " << RdKafka::err2str( rc ) );
        }
    }
    catch( ... )
    {
...
PushInputAdapter * KafkaAdapterManager::getInputAdapter( Sp2TypePtr & type, PushMode pushMode, const Dictionary & properties )
{
...
OutputAdapter * KafkaAdapterManager::getOutputAdapter( Sp2TypePtr & type, const Dictionary & properties )
{
    std::string topic = properties.get<std::string>( "topic" );
    try
    {
