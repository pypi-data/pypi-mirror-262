        ...
        if( err == RdKafka::ERR__ASSIGN_PARTITIONS )
        {
            if( !m_doneSeeking )
            {
                ...
                if( !m_startTime.isNone() )
                {
                    ...
                    if( rc )
                        SP2_THROW( RuntimeException, "Failed to get kafka offsets for starttime " << m_startTime << ": " << RdKafka::err2str( rc ) );
                }
                else
                {
                    for( auto * partition : partitions )
                                                                                           ...
                                                                                           m_running( false )
{
    ...
    if( conf -> set( "rebalance_cb", m_rebalanceCb.get(), errstr ) != RdKafka::Conf::CONF_OK )
        SP2_THROW( RuntimeException, "Failed to set rebalance callback: " << errstr );

    m_consumer.reset( RdKafka::KafkaConsumer::create( conf, errstr ) );

    if( !m_consumer )
        SP2_THROW( RuntimeException, "Failed to create consumer: " << errstr );
}
...
void KafkaConsumer::start( DateTime starttime )
{
    ...
    if( m_rebalanceCb )
    {
        ...
        else if( std::holds_alternative<TimeDelta>( startOffsetProperty ) )
        {
            ...
            SP2_THROW( TypeError, "Expected enum, datetime or timedelta for startOffset" );
    }
    else
    ...
    if( err )
        SP2_THROW( RuntimeException, "Failed to subscribe to " << m_topics.size() << " topics: " << RdKafka::err2str( err ) );
