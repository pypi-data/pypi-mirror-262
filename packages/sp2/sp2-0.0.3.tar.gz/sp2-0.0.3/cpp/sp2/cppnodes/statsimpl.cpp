...
DECLARE_CPPNODE( _in_sequence_check )
{
    ...
    INVOKE()
    {
        if( sp2.ticked( x ) != sp2.ticked( y ) )
            SP2_THROW( ValueError, "For multivariate statistics, x and y must tick in sequence." );
    };
};
...
public:
    _compute( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}
...
public:
    _bivariate_compute( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}
