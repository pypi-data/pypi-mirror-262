#ifndef _IN_SP2_ENGINE_NODE_H
#define _IN_SP2_ENGINE_NODE_H

#include <sp2/core/Exception.h>
#include <sp2/engine/AlarmInputAdapter.h>
...
public:
    ...
    virtual void createAlarm( Sp2TypePtr & type, size_t id );

    void createOutput( Sp2TypePtr & type, size_t id );
    void createBasketOutput( Sp2TypePtr & type, size_t id, size_t size );
    void createDynamicBasketOutput( Sp2TypePtr & type, size_t id );
...
protected:
    template<typename T>
    void createAlarm( Sp2TypePtr & type, size_t id );
...
inline void Node::createOutput( Sp2TypePtr & type, size_t id )
{
    SP2_ASSERT( !m_outputs[ id ] );
...
inline void Node::createBasketOutput( Sp2TypePtr & type, size_t id, size_t size )
{
    validateOutputIndex( id );
...
inline void Node::createDynamicBasketOutput( Sp2TypePtr & type, size_t id )
{
    validateOutputIndex( id );
    SP2_ASSERT( !m_outputs[ id ] );
...
template<typename T>
inline void Node::createAlarm( Sp2TypePtr & type, size_t id )
{
...
{
    SP2_ASSERT( isInputBasket( basketIdx ) );
    SP2_ASSERT( inputBasket( basketIdx ) -> isDynamicBasket() );
    ...
    if( elemId > InputId::maxElemId() )
        SP2_THROW( RangeError, "Hit dynamic key limit of " << InputId::maxElemId() );
