#ifndef _IN_SP2_ADAPTERS_STATUSADAPTER_H
#define _IN_SP2_ADAPTERS_STATUSADAPTER_H
...
using StatusLevel = sp2::Enum<StatusLevelTraits>;

class StatusAdapter : public PushInputAdapter
{
public:
    StatusAdapter( Engine * engine, Sp2TypePtr & type, PushMode pushMode, PushGroup * group = nullptr ) : PushInputAdapter( engine, type, pushMode, group )
    {
        if( type -> type() != Sp2Type::Type::STRUCT )
            SP2_THROW( ValueError, "Status Adapter can only be created with struct ts type" );

        const Sp2StructType & sType = static_cast<const Sp2StructType&>( *type.get() );
