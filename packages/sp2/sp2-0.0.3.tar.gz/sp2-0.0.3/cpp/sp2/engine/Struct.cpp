...
StructField::StructField( Sp2TypePtr type, const std::string & fieldname,
                                                             ...
                                                             m_isPartialNative( true ), m_isFullyNative( true )
{
    if( m_fields.empty() && !m_base)
        SP2_THROW( TypeError, "Struct types must define at least 1 field" );
    ...
    if( !m_fields.empty() )
        SP2_ASSERT( ( offset % m_fields[0] -> alignment() ) == 0 );
    ...
    for( size_t idx = m_firstPartialField; idx < m_fields.size(); ++idx )
    {
        ...
        if( !rv.second )
            SP2_THROW( ValueError, "sp2 Struct " << name << " attempted to add existing field " << m_fields[ idx ] -> fieldname() );
    }
}
...
std::string StructMeta::layout() const
{
    ...
    for( auto & field : m_fields )
    {
        ...
        {
            case Sp2Type::Type::BOOL:             type = 'b'; break;
            case Sp2Type::Type::INT8:             type = 'c'; break;
            case Sp2Type::Type::UINT8:            type = 'C'; break;
            case Sp2Type::Type::INT16:            type = 'h'; break;
            case Sp2Type::Type::UINT16:           type = 'H'; break;
            case Sp2Type::Type::INT32:            type = 'd'; break;
            case Sp2Type::Type::UINT32:           type = 'D'; break;
            case Sp2Type::Type::INT64:            type = 'l'; break;
            case Sp2Type::Type::UINT64:           type = 'L'; break;
            case Sp2Type::Type::DOUBLE:           type = 'f'; break;
            case Sp2Type::Type::DATETIME:         type = 't'; break;
            case Sp2Type::Type::TIMEDELTA:        type = 'e'; break;
            case Sp2Type::Type::DATE:             type = 'y'; break;
            case Sp2Type::Type::TIME:             type = 'T'; break;
            case Sp2Type::Type::ENUM:             type = 'N'; break;
            case Sp2Type::Type::STRUCT:           type = 'S'; break;
            case Sp2Type::Type::STRING:           type = 's'; break;
            case Sp2Type::Type::DIALECT_GENERIC:  type = 'G'; break;
            case Sp2Type::Type::ARRAY:            type = 'a'; break;
...
void StructMeta::copyFrom( const Struct * src, Struct * dest )
{
    ...
    if( dest -> meta() != src -> meta() &&
        !StructMeta::isDerivedType( src -> meta(), dest -> meta() ) )
            SP2_THROW( TypeError, "Attempting to copy from struct type '" << src -> meta() -> name() << "' to struct type '" << dest -> meta() -> name()
...
void StructMeta::deepcopyFrom( const Struct * src, Struct * dest )
{
    ...
    if( dest -> meta() != src -> meta() &&
        !StructMeta::isDerivedType( src -> meta(), dest -> meta() ) )
            SP2_THROW( TypeError, "Attempting to deepcopy from struct type '" << src -> meta() -> name() << "' to struct type '" << dest -> meta() -> name()
...
void StructMeta::updateFrom( const Struct * src, Struct * dest )
{
    ...
    if( dest -> meta() != src -> meta() &&
        !StructMeta::isDerivedType( src -> meta(), dest -> meta() ) )
            SP2_THROW( TypeError, "Attempting to update from struct type '" << src -> meta() -> name() << "' to struct type '" << dest -> meta() -> name()
...
size_t StructMeta::hash( const Struct * x ) const
{
    ...
    if( isNative() )
        return hash ^ sp2::hash::hash_bytes( x, size() );
