#ifndef _IN_SP2_ENGINE_TIMESERIES_H
#define _IN_SP2_ENGINE_TIMESERIES_H

#include <sp2/core/Enum.h>
#include <sp2/core/Exception.h>
#include <sp2/core/Time.h>
#include <sp2/engine/TickBuffer.h>
...
public:
    ...
    T & valueAtIndex( uint32_t index )
    {
        ...
        if( unlikely( index != 0 ) )
            SP2_THROW( RangeError, "Accessing value past index 0 when no buffering policy is set" );
        return m_lastValue;
    }
...
public:
    ...
    using DuplicatePolicyEnum = sp2::Enum<DuplicatePolicyTraits>;
...
inline int32_t TimeSeries::getValueIndex(DateTime time, DuplicatePolicyEnum duplicatePolicy) const {
    ...
    SP2_ENSURE_TRUE(duplicatePolicy!= DuplicatePolicyEnum::ALL_VALUES);
    ...
    switch(duplicatePolicy){
        ...
        case DuplicatePolicyTraits::LAST_VALUE:
            return range.first;
        default:
            SP2_THROW(InvalidArgument, "Unexpected duplicate policy" << duplicatePolicy);
    }
}
