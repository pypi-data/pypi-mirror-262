#ifndef _IN_SP2_ENGINE_TICKBUFFER_H
#define _IN_SP2_ENGINE_TICKBUFFER_H

/************************************************************************
 ** Tick buffer is the storage for SP2 time series ticks
...
inline void TickBuffer<T>::raiseRangeError( uint32_t index ) const {
    SP2_THROW( RangeError, "Invalid buffer access: index " << index << " out of range for buffer with numTicks: " << numTicks() << " capacity: " << m_capacity );
}
...
T * TickBuffer<T>::flatten( uint32_t startIndex, uint32_t endIndex, uint32_t tailPadding ) const
{
    ...
    if( startIndex < endIndex )
        SP2_THROW( RangeError, "Invalid buffer flatten: endIndex " << endIndex << " greater than startIndex " << startIndex );
    if( startIndex >= m_capacity )
        SP2_THROW( RangeError, "Invalid buffer flatten: startIndex " << startIndex << " greater than capacity " << m_capacity );
