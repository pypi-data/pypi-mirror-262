...
#include <sp2/engine/DynamicNode.h>
...
{
    //no need to keep sp2.snap inputs active
...
void DynamicNode::executeImpl()
{
    ...
    if( shapeTicked )
    {
        ...
        for( auto & event : events )
        {
            ...
            //HOWEVER there is the pitfall that a sp2.attach() input will be accessed on the same cycle as it being shutdown.  It will no longer be a valid ts in this last cycle
            ...
            //In BOTH cases, a proper check of sp2.valid() will safely return false on the removed edge
        }
    }
    ...
    //We defer sp2.stop_engine shutdowns until after the cycle because we cant shutdown / destroy
    //the dynamic engine while the node within the dynamic which is calling sp2.stop_engine is being executed!
