...
TEST( TickBufferTest, test_resize )
{
    ...
    ASSERT_THROW( buffer.valueAtIndex( 3 ), sp2::RangeError ); //exception
    ...
    ASSERT_THROW( buffer.valueAtIndex( 5 ), sp2::RangeError ); //exception
...
TEST( TickBufferTest, test_flatten )
{
    int * values_wrap, * values_nowrap, * values_single;
    {
        ...
        ASSERT_THROW( buffer.flatten( 5, 0 ), sp2::RangeError );
    }
