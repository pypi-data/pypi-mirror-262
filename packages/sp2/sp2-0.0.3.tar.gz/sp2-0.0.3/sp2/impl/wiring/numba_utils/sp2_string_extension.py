...
def _sp2_make_constant_bytes(context, builder, nbytes):
...
def sp2_unicode_to_bytes_cast(context, builder, fromty, toty, val):
    ...
    bstr = _sp2_make_constant_bytes(context, builder, src_length)
    cgutils.memcpy(builder, bstr.data, src1, bstr.nitems)
    return bstr


@numba.extending.intrinsic
def sp2_unicode_to_bytes(typingctx, s):
    # used in _to_bytes method
    assert s == numba.types.unicode_type
    sig = _BYTES_TYPE(s)

    def codegen(context, builder, signature, args):
        return sp2_unicode_to_bytes_cast(context, builder, s, _BYTES_TYPE, args[0])._getvalue()
