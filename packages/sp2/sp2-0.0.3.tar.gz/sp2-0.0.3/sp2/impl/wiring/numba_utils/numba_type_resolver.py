...
from sp2.impl.wiring.numba_utils import datetime_extension
...
class NumbaTypeResolver(object):
    ...
    _PRIMITIVE_TYPE_MAPPING = {
        ...
        datetime.datetime: datetime_extension.sp2_numba_datetime_type,
        datetime.timedelta: datetime_extension.sp2_numba_timedelta_type,
    }

    @classmethod
    def _resolve_container_type(cls, python_type):
        if Sp2TypingUtils.get_origin(python_type) is typing.Dict:
        ...
        elif Sp2TypingUtils.get_origin(python_type) is typing.List:
            (t,) = python_type.__args__
            numba_t = cls.resolve_numba_type(t)
            return numba.types.ListType(numba_t)
        elif Sp2TypingUtils.get_origin(python_type) is typing.Set:
    ...
    def resolve_numba_type(cls, python_type):
        ...
        if res is None:
            if Sp2TypingUtils.is_generic_container(python_type):
    ...
    def transform_scalar(cls, scalar, scalar_type):
        if isinstance(scalar_type, numba.types.DictType):
            ...
            return datetime_extension.sp2_numba_timedelta_type(int(scalar.total_seconds() * 1e9))
        elif isinstance(scalar, datetime.datetime):
            datetime_extension.sp2_numba_datetime_type(int((scalar - cls._EPOCH).total_seconds() * 1e9))
        else:
            return scalar
