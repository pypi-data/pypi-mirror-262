"""
Implementation of numba extensions and few wrappers to work with sp2
"""
...
def sp2_unsafe_cast_int_to_class_ptr(typingctx, src, dst):
    ...
    The int pointer is assumed to be generated by sp2_unsafe_cast_class_ptr_to_int.
    Example usage:
        ...
        class MyClass:
            ...
            def get_ptr(self):
                return sp2_unsafe_cast_class_ptr_to_int(self)
        ...
        def print_from_ptr(ptr):
            print("Ptr is ", ptr)
            my_o: MyClass = sp2_unsafe_cast_int_to_class_ptr(ptr, MyClass)
            my_o.print_hello()
    ...
    def _sp2_unsafe_cast_int_to_class_ptr(int_type, class_type):
    ...
    if isinstance(src, numba.types.Integer) and isinstance(dst, numba.types.ClassType):
        return _sp2_unsafe_cast_int_to_class_ptr(src, dst)
    return TypeError


@numba.extending.intrinsic
def sp2_unsafe_cast_class_ptr_to_int(typingctx, src):
    """Numba extension function that allows casting from class reference to int64 (ptr)
    The returned pointer only valid usage is to store it and then convert back using sp2_unsafe_cast_int_to_class_ptr
    For usage example, see documentation of sp2_unsafe_cast_int_to_class_ptr

    :param typingctx:
    :param src:
    """

    def _sp2_unsafe_cast_class_ptr_to_int(inst_type):
    ...
    if isinstance(src, numba.types.ClassInstanceType):
        return _sp2_unsafe_cast_class_ptr_to_int(src)
    return TypeError
