...
from sp2.impl.wiring.numba_utils.sp2_cpp_numba_interface import C as sp2_c

sp2_numba_datetime_type = numba.types.NPDatetime("ns")
sp2_numba_timedelta_type = numba.types.NPTimedelta("ns")
...
def sp2_datetime_from_int(typingctx, src):
    ...
    if isinstance(src, numba.types.IntegerLiteral) or src is numba.types.int64:
        return _datetime_cast_implementation(sp2_numba_datetime_type, src)
    return TypeError


@numba.extending.intrinsic
def sp2_datetime_to_int(typingctx, src):
    ...
    if src is sp2_numba_datetime_type:
        return _datetime_cast_implementation(numba.types.int64, src)
    return TypeError


@numba.extending.intrinsic
def sp2_timedelta_from_int(typingctx, src):
    ...
    """
    if isinstance(src, numba.types.IntegerLiteral) or src is numba.types.int64:
        return _datetime_cast_implementation(sp2_numba_timedelta_type, src)
    return TypeError


@numba.extending.intrinsic
def sp2_timedelta_to_int(typingctx, src):
    """Numba extension function that implementes reinterpret_cast<int64_t>(timedelta64)
    :param typingctx:
    :param src:
    :return:
    """
    if src is sp2_numba_timedelta_type:
        return _datetime_cast_implementation(numba.types.int64, src)
    return TypeError


__sp2_create_datetime_nanoseconds__ = sp2_c.__sp2_create_datetime_nanoseconds__
...
def create_datetime(year, month, day, hour=0, minute=0, second=0, microsecond=0, nanosecond=0):
    ...
    def create_datetime_impl(year, month, day, hour=0, minute=0, second=0, microsecond=0, nanosecond=0):
        return sp2_datetime_from_int(
            __sp2_create_datetime_nanoseconds__(year, month, day, hour, minute, second, microsecond * 1000 + nanosecond)
        )
...
def create_timedelta(days=0, seconds=0, microseconds=0, milliseconds=0, minutes=0, hours=0, weeks=0, nanoseconds=0):
    ...
    def create_datetime_impl(
        ...
        return sp2_timedelta_from_int(res)
