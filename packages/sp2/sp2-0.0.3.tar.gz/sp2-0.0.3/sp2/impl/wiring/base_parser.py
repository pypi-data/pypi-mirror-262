...
from sp2.impl.types.common_definitions import (
    ArgKind,
    BasketKind,
    InputDef,
    OutputBasket,
...
from sp2.impl.types.tstype import TsType
from sp2.impl.types.type_annotation_normalizer_transformer import TypeAnnotationNormalizerTransformer
    ...
    def wrapper(*args, **kwargs):
        ...
        if isinstance(node, ast.With):
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                try:
                    raise Sp2ParseError(f"Error parsing sp2 builtin: {ast.unparse(node)}")
                except AttributeError:
                    raise Sp2ParseError("Error parsing sp2 builtin function")
        else:
            # should never get here either
            raise Sp2ParseError("Unable to parse sp2 builtin function")

        if name in LEGACY_METHODS:
            if name == "__return__":
                warn(
                    "Calling __return__ is deprecated. Instead, call return sp2.output(v1=va1, v2=val2 ...)",
                    DeprecationWarning,
                )
            else:
                warn(
                    "Calling __{0}__ is deprecated: instead, use a 'with sp2.{0}()' context (see the sp2 docs)".format(
                        name[2:-2]
                    ),
                    DeprecationWarning,
                )
...
class BaseParser(ast.NodeTransformer, metaclass=ABCMeta):
    ...
    def __init__(self, name, raw_func, func_frame, debug_print=False, add_cache_control_output=False):
        ...
        self._func_globals_modified["sp2"] = sp2
    ...
    def _eval_expr(self, exp):
        return eval(
            compile(ast.Expression(body=exp), filename="<sp2>", mode="eval"),
    ...
    def _resolve_input_type_kind(cls, typ):
        ...
        elif Sp2TypingUtils.is_generic_container(typ):
            origin = Sp2TypingUtils.get_origin(typ)
    ...
    def _validate_output(cls, node):
        if not BaseParser._is_sp2_output_call(node.value) or (len(node.value.args) + len(node.value.keywords)) == 0:
            raise Sp2ParseError(
    ...
    def _parse_single_output_definition(self, name, arg_type_node, ts_idx, typ=None):
        ...
        if not kind:
            raise Sp2ParseError(f"outputs must be ts[] or basket types, got {typ}", arg_type_node.lineno)
        ...
        if (
            ...
            if tstype.isTsBasket(value):
                if arg_type_node.keywords or len(arg_type_node.args) != 1:
                    raise Sp2ParseError(f"Invalid use of '{arg_type_node.func.attr}'", arg_type_node.lineno)
                if tstype.isTsDynamicBasket(value):
                    raise Sp2ParseError("dynamic baskets cannot declare shape", arg_type_node.lineno)
        ...
        if typ is None:
            ...
            if tstype.isTsBasket(typ):
                if tstype.isTsDynamicBasket(typ):
                    ...
                    raise Sp2ParseError("output baskets must define shape using with_shape or with_shape_of")
    ...
    def _parse_output_def(self, node):
        if WARN_PYTHONIC:
            warn(
                "Declaring __outputs__ is deprecated; instead, use output annotation syntax (func -> Outputs(...)). Consult the sp2 docs.",
                DeprecationWarning,
            )

    def parse_outputs(self, node: ast.Call):
        if node.args and node.keywords:
            raise Sp2ParseError("__outputs__ must all be named or be single output, cant be both", node.lineno)

        if len(node.args) > 1:
            raise Sp2ParseError("__outputs__ single unnamed arg only", node.lineno)
    ...
    def _extract_outputs_from_body(self, body):
        ...
        if outputs_call is not None:
            body.pop(i)
            if not body:
                raise Sp2ParseError('Node body can not be empty add "pass" for an empty node', lineno)
    ...
    def _extract_single_output_definition(self, name, typ, idx, basket_container=None, enforce_shape=False):
        ...
        if not kind:
            raise Sp2ParseError(f"outputs must be ts[] or basket types, got {typ}")
    ...
    def _extract_outputs_from_return_annotation(self, returns, enforce_shape=False):
        ...
        if not (isinstance(output_dictionary_type, type) and issubclass(output_dictionary_type, Outputs)):
            ...
            try:
                ...
                raise Sp2ParseError(f"Output type must subclass `sp2.Outputs`, got {output_dictionary_type}")
    ...
    def parse_func_signature(self, funcdef):
        ...
        if args.vararg or args.kwarg:
            raise Sp2ParseError("*args and **kwargs arguments are not supported in sp2 nodes")

        if (sys.version_info.major > 3 or sys.version_info.minor >= 8) and args.posonlyargs:
            raise Sp2ParseError("position only arguments are not supported in sp2 nodes")
        ...
        for arg_idx, arg in enumerate(args.args):
            if arg.annotation is None:
                ...
                if (
                    ...
                    raise Sp2ParseError("sp2.node and sp2.graph args must be type annotated")
            else:
        ...
        if annotation_outputs and body_outputs:
            raise Sp2ParseError(
                "sp2.node and sp2.graph outputs must be via return annotation or __outputs__ call, not both"
            )
    ...
    @classmethod
    def _is_sp2_special_func_call(cls, call: ast.Call, func_name_or_list):
        name_pred = (
        ...
        #   sp2.__state__()  /  __state__()
        #        or
        #  with __state__():  with sp2.__state__():
        if isinstance(call, ast.Call) and (
            (isinstance(call.func, ast.Name) and name_pred(call.func.id))
            or (
                isinstance(call.func, ast.Attribute)
                and isinstance(call.func.value, ast.Name)
                and call.func.value.id == "sp2"
        ...
        # raise exception if trying to use __state__ or ...
            ...
            raise Sp2ParseError(f"{call.id} must be called, cannot use as a bare name")
        # ... sp2.__state__  without invocation
        elif isinstance(call, ast.Attribute) and isinstance(call.value, ast.Name) and name_pred(call.attr):
            raise Sp2ParseError(f"sp2.{call.attr} must be called, cannot use as a bare name")
        return False
    ...
    def _is_special_with_block(cls, node, func_name_or_list):
        if isinstance(node, ast.With):
            if len(node.items) == 1:
                ...
                if cls._is_sp2_special_func_call(expr, func_name_or_list):
                    return True
        return False

    def _is_special_with_or_call(cls, node, func_name_or_list):
        if cls._is_special_with_block(node, func_name_or_list) or (
            isinstance(node, ast.Expr) and cls._is_sp2_special_func_call(node.value, func_name_or_list)
        ):
            return True
        return False
    ...
    def _consume_special_block(self, body, index, name, handler_function, allow_with_block, allow_flat_call):
        ...
        if self._is_special_with_block(node, name):
            if not allow_with_block:
                raise Sp2ParseError(f"{name} can not be used in a with statement")
            handler_function(node)
            return index + 1, name
        if isinstance(node, ast.Expr) and self._is_sp2_special_func_call(node.value, name):
            if not allow_flat_call:
                raise Sp2ParseError(f"{name} must be used in a with statement")
