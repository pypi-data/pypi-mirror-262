...
from sp2.impl import builtin_functions
from sp2.impl.__sp2impl import _sp2impl
from sp2.impl.types import tstype
from sp2.impl.types.common_definitions import ArgKind, InputDef, OutputBasketContainer
from sp2.impl.warnings import WARN_PYTHONIC
from sp2.impl.wiring import Signature
from sp2.impl.wiring.ast_utils import ASTUtils
from sp2.impl.wiring.base_parser import BaseParser, Sp2ParseError, _pythonic_depr_warning
from sp2.impl.wiring.special_output_names import SP2_CACHE_ENABLED_OUTPUT
    ...
    def __call__(self, proxy, node):
        for arg in node.args:
            if isinstance(arg, ast.Starred):
                raise Sp2ParseError(f"Passing arguments with * is unsupported for {self._func_ast.name}", node.lineno)
...
class NodeParser(BaseParser):
    _SP2_NOW_FUNC = "_sp2_now"
    _SP2_ENGINE_START_TIME_FUNC = "_engine_start_time"
    _SP2_ENGINE_END_TIME_FUNC = "_engine_end_time"
    _SP2_ENGINE_STATS_FUNC = "_sp2_engine_stats"

    _SP2_STOP_ENGINE_FUNC = "_sp2_stop_engine"
    _LOCAL_METHODS = {
        _SP2_NOW_FUNC: _sp2impl._sp2_now,
        _SP2_ENGINE_START_TIME_FUNC: _sp2impl._sp2_engine_start_time,
        _SP2_ENGINE_END_TIME_FUNC: _sp2impl._sp2_engine_end_time,
        _SP2_STOP_ENGINE_FUNC: _sp2impl._sp2_stop_engine,
        _SP2_ENGINE_STATS_FUNC: _sp2impl._sp2_engine_stats,
    }
    ...
    def __init__(self, name, raw_func, func_frame, debug_print=False, add_cache_control_output=False):
        ...
        self._startblock = []
        self._stopblock = []
        self._func_globals_modified.update(builtin_functions.SP2_BUILTIN_CONTEXT_DICT)
    ...
    def _parse_alarms(self, node):
        ...
        if call.args:
            raise Sp2ParseError("__alarms__ does not accept positional arguments", call.lineno)
        ...
        for kwd in call.keywords:
            ...
            if not tstype.isTsType(typ):
                raise Sp2ParseError("__alarms__ alarms must be ts types", node.lineno)
        ...
        if body:
            ...
            for node in body:
                ...
                if not (isinstance(node, ast.AnnAssign) or isinstance(node, ast.Assign)):
                    raise Sp2ParseError("Only alarm assignments are allowed in sp2.alarms block")

                name = node.target.id if isinstance(node, ast.AnnAssign) else None
                if isinstance(node, ast.Assign):
                    if len(node.targets) != 1 or not isinstance(node.targets[0], ast.Name):
                        raise Sp2ParseError("Exactly one alarm can be assigned per line", node.lineno)
                    name = node.targets[0].id

                if not (isinstance(node.value, ast.Call) and BaseParser._is_sp2_special_func_call(node.value, "alarm")):
                    raise Sp2ParseError("Alarms must be initialized with sp2.alarm in __alarms__ block", node.lineno)
                call_node = node.value

                # handle the initial scheduling via `sp2.alarm`
                if len(call_node.keywords):
                    raise TypeError("function `sp2.alarm` does not take keyword arguments")
                if len(call_node.args) != 1:
                    raise TypeError(
                        f"function `sp2.alarm` requires a single type argument: {len(call_node.args)} arguments given"
                    )
    ...
    def _parse_state(self, node):
        ...
        if call.args:
            raise Sp2ParseError("__state__ does not accept positional arguments")
    ...
    def _ts_inproxy_expr(self, id, ctx=ast.Load()):
        if isinstance(id, ast.Name) or isinstance(id, int):
            ...
            if not input_def:
                raise Sp2ParseError(f"unrecognized input '{id.id}'", self._cur_node.lineno)
            if input_def.kind == ArgKind.SCALAR:
                raise Sp2ParseError(f"expected '{id.id}' to be a timeseries input", self._cur_node.lineno)
            ...
            if not input_def:
                raise Sp2ParseError(f"unrecognized input '{id.value.id}'", self._cur_node.lineno)
    ...
    def _ts_outproxy_expr(self, name):
        if not isinstance(name, int):
            ...
            if not output_def:
                raise Sp2ParseError(f"unrecognized output '{name}'", self._cur_node.lineno)
    ...
    def visit_Call(self, node):
        is_special_func_call = self._is_sp2_special_func_call(node, self._SPECIAL_BLOCKS)
        if (
            isinstance(node.func, ast.Attribute)
            and isinstance(node.func.value, ast.Name)
            and node.func.value.id == "sp2"
        ):
            ...
            convertor = self.METHOD_MAP.get("sp2." + node.func.attr, None)
            if convertor is None:
                raise Sp2ParseError("invalid sp2 call sp2.%s" % node.func.attr, node.lineno)
    ...
    def visit_Return(self, node):
        ...
        if self._inner_loop_count > 0:
            raise Sp2ParseError("returning from within a for or while loop is not supported", node.lineno)

        if len(self._outputs) == 0 and node.value is not None:
            raise Sp2ParseError("returning from node without any outputs defined", node.lineno)
        ...
        if len(self._outputs) == 1:
            ...
            # Allow single named outputs in sp2.output
            if BaseParser._is_sp2_output_call(node.value):
        ...
        # Multiple return values: parse proper output return syntax: return sp2.output(out1=val1, out2=val2, ...)
    ...
    def _parse_output_or_return(self, node, is_return):
        func_name = "return" if is_return else "sp2.output"
        ...
        if len(node.args) == 1:
            if len(self._signature._outputs) > 1 and self._signature.output(0).name is not None:
                ...
                ):  # if it's a basket, the arg is a dict: sp2.output({k1:v1, k2:v2})
        ...
        if len(node.args) == 2:
            if isinstance(
            ...
            ):  # if node.args[0] is an ast.Subscript then node.args[0].value.id must name an output basket: sp2.output(x[k], v)
                if is_return:
                    raise Sp2ParseError(
                ...
                if not isinstance(self._signature.output(output_name).typ, OutputBasketContainer):
                    raise Sp2ParseError("sp2.output(x[k],v) syntax can only be used on basket outputs", node.lineno)
                nodes.append(
                    ast.BinOp(
                        left=ast.Subscript(
                ...
                if not isinstance(node.args[0], ast.Name):
                    raise Sp2ParseError(f"Invalid use of {func_name} please consult documentation", node.lineno)
        ...
        for arg in node.keywords:
            if self._signature.output(arg.arg, True) is None:
                raise Sp2ParseError(f"unrecognized output '{arg.arg}'", node.lineno)
        ...
        if len(nodes) == 0:
            if not is_return:
                raise Sp2ParseError("Empty output is not allowed", node.lineno)
    ...
    def _parse_remove_dynamic_key(self, node):
        if len(self._signature._outputs) == 0:
            raise Sp2ParseError("invalid call to sp2.remove_dynamic_key on node with no outputs")
        ...
        if len(node.args) != expected_args:
            raise Sp2ParseError(
                f'sp2.remove_dynamic_key expects {expected_args} arguments for {"" if is_named_outputs else "un"}named outputs, got {len(node.args)}'
            )
        ...
        if len(node.args) == 2:
            ...
            output_def = self._signature.output(name, allow_missing=True)
            if not output_def:
                raise Sp2ParseError("unrecognized output '%s'" % name, node.lineno)
        ...
        # Convert sp2.remove_dynamic_key call to outputting dict of { key : sp2.impl.REMOVE_DYNAMIC_KEY }
        value = ast.Dict(
            keys=[key],
            values=[
                ast.Attribute(
                    value=ast.Attribute(value=ast.Name(id="sp2", ctx=ast.Load()), attr="impl", ctx=ast.Load()),
    ...
    def _parse_now(self, node):
        if len(node.args) or len(node.keywords):
            raise Sp2ParseError("sp2.now takes no arguments", node.lineno)
        return ast.Call(
            func=ast.Name(id=self._SP2_NOW_FUNC, ctx=ast.Load()), args=[self._node_proxy_expr()], keywords=[]
        )

    def _parse_stop_engine(self, node):
        args = [self._node_proxy_expr()] + node.args
        return ast.Call(func=ast.Name(id=self._SP2_STOP_ENGINE_FUNC, ctx=ast.Load()), args=args, keywords=node.keywords)

    def _parse_num_ticks(self, node):
        if len(node.args) != 1:
            raise Sp2ParseError("sp2.num_ticks expects single ts argument", node.lineno)
    ...
    def _parse_schedule_alarm_func(self, node, funcname, name_node=None):
        ...
        if not isinstance(name_node, ast.Name):
            raise Sp2ParseError(f"sp2.{funcname} expects alarm name as first argument", node.lineno)

        name = name_node.id

        input_def = self._signature.input(name, allow_missing=True)
        if not input_def:
            raise Sp2ParseError(f"unrecognized alarm '{name}'", node.lineno)

        if input_def.kind != ArgKind.ALARM:
            raise Sp2ParseError(f'cannot {funcname.replace( "_", " " )} on non-alarm input \'{name}\'', node.lineno)
    ...
    def _parse_engine_start_time(self, node):
        if len(node.args) or len(node.keywords):
            raise Sp2ParseError("sp2.engine_start_time takes no arguments", node.lineno)
        return ast.Call(
            func=ast.Name(id=self._SP2_ENGINE_START_TIME_FUNC, ctx=ast.Load()),
    ...
    def _parse_engine_end_time(self, node):
        if len(node.args) or len(node.keywords):
            raise Sp2ParseError("sp2.engine_end_time takes no arguments", node.lineno)
        return ast.Call(
            func=ast.Name(id=self._SP2_ENGINE_END_TIME_FUNC, ctx=ast.Load()),
            args=[self._node_proxy_expr()],
            keywords=[],
        )

    def _parse_sp2_enable_cache(self, node):
        if len(node.args) != 1 or node.keywords:
            raise Sp2ParseError("Invalid call to sp2.enable_cache", node.lineno)

        output = self._ts_outproxy_expr(SP2_CACHE_ENABLED_OUTPUT)
        res = ast.BinOp(left=output, op=ast.Add(), right=node.args[0])
        return res

    def _parse_sp2_engine_stats(self, node):
        if len(node.args) or len(node.keywords):
            raise Sp2ParseError("sp2.engine_stats takes no arguments", node.lineno)

        return ast.Call(
            func=ast.Name(id=self._SP2_ENGINE_STATS_FUNC, ctx=ast.Load()), args=[self._node_proxy_expr()], keywords=[]
        )
    ...
    def _validate_return_statements(self):
        ...
        if len(self._outputs) and not len(self._returned_outputs):
            raise Sp2ParseError("node has __outputs__ defined but no return or sp2.output statements")

        for output in self._outputs:
            if output.name not in self._returned_outputs:
                raise Sp2ParseError(f"output '{output.name}' is never returned")
    ...
    def _make_single_proxy_arg_func_resolver(cls, func):
        ...
        def f(node_parser, node):
            if not len(node.args):
                raise Sp2ParseError(f"{func.__name__} expects a timeseries as first positional argument", node.lineno)
    ...
    def _init_internal_maps(cls):
        cls.METHOD_MAP = {
            "sp2.now": cls._parse_now,
            "sp2.stop_engine": cls._parse_stop_engine,
            "sp2.ticked": cls._parse_ticked,
            "sp2.valid": cls._parse_valid,
            "sp2.make_passive": cls._make_single_proxy_arg_func_resolver(builtin_functions.make_passive),
            "sp2.make_active": cls._make_single_proxy_arg_func_resolver(builtin_functions.make_active),
            "sp2.remove_dynamic_key": cls._parse_remove_dynamic_key,
            # omit this as its handled in a special case
            # 'sp2.alarm': cls._parse_alarm,
            "sp2.schedule_alarm": cls._parse_schedule_alarm,
            "sp2.reschedule_alarm": cls._parse_reschedule_alarm,
            "sp2.cancel_alarm": cls._parse_cancel_alarm,
            "sp2.output": cls._parse_output,
            "__return__": cls._parse_return,
            "sp2.__return__": cls._parse_return,
            "sp2.num_ticks": cls._parse_num_ticks,
            "sp2.value_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.value_at),
            "sp2.time_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.time_at),
            "sp2.item_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.item_at),
            "sp2.values_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.values_at),
            "sp2.times_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.times_at),
            "sp2.items_at": cls._make_single_proxy_arg_func_resolver(builtin_functions.items_at),
            "sp2.set_buffering_policy": cls._parse_set_buffering_policy,
            "sp2.engine_start_time": cls._parse_engine_start_time,
            "sp2.engine_end_time": cls._parse_engine_end_time,
            "sp2.enable_cache": cls._parse_sp2_enable_cache,
            "sp2.engine_stats": cls._parse_sp2_engine_stats,
        }
