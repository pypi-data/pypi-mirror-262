...
from sp2.impl.wiring.adapters import _graph_return_adapter
...
from sp2.profiler import Profiler, graph_info
...
def _run_engine(engine, starttime, endtime, context_config=None, cache_data=None):
    ...
    if cache_config:
        from sp2.impl.wiring.cache_support.runtime_cache_manager import RuntimeCacheManager
...
class GraphRunInfo:
    ...
    def get_cur_run_times_info(cls, raise_if_missing=True):
        ...
        if info is None and raise_if_missing:
            raise RuntimeError("sp2 graph information is not available outside of sp2.run")
        return info
    ...
    **kwargs,
    ...
    with ExceptionContext():
        ...
        if isinstance(g, Context):
            ...
            engine = _sp2impl.PyEngine(**engine_settings)
