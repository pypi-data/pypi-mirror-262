...
from sp2.impl.wiring.special_output_names import SP2_CACHE_ENABLED_OUTPUT, UNNAMED_OUTPUT_NAME
...
class GraphParser(BaseParser):
    ...
    def visit_Return(self, node):
        if len(self._outputs) == 0 and node.value is not None:
            raise Sp2ParseError("returning from graph without any outputs defined", node.lineno)

        if len(self._outputs) and node.value is None:
            raise Sp2ParseError("return does not return values with non empty outputs")
    ...
    def _parse_return(self, node, special_outputs=None):
        if node.args or node.keywords:
            if not self._signature._outputs:
                raise Sp2ParseError("sp2 return trying to return values for graph with no outputs", node.lineno)
        else:
            if self._signature._outputs:
                raise Sp2ParseError("sp2 return does not return values with non empty outputs", node.lineno)
            else:
                return ast.Return()
        if len(node.args) > 1 or (len(node.args) and len(node.keywords)):
            raise Sp2ParseError(
                "sp2 return expects to be called with (value) or (output = value, output2 = value2)", node.lineno
            )
        if len(node.args) == 1:
        ...
        if len(self._signature._outputs) == 1 and self._signature._outputs[0].name is None:
            raise Sp2ParseError("Invalid return of single unnamed argument.", node.lineno)
        ...
        if len(node.keywords) == 1 and node.keywords[0].arg is None:
            if not isinstance(node.keywords[0].value, ast.Call):
                raise Sp2ParseError("only unpacking of other sp2.node or sp2.graph calls are allowed", node.lineno)
            ...
            if set(f_sig._output_map.keys()) != set(self._signature._output_map.keys()):
                raise Sp2ParseError(f"{f_name} outputs dont align with graph outputs", node.lineno)
        ...
        node.func = ast.Attribute(
            value=ast.Attribute(
                value=ast.Attribute(
                    value=ast.Name(id="sp2", ctx=ast.Load()), attr="impl", lineno=node.lineno, ctx=ast.Load()
                ),
                attr="wiring",
                lineno=node.lineno,
                ctx=ast.Load(),
    ...
    def _get_special_output_name_mapping(self):
        ...
        """
        return {SP2_CACHE_ENABLED_OUTPUT: SP2_CACHE_ENABLED_OUTPUT}

    def visit_Call(self, node: ast.Call):
        if (isinstance(node.func, ast.Name) and node.func.id == "__return__") or BaseParser._is_sp2_output_call(node):
            special_outputs = {}
            if self._add_cache_control_output:
        ...
        if (
            ...
            if len(node.args) != 1 or node.keywords:
                raise Sp2ParseError("Invalid call to sp2.set_cache_enable_ts", node.lineno)
            if self._add_cache_control_output:
                return ast.Assign(targets=[ast.Name(id=SP2_CACHE_ENABLED_OUTPUT, ctx=ast.Store())], value=node.args[0])
            else:
                raise Sp2ParseError(
                    "Invalid call to sp2.set_cache_enable_ts in graph with non controlled cache", node.lineno
                )
    ...
    def _parse_impl(self):
        ...
        if self._add_cache_control_output:
            self._funcdef.body = [
                ast.Assign(
                    targets=[ast.Name(id=SP2_CACHE_ENABLED_OUTPUT, ctx=ast.Store())],
                    value=ast.Call(
                        func=ast.Attribute(value=ast.Name(id="sp2", ctx=ast.Load()), attr="null_ts", ctx=ast.Load()),
