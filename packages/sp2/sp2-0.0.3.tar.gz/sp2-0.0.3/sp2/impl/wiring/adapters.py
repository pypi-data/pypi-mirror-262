...
from sp2.impl.mem_cache import sp2_memoized_graph_object
from sp2.impl.outputadapter import OutputAdapter  # noqa: F401
from sp2.impl.types import tstype
from sp2.impl.types.common_definitions import ArgKind, InputDef, OutputDef, PushMode
...
class AdapterDefMeta(type):
    def __init__(self, *args, **kwargs):
        ...
        if self.memoize or self.force_memoize:
            self._instantiate_func = sp2_memoized_graph_object(self._instantiate_impl, force_memoize=self.force_memoize)
...
status_adapter_def = input_adapter_def("status_adapter", _sp2impl._status_adapter, ts["T"], object, typ="T")
...
def py_pull_adapter_def(name, adapterimpl, out_type, memoize=True, force_memoize=False, **kwargs):
    ...
    :param name:         string name for the adapter
    :param adapterimpl: a derived implementation of sp2.impl.pulladapter.PullInputAdapter
    :param out_type:     the type of the output, should be a ts[] type.  Note this can use tvar types if a subsequent argument defines the tvar
    :param memoize: Specify whether the node should be memoized (default True)
    :param force_memoize: If True, the node will be memoized even if sp2.memoize(False) was called. Usually it should not be set, set
    this to True ONLY if memoization required to guarantee correctness of the function (i.e the function must be called at most once with the for each
    ...
    def impl(mgr, engine, pytype, push_mode, scalars):
        return _sp2impl._pulladapter(mgr, engine, pytype, push_mode, (adapterimpl, scalars))
...
def py_managed_adapter_def(name, adapterimpl, out_type, manager_type, memoize=True, force_memoize=False, **kwargs):
    ...
    :param name:         string name for the adapter
    :param adapterimpl: a derived implementation of sp2.impl.adaptermanager.ManagedSimInputAdapter
    :param out_type:     the type of the output, should be a ts[] type.  Note this can use tvar types if a subsequent argument defines the tvar
    ...
    def impl(mgr, engine, pytype, push_mode, scalars):
        return _sp2impl._managedsimadapter(mgr, engine, pytype, push_mode, (adapterimpl, (mgr, *scalars)))
...
def py_push_adapter_def(name, adapterimpl, out_type, manager_type=None, memoize=True, force_memoize=False, **kwargs):
    ...
    def impl(mgr, engine, pytype, push_mode, scalars):
        ...
        if mgr is not None:
            scalars = (mgr,) + scalars
        return _sp2impl._pushadapter(mgr, engine, pytype, push_mode, (adapterimpl, push_group, scalars))
...
def py_pushpull_adapter_def(
    ...
    def impl(mgr, engine, pytype, push_mode, scalars):
        ...
        if mgr is not None:
            scalars = (mgr,) + scalars
        return _sp2impl._pushpulladapter(mgr, engine, pytype, push_mode, (adapterimpl, push_group, scalars))
...
def py_output_adapter_def(name, adapterimpl, manager_type=None, memoize=True, force_memoize=False, **kwargs):
    ...
    def impl(mgr, engine, scalars):
        if mgr is not None:
            scalars = (mgr,) + scalars
        return _sp2impl._outputadapter(mgr, engine, (adapterimpl, scalars))
...
add_graph_output = output_adapter_def(
    "add_graph_output",
    _sp2impl._graph_output_adapter,
    key=object,
    input=tstype.ts["T"],
    tick_count=(int, -1),
...
_graph_return_adapter = output_adapter_def(
    "_graph_return", _sp2impl._graph_return_adapter, key=object, input=tstype.ts["T"]
)
