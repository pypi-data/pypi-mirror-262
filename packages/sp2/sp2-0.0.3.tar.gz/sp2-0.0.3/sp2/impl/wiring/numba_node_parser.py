...
from sp2.impl.types.common_definitions import ArgKind
from sp2.impl.wiring.node_parser import NodeParser, Sp2ParseError
from sp2.impl.wiring.signature import Signature
...
class _StateVarTransformer(ast.NodeTransformer):
    ...
    Example:
        ...
        x = x + y + z will be transformed to __sp2_state__.x = __sp2_state__.x +  __sp2_state__.y +  z
    """
...
class NumbaNodeParser(NodeParser):
    _STATE_ARG_NAME = "__sp2_state_ptr__"
    _STATE_INST_NAME = "__sp2_state_inst__"

    def __init__(self, name, raw_func, func_frame, debug_print=False, state_types=None):
        from sp2.impl.wiring.numba_utils.sp2_cpp_numba_interface import SP2_NUMBA_CPP_FUNCTIONS
        from sp2.impl.wiring.numba_utils.sp2_numba_functions import (
            sp2_print,
            sp2_unsafe_cast_class_ptr_to_int,
            sp2_unsafe_cast_int_to_class_ptr,
        )

        super().__init__(name=name, raw_func=raw_func, func_frame=func_frame, debug_print=debug_print)
        self._func_globals_modified.update(SP2_NUMBA_CPP_FUNCTIONS)
        self._func_globals_modified["sp2_unsafe_cast_int_to_class_ptr"] = sp2_unsafe_cast_int_to_class_ptr
        self._func_globals_modified["sp2_unsafe_cast_class_ptr_to_int"] = sp2_unsafe_cast_class_ptr_to_int
        self._func_globals_modified["__sp2__print__"] = sp2_print
        ...
        self._state_class_name = f"__SP2__STATE__{raw_func.__name__}__"
    ...
    def visit_Return(self, node):
        if len(self._outputs) > 1:
            raise Sp2ParseError("returning from node with multiple outputs, use return sp2.output(...)", node.lineno)
            ...
            res.append(
                ast.Expr(
                    value=ast.Call(
                        ...
                        args=[ast.Name(id="__sp2_node__", ctx=ast.Load()), ast.Num(n=0), return_value],
                        keywords=[],
                    )
                )
            )
    ...
    def visit_Name(self, node):
        ...
        if ts_idx is not None:
            return ast.Call(
                ...
                args=[ast.Name(id="__sp2_node__", ctx=ast.Load()), ast.Num(n=ts_idx)],
                keywords=[],
            )
        else:
            return node
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name) and node.func.id == "print":
            node.func.id = "__sp2__print__"

        return super().visit_Call(node)

    @classmethod
    def get_ts_input_value_getter_name(cls, ts_idx):
        return f"__sp2__value_getter_inp_{ts_idx}__"

    @classmethod
    def get_ts_out_value_return_name(cls, ts_idx):
        return f"__sp2__value_return_out_{ts_idx}__"

    @classmethod
    def get_arg_type_var_name(cls, arg_name):
        return f"__sp2__{arg_name}_type__"

    @classmethod
    def get_state_type_var_name(cls, arg_name):
        return f"__sp2__state__{arg_name}_type__"
    ...
    def _create_single_input_ticked_expression(self, arg):
        return self._create_single_input_builtin_call(arg, "__sp2_numba_node_ticked__")

    def _create_single_input_valid_expression(self, arg):
        return self._create_single_input_builtin_call(arg, "__sp2_numba_node_valid__")

    @classmethod
    def _sp2_single_input_builtin_function_transformer(cls, cpp_function_name):
        ...
        def _transform_sp2_call(self, node):
    ...
    def _create_state_class(
        ...
        res = ast.ClassDef(
            ...
            body=[
                ...
                ast.FunctionDef(
                    ...
                    body=[
                        ast.Return(
                            value=ast.Call(
                                func=ast.Name(id="sp2_unsafe_cast_class_ptr_to_int", ctx=l_ctx),
            ...
            decorator_list=[
                ast.Call(
                    func=ast.Attribute(value=ast.Name(id="__sp2_numba__", ctx=l_ctx), attr="jitclass", ctx=l_ctx),
    ...
    def _create_init_block(self):
        init_block = []
        init_block.append(ast.Import(names=[ast.alias(name="numba", asname="__sp2_numba__")]))
        return init_block
    ...
    def _create_jit_func(self, func_name, inner_body):
        args = self._create_ast_args(
            posonlyargs=[],
            args=[ast.arg(arg="__sp2_node__", annotation=None), ast.arg(arg=self._STATE_ARG_NAME, annotation=None)],
            kwonlyargs=[],
            defaults=[],
            vararg=None,
            kwarg=None,
        ...
        if self.has_state():
            body = [
                ast.Assign(
                    targets=[Name(id="__sp2_state_inst__", ctx=ast.Store())],
                    value=Call(
                        func=Name(id="sp2_unsafe_cast_int_to_class_ptr", ctx=ast.Load()),
                        args=[
                            Name(id="__sp2_state_ptr__", ctx=ast.Load()),
        ...
        decorator_list = [
            Call(
                func=Attribute(value=Name(id="__sp2_numba__", ctx=ast.Load()), attr="cfunc", ctx=ast.Load()),
                args=[
                    Call(
                        func=Attribute(
                            value=Attribute(
                                value=Name(id="__sp2_numba__", ctx=ast.Load()), attr="types", ctx=ast.Load()
                            ),
                            attr="void",
                            ctx=ast.Load(),
                        ),
    ...
    def _create_wrapped_inner_body(self, inner_body):
        ...
        init_state_func = self._create_jit_func("__sp2__init__state__", state_block_body)
        inner_body_func = self._create_jit_func("__sp2__impl__", inner_body)
        ...
        newbody.append(
            ast.Return(
                value=ast.Tuple(
                    elts=[
                        state_cls,
                        ast.Name(id="__sp2__init__state__", ctx=ast.Load()),
                        ast.Name(id="__sp2__impl__", ctx=ast.Load()),
                    ],
                    ctx=ast.Load(),
                )
            )
    ...
    def _init_internal_maps(cls):
        ...
        cls.METHOD_MAP["sp2.make_passive"] = cls._sp2_single_input_builtin_function_transformer(
            "__sp2_numba_node_make_passive__"
        )
        cls.METHOD_MAP["sp2.make_active"] = cls._sp2_single_input_builtin_function_transformer(
            "__sp2_numba_node_make_active__"
        )
