...
from sp2.impl.config import CacheCategoryConfig, CacheConfig, Config
...
from sp2.impl.managed_dataset.managed_dataset import ManagedDataset
from sp2.impl.managed_dataset.managed_dataset_lock_file_util import ManagedDatasetLockUtil
from sp2.impl.mem_cache import normalize_arg
from sp2.impl.struct import Struct
from sp2.impl.types import tstype
from sp2.impl.types.common_definitions import ArgKind, OutputBasketContainer, OutputDef
...
class GraphBuildPartitionCacheManager(object):
    ...
    def __init__(
        ...
        expected_outputs,
        cache_options,
        sp2_cache_start=None,
        sp2_cache_end=None,
        sp2_timestamp_shift=None,
    ):
        ...
        self._sp2_cache_start = sp2_cache_start if sp2_cache_start else Context.TLS.instance.start_time
        self._sp2_cache_end = sp2_cache_end if sp2_cache_end else Context.TLS.instance.end_time
        self._sp2_timestamp_shift = sp2_timestamp_shift if sp2_timestamp_shift else timedelta()
        missing_period_callback = _MissingPeriodCallback()
        data_for_period, is_full_period_covered = self.get_data_for_period(
            self._sp2_cache_start, self._sp2_cache_end, missing_range_handler=missing_period_callback
        )
        ...
        if is_full_period_covered:
            from sp2.adapters.parquet import ParquetReader
            ...
            reader = ParquetReader(
                ...
                start_time=sp2_cache_start,
                end_time=sp2_cache_end,
                allow_overlapping_periods=True,
                time_shift=self._sp2_timestamp_shift,
            )
    ...
    def _read_files_provider(self, dataset, data_files, lock_file_permissions, engine_lock_releaser):
        ...
        while not finished:
            ...
            if not is_file and not is_dir:
                data_files, _ = self.get_data_for_period(next_start_time, self._sp2_cache_end)
                assert data_files
                items_iter = iter(data_files.items())
                num_failures += 1
    ...
    def get_data_for_period(self, starttime: datetime, endtime: datetime, missing_range_handler):
        res, full_period_covered = self.dataset_partition.get_data_for_period(
            starttime=starttime - self._sp2_timestamp_shift,
            endtime=endtime - self._sp2_timestamp_shift,
            missing_range_handler=missing_range_handler,
        )
        if self._sp2_timestamp_shift:
            res = {
                (start + self._sp2_timestamp_shift, end + self._sp2_timestamp_shift): path
    ...
    def cache_outputs(self, outputs):
        from sp2.impl.managed_dataset.managed_parquet_writer import create_managed_parquet_writer_node
    ...
    def create_cache_manager(
        ...
        key = _CacheManagerKey(
            all_scalars, id(func), tuple(tvars.items()), sp2_cache_start, sp2_cache_end, sp2_timestamp_shift
        )
    ...
    if cache_enabled:
        ...
        for output in signature._outputs:
            if isinstance(output.typ, OutputBasketContainer):
                if Sp2TypingUtils.get_origin(output.typ.typ) is List:
                ...
                if tstype.isTsStaticBasket(output.typ):
                    if Sp2TypingUtils.get_origin(output.typ) is List:
                        raise NotImplementedError("Caching of list basket outputs is unsupported")
                    else:
                        raise TypeError(
