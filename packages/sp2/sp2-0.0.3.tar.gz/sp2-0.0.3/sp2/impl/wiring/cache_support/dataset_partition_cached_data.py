...
import sp2
from sp2.adapters.output_adapters.parquet import resolve_array_shape_column_name
...
class DatasetPartitionCachedData:
    ...
    def _get_shape_columns(self, column_list):
        for c in column_list:
            ...
            if c_type and Sp2TypingUtils.is_numpy_nd_array_type(c_type):
    ...
    def _get_array_columns(self, column_list):
        for c in column_list:
            c_type = self.metadata.columns.get(c)
            if c_type and Sp2TypingUtils.is_numpy_array_type(c_type):
                yield c
    ...
    def _load_data_split_to_columns(
        ...
        if basket_column_list:
            ...
            for column in basket_column_list:
                ...
                if issubclass(value_type, sp2.Struct):
                ...
                symbol_file = os.path.join(file_path, f"{column}__sp2_symbol.parquet")
                value_count_file = os.path.join(file_path, f"{column}__sp2_value_count.parquet")
    ...
    def _get_flat_basket_df_for_period(
        ...
        symbol_files = [os.path.join(f, f"{basket_field_name}__sp2_symbol.parquet") for f in data_files.values()]
        if load_values:
        ...
        value_count_files = [
            os.path.join(f, f"{basket_field_name}__sp2_value_count.parquet") for f in data_files.values()
        ]
        timestamp_files = [
    ...
    def get_all_basket_ids_in_range(
        ...
        symbol_column_name = "__sp2_symbol__"
