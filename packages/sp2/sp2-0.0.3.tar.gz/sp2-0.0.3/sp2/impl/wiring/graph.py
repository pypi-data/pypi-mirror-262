...
import sp2.impl.wiring.edge
from sp2.impl.constants import UNSET
from sp2.impl.error_handling import ExceptionContext
from sp2.impl.managed_dataset.dateset_name_constants import DatasetNameConstants
from sp2.impl.mem_cache import function_full_name, sp2_memoized_graph_object
from sp2.impl.types.common_definitions import InputDef
from sp2.impl.types.instantiation_type_resolver import GraphOutputTypeResolver
from sp2.impl.wiring import Signature
from sp2.impl.wiring.cache_support.dataset_partition_cached_data import DataSetCachedData, DatasetPartitionCachedData
from sp2.impl.wiring.cache_support.graph_building import (
    GraphBuildPartitionCacheManager,
    GraphCacheOptions,
    resolve_graph_cache_options,
)
from sp2.impl.wiring.context import Context
from sp2.impl.wiring.graph_parser import GraphParser
from sp2.impl.wiring.outputs import CacheWriteOnlyOutputsContainer, OutputsContainer
from sp2.impl.wiring.special_output_names import ALL_SPECIAL_OUTPUT_NAMES, UNNAMED_OUTPUT_NAME
...
class _CacheProxy:
    ...
    def __init__(self, owner, sp2_cache_start=None, sp2_cache_end=None, sp2_timestamp_shift=None):
        self._owner = owner
        self._sp2_cache_start = sp2_cache_start
        self._sp2_cache_end = sp2_cache_end
        self._sp2_timestamp_shift = sp2_timestamp_shift
    ...
    def __getitem__(self, item):
        ...
        res = _CacheProxy(self._owner, sp2_timestamp_shift=self._sp2_timestamp_shift)
        res._sp2_cache_start = item.start
        # The range values are exclusive but for caching purposes we need inclusive end time
        res._sp2_cache_end = item.stop - datetime.timedelta(microseconds=1)
        return res

    def shifted(self, sp2_timestamp_shift: datetime.timedelta):
        return _CacheProxy(
            self._owner,
            sp2_cache_start=self._sp2_cache_start,
            sp2_cache_end=self._sp2_cache_end,
            sp2_timestamp_shift=sp2_timestamp_shift,
        )
    ...
    def __call__(self, *args, _forced_tvars=None, **kwargs):
        with _ForceCached.force_cached():
            return self._owner._cached_impl(
                _forced_tvars,
                self._sp2_cache_start,
                self._sp2_cache_end,
                args,
                kwargs,
                sp2_timestamp_shift=self._sp2_timestamp_shift,
            )
...
class GraphDefMeta(type):
    def __init__(self, *args, **kwargs):
        ...
        if self.memoize or self.force_memoize:
            ...
            self._instantiate_func = sp2_memoized_graph_object(
    ...
    def cache_periods(self, start_time, end_time):
        from sp2.impl.managed_dataset.aggregation_period_utils import AggregationPeriodUtils
    ...
    def cached_data(self, data_folder=None, _forced_tvars=None) -> DatasetPartitionCachedData:
        """Get the proxy object for accessing the graph cached data.
        This is the basic interface for inspecting cache files and loading cached data as dataframes
        :param data_folder: An instance of string (data folder), sp2.Config or sp2.cache_support.CacheConfig with the appropriate cache config. Note, that
        ...
        if isinstance(data_folder, sp2.Config):
            cache_config = data_folder.cache_config
        elif isinstance(data_folder, sp2.cache_support.CacheConfig):
    ...
    def _cached_impl(self, _forced_tvars, sp2_cache_start, sp2_cache_end, args, kwargs, sp2_timestamp_shift=None):
        """A utility function to ensure that a graph is read from cache
        For example if there is a cached graph g.
        Calling g(a1, a2, ...) can either read it from cache or write the results to cache if no cached data is found.
        Calling g.cached(a1, a2, ...) forces reading from cache, if no cache data is found then exception will be raised.
        :param args: Positional arguments to the graph
        :param sp2_cache_start: The start time of the cached data before which we don't want to load any date
        :param sp2_cache_end: The end time of the cached data after which we don't want to load any date
        :param kwargs: Keyword arguments to the graph
        """
        if Context.TLS.instance.config and hasattr(Context.TLS.instance.config, "cache_config") and self._cache_options:
            read_from_cache, res, _ = self._instantiate_func(
                _forced_tvars, self._signature, args, kwargs, sp2_cache_start, sp2_cache_end, sp2_timestamp_shift
            )
    ...
    def _instantiate_impl(
        self, _forced_tvars, signature, args, kwargs, sp2_cache_start=None, sp2_cache_end=None, sp2_timestamp_shift=None
    ):
        ...
        if (
            ...
            cache_manager = GraphBuildPartitionCacheManager.create_cache_manager(
                ...
                tvars=tvars,
                sp2_cache_start=sp2_cache_start,
                sp2_cache_end=sp2_cache_end,
                sp2_timestamp_shift=sp2_timestamp_shift,
            )
    ...
    def _instantiate(self, _forced_tvars, *args, **kwargs):
        ...
        if not allow_non_cached_read:
            if isinstance(res, sp2.impl.wiring.edge.Edge):
...
def graph(
    ...
    *,
    ...
    :param force_memoize: If True, the node will be memoized even if sp2.memoize(False) was called. Usually it should not be set, set
