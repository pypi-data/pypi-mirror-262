...
from sp2.impl.types.common_definitions import ArgKind, OutputDef
from sp2.impl.types.container_type_normalizer import ContainerTypeNormalizer
from sp2.impl.wiring import runtime, tstype
from sp2.impl.wiring.edge import Edge
from sp2.impl.wiring.node import NodeDef
...
class DynamicNodeDef:
    ...
    def _create(self, engine, memo):
        ...
        def builder(sub_engine, adjusted_args):
            ...
            # make sense if we enable memoization across outter / inner graphs ( need to pass sp2_memoization instance through )
            # There is an unsolvable issue though, I think.  if we memoize something as trivial a sp2.const(1) it will never tick
            # in the dynamic
        ...
        node = _sp2impl.PyDynamicNode(engine, self._name, inputs, outputs, builder, self._dynamic_args, self._raw_args)
...
def _set_dynbasket_args(args, trigger, depth=0):
    ...
    for arg in args:
        if isinstance(arg, (tstype.SnapKeyType, tstype.AttachType)):
            if depth > 0:
                raise TypeError("sp2.snap and sp2.attach are not supported as members of containers")
...
def dynamic(trigger, sub_graph, *args, **kwargs):
    ...
    :param sub_graph: sp2.graph method to be used to create dynamic sub-graphs
    ...
    for out in sub_graph._signature._outputs:
        if out.kind.is_basket():
            raise TypeError("sp2.dynamic does not support basket outputs of sub_graph")
        sig_outputs.append(
            OutputDef(
                out.name,
    ...
    # if `sub_graph` is a bound method (e.g. calling sp2.dynamic(trigger, self.sub_graph),
    ...
    # This is just for extracting tvars and type checking at sp2.dynamic wiring time
    ...
    for idx, arg in enumerate(raw_args):
        if isinstance(arg, tstype.SnapType):
            ...
            # For now we dont allow sp2.attach on anything other than trigger, could be revisited
            # Allowing arbitrary baskets opens up issues with the key disappearing from the basket.
            dynamic_args.append((0, idx, "attach"))

            # For sp2.attach, we prep the edge as the proper single ts nodedef output.  dynamic key will set
