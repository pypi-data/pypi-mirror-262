...
import sp2
import sp2.impl.pandas_accessor  # To ensure that the sp2 accessors are registered
from sp2.impl.pandas_ext_type import is_sp2_type

_ = sp2.impl.pandas_accessor
...
def _apply_updates(
    ...
    with sp2.start():
        if throttle > timedelta(0):
            sp2.schedule_alarm(alarm, throttle, True)
    ...
    with sp2.stop():
        try:
            ...
            #     bar()
            # def main():
            #     sp2.run(my_graph, realtime=True)
    ...
    if sp2.ticked(data):
        ...
        for (idx, col), value in data.tickeditems():
            if idx not in new_rows:
                ...
                if s_has_time_col:
                    if localize:
                        row[time_col] = pytz.utc.localize(sp2.now())
                    else:
                        row[time_col] = sp2.now()
            else:
                row = new_rows[idx]
...
class Sp2PerspectiveTable:
    def __init__(
        ...
        self._static_frame = data.sp2.static_frame()
    ...
    def graph(self):
        """The sp2 graph that populates the table with ticking data"""
        if self._basket:
            _apply_updates(
                self._table,
                self._basket,
    ...
    def run_historical(self, starttime, endtime):
        ...
        """
        df = self._data.sp2.run(starttime, endtime)
    ...
    def run(self, starttime=None, endtime=timedelta(seconds=60), realtime=True, clear=False):
        ...
        if clear:
            self.clear()
        sp2.run(self.graph, starttime=starttime, endtime=endtime, realtime=realtime)

    def start(self, starttime=None, endtime=timedelta(seconds=60), *, realtime=True, clear=True, auto_shutdown=True):
        """Start a graph that sends data to the table on a sp2 engine thread.
        ...
        if clear:
            self.clear()
        self._runner = sp2.run_on_thread(
    ...
    def stop(self):
        """Stop the running sp2 engine thread"""
        if self._runner is None:
    ...
    def join(self):
        """Block until the sp2 engine thread is done"""
        if self._runner is None:
...
Sp2PerspectiveTable._add_view_methods()


def _start_many_graph(tables: [Sp2PerspectiveTable]):
    for table in tables:
        table.graph()


class Sp2PerspectiveMultiTable:
    """A class to hold multiple Sp2Perspective tables and coordinate their running/stopping."""

    def __init__(self, tables: {str: Sp2PerspectiveTable}):
        self._tables = tables
        self._runner = None
    ...
    def get_widget(self, widget="Tab", config=None):
        ...
        :param config: An optional dictionary, keyed with the names of tables. The values of the dictionary are
            kwargs to pass to the Sp2PerspectiveTable.get_widget call
        """
        import ipywidgets
