...
import sp2
from sp2.impl.types.tstype import TsType, isTsType, ts
    ...
    def __init__(self, values, dtype=None, copy=False):
        ...
        if dtype is None:
            try:
                ...
                raise ValueError("Either a sp2.Edge must be provided or a dtype")
...
def _binary_op(x: ts["T"], y: ts["T"], op: object) -> ts["T"]:
    if sp2.ticked(x, y) and sp2.valid(x, y):
        return op(x, y)
...
def _reduce(x: [ts["T"]], typ: "T", func: object, args: object = (), kwargs: object = {}) -> ts["T"]:
    ...
    # If one wanted to reduce only over valid elements, then in many cases you could pre-apply sp2.default
...
def is_sp2_type(arr_or_dtype) -> bool:
