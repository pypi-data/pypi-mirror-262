...
class StructMeta(_sp2typesimpl.PyStructMeta):
    def __new__(cls, name, bases, dct):
        ...
        if annotations:
            for k, v in annotations.items():
                actual_type = v
                if Sp2TypingUtils.is_generic_container(v):
                    actual_type = ContainerTypeNormalizer.normalized_type_to_actual_python_type(v)
                    if Sp2TypingUtils.is_generic_container(actual_type):
...
class Struct(_sp2typesimpl.PyStruct, metaclass=StructMeta):
    @classmethod
    ...
    def fromts(cls, trigger=None, /, **kwargs):
        ...
        return sp2.struct_fromts(cls, kwargs, trigger)
    ...
    def collectts(cls, **kwargs):
        ...
        return sp2.struct_collectts(cls, kwargs)
    ...
    def _obj_from_python(cls, json, obj_type):
        ...
        if Sp2TypingUtils.is_generic_container(obj_type):
            if Sp2TypingUtils.get_origin(obj_type) in (typing.List, typing.Set, typing.Tuple):
            ...
            elif Sp2TypingUtils.get_origin(obj_type) is typing.Dict:
            ...
            elif Sp2TypingUtils.get_origin(obj_type) in (sp2.typing.NumpyNDArray, sp2.typing.Numpy1DArray):
                return json
            else:
    ...
    def __getstate__(self):
        ...
        for k in self.__full_metadata_typed__:
            v = getattr(self, k, sp2.UNSET)
            if v is not sp2.UNSET:
                kwargs[k] = v
        return kwargs
