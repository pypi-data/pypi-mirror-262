...
import sp2
from sp2.adapters.output_adapters.parquet import ParquetOutputConfig, ParquetWriter
from sp2.cache_support import CacheConfig
...
from sp2.utils.file_permissions import apply_file_permissions
from sp2.utils.lock_file import MultipleFilesLock
...
class _SingleBasketMergeData:
    def __init__(self, basket_name, basket_types, input_files, basket_data_input_files):
        ...
        self.count_column_name = f"{basket_name}__sp2_value_count"
        if issubclass(self.basket_types.value_type, sp2.Struct):
            self.data_column_names = [f"{basket_name}.{c}" for c in self.basket_types.value_type.metadata()]
        else:
            self.data_column_names = [basket_name]
        self.symbol_column_name = f"{basket_name}__sp2_symbol"
...
class _MergeFileInfo(sp2.Struct):
    file_path: str
