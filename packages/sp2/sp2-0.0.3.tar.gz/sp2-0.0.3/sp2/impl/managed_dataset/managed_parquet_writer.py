...
import sp2
from sp2.adapters.parquet import ParquetOutputConfig, ParquetWriter
...
from sp2.impl.managed_dataset.managed_dataset import ManagedDatasetPartition
from sp2.impl.managed_dataset.managed_dataset_merge_utils import _create_wip_file
from sp2.impl.wiring import Context
from sp2.impl.wiring.cache_support.partition_files_container import PartitionFileContainer
from sp2.impl.wiring.outputs import OutputsContainer
from sp2.impl.wiring.special_output_names import ALL_SPECIAL_OUTPUT_NAMES, SP2_CACHE_ENABLED_OUTPUT
...
def _cache_filename_provider_custom_time(
    ...
    timestamp_ts: sp2.ts[datetime.datetime],
) -> sp2.ts[str]:
    with sp2.state():
...
def _cache_filename_provider(
    ...
    config: Optional[ParquetOutputConfig],
    split_columns_to_files: Optional[bool],
    cache_control_ts: sp2.ts[bool],
    default_cache_enabled: bool,
) -> sp2.ts[str]:
    with sp2.alarms():
        a_update_file_alarm = sp2.alarm(bool)
    ...
    with sp2.start():
        ...
        sp2.schedule_alarm(a_update_file_alarm, datetime.timedelta(), False)
    ...
    with sp2.stop():
        ...
        if "s_cur_file_path" in locals() and s_cur_file_path:
            if s_cache_enabled:
                PartitionFileContainer.get_instance().add_generated_file(
                    dataset_partition, s_last_start_time, sp2.now(), s_cur_file_path, config
                )

    if sp2.ticked(cache_control_ts):
        if cache_control_ts:
            ...
            if not s_cache_enabled:
                ...
                s_last_start_time = sp2.now()
                cutoff_time = s_data_paths.get_file_cutoff_time(s_last_start_time)
                sp2.schedule_alarm(a_update_file_alarm, cutoff_time, False)
                return s_cur_file_path
        else:
    ...
    if sp2.ticked(a_update_file_alarm) and s_last_start_time != sp2.now():
        s_cur_file_path = _finalize_current_output_file(
            s_data_paths,
            config,
            dataset_partition,
...
@sp2.node
def _serialize_value(value: sp2.ts["T"], type_serializer: CacheObjectSerializer) -> sp2.ts[bytes]:
    if sp2.ticked(value):
        sp2.output(type_serializer.serialize_to_bytes(value))
...
def create_managed_parquet_writer_node(
    ...
    if controlled_cache:
        cache_control_ts = values[SP2_CACHE_ENABLED_OUTPUT]
    else:
        cache_control_ts = sp2.const(True)
        default_cache_enabled = True

    if not isinstance(values, OutputsContainer):
        values = OutputsContainer(**{DatasetNameConstants.UNNAMED_OUTPUT_NAME: values})

    if data_timestamp_column_name and data_timestamp_column_name != DatasetNameConstants.SP2_TIMESTAMP:
        timestamp_ts = values
    ...
    for key, value in values._items():
        ...
        elif isinstance(value.tstype.typ, type) and issubclass(value.tstype.typ, sp2.Struct):
    ...
    if (
        ...
        and data_timestamp_column_name != DatasetNameConstants.SP2_TIMESTAMP
    ):
        raise RuntimeError(
