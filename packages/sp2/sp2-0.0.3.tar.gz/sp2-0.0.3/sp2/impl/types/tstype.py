...
class DynamicBasketMeta(type):
    def __getitem__(self, args):
        if not isinstance(args, tuple) or len(args) != 2:
            raise ValueError("sp2.DynamicBasket[] requires keys_type,value_type args")
...
def isTsType(t):
    return Sp2TypingUtils.is_generic_container(t) and Sp2TypingUtils.get_origin(t) is TsType


def isTsBasket(t):
    return Sp2TypingUtils.is_generic_container(t) and (
        (Sp2TypingUtils.get_origin(t) is typing.Dict and isTsType(t.__args__[1]))
        or (Sp2TypingUtils.get_origin(t) is typing.List and isTsType(t.__args__[0]))
    )


def isTsDynamicBasket(t):
    return (
        Sp2TypingUtils.is_generic_container(t)
        and Sp2TypingUtils.get_origin(t) is typing.Dict
...
class SnapType:
    def __init__(self, edge):
        ...
        if not isinstance(edge, Edge):
            raise TypeError("sp2.snap() only supports snapping of single timeseries edge")
    ...
    def __repr__(self):
        return f"sp2.snap(ts[{self.ts_type.typ.__name__}])"
...
class SnapKeyType:
    ...
    def __repr__(self):
        return f"sp2.snapkey(ts[{self.key_tstype.typ.__name__}])"
...
class AttachType:
    ...
    def __repr__(self):
        return f"sp2.attach({{ ts[{self.key_tstype.typ.__name__}], ts[{self.value_tstype.typ.__name__}] }})"
