...
from sp2.impl.types.common_definitions import ArgKind, BasketKind, InputDef, OutputDef
from sp2.impl.types.container_type_normalizer import ContainerTypeNormalizer
from sp2.impl.types.numpy_type_util import map_numpy_dtype_to_python_type
from sp2.impl.types.tstype import AttachType, SnapKeyType, SnapType, TsType, isTsDynamicBasket
...
class UpcastRegistry(object):
    ...
    def resolve_type(self, expected_type: type, new_type: type, allow_subtypes: bool, raise_on_error=True):
        ...
        if res is None:
            if Sp2TypingUtils.is_generic_container(expected_type):
                expected_type_base = Sp2TypingUtils.get_orig_base(expected_type)
                if expected_type_base is new_type:
                    return expected_type
                if Sp2TypingUtils.is_generic_container(new_type):
                    expected_origin = Sp2TypingUtils.get_origin(expected_type)
                    new_type_origin = Sp2TypingUtils.get_origin(new_type)
                    array_types = (sp2.typing.Numpy1DArray, sp2.typing.NumpyNDArray)
                    if (
                        expected_origin in array_types
                        and new_type_origin in array_types
                        and expected_type.__args__ == new_type.__args__
                    ):
                        return sp2.typing.NumpyNDArray[new_type.__args__[0]]
                if raise_on_error:
                    raise TypeError(f"Incompatible types {expected_type} and {new_type}")
                else:
                    return None
            elif Sp2TypingUtils.is_generic_container(new_type):
                if Sp2TypingUtils.get_orig_base(new_type) is expected_type:
                    return expected_type
                elif raise_on_error:
                    raise TypeError(f"Incompatible types {expected_type} and {new_type}")
                else:
            ...
            if allow_subtypes and inspect.isclass(expected_type) and inspect.isclass(new_type):
                if issubclass(expected_type, new_type):
                    ...
                    return expected_type if Sp2TypingUtils.is_generic_container(expected_type) else new_type
                elif issubclass(new_type, expected_type):
                    return new_type if Sp2TypingUtils.is_generic_container(new_type) else expected_type
            if raise_on_error:
                raise TypeError(f"Incompatible types {expected_type} and {new_type}")
        return res
...
class TypeMismatchError(TypeError):
    ...
    def pretty_typename(cls, typ):
        if Sp2TypingUtils.is_generic_container(typ):
            return str(typ)
        elif Sp2TypingUtils.is_forward_ref(typ):
...
class _InstanceTypeResolverBase(metaclass=ABCMeta):
    ...
    def _resolve_types(self):
        for arg, in_out_def in zip(self._arguments, self._input_or_output_definitions):
            ...
            if in_out_def.kind.is_single_ts():
                ...
                if Sp2TypingUtils.is_generic_container(in_out_def.typ):
    ...
    def _is_expected_generic_meta(self, typ, expected_generic_meta):
        is_generic_container = Sp2TypingUtils.is_generic_container(typ)
        return (is_generic_container and Sp2TypingUtils.get_origin(typ) is expected_generic_meta) or (
    ...
    def _rec_validate_type_spec_vs_type_spec_and_resolve_tvars(
        ...
        if expected_sub_type is typing.Any:
            ...
            return True
        elif Sp2TypingUtils.is_forward_ref(expected_sub_type):
            ...
            return True
        elif Sp2TypingUtils.is_generic_container(expected_sub_type):
            origin_typ = Sp2TypingUtils.get_origin(expected_sub_type)
            if not self._is_expected_generic_meta(actual_sub_type, origin_typ):
                ...
                if origin_typ is not sp2.typing.NumpyNDArray or not self._is_expected_generic_meta(
                    actual_sub_type, sp2.typing.Numpy1DArray
                ):
                    return False
            ...
            elif origin_typ in (sp2.typing.Numpy1DArray, sp2.typing.NumpyNDArray):
                # We want to support also passing ndarray wherever sp2.typing.NumpyArray[...] is expected
                if Sp2TypingUtils.is_generic_container(actual_sub_type):
                    return self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(
                        actual_sub_type.__args__[0], expected_sub_type.__args__[0], is_in_ts=is_in_ts
                    )
                else:
        ...
        elif Sp2TypingUtils.is_union_type(expected_sub_type):
    ...
    def _add_scalar_value(self, arg, in_out_def):
        ...
        if isinstance(inp_def_type, str):
            ...
            if tvar[0] == "~":  # Passed by value
                ...
                if not Sp2TypingUtils.is_type_spec(typ):
    ...
    def _is_scalar_value_matching_spec(self, inp_def_type, arg):
        ...
        if (
            ...
            return True
        if Sp2TypingUtils.is_union_type(inp_def_type):
    ...
    def _rec_validate_container_and_resolve_tvars(self, sub_arg, sub_type_def):
        ...
        if sub_type_def is typing.Any:
            ...
            return True
        elif Sp2TypingUtils.is_forward_ref(sub_type_def):
            self._add_t_var_resolution(sub_type_def.__forward_arg__, type(sub_arg))
            return True
        elif Sp2TypingUtils.is_generic_container(sub_type_def):
            # TODO: THESE CHECKS MIGHT BE EXPENSIVE AND WE MIGHT WANT TO HAVE CONFIG TO DISABLE THEM

            if Sp2TypingUtils.get_origin(sub_type_def) is typing.List:
                ...
                for v in sub_arg:
                    if not self._rec_validate_container_and_resolve_tvars(v, sub_type):
                        return False
            elif Sp2TypingUtils.get_origin(sub_type_def) is typing.Set:
                ...
                for v in sub_arg:
                    if not self._rec_validate_container_and_resolve_tvars(v, sub_type):
                        return False
            elif Sp2TypingUtils.get_origin(sub_type_def) is typing.Dict:
    ...
    def _add_list_basket_ts_value(self, args, in_out_def):
        assert Sp2TypingUtils.get_origin(in_out_def.typ) is typing.List
    ...
    def _add_dict_basket_ts_value(self, args, in_out_def):
        assert Sp2TypingUtils.get_origin(in_out_def.typ) is typing.Dict
    ...
    def _resolve_tvar_container_internal_types(self, tvar, container_typ, arg, raise_on_error=True):
        ...
        if isinstance(arg, set):
            ...
            if arg.ndim > 1:
                res = sp2.typing.NumpyNDArray[python_type]
            else:
                res = sp2.typing.Numpy1DArray[python_type]
        else:
...
class InputInstanceTypeResolver(_InstanceTypeResolverBase):
    ...
    def _add_ts_value(self, arg, in_out_def):
        if isinstance(arg, AttachType):
            # For sp2.attach we keep the argument as a scalar, but type check as the value timeseries of the dynamic basket
            if not self._rec_validate_type_spec_vs_type_spec_and_resolve_tvars(
...
class GraphOutputTypeResolver(_InstanceTypeResolverBase):
    ...
    def _get_basket_kind(self, in_out_def):
        ...
        if Sp2TypingUtils.get_origin(in_out_def.typ) is typing.Dict:
            return BasketKind.DICT
        else:
            assert Sp2TypingUtils.get_origin(in_out_def.typ) is typing.List
            return BasketKind.LIST
