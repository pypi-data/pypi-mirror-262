...
def _pandas_at(
    ...
    with sp2.start():
        ...
        if isinstance(window, int):
            assert window > 0
            sp2.set_buffering_policy(xs, tick_count=window)
            window -= 1
        elif isinstance(window, timedelta):
            assert window > timedelta(0)
            sp2.set_buffering_policy(xs, tick_history=window)
    ...
    if sp2.ticked(trigger):
        if (
            ...
            keys = xs.keys()
            times, values = sp2.items_at(xs[keys[0]], -window, None)
            idx = pandas.DatetimeIndex(times)
            arrays = {keys[0]: values}
            for k in keys[1:]:
                arrays[k] = sp2.values_at(xs[k], -window, None)
            return pandas.DataFrame(arrays, columns=None, index=idx, copy=False)
        else:
            data = {}
            lengths = set()
            for k in xs.keys():
                times, values = sp2.items_at(xs[k], -window, None)
                data[k] = (times, values)
...
def make_pandas(
    ...
    if wait_all_valid:
        ...
        trigger = sp2.filter(all_valid, trigger)  # To make sure we don't generate empty data frames
    for k, x in data.items():
        if tindex is None:
            sampled[k] = data[k]
        else:
            sampled[k] = sp2.sample(tindex, x)
        if wait_all_valid:
            sampled[k] = sp2.filter(all_valid, sampled[k])
