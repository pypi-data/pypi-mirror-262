...
import sp2
from sp2 import ts
from sp2.impl.pandas_ext_type import TsDtype, is_sp2_type
from sp2.impl.struct import defineNestedStruct
...
@sp2.node
def _basket_synchronize(xs: [ts["T"]], threshold: timedelta) -> sp2.OutputBasket(List[ts["T"]], shape_of="xs"):
    with sp2.alarms():
    ...
    if sp2.ticked(a_end) or len(s_current) == len(xs):
...
def _from_series(series: pd.Series, drop_na: bool = False) -> Edge:
    ...
    if len(series) == 0:
        return sp2.null_ts(typ)
    else:
        return sp2.curve(typ, (idx.values, values))
...
@register_series_accessor("sp2")
class Sp2SeriesAccessor(object):
    ...
    @staticmethod
    def _validate(obj):
        if not is_sp2_type(obj):
            raise AttributeError("Cannot use 'sp2' accessor on objects of " "dtype '{}'.".format(obj.dtype))
    ...
    def pipe(self, node, *args, **kwargs):
        """Call Edge.pipe to apply a sp2 node to each non-null element of a series, generating a new series.
    ...
    def binop(self, node, other, *args, **kwargs):
        ...
        :example: series1.sp2.binop(filter, series2)
        """
    ...
    def sample(self, trigger: Union[timedelta, np.timedelta64, pd.Timedelta, ts[object]]):
        ...
        :return: A TsDtype series containing the outputs of sp2.sample applied to each non-null element of the series
        """
        ...
        if isinstance(trigger, timedelta):
            trigger = sp2.timer(trigger, True)
        return self.pipe((sp2.sample, "x"), trigger=trigger)

    @staticmethod
    def _flatten(series, columns, prepend_name, delim, recursive):
        struct = series.dtype.subtype
        if not issubclass(struct, sp2.Struct):
        ...
        for col in columns:
            ...
            if recursive and issubclass(meta[col], sp2.Struct):
                data.update(Sp2SeriesAccessor._flatten(col_series, None, prepend_name, delim, recursive))
            else:
    ...
    def _eval_graph(self, tick_count: int = -1, tick_history: timedelta = timedelta(), snap: bool = False):
        for rowidx, edge in self._obj.items():
            if isinstance(edge, Edge):
                sp2.add_graph_output((self._obj.name, rowidx), edge, tick_count=tick_count, tick_history=tick_history)
        if snap:
            sp2.stop_engine(self.valid())
    ...
    def run(
        ...
        :param realtime: Whether to use the realtime sp2 engine
    ...
    def show_graph(self):
        ...
        import sp2.showgraph

        buffer = sp2.showgraph.generate_graph(self._eval_graph, "png")
        return Image.open(buffer)


@register_series_accessor("to_sp2")
class ToSp2SeriesAccessor(object):
    def __init__(self, pandas_obj):
        self._obj = pandas_obj
    ...
    def __call__(self, drop_na: bool = False):
        ...
        :param drop_na: If True, will drop na types from series of float and object types. For all other series types,
            na's are dropped by default for sp2's type consistency.
        """
...
@register_dataframe_accessor("sp2")
class Sp2DataFrameAccessor(object):
    def __init__(self, pandas_obj):
        self._obj = pandas_obj

    def static_frame(self):
        """Return the static part of the data frame (columns not of dtype TsDtype)"""
        cols = [col for col, series in self._obj.items() if not is_sp2_type(series)]
        return self._obj[cols]

    def ts_frame(self):
        """Return the sp2 part of the data frame (columns of type TsDtype)"""
        cols = [col for col, series in self._obj.items() if is_sp2_type(series)]
        return self._obj[cols]

    def _eval_graph(self, tick_count: int = -1, tick_history: timedelta = timedelta(), snap: bool = False):
        df_ts = self.ts_frame()
        if df_ts.index.has_duplicates:
            raise ValueError("Cannot run sp2 on a frame with duplicate row indices.")
        if df_ts.columns.has_duplicates:
            raise ValueError("Cannot run sp2 on a frame with duplicate column indices.")
        for col, series in df_ts.items():
            series.sp2._eval_graph(tick_count, tick_history, snap)
        ...
        if collect:
            ...
            if orig_frame.empty:
                ...
                for col, s in orig_frame.items():
                    orig_frame[col] = s.fillna(sp2.null_ts(s.dtype.subtype))
                ...
                series = collected.sp2.run(starttime, endtime, realtime, tick_count, tick_history, snap)
            ...
            for col, coldata in rawdata.items():
                data[col] = self._obj[col].sp2._parse_results(coldata, col, snap)
            if data:
                idx = None
            else:
                idx = _empty_index_like(self._obj.index, snap)
    ...
    def sample(self, trigger: Union[timedelta, np.timedelta64, pd.Timedelta, ts[object]], inplace: bool = False):
        ...
        for col, series in self._obj.items():
            if is_sp2_type(series):
                data[col] = series.sp2.sample(trigger)
    ...
    def _collect(self, data, struct_type):
        ...
        def row_collect(row):
            ...
            if not data:
                return sp2.null_ts(struct_type)
...
@register_dataframe_accessor("to_sp2")
class ToSp2FrameAccessor(object):
    def __init__(self, pandas_obj):
        self._obj = pandas_obj

    def __call__(self, columns=None, agg="last", drop_na=False):
        """Return an Edge from the series
        :param columns: The set of columns to turn into sp2 edges. If none are provided, all will be. Other columns
        ...
        for col, series in self._obj.items():
            if columns is None or col in columns:
                outputs[col] = series.to_sp2(drop_na)
            else:
                outputs[col] = series.to_sp2._static_agg(agg)
