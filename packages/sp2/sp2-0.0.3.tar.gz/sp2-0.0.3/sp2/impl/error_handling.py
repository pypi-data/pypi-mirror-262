...
class ExceptionContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if not self.PRINT_EXCEPTION_FULL_STACK:
            from sp2.impl.wiring.base_parser import Sp2ParseError

            if exc_val is not None:
                ast_folder = os.path.dirname(ast.__file__)
                impl_folder = os.path.dirname(sp2.impl.__file__)
                ...
                new_tb = None
                if filtered_tb:
                    new_tb = _sp2impl.create_traceback(
                        None, filtered_tb[-1].tb_frame, filtered_tb[-1].tb_lasti, filtered_tb[-1].tb_lineno
                    )
                    for old_tb in reversed(filtered_tb[:-1]):
                        new_tb = _sp2impl.create_traceback(new_tb, old_tb.tb_frame, old_tb.tb_lasti, old_tb.tb_lineno)

                if exc_type is Sp2ParseError and exc_val.filename is not None and exc_val.lineno is not None:
                    new_tb = _sp2impl.create_traceback(new_tb, exc_val.frame, exc_val.lineno, exc_val.lineno)
