...
from sp2.impl.types.common_definitions import OutputBasket, Outputs
from sp2.impl.types.tstype import ts
from sp2.impl.wiring import DelayedEdge, Edge, OutputsContainer, graph, input_adapter_def, node
from sp2.impl.wiring.delayed_node import DelayedNodeWrapperDef
from sp2.lib import _sp2baselibimpl
...
const = input_adapter_def("sp2.const", _sp2impl._const, ts["T"], value="~T", delay=(timedelta, timedelta()))
_timer = input_adapter_def(
    "sp2.timer", _sp2impl._timer, ts["T"], interval=timedelta, value=("~T", True), allow_deviation=(bool, False)
)
...
def timer(interval: timedelta, value: "~T" = True, allow_deviation: bool = False) -> ts["T"]:
    if interval <= timedelta():
        raise ValueError("sp2.timer interval must be > 0")
...
class LogSettings:
    ...
    @classmethod
    def set(cls, logger_name="sp2", logging_tz=None):
    ...
    def with_set(cls, logger_name="sp2", logging_tz=None):
...
def _list_basket_to_string_ts(x: [ts["T"]]) -> ts[str]:
    value = ",".join([str(x[i]) if sp2.ticked(x[i]) else "" for i in range(len(x))])
    return f"[{value}]"
...
def log(
    ...
    """
    Logs a time-series value during the execution of a sp2 graph

    Arguments:
    level: log level to use e.g. CRITICAL
    tag: label for the logged entry e.g. "x"
    x: the time-series to log
    logger: optional, the logger to use. If not set, the default sp2 logger is used
...
def get_basket_field(dict_basket: {"K": ts["V"]}, field_name: str) -> OutputBasket(
    ...
    :return:
    """
    if isinstance(dict_basket, sp2.impl.wiring.cache_support.graph_building.WrappedCachedStructBasket):
        return dict_basket.get_basket_field(field_name)
    else:
        return {k: getattr(v, field_name) for k, v in dict_basket.items()}


@node(cppimpl=_sp2baselibimpl.sample)
def sample(trigger: ts["Y"], x: ts["T"]) -> ts["T"]:
    ...
    if sp2.ticked(trigger) and sp2.valid(x):
        return x


@node(cppimpl=_sp2baselibimpl.firstN)
...
@node(cppimpl=_sp2baselibimpl.count)
...
@node(cppimpl=_sp2baselibimpl._delay_by_timedelta)
...
@node(cppimpl=_sp2baselibimpl._delay_by_ticks)
def _delay_by_ticks(x: ts["T"], delay: int) -> ts["T"]:
    with sp2.start():
        assert delay > 0
        sp2.set_buffering_policy(x, tick_count=delay + 1)

    if sp2.ticked(x) and sp2.num_ticks(x) > delay:
        return sp2.value_at(x, -delay)
...
def _lag(x: ts["T"], lag: typing.Union[timedelta, int]) -> ts["T"]:
    ...
    if isinstance(lag, int):
        ...
        return sp2.sample(x, sp2.delay(x, lag))
...
@node(cppimpl=_sp2baselibimpl.merge)
...
@node(cppimpl=_sp2baselibimpl.split)
def split(flag: ts[bool], x: ts["T"]) -> Outputs(false=ts["T"], true=ts["T"]):
    ...
    if sp2.ticked(x) and sp2.valid(flag):
        if flag:
            sp2.output(true=x)
        else:
            sp2.output(false=x)
...
@node(cppimpl=_sp2baselibimpl.cast_int_to_float)
...
@node(cppimpl=_sp2baselibimpl.filter)
...
def _drop_dups(x: ts["T"]) -> ts["T"]:
    with sp2.start():
        s_prev = sp2.impl.constants.UNSET

    if sp2.ticked(x) and x != s_prev:
        s_prev = x
        return x


@node(cppimpl=_sp2baselibimpl._drop_dups_float)
...
@node(cppimpl=_sp2baselibimpl.drop_nans)
...
@node(cppimpl=_sp2baselibimpl.unroll)
...
@node(cppimpl=_sp2baselibimpl.collect)
...
def gate(x: ts["T"], release: ts[bool]) -> ts[["T"]]:
    ...
    if sp2.valid(release) and release and len(s_pending):
        out = s_pending
        s_pending = []
        return out
...
def default(x: ts["T"], default: "~T", delay: timedelta = timedelta()) -> ts["T"]:
    ...
    default = sp2.const(default, delay=delay)
    if delay != timedelta():
        default = sp2.firstN(sp2.merge(x, default), 1)
    return sp2.merge(x, default)
...
@node(cppimpl=_sp2baselibimpl.multiplex)
def multiplex(x: {"K": ts["T"]}, key: ts["K"], tick_on_index: bool = False, raise_on_bad_key: bool = False) -> ts["T"]:
    ...
    if s_key_valid:
        if sp2.ticked(x[key]) or (tick_on_index and sp2.ticked(key) and sp2.valid(x[key])):
            sp2.output(x[key])


@node(cppimpl=_sp2baselibimpl.demultiplex)
def demultiplex(x: ts["T"], key: ts["K"], keys: ["K"], raise_on_bad_key: bool = False) -> OutputBasket(
    ...
    if sp2.ticked(x) and sp2.valid(key):
        if key in s_keys:
            sp2.output({key: x})
...
# @node(cppimpl=_sp2baselibimpl.dynamic_demultiplex)
@node
...
# @node(cppimpl=_sp2baselibimpl.dynamic_collect)
@node
...
@node(cppimpl=_sp2baselibimpl.exprtk_impl)
def _sp2_exprtk_impl(
    expression_str: str,
...
@node(cppimpl=_sp2baselibimpl.struct_field)
...
@node(cppimpl=_sp2baselibimpl.struct_fromts)
...
def wrap_feedback(i: ts["T"]) -> ts["T"]:
    ...
    :return:
    """
    feedback = sp2.feedback(i.tstype.typ)
    feedback.bind(i)
    return feedback.out()
...
@node(cppimpl=_sp2baselibimpl.times)
...
def static_cast(x: ts["T"], outType: "U") -> ts["U"]:
    ...
    if not issubclass(outType, x.tstype.typ) or (outType is bool and x.tstype.typ is int):
        raise TypeError(f"Unable to sp2.static_cast edge of type {x.tstype.typ.__name__} to {outType.__name__}")
