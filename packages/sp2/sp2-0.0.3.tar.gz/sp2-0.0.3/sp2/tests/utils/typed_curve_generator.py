...
class _SimpleStruct(sp2.Struct):
    value1: int
    value2: float
...
class TypedCurveGenerator(object):
    class SimpleEnum(sp2.Enum):
        VALUE1 = sp2.Enum.auto()
        VALUE2 = sp2.Enum.auto()
        VALUE3 = sp2.Enum.auto()

    SimpleStruct = _SimpleStruct
    SimpleSubStruct = _SimpleSubStruct

    class NestedStruct(sp2.Struct):
        value1: str
    ...
    @sp2.node
    def _transform_int_ts_node(self, input: sp2.ts[int], typ: "T", cast_function: object) -> sp2.ts["T"]:
    ...
    def _transform_int_curve_to_type(self, typ, input: sp2.ts[int]):
        if typ is bool:
            cast_func = bool
        elif typ is int:
            cast_func = int
