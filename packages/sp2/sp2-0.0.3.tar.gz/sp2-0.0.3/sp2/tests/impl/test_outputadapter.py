...
import sp2
from sp2 import ts
from sp2.impl.adaptermanager import AdapterManagerImpl
from sp2.impl.outputadapter import OutputAdapter
from sp2.impl.pushadapter import PushInputAdapter
from sp2.impl.wiring import py_output_adapter_def, py_push_adapter_def
...
class MyAdapterManager(AdapterManagerImpl):
    ...
    def subscribe(self, symbol):
        return _my_input_adapter(self, symbol, push_mode=sp2.PushMode.NON_COLLAPSING)
...
def my_graph_basic():
    ...
    curve = sp2.curve(data=[(timedelta(seconds=1), d) for d in data], typ=object)
...
def my_graph_with_manager():
    ...
    sp2.print("data_1", data_1)
    sp2.print("data_2", data_2)
    sp2.print("data_3", data_3)
...
class TestPythonOutputAdapter(unittest.TestCase):
    def test_basic(self):
        sp2.run(my_graph_basic, starttime=datetime.utcnow(), endtime=timedelta(seconds=5), realtime=False)
        self.assertEqual(
            output_buffer[0], '[{"a": 1, "b": 2, "c": 3}, {"a": 1, "b": 2, "c": 3}, {"a": 1, "b": 2, "c": 3}]'
        )

    def test_with_manager(self):
        sp2.run(my_graph_with_manager, starttime=datetime.utcnow(), endtime=timedelta(seconds=1), realtime=True)
