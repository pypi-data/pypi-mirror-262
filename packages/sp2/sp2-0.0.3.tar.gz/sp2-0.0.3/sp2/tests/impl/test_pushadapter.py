...
from sp2.impl.pushadapter import PushBatch, PushGroup, PushInputAdapter
...
class TestPushAdapter(unittest.TestCase):
    def test_basic(self):
        ...
        def check(burst: ts[["T"]], lv: [ts["T"]], nc: ts["T"]):
            ...
            if sp2.ticked(lv) and sp2.valid(lv):
    ...
    def test_multiple_ticks_in_batch(self):
        ...
        def graph():
            ...
            sp2.add_graph_output("v", b_to_d(b))

            stop = nc2 == sp2.const(8)
            stop = sp2.filter(stop, stop)
            sp2.stop_engine(stop)

        result = sp2.run(graph, starttime=datetime.utcnow(), endtime=timedelta(seconds=30), realtime=True)["v"]
