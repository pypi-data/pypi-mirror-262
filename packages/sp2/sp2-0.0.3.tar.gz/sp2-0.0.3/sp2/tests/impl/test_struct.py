...
class StructNoDefaults(sp2.Struct):
    b: bool
    i: int
    f: float
    s: str
...
class StructWithDefaults(sp2.Struct):
    b: bool
    i: int = 123
    f: float
    s: str = "456"
...
class StructWithMethods(sp2.Struct):
    i: int
    i2: int
...
class BaseNonNative(sp2.Struct):
    s: str
    l: list
    o: object


class BaseMixed(sp2.Struct):
    i: int
    b: bool
    f: float
    s: str
...
class StructWithStruct(sp2.Struct):
    a: int
    s: BaseNative


class StructWithMutableStruct(sp2.Struct):
    a: int
    s: BaseNonNative


class StructWithLists(sp2.Struct):
    # native_list: [int]
...
class TestSp2Struct(unittest.TestCase):
    def test_basic(self):
    ...
    def test_deepcopy(self):
        ...
        class Inner(sp2.Struct):
            v: int

        class Outer(sp2.Struct):
            a3: [object]
    ...
    def test_from_dict_loop_with_generic_typing(self):
        class MyStruct(sp2.Struct):
            ...
            np_arr: sp2.typing.NumpyNDArray[float]
    ...
    def test_struct_instance(self):
        """was a crash"""
        with self.assertRaisesRegex(TypeError, "sp2.Struct cannot be instantiated"):
            _ = sp2.Struct()
    ...
    def test_gc(self):
        ...
        @sp2.node
        def gen(x: sp2.ts[bool]) -> sp2.ts[FOO]:
            if sp2.ticked(x):
                return FOO(s="test", l=[1, 2, 3, 4])

        @sp2.node(memoize=False)
        def consume(x: sp2.ts[FOO]):
            if sp2.ticked(x):
        ...
        sp2.run(graph, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=5))
        ...
        @sp2.node
        def gen2(x: sp2.ts[bool]) -> sp2.ts[FOO]:
            if sp2.ticked(x):
        ...
        @sp2.node(memoize=False)
        def consume2(x: sp2.ts[FOO]):
            with sp2.state():
                s_x = None

            if sp2.ticked(x):
                # Keep reference around every3 ticks
                if sp2.num_ticks(x) % 3 == 0:
                    s_x = x
        ...
        with AssertLeakFree():
            for _ in range(100):
                sp2.run(graph2, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=1000))
    ...
    def test_defineNestedStruct(self):
        ...
        self.assertTrue(issubclass(d, sp2.Struct))
    ...
    def test_struct_type_alloc(self):
        ...
        for i in range(1000):
            ...
            ts = getattr(sp2.const(s), fieldname)
            sp2.run(ts, starttime=datetime.utcnow(), endtime=timedelta())
    ...
    def test_struct_printing(self):
        ...
        class StructI(sp2.Struct):
            a: int
            b: str
            c: dict

        class StructK(sp2.Struct):
            a: float
            b: float

        class StructJ(sp2.Struct):
            a: int
            b: dict
            c: StructK
    ...
    def test_disappearing_dynamic_types(self):
        """Was a BUG due to missing refcount: https://github.com/Point72/sp2/issues/74"""
