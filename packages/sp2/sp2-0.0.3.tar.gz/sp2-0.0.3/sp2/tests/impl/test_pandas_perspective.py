...
try:
    ...
    from sp2.impl.pandas_perspective import Sp2PerspectiveMultiTable, Sp2PerspectiveTable
except ImportError:
    raise unittest.SkipTest("skipping perspective tests")


class TestSp2PerspectiveTable(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_not_running(self):
        table = Sp2PerspectiveTable(self.df)
        self.assertRaises(ValueError, table.stop)
        self.assertRaises(ValueError, table.join)

    def test_keep_history(self):
        target = self.df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        ...
        table = Sp2PerspectiveTable(self.df, index_col="my_index", time_col="my_timestamp")
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        self.check_table_history(table, target, "my_index", "my_timestamp")

        table = Sp2PerspectiveTable(self.df, throttle=timedelta(seconds=10))
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        self.check_table_history(table, target, "index", "timestamp")

        table = Sp2PerspectiveTable(self.df, throttle=timedelta(seconds=0))
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        self.check_table_history(table, target, "index", "timestamp")

        self.assertRaises(ValueError, Sp2PerspectiveTable, self.df, time_col=None, keep_history=True)
    ...
    def test_limit(self):
        table = Sp2PerspectiveTable(self.df, limit=3)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=20))
        table.join()
        out = table.to_df()
        self.assertEqual(len(out), 3)

        target = self.df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=20))
        ...
        self.assertRaises(ValueError, Sp2PerspectiveTable, self.df, keep_history=False, limit=3)

    def test_localize(self):
        # In this mode, the timestamp column should be in "local time", not utc
        self.df["datetime_col"] = pd.Series(
            [sp2.const(datetime(2020, 1, 1, 1)), sp2.const(datetime(2020, 1, 1, 2)), np.nan],
            dtype=TsDtype(datetime),
            index=self.idx,
        )
        table = Sp2PerspectiveTable(self.df, time_col="my_timestamp", keep_history=False, localize=True)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        out = table.to_df()
        import dateutil
    ...
    def test_snap(self):
        ...
        table = Sp2PerspectiveTable(self.df, index_col=index_col, time_col=time_col, keep_history=False)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        out = table.to_df().convert_dtypes()
        pd.testing.assert_frame_equal(out, target)

        table = Sp2PerspectiveTable(self.df, index_col=index_col, time_col=None, keep_history=False)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=4))
        table.join()
        out = table.to_df().convert_dtypes()
    ...
    def test_run_types(self):
        ...
        s_float = pd.Series([sp2.const(0.1) for _ in self.idx], dtype=TsDtype(float), index=self.idx)
    ...
    def test_run_historical(self):
        index_col = "my_index"
        time_col = "my_timestamp"
        table = Sp2PerspectiveTable(self.df, index_col=index_col, time_col=time_col)
        ...
        table = Sp2PerspectiveTable(self.df, index_col=index_col, time_col=time_col, limit=3)
    ...
    def test_real_time(self):
        table = Sp2PerspectiveTable(self.df, keep_history=False)
    ...
    def test_empty(self):
        table = Sp2PerspectiveTable(self.df.sp2.static_frame(), keep_history=True)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        table.join()
        df1 = table.to_df()
        ...
        table = Sp2PerspectiveTable(self.df.sp2.static_frame(), time_col=None, keep_history=False)
        table.start(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        table.join()
        df2 = table.to_df()
        pd.testing.assert_frame_equal(df2, self.df.sp2.static_frame().reset_index())
    ...
    def test_get_widget(self):
        ...
        table = Sp2PerspectiveTable(self.df, index_col="my_index", time_col=None, keep_history=False)
        widget = table.get_widget()
        self.assertIsInstance(widget, perspective.PerspectiveWidget)
    ...
    def test_multi_table(self):
        tables = {"test1": Sp2PerspectiveTable(self.df), "test2": Sp2PerspectiveTable(self.df, keep_history=False)}
        tables["test3"] = tables["test1"]
        multi = Sp2PerspectiveMultiTable(tables)
        ...
        multi.stop()
        self.assertEqual(len(multi["test1"].to_df()), 6)  # From the two sp2.const ticks
        self.assertEqual(len(multi["test2"].to_df()), 3)
        self.assertEqual(len(multi["test3"].to_df()), len(multi["test1"].to_df()))
