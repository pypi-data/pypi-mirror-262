...
import sp2
from sp2.impl.pandas_ext_type import TsArray, TsDtype
...
def edge_equality_mock(monkeypatch):
    """Because sp2 defines Edge equality to return another Edge, but the provided pandas tests rely on a standard
...
def data(pytype):
    ...
    * data[0] and data[1] should not be equal
    """
    return TsArray([sp2.const(pytype(i + 1)) for i in range(100)])


@pytest.fixture
def data_missing(pytype):
    """Length-2 array with [NA, Valid]"""
    return TsArray([np.NaN, sp2.const(pytype(1))])
...
def data_for_grouping(dtype, pytype):
    ...
    """
    a = sp2.const(pytype(0))
    b = sp2.const(pytype(1))
    c = sp2.const(pytype(2))
    na = np.nan
...
def data_for_twos(pytype):
    ...
    return TsArray([sp2.const(pytype(2)) for _ in range(100)])
