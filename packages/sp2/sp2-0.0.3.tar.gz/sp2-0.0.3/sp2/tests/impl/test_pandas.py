...
import sp2
from sp2.impl.pandas import make_pandas
...
class TestPandas(unittest.TestCase):
    def test_make_pandas_basic(self):
        ...
        x = sp2.count(sp2.timer(dt1))
        y = sp2.count(sp2.timer(dt2))
        out1 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 20, None), starttime=start, endtime=end)[0]
        out2 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 20, tindex=x), starttime=start, endtime=end)[0]
        out3 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 2, tindex=x), starttime=start, endtime=end)[0]
    ...
    def test_make_pandas_window(self):
        ...
        out1 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 2, None), starttime=start, endtime=end)[0]
        # Test that a timedelta window works the same way
        out2 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, dt1, None), starttime=start, endtime=end)[0]
        # Test a timedelta window that's too small
        out3 = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, dt1 / 2, None), starttime=start, endtime=end)[0]
        for out in [out1, out2, out3]:
    ...
    def test_make_pandas_init(self):
        ...
        out2 = sp2.run(lambda: make_pandas(q, {"x": x, "y": y}, 20, wait_all_valid=True), starttime=start, endtime=end)[
            0
        ]
...
class TestDataFrame(unittest.TestCase):
    def test_pandas_ts(self):
        ...
        x = sp2.count(sp2.timer(dt1))
        y = sp2.count(sp2.timer(dt2))
        df = sp2.DataFrame({"x": x, "y": y})

        # Case 1: Simplest use case
        out = sp2.run(lambda: df.to_pandas_ts("y", 2), starttime=start, endtime=end)[0]
        target = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 2), starttime=start, endtime=end)[0]
        ...
        # Case 2: Pass tindex
        out = sp2.run(lambda: df.to_pandas_ts("y", 2, "x"), starttime=start, endtime=end)[0]
        target = sp2.run(lambda: make_pandas(y, {"x": x, "y": y}, 2, tindex=x), starttime=start, endtime=end)[0]
        self.assertEqual(len(out), len(target))
        for i in range(len(out)):
            self.assertEqual(out[i][0], target[i][0])
            pd.testing.assert_frame_equal(out[i][1], target[i][1])
        ...
        out = sp2.run(lambda: df.to_pandas_ts(df["x"], 2, df["x"], wait_all_valid=False), starttime=start, endtime=end)[
            0
        ]
        target = sp2.run(
    ...
    def test_pandas_ts_types(self):
        ...
        x = sp2.count(sp2.timer(dt1))
        y = sp2.count(sp2.timer(dt2))
        df = sp2.DataFrame({("A", "x"): x, ("B", "x"): sp2.const("Test"), ("A", "y"): y, ("B", "y"): sp2.const(start)})
        out = sp2.run(lambda: df.to_pandas_ts(("A", "y"), 2, ("A", "x")), starttime=start, endtime=end)[0]
        self.assertEqual(len(out), 3)
        idx = pd.DatetimeIndex([start + dt2 + dt1, start + dt2 + 2 * dt1])
        target = pd.DataFrame(
