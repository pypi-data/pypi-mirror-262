...
from sp2.impl.pushpulladapter import PushGroup, PushPullInputAdapter
from sp2.impl.wiring import py_pushpull_adapter_def
...
class TestPushPullAdapter(unittest.TestCase):
    def test_basic(self):
        ...
        def graph(num_sim, num_rt, num_sim_repeats):
            ...
            stop = sp2.count(nc) == expected_ticks
    ...
    def test_out_of_order(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "tried to push a sim tick after live tick"):
            sp2.run(graph, starttime=datetime.utcnow(), endtime=timedelta(seconds=10))
    ...
    def test_historical(self):
        ...
        graph_out = sp2.run(graph, starttime=datetime.utcnow(), endtime=timedelta(seconds=10), realtime=False)
