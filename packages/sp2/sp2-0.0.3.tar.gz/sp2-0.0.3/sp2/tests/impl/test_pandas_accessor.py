...
import sp2
import sp2.impl.pandas_accessor
from sp2.impl.pandas_ext_type import TsDtype
from sp2.impl.wiring.edge import Edge

_ = sp2.impl.pandas_accessor  # To prevent IDE from removing import
...
class TestSp2SeriesAccessor(unittest.TestCase):
    def setUp(self) -> None:
        idx = ["ABC", "DEF", "GJH"]
        self.series = pd.Series(
            [sp2.const(99.0), sp2.timer(timedelta(seconds=1), 103.0), np.nan], dtype=TsDtype(float), index=idx
        )
    ...
    def test_apply(self):
        ...
        out = self.series.sp2.apply(f, 1, z=2)
        ...
        out = self.empty.sp2.apply(f, 1, z=2)
        self.assertEqual(out.dtype, self.empty.dtype)

    def test_pipe(self):
        trigger = sp2.timer(timedelta(hours=1))
        out = self.series.sp2.pipe((sp2.sample, "x"), trigger=trigger)
        for idx, edge in self.series.items():
            if pd.isnull(edge):
                self.assertTrue(pd.isnull(out[idx]))
            else:
                self.assertTrue(edge_eq(out[idx], edge.pipe((sp2.sample, "x"), trigger=trigger)))
    ...
    def test_binop(self):
        flag = pd.Series(
            [sp2.const(True), sp2.timer(timedelta(seconds=1), False), np.nan],
            dtype=TsDtype(bool),
            index=self.series.index,
        )
        out = flag.sp2.binop(sp2.filter, self.series)
        for idx, edge in self.series.items():
            if pd.isnull(edge):
                self.assertTrue(pd.isnull(out[idx]))
            else:
                self.assertTrue(edge_eq(out[idx], sp2.filter(flag[idx], edge)))

        flag = flag.reset_index(drop=True)
        self.assertRaises(ValueError, flag.sp2.binop, sp2.filter, self.series)

        flag = pd.Series([], dtype=TsDtype(bool))
        out = flag.sp2.binop(sp2.filter, self.empty)
        self.assertEqual(out.dtype, flag.dtype)

    def test_sample(self):
        trigger = sp2.timer(timedelta(hours=1))
        out1 = self.series.sp2.sample(trigger)
        out2 = self.series.sp2.sample(timedelta(hours=1))
        out3 = self.series.sp2.sample(pd.Timedelta(hours=1))
        for idx, edge in self.series.items():
            if pd.isnull(edge):
                ...
                self.assertTrue(edge_eq(out1[idx], sp2.sample(trigger, edge)))
                self.assertTrue(edge_eq(out2[idx], sp2.sample(trigger, edge)))
                self.assertTrue(edge_eq(out3[idx], sp2.sample(trigger, edge)))

        out = self.empty.sp2.sample(trigger)
        self.assertEqual(out.dtype, self.empty.dtype)
    ...
    def test_flatten(self):
        ...
        series = pd.Series([sp2.const(MyStruct(a=4.0, b="x", c=SubStruct()))], dtype=TsDtype(MyStruct), name="test")
        out = series.sp2.flatten()
        ...
        # Change delimeter
        out = series.sp2.flatten(delim="_")
        ...
        # Don't prepend name
        out = series.sp2.flatten(prepend_name=False)
        ...
        # Not recursive
        out = series.sp2.flatten(recursive=False)
    ...
    def test_valid(self):
        out = sp2.run(self.series.sp2.valid, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=1))
        self.assertEqual(out, {0: [(datetime(2020, 1, 1, 0, 0, 1), True)]})

    def test_synchronize(self):
        x = sp2.const(100.0)
        series = pd.Series(
            [x, sp2.delay(x, timedelta(seconds=1)), np.nan, sp2.delay(x, timedelta(seconds=2))], dtype=TsDtype(float)
        )
        out = series.sp2.synchronize(timedelta(seconds=10)).sp2.run(
        ...
        out = series.sp2.synchronize(timedelta(seconds=1)).sp2.run(
    ...
    def test_run(self):
        out = self.series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        ...
        out = self.series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=3), tick_count=2)
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("DEF", pd.Timestamp("2020-01-01 00:00:02")),
        ...
        out = self.series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(0))
    ...
    def test_run_empty(self):
        out = self.empty.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        self.assertEqual(len(out), 0)
        self.assertEqual(out.index.nlevels, 2)
        self.assertIsInstance(out.index.levels[-1], pd.DatetimeIndex)

        out = self.empty_multi.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        self.assertEqual(len(out), 0)
    ...
    def test_run_repeat(self):
        ...
        t = datetime(2020, 1, 1)
        data = sp2.curve(float, [(t, 99.0), (t, 98.0)])
        self.series = pd.Series([data, data + 4, np.nan], dtype=TsDtype(float), index=self.series.index)
        out = self.series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=0))
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("ABC", pd.Timestamp("2020-01-01")),
    ...
    def test_snap(self):
        out = self.series.sp2.snap(starttime=datetime(2020, 1, 1))
        idx = pd.Index(["ABC", "DEF"])
        target = pd.Series([99.0, 103.0], index=idx)
        pd.testing.assert_series_equal(out, target)

        out = self.series.sp2.snap(timeout=timedelta(seconds=0.5), starttime=datetime(2020, 1, 1))
        idx = pd.Index(["ABC"])
        target = pd.Series([99.0], index=idx)
        pd.testing.assert_series_equal(out, target)
    ...
    def test_snap_nodata(self):
        self.series = self.series[["DEF", "GJH"]]
        out = self.series.sp2.snap(timeout=timedelta(0))
        self.assertEqual(len(out), 0)
        self.assertEqual(out.dtype, float)
        self.assertEqual(out.index.dtype, self.series.index.dtype)

    def test_snap_empty(self):
        out = self.empty.sp2.snap(starttime=datetime(2020, 1, 1))
        pd.testing.assert_index_equal(out.index, self.empty.index)

        out = self.empty_multi.sp2.snap(starttime=datetime(2020, 1, 1))
        pd.testing.assert_index_equal(out.index, self.empty_multi.index)


class TestSp2DataFrameAccessor(unittest.TestCase):
    def setUp(self) -> None:
        self.idx = ["ABC", "DEF", "GJH"]
        bid = pd.Series(
            [sp2.const(99.0), sp2.timer(timedelta(seconds=1), 103.0), np.nan], dtype=TsDtype(float), index=self.idx
        )
        ask = pd.Series(
            [sp2.const(100.0), sp2.timer(timedelta(seconds=2), 104.0), sp2.const(100.0)],
            dtype=TsDtype(float),
            index=self.idx,
        )
        sector = ["X", "Y", "X"]
    ...
    def test_run(self):
        out = self.df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("DEF", pd.Timestamp("2020-01-01 00:00:01")),
        ...
        out = self.df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2), tick_count=1)
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("DEF", pd.Timestamp("2020-01-01 00:00:02")),
    ...
    def test_run_ts_only(self):
        out = self.df.sp2.ts_frame().sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("DEF", pd.Timestamp("2020-01-01 00:00:01")),
    ...
    def test_run_static_only(self):
        out = self.df.sp2.static_frame().sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
    ...
    def test_run_nodata(self):
        ...
        out = self.df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=0))
    ...
    def test_run_duplicate_static(self):
        idx = ["ABC", "DEF", "ABC"]
        self.df.index = idx
        self.assertRaises(ValueError, self.df.sp2.run, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
    ...
    def test_run_repeat(self):
        ...
        t = datetime(2020, 1, 1)
        bid = pd.Series(
            [sp2.curve(float, [(t, 99.0), (t, 98.0)]), sp2.curve(float, [(t, 103.0), (t, 102.0)]), np.nan],
            dtype=TsDtype(float),
            index=self.idx,
        )
        ask = pd.Series(
    ...
    def test_run_types(self):
        ...
        self.idx = ["ABC", "DEF", "GJH"]
        s_str = pd.Series([sp2.const("a") for _ in self.idx], dtype=TsDtype(str), index=self.idx)
        s_int = pd.Series([sp2.const(0) for _ in self.idx], dtype=TsDtype(int), index=self.idx)
        s_float = pd.Series([sp2.const(0.0) for _ in self.idx], dtype=TsDtype(float), index=self.idx)
        s_bool = pd.Series([sp2.const(True) for _ in self.idx], dtype=TsDtype(bool), index=self.idx)
        s_date = pd.Series([sp2.const(date.min) for _ in self.idx], dtype=TsDtype(date), index=self.idx)
        s_object = pd.Series([sp2.const(MyObject()) for _ in self.idx], dtype=TsDtype(MyObject), index=self.idx)
        self.df = pd.DataFrame(
            {
                "s_str": s_str,
                "s_int": s_int,
    ...
    def test_snap(self):
        out = self.df.sp2.snap(starttime=datetime(2020, 1, 1))
    ...
    def test_snap_ts_only(self):
        out = self.df.sp2.ts_frame().sp2.snap(starttime=datetime(2020, 1, 1))
    ...
    def test_snap_static_only(self):
        out = self.df.sp2.static_frame().sp2.snap(starttime=datetime(2020, 1, 1))
        pd.testing.assert_frame_equal(out, self.df.sp2.static_frame().iloc[[]])

    def test_snap_nodata(self):
        self.df = self.df.loc[["DEF"]]
        out = self.df.sp2.snap(timeout=timedelta(seconds=0))
        self.assertTrue(out.empty)
        pd.testing.assert_index_equal(out.columns, pd.Index(["name", "bid", "ask", "sector"]))
    ...
    def test_sample(self):
        trigger = sp2.timer(timedelta(hours=1))
        out1 = self.df.sp2.sample(trigger)
        out2 = self.df.sp2.sample(timedelta(hours=1))
        out3 = self.df.sp2.sample(pd.Timedelta(hours=1))
        target = self.df.copy()
        target["bid"] = target["bid"].sp2.sample(trigger)
        target["ask"] = target["ask"].sp2.sample(trigger)
        start = datetime(2020, 1, 1)
        end = datetime(2020, 1, 2)
        target = target.sp2.run(start, end)
        pd.testing.assert_frame_equal(out1.sp2.run(start, end), target)
        pd.testing.assert_frame_equal(out2.sp2.run(start, end), target)
        pd.testing.assert_frame_equal(out3.sp2.run(start, end), target)

        self.df.sp2.sample(trigger, inplace=True)
        pd.testing.assert_frame_equal(self.df.sp2.run(start, end), target)

    def test_collect(self):
        out = self.df.sp2.collect()
        ...
        self.assertTrue(issubclass(C, sp2.Struct))
        ...
        out = out.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        idx = pd.MultiIndex.from_tuples(
            [
                ("ABC", pd.Timestamp("2020-01-01")),
                ("DEF", pd.Timestamp("2020-01-01 00:00:01")),
        ...
        out = self.df.sp2.collect(columns=["bid"], struct_type=MyStruct)
        self.assertIsInstance(out, pd.Series)
    ...
    def test_collect_nested(self):
        ...
        series = pd.Series([sp2.const(MyStruct(a=4.0, b="x", c=SubStruct()))], dtype=TsDtype(MyStruct))
        df = series.sp2.flatten()
        out = df.sp2.collect()
        ...
        out = df.sp2.collect(columns=["a", "c fld"], struct_type=MyStruct)
        self.assertIsInstance(out, pd.Series)
        self.assertEqual(out.dtype, TsDtype(MyStruct))
        out = out.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
    ...
    def _performance_test(self, starttime, endtime, N, M, dt, offset, collect, seed=1234, profile=False):
        ...
        fields = {
            f"field_{i}": pd.Series(
                [sp2.timer(dt + i * offset, 100.0 + i) for _ in self.idx], dtype=TsDtype(float), index=self.idx
            )
            for i in range(M)
        }
        ...
        if profile:
            ...
            pr.enable()
        start = time.time()
        out = self.df.sp2.run(starttime=starttime, endtime=endtime, collect=collect)
        end = time.time()
...
class TestToSp2SeriesAccessor(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_to_sp2_null(self):
        inp = pd.Series([], index=pd.DatetimeIndex([]), dtype=float)
        edge = inp.to_sp2()
        self.assertEqual(edge.tstype.typ, float)
        self.assertEqual(edge.nodedef.__class__, sp2.null_ts)

    def test_to_sp2_float(self):
    ...
    def test_to_sp2_float_drop_na(self):
        inp = pd.Series([99.0, np.nan, 103.0], index=self.idx)
        edge = inp.to_sp2(drop_na=True)
        out = edge.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        target = [(datetime(2020, 1, 1, 0, 0), 99.0), (datetime(2020, 1, 1, 0, 0, 2), 103.0)]
        self.assertListEqual(out, target)

    def test_to_sp2_int(self):
    ...
    def test_to_sp2_multiindex(self):
        inp = pd.Series([99.0, 103.0, 103.0], index=self.midx)
        series = inp.to_sp2()
        self.assertEqual(series.dtype, TsDtype(float))
        out = series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
    ...
    def test_to_sp2_multiindex_drop_na(self):
        inp = pd.Series([99.0, np.nan, 103.0], index=self.midx)
        series = inp.to_sp2(drop_na=True)
        self.assertEqual(series.dtype, TsDtype(float))
        out = series.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        pd.testing.assert_series_equal(out, inp.dropna())


class TestToSp2FrameAccessor(unittest.TestCase):
    def setUp(self) -> None:
        self.idx = pd.DatetimeIndex(
            [pd.Timestamp("2020-01-01"), pd.Timestamp("2020-01-01 00:00:01"), pd.Timestamp("2020-01-01 00:00:02")]
        )
    ...
    def test_single_index(self):
        ...
        out = df.to_sp2(columns=["bid", "ask"], agg=pd.Series.mode)
        self.assertIsInstance(out, dict)
    ...
    def test_multiindex(self):
        ...
        out = df.sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        pd.testing.assert_frame_equal(out, self.df)

        df = self.df.to_sp2(columns=["bid", "ask"])
        np.testing.assert_array_equal(df.columns, self.df.columns)
        ...
        df = self.df.to_sp2(columns=["bid", "ask"], agg="first")
        ...
        df = self.df.to_sp2(columns=[], agg="first")
    ...
    def test_multiindex_drop_na(self):
        df = self.df.to_sp2(drop_na=True)
        np.testing.assert_array_equal(df.columns, self.df.columns)
        self.assertEqual(df["name"].dtype, TsDtype(str))
        self.assertEqual(df["bid"].dtype, TsDtype(float))
        self.assertEqual(df["ask"].dtype, TsDtype(float))
        ...
        out = df["ask"].sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        pd.testing.assert_series_equal(out, self.df["ask"].dropna())

        out = df["bid"].sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
        pd.testing.assert_series_equal(out, self.df["bid"].dropna())

        out = df[["name", "sector"]].sp2.run(starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=2))
