...
import sp2
from sp2.adapters.output_adapters.parquet import ParquetOutputConfig
from sp2.adapters.parquet import ParquetReader, ParquetWriter


class PriceQuantity(sp2.Struct):
    PRICE: float
    SIZE: int
    SIDE: str
    SYMBOL: str
...
class PriceQuantity2(sp2.Struct):
    price: float
    quantity: int
    side: str
...
class TestParquetReader(unittest.TestCase):
    def do_test_body(self, filename, arrow=False):
        @sp2.graph
        def write_ts_to_file(x: sp2.ts["T"]):
            temp_file = tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w")
            temp_file.close()
            sp2.schedule_on_engine_stop(lambda: os.unlink(temp_file.name))
            if arrow:
        ...
        def graph():
            ...
            sp2.add_graph_output("aapl", aapl)
            sp2.add_graph_output("ibm", ibm)
            sp2.add_graph_output("aapl2", aapl2)
            sp2.add_graph_output("aapl_price", aapl_price)
            sp2.add_graph_output("all", all)

        result = sp2.run(graph, starttime=datetime(2020, 3, 3, 9, 30))
    ...
    def test_parquet_output_file_rotation(self):
        ...
        def g(d: str, empty_last_file: bool = True):
            ...
            data = sp2.curve(int, values)
            rotate_file = sp2.curve(
                str,
                [
            ...
            parquet_writer = ParquetWriter(
                ...
                timestamp_column_name="sp2_timestamp",
        ...
        with tempfile.TemporaryDirectory() as d:
            sp2.run(g, d, starttime=datetime.now(), endtime=timedelta(seconds=20))
        ...
        with tempfile.TemporaryDirectory() as d:
            sp2.run(g, d, False, starttime=datetime.now(), endtime=timedelta(seconds=20))
            dfs = [pandas.read_parquet(os.path.join(d, f"out{i}.parquet")) for i in range(5)]
            self.assertEqual(dfs[0].my_out.tolist(), list(range(4)))
            self.assertEqual(dfs[1].my_out.tolist(), [4])
            self.assertEqual(dfs[2].my_out.tolist(), [])
    ...
    def test_parquet_writer(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            for timestamp_column_name in ["timestamp", None]:
                ...
                @sp2.graph
                def graph():
                    x = sp2.curve(int, [(timedelta(seconds=v + 1), v + 1) for v in range(10)])
                    y = sp2.curve(float, [(timedelta(seconds=v + 1), v * 10.0) for v in range(10)])
                ...
                sp2.run(graph, starttime=start_time)
    ...
    def test_parquet_multiple_values_same_tick(self):
        ...
        def writer_g(output_file: str):
            symbol = sp2.curve(str, [(timedelta(0), "AAPL"), (timedelta(0), "IBM"), (timedelta(0), "Z")])
            value = sp2.curve(float, [(timedelta(0), 100.0), (timedelta(0), 200.0), (timedelta(0), 300.0)])
            parquet_writer = ParquetWriter(output_file, "sp2_timestamp")
            parquet_writer.publish("symbol", symbol)
            parquet_writer.publish("value", value)
            sp2.add_graph_output("symbol", symbol)
            sp2.add_graph_output("value", value)

        @sp2.graph
        def reader_g1(input_file: str):
            reader = ParquetReader(input_file, "symbol", "sp2_timestamp")
            sp2.add_graph_output("symbol", reader.subscribe_all(str, "symbol"))
            sp2.add_graph_output("value", reader.subscribe_all(float, "value"))

        @sp2.graph
        def reader_g2(input_file: str):
            reader = ParquetReader(input_file, "symbol", "sp2_timestamp")
            reader_all = ParquetReader(input_file, "symbol", "sp2_timestamp")
            ...
            sp2.add_graph_output("all_symbols", reader_all.subscribe_all(str, "symbol"))
            sp2.add_graph_output("all_values", reader_all.subscribe_all(float, "value"))
        ...
        with tempfile.TemporaryDirectory(prefix="sp2_unit_tests") as tmp_folder:
            file_name = os.path.join(tmp_folder, "data.parquet")
            ref_data = sp2.run(writer_g, file_name, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=10))
            read_data1 = sp2.run(reader_g1, file_name, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=10))
            self.assertEqual(ref_data, read_data1)
            read_data2 = sp2.run(reader_g2, file_name, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=10))
            read_data3 = sp2.run(reader_g3, file_name, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=10))
    ...
    def test_duplicate_field_name(self):
        ...
        def writer_graph(file_name, arrow: bool = False):
            ...
            with self.assertRaisesRegex(KeyError, ".*Publishing duplicate column names in parquet/arrow file: value.*"):
                parquet_writer.publish("value", sp2.null_ts(int))

        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as f:
            f.close()
            sp2.run(writer_graph, file_name=f.name, starttime=datetime(2020, 1, 1), endtime=timedelta(seconds=10))
            os.unlink(f.name)
    ...
    def test_non_python_c_fields(self):
        ...
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as f:
            ...
            for field_type in (int, float):
                ...
                res = sp2.run(g, starttime=start_t, endtime=timedelta(seconds=10))
    ...
    def test_in_memory_arrow_read(self):
        ...
        def read_tables(tables: object):
            ...
            sp2.add_graph_output("o", records)
        ...
        resT1 = sp2.run(read_tables, t1, starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t1.to_pandas() == res_to_df(resT1)).all().all())
        # Single table with multiple chunks of data in it
        resT = sp2.run(read_tables, pyarrow.concat_tables([t1, t2]), starttime=s, endtime=timedelta(seconds=10))
        ...
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # List of tables
        resT = sp2.run(read_tables, [t1, t2], starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # List of tables with empty tables
        resT = sp2.run(read_tables, [empty_t, t1, empty_t, t2, empty_t], starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # Generator of tables
        resT = sp2.run(read_tables, (v for v in [t1, t2]), starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # A function that's given the start and end time of the run generates the tables
        resT = sp2.run(read_tables, lambda s, e: (v for v in [t1, t2]), starttime=s, endtime=timedelta(seconds=10))
        self.assertTrue((t.to_pandas() == res_to_df(resT)).all().all())
        # Empty table
        resEmpty = sp2.run(read_tables, empty_t, starttime=s, endtime=timedelta(seconds=10))
        self.assertEqual(len(resEmpty["o"]), 0)
        resEmpty = sp2.run(read_tables, [empty_t, empty_t], starttime=s, endtime=timedelta(seconds=10))
        self.assertEqual(len(resEmpty["o"]), 0)
        with self.assertRaisesRegex(TypeError, ".*Expected PyTable from generator, got str.*"):
            sp2.run(read_tables, ["dummy"], starttime=s, endtime=timedelta(seconds=10))
    ...
    def test_bytes_read_write(self):
        ...
        def g_write(temp_file: object):
            writer = ParquetWriter(temp_file, "sp2_timestamp")
            writer.publish("value", sp2.const(VALUE))

        @sp2.node
        def pass_through(x: sp2.ts[bytes]) -> sp2.ts[bytes]:
            self.assertEqual(x, VALUE)
            return x

        @sp2.graph
        def g_read(temp_file: object) -> sp2.Outputs(v1=sp2.ts[bytes], v2=sp2.ts[bytes]):
            reader = ParquetReader(temp_file, time_column="sp2_timestamp")
            res = reader.subscribe_all(bytes, "value")
            return sp2.output(v1=res, v2=pass_through(res))

        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file:
            sp2.set_capture_cpp_backtrace(True)
            temp_file.close()
            s = datetime(2021, 1, 1)
            e = timedelta(seconds=10)
            sp2.run(g_write, temp_file.name, starttime=s, endtime=e)
            res = sp2.run(g_read, temp_file.name, starttime=s, endtime=e)
    ...
    def test_parquet_mismatched_column_type_error(self):
        ...
        def g_write(temp_file: object):
            writer = ParquetWriter(temp_file, "sp2_timestamp")
            writer.publish("symbol", sp2.const(123))
            writer.publish("value", sp2.const(1))

        @sp2.graph
        def g_read(temp_file: object) -> sp2.ts[int]:
            reader = ParquetReader(temp_file, symbol_column="symbol", time_column="sp2_timestamp")
            res = reader.subscribe("sym", int, "value")
            return res

        @sp2.graph
        def g_read2(temp_file: object) -> sp2.ts[str]:
            reader = ParquetReader(temp_file, time_column="sp2_timestamp")
        ...
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file:
            ...
            with self.assertRaisesRegex(
                ...
                sp2.run(g_read, temp_file.name, starttime=s, endtime=e)
            with self.assertRaisesRegex(
                TypeError, ".*Unexpected column type for column value , expected STRING got int64.*"
            ):
                sp2.run(g_read2, temp_file.name, starttime=s, endtime=e)

    def test_parquet_schema_change(self):
        class SubStruct(sp2.Struct):
            value2: int
        ...
        def g_write(temp_file: object, base_value: int, write_struct2: bool = False, write_value2: bool = False):
            writer = ParquetWriter(temp_file, "sp2_timestamp")
            writer.publish("symbol", sp2.const("sym"))
            if write_struct2:
                sub_struct = SubStruct()
                res_struct = MyStruct2(value=base_value, sub=sub_struct)
                if write_value2:
                    sub_struct.value2 = base_value * 10
                writer.publish_struct(sp2.const(res_struct))
            else:
                writer.publish_struct(sp2.const(MyStruct(value=base_value)))

        @sp2.graph
        def g_read(temp_files: object, read_value2: bool = False, allow_missing_columns: bool = True) -> sp2.Outputs(
            value=sp2.ts[int], value2=sp2.ts[int]
        ):
            reader = ParquetReader(
                temp_files,
                symbol_column="symbol",
                time_column="sp2_timestamp",
            ...
            if read_value2:
                ...
                value2 = sp2.null_ts(int)
            return sp2.output(value=value, value2=value2)

        @sp2.graph
        def g_read_struct(temp_files: object, struct_type: "T", allow_missing_columns: bool = True) -> sp2.ts["T"]:
            reader = ParquetReader(
                temp_files,
                symbol_column="symbol",
                time_column="sp2_timestamp",
        ...
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file1:
            temp_file1.close()
            with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file2:
                temp_file2.close()
                with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file3:
                    temp_file3.close()
                    with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file4:
                        temp_file4.close()
                        ...
                        sp2.run(g_write, temp_file1.name, 1, starttime=s1, endtime=e1)
                        sp2.run(g_write, temp_file2.name, 2, True, starttime=s2, endtime=e2)
                        sp2.run(g_write, temp_file3.name, 3, True, True, starttime=s3, endtime=e3)
                        sp2.run(g_write, temp_file4.name, 4, starttime=s4, endtime=e4)
                        ...
                        with self.assertRaisesRegex(RuntimeError, ".*Missing column sub.*"):
                            sp2.run(g_read, file_names, True, starttime=s1, endtime=e4, allow_missing_columns=False)
                        ...
                        res = sp2.run(g_read, file_names, True, starttime=s1, endtime=e4)
                        res2 = sp2.run(
                            g_read_struct, file_names, MyStruct, starttime=s1, endtime=e4, allow_missing_columns=False
                        )
                        res3 = sp2.run(g_read_struct, file_names, MyStruct2, starttime=s1, endtime=e4)
    ...
    def test_parquet_missing_files_read(self):
        ...
        def g_write(f1: str, f2: str):
            ...
            writer1.publish("value", sp2.curve(int, [(timedelta(seconds=1), 1), (timedelta(seconds=2), 2)]))
            writer2.publish("value", sp2.curve(int, [(timedelta(seconds=3), 3), (timedelta(seconds=4), 4)]))

        @sp2.graph
        def g_read(file_names: object, allow_missing_files: bool = False) -> sp2.ts[int]:
        ...
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests_f1", mode="w") as temp_file1:
            temp_file1.close()
            with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests_f2", mode="w") as temp_file2:
                temp_file2.close()
                sp2.set_capture_cpp_backtrace(True)
                sp2.run(
                    g_write,
    ...
    def test_bad_field_map(self):
        ...
        sp2.run(g, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))

    def test_numpy_array_on_struct_with_field_map(self):
        class MyStruct(sp2.Struct):
            v1: int
            v2: str
            v3: sp2.typing.Numpy1DArray[str]
            v4: sp2.typing.Numpy1DArray[int]
        ...
        @sp2.graph
        def write_data(file_name: str) -> sp2.ts[MyStruct]:
        ...
        @sp2.graph
        def read_data(file_name: str) -> sp2.ts[MyStruct]:
        ...
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file:
            temp_file.close()
            v1 = sp2.run(write_data, temp_file.name, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
            v2 = sp2.run(read_data, temp_file.name, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
    ...
    def test_string_array_with_nulls(self):
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file:
            ...
            def reader_g():
                ...
                return reader.subscribe_all(sp2.typing.Numpy1DArray[str], "v")

            with self.assertRaisesRegex(
                ValueError, ".*Can't read empty value to array from arrow array of type utf8.*"
            ):
                sp2.run(reader_g, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
    ...
    def test_float_array_with_nulls(self):
        with tempfile.NamedTemporaryFile(prefix="sp2_unit_tests", mode="w") as temp_file:
            ...
            def reader_g():
                reader = ParquetReader(temp_file.name, time_column="t", tz=pytz.utc)
                return reader.subscribe_all(sp2.typing.Numpy1DArray[float], "v")

            res = sp2.run(reader_g, starttime=datetime(2022, 1, 1), endtime=timedelta(seconds=10))
            res_a = res[0][0][1]
    ...
    def test_all_types(self):
        ...
        class BaseNative(sp2.Struct):
            i: int
            b: bool
            f: float

        class AllTypes(sp2.Struct):
            b: bool
            i: int
            d: float
            dt: datetime
        ...
        @sp2.graph
        def write_data(file_name: str, binary_arrow: bool) -> sp2.ts[AllTypes]:
            parquet_writer = ParquetWriter(
                file_name=file_name,
                timestamp_column_name="TIME",
            ...
            values = sp2.curve(sType, [(timedelta(0), v1), (timedelta(0), v2), (timedelta(seconds=5), v3)])
        ...
        @sp2.node
        def _check_equal(field: str, x: sp2.ts[object], y: sp2.ts[object]):
            with sp2.state():
                s_count = 0
        ...
        @sp2.graph
        def read_data(file_name: str, binary_arrow: bool) -> sp2.ts[AllTypes]:
    ...
    def test_write_metadata(self):
        ...
        def write_data(
            ...
            values = sp2.const(MyStruct(a=1, b=2.0))
            parquet_writer.publish_struct(values)
            return values
    ...
    def test_parquet_int_symbol(self):
        @sp2.node
        def _mod(x: sp2.ts[int], mod: int) -> sp2.ts[int]:
            return x % mod
        ...
        def g_write(temp_file: object):
            ...
            data = sp2.count(sp2.timer(timedelta(seconds=0.1)))
            symbol = _mod(data, 3)
        ...
        def g_read(temp_file: object):
            ...
            for symbol in range(3):
                sp2.add_graph_output(symbol, reader.subscribe(symbol, int, "value"))
    ...
    def test_parquet_polars_read(self):
        ...
        def test_tz(file_name: str, start: datetime):
            data = sp2.curve(float, [(start, 1.0)])

            writer = ParquetWriter(
                file_name=file_name, timestamp_column_name="sp2_time", config=ParquetOutputConfig(allow_overwrite=True)
            )
        ...
        with tempfile.NamedTemporaryFile(suffix=".parquet") as output:
            output.file.close()
            g = sp2.run(test_tz, output.name, start, starttime=start)

            struct_df = polars.read_parquet(output.name)

            self.assertEqual(len(struct_df), 1)
            self.assertEqual(struct_df["sp2_time"][0].tzinfo.key, "UTC")
            self.assertEqual(struct_df["sp2_time"][0].timestamp(), start.timestamp())
