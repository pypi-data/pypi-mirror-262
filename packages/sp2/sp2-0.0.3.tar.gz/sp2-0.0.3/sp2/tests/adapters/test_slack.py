...
import sp2
from sp2 import ts
from sp2.adapters.slack import SlackAdapterManager, SlackMessage, mention_user
...
def send_fake_message(clientmock: MagicMock, requestmock: MagicMock, am: SlackAdapterManager) -> ts[bool]:
    with sp2.alarms():
        a_send = sp2.alarm(bool)
    with sp2.start():
        sp2.schedule_alarm(a_send, timedelta(seconds=1), True)
    if sp2.ticked(a_send):
        if a_send:
            am._process_slack_message(clientmock, requestmock)
            sp2.schedule_alarm(a_send, timedelta(seconds=1), False)
        else:
            return True
...
class TestSlack:
    ...
    def test_slack(self, payload):
        with patch("sp2.adapters.slack.SocketModeClient") as clientmock:
            ...
            def graph():
                ...
                # send a response
                resp = hello(sp2.unroll(am.subscribe()))
                am.publish(resp)

                # do a react
                rct = react(sp2.unroll(am.subscribe()))
                am.publish(rct)

                sp2.add_graph_output("response", resp)
                sp2.add_graph_output("react", rct)

                # stop after first messages
                done_flag = (sp2.count(stop) + sp2.count(resp) + sp2.count(rct)) == 3
                sp2.stop_engine(stop)

            # run the graph
            resp = sp2.run(graph, realtime=True)
