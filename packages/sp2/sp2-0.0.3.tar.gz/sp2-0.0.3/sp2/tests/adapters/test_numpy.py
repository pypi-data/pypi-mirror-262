...
def g(typ: object, dts: np.ndarray, values: np.ndarray):
    x = sp2.curve(typ=typ, data=(dts, values))
    sp2.add_graph_output("out", x)
...
class TestNumpyAdapter(unittest.TestCase):
    def test_int(self):
        raw_vals = [7, -13, 21]
        res = sp2.run(g, typ=int, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)
    ...
    def test_float(self):
        ...
        res = sp2.run(g, typ=float, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))
    ...
    def test_bool(self):
        ...
        res = sp2.run(g, typ=bool, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))
    ...
    def test_obj(self):
        ...
        res = sp2.run(g, typ=Foo, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))
    ...
    def test_complex(self):
        ...
        with self.assertRaisesRegex(ValueError, "numpy complex type only supported with dtype='object'"):
            sp2.run(g, typ=complex, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)

    def test_string(self):
        raw_vals = ["spam", "ff", "pancakes"]
        res = sp2.run(g, typ=str, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))
    ...
    def test_old_string(self):
        ...
        res = sp2.run(g, typ=str, values=np.array(raw_vals, dtype="S"), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))

    def test_char(self):
        raw_vals = ["a", "b", "c"]
        res = sp2.run(g, typ=str, values=np.array(raw_vals, dtype="c"), dts=test_dts_ndarray, starttime=test_starttime)
        self.assertEqual(res["out"], list(zip(test_dts, raw_vals)))
    ...
    def test_timestamps(self):
        ...
        res = sp2.run(g, typ=int, values=np.array(raw_vals), dts=dts_ndarray, starttime=starttime)
        ...
        with self.assertRaisesRegex(ValueError, "timestamps ndarray must be dtype of datetime64 or object"):
            res = sp2.run(g, typ=int, values=np.array(raw_vals), dts=np.array(raw_vals), starttime=starttime)
            self.assertEqual(res["out"], list(zip(dts, raw_vals)))
    ...
    def test_array(self):
        ...
        # res = sp2.run(g, typ=str, values=np.array(raw_vals, dtype='object'), dts=test_dts_ndarray, starttime=test_starttime)
        # self.assertEqual(res['out'], list(zip(test_dts, raw_vals)))
    ...
    def test_numpy_arrays(self):
        ...
        for dtype in ("f", "d", "object"):
            res = sp2.run(
                g,
                typ=sp2.typing.Numpy1DArray[float],
        ...
        for dtype in ("b", "h", "i", "l", "object"):
            res = sp2.run(
                g,
                typ=sp2.typing.Numpy1DArray[int],
                values=np.array(raw_vals, dtype=dtype),
                dts=test_dts_ndarray,
                starttime=test_starttime,
            )
        ...
        for dtype in ("U", "U10", "object"):
            res = sp2.run(
                g,
                typ=sp2.typing.Numpy1DArray[str],
                values=np.array(raw_vals, dtype=dtype),
                dts=test_dts_ndarray,
                starttime=test_starttime,
            )
        ...
        res = sp2.run(
            g,
            typ=sp2.typing.Numpy1DArray[Foo],
        ...
        for dtype in ("f", "d", "object"):
            res = sp2.run(
                g,
                typ=sp2.typing.NumpyNDArray[float],
                values=np.array(raw_vals, dtype=dtype),
                dts=test_dts_ndarray,
                starttime=test_starttime,
            )
        ...
        for dtype in ("b", "h", "i", "l", "object"):
            res = sp2.run(
                g,
                typ=sp2.typing.NumpyNDArray[int],
                values=np.array(raw_vals, dtype=dtype),
                dts=test_dts_ndarray,
                starttime=test_starttime,
            )
        ...
        res = sp2.run(
            g, typ=sp2.typing.NumpyNDArray[float], values=transposed, dts=test_dts_ndarray, starttime=test_starttime
        )
        ...
        res = sp2.run(
            g, typ=sp2.typing.NumpyNDArray[str], values=reshaped, dts=test_dts_ndarray, starttime=test_starttime
        )
        ...
        res = sp2.run(
            g, typ=sp2.typing.NumpyNDArray[float], values=flipped, dts=test_dts_ndarray, starttime=test_starttime
        )
        ...
        res = sp2.run(
            g, typ=sp2.typing.NumpyNDArray[float], values=broadcast, dts=test_dts_ndarray, starttime=test_starttime
        )
        ...
        res = sp2.run(g, typ=float, values=flipped_1D, dts=test_dts_ndarray, starttime=test_starttime)
    ...
    def test_forward_past_start(self):
        ...
        res = sp2.run(g, typ=int, values=np.array(raw_vals), dts=test_dts_ndarray, starttime=starttime)
    ...
    def test_ndarray_len_match(self):
        with self.assertRaisesRegex(ValueError, "ndarrays passed to sp2.curve must be of equal length"):
            sp2.run(g, typ=int, values=np.array([1, 2]), dts=test_dts_ndarray, starttime=test_starttime)

    def test_type_match(self):
        with self.assertRaisesRegex(ValueError, "numpy type .* requires float output type"):
            sp2.run(g, typ=int, values=np.array([1.1, 2.2, 3.3]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires int output type"):
            sp2.run(g, typ=float, values=np.array([1, 2, 3]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires int output type"):
            sp2.run(g, typ=str, values=np.array([1, 2, 3]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires bool output type"):
            sp2.run(g, typ=int, values=np.array([True, False, True]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires int output type"):
            sp2.run(g, typ=bool, values=np.array([1, 2, 3]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires string output type"):
            sp2.run(g, typ=float, values=np.array(["aa", "bb", "cc"]), dts=test_dts_ndarray, starttime=test_starttime)
        with self.assertRaisesRegex(ValueError, "numpy type .* requires datetime output type"):
            sp2.run(g, typ=int, values=test_dts_ndarray, dts=test_dts_ndarray, starttime=test_starttime)
