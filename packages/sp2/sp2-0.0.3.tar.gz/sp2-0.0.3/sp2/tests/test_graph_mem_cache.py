...
import sp2
from sp2 import ts
from sp2.impl.mem_cache import Sp2GraphObjectsMemCache, sp2_memoized
from sp2.impl.wiring.runtime import build_graph


class TestGraphMemCache(unittest.TestCase):
    def test_basic_functionality(self):
        with Sp2GraphObjectsMemCache.new_context():
            ...
            with Sp2GraphObjectsMemCache.new_context() as c:
                orig_context = c
    ...
    def _subgraph_aux(self, memoize=True, force_memoize=False):
        ...
        node_decor = (
            sp2.node if memoize and not force_memoize else sp2.node(None, memoize=False, force_memoize=force_memoize)
        )
        graph_decor = (
            sp2.graph if memoize and not force_memoize else sp2.graph(None, memoize=False, force_memoize=force_memoize)
        )

        @node_decor
        def f(
            inp_ts: sp2.ts["T"], inp_basket: [sp2.ts["T"]], inp_basket2: {str: sp2.ts["T"]}, scalar: object
        ) -> sp2.ts["T"]:
            if sp2.ticked(inp_ts):
                return inp_ts

        @graph_decor
        def sub_graph() -> sp2.Outputs(e1=ts[int], e2=ts[int], e3=ts[int], e4=ts[float]):
            mutable_count[0] += 1
            c1 = sp2.const(1)
            c2 = sp2.const(1)
            c3 = sp2.const(1.0)
            ...
            return sp2.output(e1=e1, e2=e2, e3=e3, e4=e4)
