...
import sp2
from sp2.random import brownian_motion, brownian_motion_1d, poisson_timer
from sp2.typing import Numpy1DArray, NumpyNDArray


class TestPoissonTimer(unittest.TestCase):
    def test_simple(self):
        rate = sp2.const(2.0)  # two events per second
        out = sp2.run(
    ...
    def test_delayed_start(self):
        rate = sp2.const(0.1, delay=timedelta(days=1))  # two events per second
        out = sp2.run(
    ...
    def test_changing_rate(self):
        ...
        out = sp2.run(poisson_timer, rate, seed=1234, starttime=datetime(2020, 1, 1), endtime=datetime(2020, 1, 1, 3))[
            0
        ]
...
class TestBrownianMotion(unittest.TestCase):
    def test_simple_increments(self):
        ...
        n_seconds = 2
        trigger = sp2.timer(timedelta(seconds=n_seconds))
        out = sp2.run(
            brownian_motion,
            trigger,
            sp2.const(mean),
    ...
    def test_brownian_motion_1d(self):
        ...
        out = sp2.run(
            brownian_motion,
            trigger,
            sp2.const(np.array([mean])),
            sp2.const(np.array([[cov]])),
            seed=1234,
            starttime=datetime(2020, 1, 1),
            endtime=timedelta(seconds=100),
        )[0]
    ...
    def test_change_at_trigger(self):
        ...
        cov = np.array([[0.0]])
        trigger = sp2.timer(timedelta(seconds=1))
        drift = sp2.curve(Numpy1DArray[float], [(datetime(2020, 1, 1), mean1), (datetime(2020, 1, 1, 0, 0, 1), mean2)])
        cov = sp2.const(cov)
        out = sp2.run(
            brownian_motion,
            trigger,
    ...
    def test_change_between_triggers(self):
        ...
        mean2 = np.array([2.0])
        cov = np.array([[0.0]])
        trigger = sp2.timer(timedelta(seconds=2))
        drift = sp2.curve(Numpy1DArray[float], [(datetime(2020, 1, 1), mean1), (datetime(2020, 1, 1, 0, 0, 1), mean2)])
        cov = sp2.const(cov)
        out = sp2.run(
            brownian_motion,
    ...
    def test_changing_parameters(self):
        ...
        n_seconds = 1
        trigger = sp2.timer(timedelta(seconds=n_seconds))
        drift = sp2.curve(Numpy1DArray[float], [(datetime(2020, 1, 1, 0), mean1), (datetime(2020, 1, 1, 1), mean2)])
        cov = sp2.curve(NumpyNDArray[float], [(datetime(2020, 1, 1, 0), cov1), (datetime(2020, 1, 1, 1), cov2)])
        out = sp2.run(
            brownian_motion,
            trigger,
            drift,
    ...
    def disable_test_performance(self):
        ...
        def graph():
            out = brownian_motion(trigger, sp2.const(mean), sp2.const(cov), seed=1234)
            sp2.add_graph_output("BM", out, tick_count=1)
