...
from sp2.adapters.utils import (
...
from sp2.impl.wiring import input_adapter_def, output_adapter_def, status_adapter_def
from sp2.lib import _kafkaadapterimpl
...
class KafkaStartOffset(sp2.Enum):
    EARLIEST = 1  # Replay all of history
    LATEST = 2  # Start from new msgs
    START_TIME = 3  # Start from sp2 run starttime
...
class KafkaAdapterManager:
    ...
    def subscribe(
        ...
        push_mode: sp2.PushMode = sp2.PushMode.LAST_VALUE,
        ...
        if not field_map and issubclass(ts_type, sp2.Struct):
    ...
    def status(self, push_mode=sp2.PushMode.NON_COLLAPSING):
        ts_type = Status
