...
@sp2.node
def flatten_numpy_array(x: sp2.ts[sp2.typing.NumpyNDArray["T"]]) -> sp2.Outputs(
    value=sp2.ts[sp2.typing.Numpy1DArray["T"]], shape=sp2.ts[sp2.typing.Numpy1DArray[int]]
):
    ...
    """
    if sp2.ticked(x):
        return sp2.output(value=x.reshape(-1), shape=numpy.array(x.shape, dtype=int))


@sp2.node
def reshape_numpy_array(
    value: sp2.ts[sp2.typing.Numpy1DArray["T"]], shape: sp2.ts[sp2.typing.Numpy1DArray[int]]
) -> sp2.ts[sp2.typing.NumpyNDArray["T"]]:
    ...
    """
    assert sp2.ticked(value)
    assert sp2.ticked(shape)
