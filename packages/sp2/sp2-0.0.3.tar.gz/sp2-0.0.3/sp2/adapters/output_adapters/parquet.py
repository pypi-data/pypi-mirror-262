...
from sp2.impl.wiring.adapters import output_adapter_def
from sp2.lib import _parquetadapterimpl

__all__ = ["ParquetOutputConfig", "ParquetWriter"]

_ARRAY_DIMENSIONS_SUFFIX = "_sp2_dimensions"
K = TypeVar("K")
V = TypeVar("V")
...
class ParquetWriter:
    ...
    def publish_struct(self, value: ts[Struct], field_map: Dict[str, str] = None):
        ...
        for k, v in value.tstype.typ.metadata(typed=True).items():
            if Sp2TypingUtils.is_numpy_array_type(v) and k in field_map:
    ...
    def publish(self, column_name, value: ts[object], array_dimensions_column_name=None):
        ...
        array). The column of the dimensions is specified by this argument, if None provided then default column name will be created by appending '_sp2_dimensions' suffix
        """
        from sp2.adapters.output_adapters.parquet_utility_nodes import flatten_numpy_array

        properties = {"column_name": column_name}
        ts_type = value.tstype.typ

        if Sp2TypingUtils.is_numpy_array_type(ts_type):
            if Sp2TypingUtils.get_origin(ts_type) is sp2.typing.NumpyNDArray:
    ...
    def publish_dict_basket(self, column_name, value, key_type, value_type):
        ...
        if self._filename_provider is not None:
            ...
            filename_provider = sp2.null_ts(str)

        if (
            Sp2TypingUtils.is_generic_container(value_type)
            and Sp2TypingUtils.get_orig_base(value_type) is numpy.ndarray
        ):
