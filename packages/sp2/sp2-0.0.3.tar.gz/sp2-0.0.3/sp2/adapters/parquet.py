...
import sp2
from sp2.adapters.output_adapters.parquet import ParquetOutputConfig, ParquetWriter, resolve_array_shape_column_name
from sp2.adapters.status import Status
...
from sp2.impl.wiring import input_adapter_def, status_adapter_def
from sp2.impl.wiring.node import node
from sp2.lib import _parquetadapterimpl
...
class ParquetReader:
    def __init__(
        ...
        if read_from_memory_tables:
            if not _CAN_READ_ARROW_BINARY:
                raise TypeError("SP2 Cannot load binary arrows derived from pyarrow versions less than 4.0.1")
    ...
    def _subscribe_impl(self, symbol, typ, field_map, push_mode, basket_name=None, array_dimensions_column_name=None):
        ...
        if field_map is None:
            if basket_name is None:
                assert isinstance(typ, sp2.impl.struct.StructMeta), "field_map must be provided for non struct types"
        ...
        if field_map is not None:
            ...
            if isinstance(field_map, dict):
                bad_field_map = not issubclass(typ, sp2.Struct)
                if not bad_field_map:
                    meta_typed = typ.metadata(typed=True)
                    new_field_map = {}
                    for k, v in field_map.items():
                        field_typ = meta_typed[v]
                        if Sp2TypingUtils.is_numpy_array_type(field_typ):
        ...
        if Sp2TypingUtils.is_generic_container(typ) and Sp2TypingUtils.get_orig_base(typ) is numpy.ndarray:
            value_type = typ.__args__[0]
            if Sp2TypingUtils.get_origin(typ) is sp2.typing.NumpyNDArray:
                ...
                flat_values = self._subscribe_impl(
                    symbol=symbol,
                    typ=sp2.typing.Numpy1DArray[typ.__args__[0]],
                    field_map=field_map,
                    push_mode=push_mode,
                )
                ...
                from sp2.adapters.output_adapters.parquet_utility_nodes import reshape_numpy_array
    ...
    def subscribe(
        ...
        :param typ: The type of the SP2 time series subscription. Can either be a primitive type like int or alternatively a type
        that inherits from sp2.Struct, in which case each instance of the struct will be constructed from the matching file columns.
        :param field_map: A map of the fields from parquet columns for the SP2 time series. If typ is a primitive, then field_map should be
        a string specifying the column name, if typ is a sp2 Struct then field_map should be a str->str dictionary of the form
        ...
        If the read array is NumpyNDArray and the column name is not specified, then the default column name will be used by appending the '_sp2_dimensions' to the
        column name.
        """
