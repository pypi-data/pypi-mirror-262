...
def graph_info(g, *args, **kwargs):
    """
    Returns static information about the given sp2.graph in the form of a GraphInfo object

    Input:
        graph: the sp2 graph to analyze
...
class ProfilerInfo(Struct):
    ...
    def print_stats(self, sort_by: str = "total_time", max_nodes: int = 100):
        """
        Prints profiling statistics for the sp2 graph
...
def _profile(prof_info: ProfilerInfo, trigger: ts[Future] = nullts()):
    with sp2.stop():
        prof_info.from_engine(sp2.engine_stats())  # set the Profiler object in-memory

    """
    Returns runtime information about the given sp2.graph in the form of a ProfilerInfo object
    ...
    if sp2.ticked(trigger):
        prof_info.from_engine(sp2.engine_stats())
