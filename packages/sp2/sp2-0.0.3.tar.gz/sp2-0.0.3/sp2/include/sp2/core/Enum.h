#ifndef _IN_SP2_CORE_ENUM_H
#define _IN_SP2_CORE_ENUM_H
    ...
    struct ReverseMap : public std::unordered_map<const char *, typename EnumTraits::_enum, hash::CStrHash, hash::CStrEq>
    {
        ...
        EnumV fromString( const char *s ) const
        {
            ...
            if( it == this -> end() )
            {
                ...
                SP2_THROW( ValueError, "Unrecognized enum value: " << s << " for enum " << typeid( EnumTraits ).name() );
            }
        ...
        void addAliases( const Aliases &aliases )
        {
            for( auto &entry : aliases )
            {
                ...
                if( it == this -> end() )
                    SP2_THROW(ValueError, "Invalid Enum alias, unrecognized enum value " << entry.first);

                if( this -> find( entry.second.c_str() ) != this -> end() )
                    SP2_THROW(ConfigException, "Attempting to add conflicting alias " << entry.second << " to "
...
Enum<EnumTraits>::Enum( UType v )
{
    if( v < 0 || v >= numTypes() )
    {
        if( UnknownOnInvalidValue<EnumTraits>(0) )
            ...
            SP2_THROW(ValueError, "enum value: " << v << " out of range for enum " << typeid(EnumTraits).name());
    } else
...
template<typename EnumTraits>
struct hash<sp2::Enum<EnumTraits>> {
    size_t operator()(sp2::Enum<EnumTraits> e) const {
        return std::hash<typename sp2::Enum<EnumTraits>::EnumV>()(e);
    }
};

}

#define INIT_SP2_ENUM(ENUM, ...)                                \
