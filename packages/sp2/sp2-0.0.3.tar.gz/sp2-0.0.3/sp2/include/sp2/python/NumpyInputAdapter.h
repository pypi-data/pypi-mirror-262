#ifndef _IN_SP2_PYTHON_NUMPYINPUTADAPTER_H
#define _IN_SP2_PYTHON_NUMPYINPUTADAPTER_H
...
class NumpyCurveAccessor
{
    ...
    // Thus, you can get the nested Numpy array values in sp2.curve with zero-copy operations
...
public:
    ...
    NumpyCurveAccessor( PyArrayObject * arr )
    {
        ...
        if( m_nd < 2 )
            SP2_THROW( sp2::TypeError, "NumpyCurveAccessor is inefficient for a 1-D Numpy array: use PyArray_GETPTR1 to access indexed values" );
    ...
    {
        if( index >= m_outerDim )
            SP2_THROW( sp2::TypeError, "Requested data index out of range in NumpyCurveAccessor" );
...
public:
    NumpyInputAdapter( Engine * engine, Sp2TypePtr & type, PyArrayObject * datetimes,
                                                  ...
                                                  m_val_scale( 0 ), m_index( 0 )
    {
        ...
        validateNumpyTypeVsSp2Type( type, out_type );


        auto dt_type = dts_descr -> type;
        if( dt_type != NPY_DATETIMELTR && dt_type != NPY_OBJECTLTR )
            SP2_THROW( ValueError, "timestamps ndarray must be dtype of datetime64 or object, got type code of " << dt_type );

        if( dt_type == NPY_DATETIMELTR )
        {
            auto base = sp2::python::datetimeUnitFromDescr( dts_descr );
            m_dt_scale = sp2::python::scalingFromNumpyDtUnit( base );
        }
        else
        ...
        {
            auto val_base = sp2::python::datetimeUnitFromDescr( vals_descr );
            m_val_scale = sp2::python::scalingFromNumpyDtUnit( val_base );
        }
    }
    ...
    void start( DateTime start, DateTime end ) override
    {
        while( m_index < m_size )
        {
            ...
            if (m_dt_scale == 0)
            {
                ...
                t = fromPython<sp2::DateTime>( *( PyObject ** ) data );
            }
            else
            {
...
{
    sp2::python::stringFromNumpyStr( data, value, m_val_type, m_elem_size );
}

template<>
void NumpyInputAdapter<sp2::DateTime>::setValue( sp2::DateTime & value, void * data )
{
...
template<>
void NumpyInputAdapter<sp2::TimeDelta>::setValue( sp2::TimeDelta & value, void * data )
{
    int64_t dt_val = *( static_cast<int64_t*>(data) );
...
void NumpyInputAdapter<int64_t>::setValue( int64_t & value, void * data )
{
    switch( m_val_type )
    {
        ...
        default:
            SP2_THROW( ValueError, "NumpyInputAdapter<int64_t>::setValue sees invalid numpy type " << m_val_type );
    }
}
...
void NumpyInputAdapter<double>::setValue( double & value, void * data )
{
    switch( m_val_type )
    {
        ...
        default:
            SP2_THROW( ValueError, "NumpyInputAdapter<double>::setValue sees invalid numpy type " << m_val_type );
    }
};
