#ifndef _IN_SP2_PYTHON_NUMPY_CONVERSIONS_H
#define _IN_SP2_PYTHON_NUMPY_CONVERSIONS_H

#include <sp2/engine/sp2_autogen/autogen_types.h>
...
inline PyObject * as_nparray( const sp2::TimeSeriesProvider * ts, const TickBuffer<T> * tickBuffer, const T & lastValue,
...
inline PyObject * as_nparray( const sp2::TimeSeriesProvider * ts, const TickBuffer<DateTime> * tickBuffer, const DateTime & lastValue,
                              int32_t startIndex, int32_t endIndex, bool tailPadding )
{
    int32_t len = startIndex - endIndex + 1;
    if( len <= 0 || !ts -> valid() || ( !tickBuffer && endIndex != 0 ) )
...
inline PyObject * as_nparray( const sp2::TimeSeriesProvider * ts, const TickBuffer<TimeDelta> * tickBuffer, const TimeDelta & lastValue,
                              int32_t startIndex, int32_t endIndex, bool tailPadding )
{
    int32_t len = startIndex - endIndex + 1;
    if( len <= 0 || !ts -> valid()|| ( !tickBuffer && endIndex != 0 ) )
...
template<typename T>
inline PyObject * createNumpyArray( ValueType valueType, const sp2::TimeSeriesProvider * ts, int32_t startIndex, int32_t endIndex,
