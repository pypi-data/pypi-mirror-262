#ifndef _IN_SP2_ENGINE_STRUCT_H
#define _IN_SP2_ENGINE_STRUCT_H
...
public:
    ...
    bool isNative() const                 { return m_type -> type() <= Sp2Type::Type::MAX_NATIVE_TYPE; }
...
protected:
    ...
    StructField( Sp2TypePtr type, const std::string & fieldname,
...
{
    static_assert( Sp2Type::Type::fromCType<T>::type <= Sp2Type::Type::MAX_NATIVE_TYPE );
    static_assert( sizeof(T) == alignof(T) );

public:
    NativeStructField() {}
    NativeStructField( const std::string & fieldname ) : NativeStructField( Sp2Type::fromCType<T>::type(), fieldname )
    {
    }
...
protected:
    NativeStructField( Sp2TypePtr type, const std::string & fieldname ) : StructField( type, fieldname, sizeof( T ), alignof( T ) )
    {}
};
...
class Sp2EnumStructField final : public NativeStructField<Sp2Enum>
{
public:
    Sp2EnumStructField( Sp2TypePtr type, const std::string & fieldname ) : NativeStructField( type, fieldname )
    {}
};
...
public:
    const T & value( const Struct * s ) const
    {
        SP2_THROW( NotImplemented, "Struct fields are not supported for type " << Sp2Type::Type::fromCType<T>::type );
    }
...
{
public:
    NonNativeStructField( Sp2TypePtr type, const std::string &fieldname, size_t size, size_t alignment ) :
...
{
public:
    using CType = sp2::Sp2Type::StringCType;

    StringStructField( Sp2TypePtr type, const std::string & fieldname ) :
...
{
    using CType = typename sp2::Sp2Type::Type::toCType<Sp2Type::Type::ARRAY,ElemT>::type;

    //template<typename T, std::enable_if_t<Sp2Type::isNative(Sp2Type::fromCType<T>::type), bool> = true>
    template<typename T>
    static std::enable_if_t<Sp2Type::isNative(Sp2Type::Type::fromCType<T>::type), void> deepcopy( const std::vector<T> & src, std::vector<T> & dest )
    {
        dest = src;
    }
...
public:
    ArrayStructField( Sp2TypePtr arrayType, const std::string & fieldname ) :
...
public:
    DialectGenericStructField( const std::string & fieldname, size_t size, size_t alignment ) :
        NonNativeStructField( Sp2Type::DIALECT_GENERIC(), fieldname, size, alignment )
    {}
...
std::shared_ptr<typename StructField::upcast<T>::type> StructMeta::getMetaField( const char * fieldname, const char * expectedtype )
{
    ...
    if( !field_ )
        SP2_THROW( TypeError, "Struct type " << name() << " missing required field " << fieldname << " for " << expectedtype );
    ...
    if( !typedfield )
        SP2_THROW( TypeError, expectedtype << " - provided struct type " << name() << " expected type " << Sp2Type::Type::fromCType<T>::type << " for field " << fieldname
...
{
public:
    StructStructField( Sp2TypePtr sp2Type, const std::string &fieldname ) :
        NonNativeStructField( sp2Type, fieldname, sizeof( StructPtr ), alignof( StructPtr ) )
    {
        SP2_ASSERT( sp2Type -> type() == Sp2Type::Type::STRUCT );
        m_meta = std::static_pointer_cast<const Sp2StructType>( sp2Type ) -> meta();
    }
...
template<> struct StructField::upcast<Sp2Enum>   { using type = Sp2EnumStructField; };
...
#endif
template<> struct StructField::upcast<sp2::DialectGenericType> { using type = DialectGenericStructField; };
...
template<>
struct hash<sp2::StructPtr>
{
    size_t operator()( const sp2::StructPtr & s ) const
    {
        return s -> hash();
    }
};
