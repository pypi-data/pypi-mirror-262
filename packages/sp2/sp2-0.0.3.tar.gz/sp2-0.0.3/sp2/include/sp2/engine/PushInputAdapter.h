#ifndef _IN_SP2_ENGINE_PUSHINPUTADAPTER_H
#define _IN_SP2_ENGINE_PUSHINPUTADAPTER_H
...
{
public:
    PushInputAdapter( Engine * engine, Sp2TypePtr & type, PushMode pushMode,
...
inline bool PushInputAdapter::consumeEvent( PushEvent * event, std::vector<PushGroup *> & dirtyGroups )
{
    ...
    bool consumed = switchSp2Type( dataType(),
                                   [ event ]( auto tag )
                                   {
...
{
    SP2_ASSERT( m_group == nullptr || m_group == event -> adapter() -> group() );
