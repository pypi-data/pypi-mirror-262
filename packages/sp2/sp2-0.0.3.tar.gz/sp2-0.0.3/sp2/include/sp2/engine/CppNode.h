#ifndef _IN_SP2_ENGINE_CPPNODE_H
#define _IN_SP2_ENGINE_CPPNODE_H
...
public:
    ...
    const InOutDef & tsinputDef( const char * inputName ) const
    {
        ...
        if( it == m_nodedef -> inputs.end() )
            SP2_THROW( ValueError, "CppNode failed to find input " << inputName << " on node " << name() );
    ...
    const InOutDef & tsoutputDef( const char * outputName ) const
    {
        ...
        if( it == m_nodedef -> outputs.end() )
            SP2_THROW( ValueError, "CppNode failed to find output " << outputName << " on node " << name() );
    ...
    const InOutDef & alarmDef( const char * alarmName ) const
    {
        ...
        if( !def.isAlarm )
            SP2_THROW( TypeError, "CppNode expected alarm " << alarmName << " but found it as an input on node " << name() );
        return def;
    }
    ...
    T scalarValue( const char * scalarName ) const
    {
        ...
        if( !m_nodedef -> scalars.exists( scalarName ) )
            SP2_THROW( ValueError, "CppNode failed to find scalar " << scalarName << " on node " << name() );
    ...
    using Creator = std::function<CppNode*( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef )>;

protected:
    CppNode( Engine * engine,
             const NodeDef & nodedef ) : sp2::Node( asSp2NodeDef( nodedef ), engine ),
                                         m_nodedef( &nodedef )
    {}

    sp2::NodeDef asSp2NodeDef( const NodeDef & nodedef ) const
    {
        if( nodedef.inputs.size() > InputId::maxInputs() )
            SP2_THROW( ValueError, "number of inputs exceeds limit of " << InputId::maxInputs() << " on node " << name() );

        if( nodedef.outputs.size() > OutputId::maxOutputs() )
            SP2_THROW( ValueError, "number of outputs exceeds limit of " << OutputId::maxOutputs() << " on node " << name() );

        return sp2::NodeDef{ INOUT_ID_TYPE( nodedef.inputs.size() ), INOUT_ID_TYPE( nodedef.outputs.size() ) };
    }

    void validateNodeDef() const
    {
        if( !m_nodedef )
            SP2_THROW( RuntimeException, "CppNode cpp nodedef information is only available during INIT" );
    }
    ...
    public:
                                                                 ...
                                                                 m_id( 0 )
        {
            ...
            if( def.isAlarm )
                SP2_THROW( TypeError, "CppNode expected input " << name << " but found it as an alarm on node " << m_node.name() );
        ...
        const Sp2Type * type() const { return ts() -> type(); }
    ...
    public:
         ...
         */
        const Sp2TypePtr& type() const {return m_type;}
    ...
    struct GenericAlarmWrapper : public GenericInputWrapper
    {
        ...
        Scheduler::Handle scheduleAlarm( DateTime time, const GenericInputWrapper & input )
        {
            ...
            return switchSp2Type( input.type(), [this,time,&input]( auto tag )
                           {
    ...
    public:
        ...
        {
            assert( input.type() == type() );
            switchSp2Type( input.type(), [this,&input]( auto tag )
                           {
                               using T = typename decltype(tag)::type;
    ...
    //for python-like sp2 namespace interface
    class SP2
    {
    public:
...
template<>
struct sp2::CppNode::InputTypeHelper<sp2::CppNode::Generic>
{
    using type = GenericInputWrapper;
};

template<>
struct sp2::CppNode::ListInputBasketTypeHelper<sp2::CppNode::Generic>
{
    using type = ListInputBasketWrapper<GenericInputWrapper>;
};

template<>
struct sp2::CppNode::DictInputBasketTypeHelper<sp2::CppNode::Generic>
{
    using type = DictInputBasketWrapper<GenericInputWrapper>;
};

template<>
struct sp2::CppNode::OutputTypeHelper<sp2::CppNode::Generic>
{
    using type = GenericOutputWrapper;
};

template<>
struct sp2::CppNode::ListOutputBasketTypeHelper<sp2::CppNode::Generic>
{
    using type = ListOutputBasketWrapper<GenericOutputWrapper>;
};

template<>
struct sp2::CppNode::DictOutputBasketTypeHelper<sp2::CppNode::Generic>
{
    using type = DictOutputBasketWrapper<GenericOutputWrapper>;
};

template<>
struct sp2::CppNode::AlarmTypeHelper<sp2::CppNode::Generic>
{
    using type = GenericAlarmWrapper;
};


#define DECLARE_CPPNODE( Name ) class Name : public sp2::CppNode

#define _STATIC_CREATE_METHOD( Class ) \
    static sp2::CppNode * create( Engine * engine, const sp2::CppNode::NodeDef & nodedef ) \
    { \
...
public:                                                      \
    ...
    Class( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
...
#define SCALAR_INPUT( Type, Name )                      sp2::CppNode::Scalar<Type> Name{#Name,*this};
#define SCALAR_INPUT_RENAMED( Type, DeclName, VarName ) sp2::CppNode::Scalar<Type> VarName{#DeclName,*this};
...
#define SP2_OUTPUT( Value ) __unnamed_output.output( Value )
#define RETURN( Value ) do{ SP2_OUTPUT( Value ); return; } while( 0 );

#define SINGLE_ARG(...) __VA_ARGS__
#define CPPNODE_CREATE_METHOD( Name ) Name##_create_method
#define CPPNODE_CREATE_FWD_DECL( Namespace, Name ) namespace Namespace { sp2::CppNode * CPPNODE_CREATE_METHOD( Name )( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ); }
#define EXPORT_CPPNODE( Name ) sp2::CppNode * CPPNODE_CREATE_METHOD( Name )( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) { return Name::create( engine, nodedef ); }
#define EXPORT_TEMPLATE_CPPNODE( Name, Typed ) sp2::CppNode * CPPNODE_CREATE_METHOD( Name )( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) { return Typed::create( engine, nodedef ); }
