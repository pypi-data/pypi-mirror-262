#ifndef _IN_SP2_ENGINE_SCHEDULER_H
#define _IN_SP2_ENGINE_SCHEDULER_H

#include <sp2/core/BasicAllocator.h>
...
{
    if( handle.expired() )
        SP2_THROW( ValueError, "attempting to reschedule expired handle" );
...
template<>
struct hash<sp2::Scheduler::Handle>
{
    size_t operator()( const sp2::Scheduler::Handle & handle ) const
    {
        return handle.hash();
    }
};
