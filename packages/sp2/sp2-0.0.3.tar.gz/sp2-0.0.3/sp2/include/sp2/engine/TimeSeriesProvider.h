#ifndef _IN_SP2_ENGINE_TIMESERIESPROVIDER_H
#define _IN_SP2_ENGINE_TIMESERIESPROVIDER_H

#include <sp2/core/Exception.h>
#include <sp2/engine/Sp2Type.h>
#include <sp2/engine/EventPropagator.h>
...
public:
    ...
    void init( const Sp2TypePtr & type, Node * node = nullptr );
    void reset();

    void setNode( Node * node )
    {
        SP2_ASSERT( m_node == nullptr );
        m_node = node;
    }

    //We want want to find a better way to get 0-rank, node is only kept here for ranking logic seems wasteful
    Node *    node() const       { return m_node; }
    const Sp2Type * type() const { return m_type; }
...
inline void TimeSeriesProvider::outputTickTyped( uint64_t cycleCount, DateTime timestamp, const T & value, bool propagate_ )
{
    ...
    SP2_ASSERT( Sp2Type::Type::fromCType<T>::type == m_type -> type() );

    if( m_lastCycleCount == cycleCount )
        SP2_THROW( RuntimeException, "Attempted to output twice on the same engine cycle at time " << timestamp );
...
inline T & TimeSeriesProvider::reserveTickTyped( uint64_t cycleCount, DateTime timestamp )
{
    ...
    if( m_lastCycleCount == cycleCount )
        SP2_THROW( RuntimeException, name() << " attempted to output twice on the same engine cycle at time " << timestamp );
