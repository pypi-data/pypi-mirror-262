#ifndef _IN_SP2_ENGINE_WINDOWBUFFER_H
#define _IN_SP2_ENGINE_WINDOWBUFFER_H
...
{
    if( unlikely( empty() ) )
        SP2_THROW( RangeError, "Cannot pop from empty window buffer" );
...
{
    if( n > m_count )
        SP2_THROW( RangeError, "Window buffer of size " << m_count << " does not contain " << n << " elements for removal." );
    m_leftIndex += n;
...
inline FixedSizeWindowBuffer<T>::FixedSizeWindowBuffer( int64_t capacity )
{
    ...
    {
        SP2_THROW( ValueError, "Tick specified interval is too large To use an expanding window, set interval=None.");
    }
