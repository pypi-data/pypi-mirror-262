#ifndef _IN_SP2_CORE_DICTIONARY_H
#define _IN_SP2_CORE_DICTIONARY_H
...
inline const Dictionary::Value& Dictionary::getUntypedValue( const std::string &key ) const
{
    auto it = m_map.find( key );
    if( it == m_map.end() )
        SP2_THROW( KeyError, "Dictionary missing key \"" << key << "\"" );
...
template<> inline int32_t  Dictionary::cast<uint32_t,int32_t>( const uint32_t & value )
{
    if( value > (uint32_t) std::numeric_limits<int32_t>::max() )
        SP2_THROW( RangeError, "Dictionary value for uint32_t ( " << value << " ) is out of range for int32_t cast" );
    return value;
}
...
{
    if( value < 0 )
        SP2_THROW( RangeError, "Dictionary value for int32_t ( " << value << " ) is out of range for uint32_t cast" );
    return value;
}
...
template<> inline int64_t  Dictionary::cast<uint64_t,int64_t>( const uint64_t & value )
{
    if( value > ( uint64_t ) std::numeric_limits<int64_t>::max() )
        SP2_THROW( RangeError, "Dictionary value for uint64_t ( " << value << " ) is out of range for int64_t cast" );
    return value;
}
...
{
    if( value < 0 )
        SP2_THROW( RangeError, "Dictionary value for int64_t ( " << value << " ) is out of range for uint64_t cast" );
    return value;
}
...
inline typename Dictionary::ReturnType<T>::Type Dictionary::extractValue( const std::string & key, const Value & value )
{
    ...
    catch( const std::bad_variant_access & err )
    {
        ...
        catch( const std::bad_variant_access & err )
        {
            ...
            SP2_THROW( TypeError, "Dictionary type-mismatch on key \"" << key << "\".  Expected type \"" << cpp_type_name<T>() << "\" got type: \"" << curtypename << "\"" );
        }
    }
}
...
template<>
struct hash<sp2::Dictionary>
{
    size_t operator()( const sp2::Dictionary & d ) const
    {
        return d.hash();
    }
};
