...
#include <sp2/engine/WindowBuffer.h>
...
public:
    _generic_tick_window_updates( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}

    START()
    {
        if( interval <= 0 )
            SP2_THROW( ValueError, "Tick interval needs to be positive" );
        sp2.make_passive( x );
    }
    ...
    INVOKE()
    {
        ...
        if( sp2.ticked( trigger ) || ( s_first && sp2.ticked( x ) && sp2.ticked( sampler )  ) )
        {
            s_first = false;
            int64_t total_count = sp2.count( sampler );
...
protected:
    ...
    // ]
    // def g():
    //   d = sp2.curve(float, data)
    //   trigger = sp2.curve(bool, trigger_data)
    //   m = sp2.stats.max(d, timedelta(minutes=31), min_window=timedelta(), trigger=trigger)
    //   sp2.print('trigger', trigger)
    //   sp2.print('d', d)
    //   sp2.print('m', m)
    // sp2.run(g, starttime=datetime(2023,2,17,12), endtime=datetime(2023, 2, 17, 18, 25))
    //
...
public:
    ...
    _generic_time_window_updates( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}

    START()
    {
        if( TimeDelta( interval ) < TimeDelta::ZERO() )
            SP2_THROW( ValueError, "Time interval needs to be non-negative" );
    ...
    INVOKE()
    {
        ...
        if( sp2.ticked( trigger ) || ( s_first && sp2.ticked( x ) && sp2.ticked( sampler ) ) )
        {
            ...
            // Handle removals
            int64_t count = sp2.count( sampler );
...
public:
    ...
    _generic_cross_sectional( sp2::Engine * engine, const sp2::CppNode::NodeDef & nodedef ) : sp2::CppNode( engine, nodedef )
    {}
