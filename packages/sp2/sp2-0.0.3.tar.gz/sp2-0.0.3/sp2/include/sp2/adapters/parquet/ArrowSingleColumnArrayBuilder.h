#ifndef _IN_SP2_ADAPTERS_PARQUET_ArrowSingleColumnArrayBuilder_H
#define _IN_SP2_ADAPTERS_PARQUET_ArrowSingleColumnArrayBuilder_H
...
public:
    ...
    virtual void handleRowFinished() override
    {
        if( m_hasValue )
        {
            ...
            SP2_TRUE_OR_THROW_RUNTIME( m_builderPtr -> Append().ok(), "Failed to append struct" );
        }
        else
        {
...
public:
    ...
    virtual void handleRowFinished() override
    {
        ...
        {
            SP2_TRUE_OR_THROW_RUNTIME( m_builderPtr -> Append().ok(), "Failed to append list" );
...
private:
    static std::shared_ptr<arrow::ArrayBuilder> createValueArrayBuilderForType( const std::shared_ptr<::arrow::DataType> &childType)
    {
        switch(childType->id())
        {
            ...
            default:
                SP2_THROW(TypeError, "Trying to create arrow list array builder for unsupported element type " << childType->name());
        }
    }
...
public:
    ...
    {
        SP2_TRUE_OR_THROW_RUNTIME( m_builderPtr -> Reserve( getChunkSize() ).ok(), "Failed to reserve arrow array size" );
    }
    ...
    {
        std::shared_ptr<arrow::Array> array;
        SP2_TRUE_OR_THROW_RUNTIME( m_builderPtr -> Finish( &array ).ok(), "Failed to create arrow array" );
        return array;
    }
...
class DatetimeArrayBuilder : public BaseTypedArrayBuilder<sp2::DateTime, arrow::TimestampBuilder>
{
public:
    DatetimeArrayBuilder( std::string columnName, std::uint32_t chunkSize )
            : BaseTypedArrayBuilder<sp2::DateTime, arrow::TimestampBuilder>( columnName,
                                                                             chunkSize,
...
class DateArrayBuilder : public BaseTypedArrayBuilder<sp2::Date, arrow::Date32Builder>
{
public:
    DateArrayBuilder( std::string columnName, std::uint32_t chunkSize )
            : BaseTypedArrayBuilder<sp2::Date, arrow::Date32Builder>( columnName,
                                                                      chunkSize )
    {
    }
...
private:
    static sp2::Date &getOrigin()
    {
        static sp2::Date origin{ 1970, 1, 1 };
        return origin;
    }
};

class TimeArrayBuilder : public BaseTypedArrayBuilder<sp2::Time, arrow::Time64Builder>
{
public:
    TimeArrayBuilder( std::string columnName, std::uint32_t chunkSize )
        : BaseTypedArrayBuilder<sp2::Time, arrow::Time64Builder>( columnName, chunkSize,
...
class TimedeltaArrayBuilder : public BaseTypedArrayBuilder<sp2::TimeDelta, arrow::DurationBuilder>
{
public:
    TimedeltaArrayBuilder( std::string columnName, std::uint32_t chunkSize )
            : BaseTypedArrayBuilder<sp2::TimeDelta, arrow::DurationBuilder>(
