...
class DataFrame:
    ...
    def _filter(self, filter_ts):
        ...
        for col, v1 in zip(self._columns, self._data.values()):
            data[col] = sp2.baselib.filter(filter_ts, v1)
    ...
    def __getitem__(self, columns):
        if isinstance(columns, Edge):
            if columns.tstype.typ is not bool:
                raise KeyError(
                    "sp2.DataFrame access by edge expected ts[bool] got ts[" + columns.tstype.typ.__name__ + "]"
                )
    ...
    def __repr__(self):
        return "sp2.DataFrame( %s )" % (", ".join(f"{k} = ts[{v.tstype.typ.__name__}]" for k, v in self._data.items()))
    ...
    def _eval_graph(self):
        ...
        for k, v in self._data.items():
            sp2.add_graph_output(k, v)

    def _eval(self, starttime: datetime, endtime: datetime = None, realtime: bool = False):
        import sp2

        return sp2.run(self._eval_graph, starttime=starttime, endtime=endtime, realtime=realtime)
    ...
    def show_graph(self):
        ...
        buffer = sp2.showgraph.generate_graph(self._eval_graph)
    ...
    def to_perspective(self, starttime: datetime, endtime: datetime = None, realtime: bool = False):
        ...
        @sp2.node
        def apply_updates(table: object, data: {str: sp2.ts[object]}, timecol: str, throttle: timedelta):
            with sp2.alarms():
