...
# We need to patch mock modules into sys.modules to avoid pulling in sp2/__init__.py and all of its baggage, which include imports of
# _sp2impl, which would be a circular dep
spec = importlib.util.find_spec("sp2")
loader = importlib.machinery.SourceFileLoader(spec.name, spec.origin)
spec = importlib.util.spec_from_loader("sp2", loader)
sp2_mod = importlib.util.module_from_spec(spec)
sys.modules["sp2"] = sp2_mod

from sp2.impl.struct import Struct  # noqa: E402
from sp2.impl.enum import Enum  # noqa: E402


def struct_type(type_info):
    return f"""sp2::autogen::{type_info['pytype'].__name__}::Ptr"""


def enum_type(type_info):
    return f"""sp2::autogen::{type_info['pytype'].__name__}"""


def array_type(type_info):
    elem_typeinfo = type_info["elemtype"]
    elemctype = SP2_CPP_TYPE_MAP[elem_typeinfo["type"]]
...
SP2_CPP_TYPE_MAP = {
    ...
    "UINT64": "uint64_t",
    "DOUBLE": "double",
    "DATETIME": "sp2::DateTime",
    "TIMEDELTA": "sp2::TimeDelta",
    "DATE": "sp2::Date",
    "STRING": "std::string",
    "DIALECT_GENERIC": "sp2::DialectGenericType",
    "STRUCT": struct_type,
    "ENUM": enum_type,
    "ARRAY": array_type,
}
...
class CodeGenerator:
        ...
        for struct_type in self._struct_types:
            ...
            for field in fields:
                ...
                if extmod is not None:
                    headers.add(extmod.SP2_AUTOGEN_HINTS["cpp_header"])
    ...
    def generate_header_code(self):
        include_guard = "_IN_SP2_AUTOGEN_" + self._module_name.replace(".", "_").upper()
        out = f"""
...
"""
    ...
    def _generate_headers(self):
        common_headers = ["sp2/core/Exception.h", "sp2/engine/Struct.h", "cstddef"]
    ...
    def _generate_enum_class(self, enum_type):
        ...
        out = f"""
class {enum_name} : public sp2::Sp2Enum
{{
public:
...
{sp2enum_decls}
    ...
    {enum_name}( const sp2::Sp2Enum & v ) : sp2::Sp2Enum( v ) {{ SP2_TRUE_OR_THROW( v.meta() == s_meta.get(), AssertionError, "Mismatched enum meta" ); }}
...
"""
    ...
    {{
        ...
        if( !{fieldname}_isSet() )
            SP2_THROW( sp2::ValueError, "field {fieldname} on struct {struct_name} is not set" );
...
"""
        ...
        base_class = "sp2::Struct" if base_struct is None else base_struct.__name__
...
public:
    ...
    using Ptr = sp2::TypedStructPtr<{struct_name}>;
    ...
    Ptr copy() const {{ return sp2::structptr_cast<{struct_name}>( Struct::copy() ); }}
...
{assertions}
    ...
    {{
        ...
        sp2::python::PyStructMeta * pymeta = ( sp2::python::PyStructMeta * ) structType;
...
sp2::StructMetaPtr {struct_name}::s_meta;
"""
...
{{
    ...
    {{
        ...
        // should add some assertion here..
        sp2::python::PySp2EnumMeta * pymeta = ( sp2::python::PySp2EnumMeta * ) enumType;
...
std::shared_ptr<sp2::Sp2EnumMeta> {enum_name}::s_meta;
{static_decls}
"""
...
# Test2 = sp2.impl.struct.defineStruct( 'Test2', { 'A' + str(i) : bool for i in range(25 )})
