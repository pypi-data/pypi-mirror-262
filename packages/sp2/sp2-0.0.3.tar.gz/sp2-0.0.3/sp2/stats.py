...
import sp2
from sp2 import ts
from sp2.lib import _sp2npstatsimpl, _sp2statsimpl
...
@sp2.node(cppimpl=_sp2statsimpl._tick_window_updates)
def _tick_window_updates(
    x: ts[float], interval: int, trigger: ts[object], sampler: ts[object], reset: ts[object], recalc: ts[object]
) -> sp2.Outputs(additions=ts[[float]], removals=ts[[float]]):
    raise NotImplementedError("_tick_window_updates only implemented in C++")
    return sp2.output(additions=0, removals=0)


@sp2.node(cppimpl=_sp2statsimpl._time_window_updates)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_tick_window_updates)
...
) -> sp2.Outputs(additions=ts[[np.ndarray]], removals=ts[[np.ndarray]]):
    raise NotImplementedError("_np_tick_window_updates only implemented in C++")
    return sp2.output(additions=0, removals=0)


@sp2.node(cppimpl=_sp2npstatsimpl._np_time_window_updates)
...
def _window_updates(
    ...
    reset: ts[object],
    recalc: ts[object],
) -> sp2.Outputs(additions=ts[[typing.Union[float, np.ndarray]]], removals=ts[[typing.Union[float, np.ndarray]]]):
    """
    ...
    return sp2.output(additions=additions, removals=removals)


@sp2.node(cppimpl=_sp2statsimpl._min_hit_by_tick)
def _min_hit_by_tick(x: ts["T"], min_window: int, trigger: ts[object]) -> ts[bool]:
    if sp2.ticked(trigger):
        if sp2.num_ticks(x) >= min_window:
            sp2.make_passive(trigger)
            return True
...
def _min_hit(x: ts["T"], min_window: typing.Union[timedelta, int], trigger: ts[object]) -> ts[bool]:
    ...
    return sp2.const(True, delay=min_window)


@sp2.node(cppimpl=_sp2statsimpl._in_sequence_check)
def _in_sequence_check(x: ts["T"], y: ts["T"]):
    raise NotImplementedError("_in_sequence_check only implemented in C++")


@sp2.node(cppimpl=_sp2statsimpl._sync_nan_f)
def _sync_nan_f(x: ts[float], y: ts[float]) -> sp2.Outputs(x_sync=ts[float], y_sync=ts[float]):
    raise NotImplementedError("_sync_nan_f only implemented in C++")
    return sp2.output(x_sync=0, y_sync=0)


@sp2.node(cppimpl=_sp2npstatsimpl._sync_nan_np)
def _sync_nan_np(x: ts[np.ndarray], y: ts[np.ndarray]) -> sp2.Outputs(x_sync=ts[np.ndarray], y_sync=ts[np.ndarray]):
    raise NotImplementedError("_sync_nan_np only implemented in C++")
    return sp2.output(x_sync=0, y_sync=0)


@sp2.graph
def _sync_nan(x: ts[typing.Union[float, np.ndarray]], y: ts[typing.Union[float, np.ndarray]]) -> sp2.Outputs(
...
@sp2.node(cppimpl=_sp2npstatsimpl._list_to_np)
def list_to_numpy(x: [ts[float]], fillna: bool = False) -> ts[sp2.typing.Numpy1DArray[float]]:
    """
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_to_list)
def numpy_to_list(x: ts[np.ndarray], n: int) -> sp2.OutputBasket(List[ts[float]], shape="n"):
...
def _setup(x, interval, min_window, trigger, sampler, reset, weights=None, ignore_weights=False, recalc=None):
    ...
    if reset is not None:
        if recalc is not None:
            ...
            recalc = sp2.null_ts(bool)
            clear_stat = reset
    else:
        reset = sp2.null_ts(bool)
        if recalc is not None:
            clear_stat = recalc
        else:
            recalc = sp2.null_ts(bool)
            clear_stat = sp2.null_ts(bool)
    ...
    if (isinstance(min_window, int) and min_window > 1) or (
        ...
        min_hit = sp2.default(_min_hit(sampler, min_window, trigger), False)

    series = x
    if x.tstype.typ is int:
        series = sp2.cast_int_to_float(x)

    if weights is not None:
        if weights.tstype.typ is int:
            weights = sp2.cast_int_to_float(weights)
...
@sp2.node(cppimpl=_sp2statsimpl._count)
def _count(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_count)
def _np_count(
...
@sp2.node(cppimpl=_sp2statsimpl._sum)
def _sum(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._kahan_sum)
def _kahan_sum(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_sum)
def _np_sum(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_kahan_sum)
def _np_kahan_sum(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._mean)
def _mean(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_mean)
def _weighted_mean(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_mean)
def _np_mean(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._var)
def _var(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._sem)
def _sem(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_var)
def _weighted_var(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_sem)
def _weighted_sem(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_mean)
...
@sp2.node(cppimpl=_sp2statsimpl._covar)
def _covar(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2statsimpl._corr)
def _corr(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_covar)
def _weighted_covar(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_corr)
def _weighted_corr(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_var)
def _np_var(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_sem)
def _np_sem(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_covar)
def _np_covar(
    x_add: ts[[np.ndarray]],
    x_rem: ts[[np.ndarray]],
    w_add: ts[[np.ndarray]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_corr)
def _np_corr(
    x_add: ts[[np.ndarray]],
    x_rem: ts[[np.ndarray]],
    w_add: ts[[np.ndarray]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_var)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_sem)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_covar)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_corr)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_cov_matrix)
def _np_cov_matrix(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_corr_matrix)
def _np_corr_matrix(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_cov_matrix)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_corr_matrix)
...
@sp2.node(cppimpl=_sp2statsimpl._skew)
def _skew(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._kurt)
def _kurt(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_skew)
def _weighted_skew(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2statsimpl._weighted_kurt)
def _weighted_kurt(
    x_add: ts[[float]],
    x_rem: ts[[float]],
    y_add: ts[[float]],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_skew)
def _np_skew(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_kurt)
def _np_kurt(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_skew)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_weighted_kurt)
...
@sp2.node(cppimpl=_sp2statsimpl._first)
def _first(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_first)
def _np_first(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._last)
def _last(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_last)
def _np_last(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._unique)
def _unique(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_unique)
def _np_unique(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._prod)
def _prod(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_prod)
def _np_prod(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._quantile)
def _quantile(
    additions: ts[[float]],
    removals: ts[[float]],
    ...
    min_data_points: int,
    ignore_na: bool,
) -> sp2.OutputBasket(List[ts[float]], shape="nq"):
    raise NotImplementedError("_quantile only implemented in C++")
    return 0


@sp2.node(cppimpl=_sp2npstatsimpl._np_quantile)
def _np_quantile(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    quants: typing.List[float],
    ...
    min_data_points: int,
    ignore_na: bool,
) -> sp2.OutputBasket(List[ts[np.ndarray]], shape="nq"):
    raise NotImplementedError("_np_quantile only implemented in C++")
    return 0


@sp2.node(cppimpl=_sp2statsimpl._min_max)
def _min_max(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_min_max)
def _np_min_max(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._rank)
def _rank(
    additions: ts[[float]],
    removals: ts[[float]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_rank)
def _np_rank(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    trigger: ts[object],
...
@sp2.node(cppimpl=_sp2statsimpl._arg_min_max)
def _arg_min_max(
    x: ts[float],
    removals: ts[[float]],
    max: bool,
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_arg_min_max)
def _np_arg_min_max(
    x: ts[np.ndarray],
    removals: ts[[np.ndarray]],
    max: bool,
...
@sp2.node(cppimpl=_sp2statsimpl._ema_compute)
def _ema_compute(
    additions: ts[[float]],
    removals: ts[[float]],
    alpha: float,
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_ema_compute)
def _np_ema_compute(
    additions: ts[[np.ndarray]],
    removals: ts[[np.ndarray]],
    alpha: float,
...
@sp2.node(cppimpl=_sp2statsimpl._ema_adjusted)
def _ema_adjusted(
    additions: ts[[float]],
    removals: ts[[float]],
    alpha: float,
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_ema_adjusted)
...
@sp2.node(cppimpl=_sp2statsimpl._ema_timewise)
def _ema_timewise(
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_ema_timewise)
...
@sp2.node(cppimpl=_sp2statsimpl._ema_debias_halflife)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_ema_debias_halflife)
...
@sp2.node(cppimpl=_sp2statsimpl._ema_debias_alpha)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_ema_debias_alpha)
...
@sp2.node(cppimpl=_sp2statsimpl._cross_sectional_as_list)
...
@sp2.node(cppimpl=_sp2npstatsimpl._cross_sectional_as_np)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_cross_sectional_as_list)
...
@sp2.node(cppimpl=_sp2npstatsimpl._np_cross_sectional_as_np)
...
def count(
    ...
    if min_hit is not None:
        return sp2.filter(min_hit, edge)
    return edge
...
def mean(
    ...
    if series.tstype.typ is float:
        if weights is not None:
            weight_updates = _window_updates(sp2.sample(sampler, weights), interval, trigger, sampler, reset, recalc)
            edge = _weighted_mean(
                updates.additions,
                updates.removals,
...
def cov(
    ...
    if series.tstype.typ is float:
        if weights is not None:
            w_upd = _window_updates(sp2.sample(sampler, weights), interval, trigger, sampler, reset, recalc)
...
def ema(
    ...
    if min_periods > 1:
        return sp2.filter(_min_hit_by_tick(series, min_periods, trigger), edge)
    else:
        return edge
...
def ema_cov(
    ...
    if bias:
        ...
        return sp2.multiply(factor, bias_cov)
