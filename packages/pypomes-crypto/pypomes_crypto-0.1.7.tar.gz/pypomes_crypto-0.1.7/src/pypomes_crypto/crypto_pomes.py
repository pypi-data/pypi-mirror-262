...
def crypto_validate_p7s(errors: list[str], p7s_file: str | bytes, p7s_payload: str | bytes) -> bool:
    """
    ...
    if result:
        ...
        try:
            sig_scheme: pkcs1_15.PKCS115_SigScheme = pkcs1_15.new(pkcs7.public_key)
            sig_scheme.verify(pkcs7.payload_hash, pkcs7.signature)
        except ValueError:
            result = False
            if isinstance(errors, list):
