    ...
    def build_fast(cls, path, images, basic=None, step=None, base=None, args=None, push=True, push_only_last=False):
        ...
        def _do_build_build():
            ...
            for i, c in enumerate(content_build):
                ...
                for image_name in images:
                    fr, fd = f'/dekartifacts/build/{image_name}/env', f'{td}/env.yaml'
                    remove_path(fd)
                    cls.cp(image_name, fr, fd, ignore=True)
                    if os.path.exists(fd):
                        env_map[image_name] = assign(env_map.get(image_name, {}), yaml.load(fd))
            result = {}
            for image_name in images:
                result[image_name] = "\n" + "\n".join(
                    f'ENV {kk} {vv}' for kk, vv in env_map.get(image_name, {}).items()
                ) + "\n"
            return result
        ...
        def _do_build_result():
            for image_name in images:
                ...
                write_file(os.path.join(path, '.context'), ma=os.path.join(path, image_name))
                for i, c in enumerate(contents):
                    ...
                    if is_last or has_updated or not has_basic or not cls.remote_exist(current_image):
                        _do_prepare('\n'.join(
                            [content_args, *build_from, self_from, c, env_from_build[image_name] if i == 0 else '']
                        ))
