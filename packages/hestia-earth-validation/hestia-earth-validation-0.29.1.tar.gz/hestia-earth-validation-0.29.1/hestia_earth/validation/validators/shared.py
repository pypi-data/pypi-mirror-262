...
def _is_waterRegime_sum(practice: dict):
    ...
    value = get_table_value(lookup, 'termid', term_id, column_name('percentAreaSumIs100'))
    return not (not value)


def _is_tillage_sum(practice: dict):
    term = practice.get('term')
    term_id = term.get('@id')
    lookup = download_lookup(f"{term.get('termType')}.csv")
    value = get_table_value(lookup, 'termid', term_id, column_name('unique'))
    return not (not value)


_VALIDATE_SUM_100_PERCENT_FILTER = {
    TermTermType.TILLAGE: _is_tillage_sum,
    TermTermType.WATERREGIME: _is_waterRegime_sum
}


def _validate_list_sum_100_percent(cycle: dict, list_key: str, term_type: TermTermType, with_min_value: bool):
    blank_nodes = [
        p for p in filter_list_term_type(cycle.get(list_key, []), term_type) if all([
            # only account for practices with `%` or `% area` units
            p.get('term', {}).get('units', '').startswith('%'),
            _VALIDATE_SUM_100_PERCENT_FILTER.get(term_type, lambda *args: True)(p)
        ])
    ]
    min_value = 99.5
    max_value = 100.5
    total_value, valid_sum = valid_list_sum(blank_nodes)
    valid = all([
        total_value <= max_value,
        not with_min_value or min_value <= total_value
    ])
    return not blank_nodes or valid or {
        'level': 'error',
        'dataPath': f".{list_key}",
        'message': f"value should sum to {'' if with_min_value else 'maximum '}100 across all {term_type.value}",
        'params': {
            'sum': total_value,
            'max': max_value
        } | ({'min': min_value} if with_min_value else {})
    }


def validate_list_sum_100_percent(
    node: dict, list_key: str, term_types: List[TermTermType] = [], with_min_value: bool = True
):
    return _filter_list_errors([
        _validate_list_sum_100_percent(node, list_key, term_type, with_min_value) for term_type in term_types
    ])
