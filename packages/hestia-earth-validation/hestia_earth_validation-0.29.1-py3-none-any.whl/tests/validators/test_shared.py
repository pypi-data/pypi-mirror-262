...
_LIST_SUM_100_TERM_TYPES = [TermTermType.CROPRESIDUEMANAGEMENT]


def test_validate_list_sum_100_percent():
    # no practices should be valid
    cycle = {'practices': []}
    assert validate_list_sum_100_percent(cycle, 'practices', _LIST_SUM_100_TERM_TYPES) is True

    with open(f"{fixtures_folder}/list-values-sum-100/total-90.json") as f:
        cycle = json.load(f)
    assert validate_list_sum_100_percent(cycle, 'practices', _LIST_SUM_100_TERM_TYPES, with_min_value=True) == {
        'level': 'error',
        'dataPath': '.practices',
        'message': 'value should sum to 100 across all cropResidueManagement',
        'params': {
            'sum': 90,
            'min': 99.5,
            'max': 100.5
    ...
    assert validate_list_sum_100_percent(cycle, 'practices', _LIST_SUM_100_TERM_TYPES, with_min_value=False) is True

    with open(f"{fixtures_folder}/list-values-sum-100/total-100.json") as f:
        cycle = json.load(f)
    assert validate_list_sum_100_percent(cycle, 'practices', _LIST_SUM_100_TERM_TYPES) is True

    with open(f"{fixtures_folder}/list-values-sum-100/total-110.json") as f:
        cycle = json.load(f)
    assert validate_list_sum_100_percent(cycle, 'practices', _LIST_SUM_100_TERM_TYPES, with_min_value=False) == {
        'level': 'error',
        'dataPath': '.practices',
        'message': 'value should sum to maximum 100 across all cropResidueManagement',
        'params': {
            'sum': 110,
            'max': 100.5
        }
...
def test_validate_list_percent_requires_value_valid():
    # no practices should be valid
    cycle = {'practices': []}
    assert validate_list_percent_requires_value(cycle, 'practices', _LIST_SUM_100_TERM_TYPES) is True
...
def test_validate_list_percent_requires_value_invalid():
    with open(f"{fixtures_folder}/list-percent-value/invalid.json") as f:
        cycle = json.load(f)
    assert validate_list_percent_requires_value(cycle, 'practices', _LIST_SUM_100_TERM_TYPES) == {
        'level': 'error',
