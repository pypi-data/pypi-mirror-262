...
class DockerYardEnv(gym.Env):
    ...
    def create_game(self):
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.float32([max_row for _ in range(len(self.observation_list))]), dtype=np.float32
        )
    ...
    def reset(self, **kwargs):
        ...
        if seed is not None:
            pass
        self.create_game()
        source_observation_space = np.array(self.docker_game.get_observation_space()).astype(np.float32)
        self.success_times = 0
        self.wrong_times = 0
        self.total_reward = 0
    ...
    def step(self, action):
        ...
        return np.array(current_observation).astype(np.float32), step_reward, terminated, truncated, {}