...
from findly.unified_reporting_sdk.common.common_parser import (
    CommonParser,
    RESERVED_TOTAL,
)
...
from findly.unified_reporting_sdk.common.where_string_comparison import (
...
METRICS_JSONL_PATH = os.path.join(os.path.dirname(script_path), "metadata", "metrics.jsonl")
DIMENSION_JSONL_PATH = os.path.join(os.path.dirname(script_path), "metadata", "dimensions.jsonl")
...
class FbAdsQueryArgsParser(CommonParser):
    ...
    async def parse_report_response_to_dataframe(
        ...
        try:
            ...
            parsed_dates = await self.get_date_ranges(
                date_where_clause_str=query_args.date_ranges,
            )
    ...
    async def parse_query_args_to_request_params(
        ...
        fb_ads_date_ranges = await self.get_date_ranges(
            date_where_clause_str=query_args.date_ranges,
        )
    ...
    async def get_order_by(
        ...
        if DATE in order_by_candidates:
            order_by_candidates.remove(DATE)
        if YEARMONTH in order_by_candidates:
            order_by_candidates.remove(YEARMONTH)

        order_by_list = self.parse_string_list(order_by)
        ...
        for order_by_item in order_by_list:
            if order_by_item.startswith("-"):
                ...
                if order_by_item in order_by_candidates:
                    final_order_by.append(order_by_item + FB_ADS_ORDER_DESC)
                    break
            else:
                if order_by_item in order_by_candidates:
                    final_order_by.append(order_by_item + FB_ADS_ORDER_ASC)
                    break
