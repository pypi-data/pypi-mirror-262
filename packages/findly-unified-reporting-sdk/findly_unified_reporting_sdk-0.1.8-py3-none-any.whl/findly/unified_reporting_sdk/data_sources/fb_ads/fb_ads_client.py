...
class FbAdsClient(ReportsClient):
    ...
    async def list_all_ad_accounts(self) -> Optional[List[str]]:
        """
    ...
    async def query(
        ...
        **kwargs: Dict,
        ...
        insights = None
        try:
            insights, summary = await self._query(**params)
        ...
        **kwargs: Dict,
        ...
        try:
            if dimension.type == DimensionType.FB_ADS_BREAKDOWN:
                ...
                result = [insight[dimension.name] for insight in response[:top_n]]
                ...
                if len(response) != 1:
                    ...
                    # If the response length is 0, return an empty list
                    # If the response length is > 1, return the top N values for the first element
                    # but keep a warning in the logs so we know that something is wrong
                    if len(response) == 0:
                        return []
                for action in response[0]["actions"]:
