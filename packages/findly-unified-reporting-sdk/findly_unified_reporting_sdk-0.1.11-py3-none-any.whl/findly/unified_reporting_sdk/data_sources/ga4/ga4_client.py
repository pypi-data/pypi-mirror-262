...
from findly.unified_reporting_sdk.util.typed_cache import typed_cache
...
class GA4Client(ReportsClient):
    ...
    @typed_cache(ttl=3600, cache=Cache.MEMORY)  # Cache results for 1 hr
    ...
    @typed_cache(ttl=3600, cache=Cache.MEMORY)  # Cache results for 1 hour
    ...
    @typed_cache(ttl=300, cache=Cache.MEMORY)  # Cache results for 5 minutes
    async def _query(
        self,
        metrics: List[Metric],
