...
class GA4Client(ReportsClient):
    ...
    async def _decorated_get_domain_name(self, property_id: str, **kwargs: str) -> Optional[str]:
    ...
    async def is_metric_empty(
        ...
        **kwargs: str
        ...
        return await self._decorated_is_metric_empty(metric=metric, property_id=property_id, **kwargs)

    @cached(
        ttl=3600, cache=Cache.MEMORY, skip_cache_func=lambda x: x is None
    )  # Cache results for 1 hour
    async def _decorated_get_metadata(self, property_id: str, **kwargs: str) -> Optional[Metadata]:
