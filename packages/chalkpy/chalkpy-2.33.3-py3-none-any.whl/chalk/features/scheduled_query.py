...
class ScheduledQuery:
    def __init__(
        ...
        """
        Creates an offline query which runs on a schedule. Cron queries do not produce datasets,
        but persists computation results in the feature store.

        By default, cron queries use incrementalization to only ingest data which has been updated since the last run.

        Parameters
        ----------
        name
            A unique name for the cron query. Cron queries with the same name will share status and incrementalization metadata.
        schedule
        ...
        Examples
        --------
        >>> from chalk.features import CronQuery
        >>> # this cron query will automatically run every 5 minutes after chalk apply
        >>> CronQuery(
        ...     "ingest_users",
        ...     schedule="*/5 * * * *",
        ...     output=[User],
