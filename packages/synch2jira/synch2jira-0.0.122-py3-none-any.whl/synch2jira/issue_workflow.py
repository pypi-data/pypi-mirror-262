...
class WorkFlow:
    ...
    def get_duration_between_two_state(issue_key, state1, state2):
        if IssueWokflow.did_issue_have_state(issue_key, state1) and IssueWokflow.did_issue_have_state(issue_key,
            ...
            return [issue_key, worflow2.from_time, WorkFlow.convert_date_time_to_date(worflow1.from_time),
                    timestamp_to_jour, worflow1.updated if date1 > date2 else worflow2.updated]
