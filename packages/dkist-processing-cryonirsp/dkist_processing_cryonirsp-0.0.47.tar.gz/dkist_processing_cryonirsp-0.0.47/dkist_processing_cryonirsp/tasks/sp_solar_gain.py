...
class SPSolarGainCalibration(CryonirspTaskBase):
            ...
            for exposure_time in target_exposure_times:
                for beam in range(1, self.constants.num_beams + 1):
                    ...
                    if self.parameters.fringe_correction_on:
                        with self.apm_processing_step(
                            f"Computing final solar gain based on fringe-corrected flux-scaled lamp gain for {beam = } and {exposure_time = }"
                        ):
                            # Compute a solar gain based on a fringe-corrected lamp gain
                            final_gain = self._compute_fringe_corrected_gain(beam, exposure_time)
    ...
    def _distort_char_spectrum(self, char_spec: np.ndarray) -> np.ndarray:
        ...
        Returns
        -------
        The characteristic spectrum fith spectral curvature distortion applied
        """
    ...
    def _compute_fringe_corrected_gain(self, beam: int, exposure_time: float) -> np.ndarray:
        """
